...
static torch::Tensor jblas_quantize(const torch::Tensor& Fp32Wei, bool transpose, int64_t bits, const std::string& alg,
                                    int64_t block_size, const std::string& compute_type) {
  return quant_launcher(Fp32Wei, transpose, bits, alg, block_size, compute_type);
}

static void jblas_symqdq_weight(torch::Tensor& Fp32Wei, bool transpose, int64_t bits, int64_t block_size) {
