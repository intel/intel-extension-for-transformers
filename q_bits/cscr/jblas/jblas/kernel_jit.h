 ...
 public:
  ...
  void codeGen(std::vector<kernel::jit_injector::eltwise_injector>& injectors) {
    inLocalLabel();  // use local label for multiple instance
    {
      ...
      const Xbyak::Reg64& reg_elt_constv = st.t[9];
      ...
      for (int i = 0; i < ColUnroll; i++) {
        ...
        for (int j = 0; j < injectors.size(); j++) injectors[j].vector_compute(Xbyak::Zmm(i), j * 3 * sizeof(float));
      ...
      for (int i = 0; i < injectors.size(); i++) injectors[i].vector_compute(Xbyak::Zmm(0), i * 3 * sizeof(float));
  ...
  static JBLAS_CODE forward(void* srcptr, void* dstptr, int row, int col, int srcstride, int dststride) {
    ...
    auto param = params{srcptr, dstptr, nullptr, col, srcstride, dststride};
  ...
  static JBLAS_CODE forward_with_gelu_and_linear(void* srcptr, void* dstptr, int row, int col, int srcstride,
    ...
    static std::vector<kernel::jit_injector::eltwise_injector> p = {{GELU}, {LINEAR}};
