    ...
    def __call__(self, model):
        ...
        for node in model.nodes:
            if node.op_type == "Reshape" and node.input_tensors[0].source_op:
                ...
                if pre_node.op_type in EXECUTOR_TYPE and (
                    EXECUTOR_TYPE[pre_node.op_type] == "InnerProduct"
                    or EXECUTOR_TYPE[pre_node.op_type] == "Matmul"
                ):
                    ...
                    if node.output_tensors[0].dest_op != []:
                        next_node = model.get_node_by_name(
        ...
        for i in range(len(pattern_mapping_config["StableDiffusion_ReshapeFusion"])):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "StableDiffusion_ReshapeFusion", pattern_dict, model
            )

            if len(new_node_names) != 0:
                logger.info("StableDiffusion_ReshapeFusion matched...")
                logger.debug(
                    "StableDiffusion_ReshapeFusion = {}".format(new_node_names)
                )
                for i in range(len(new_node_names)):
                    ...
                    if len(ret_old_nodes[i][3].output_tensors[0].dest_op) == 3:
                        ...
                        matMulWithBiasAdd_node.input_tensors[
                            3
                        ] = gather_node.output_tensors[0]
        return model
