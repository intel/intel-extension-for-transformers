...
class QuantizedGraphDtypeCheck(Pattern):
    ...
    def __call__(self, model):
        ...
        def _insert_quant_node(pre_node, model, graph_dtype):
            ...
            pre_node.output_tensors[0].name = (
                pre_node.output_tensors[0].name + "_before_quant"
            )
        ...
        def _scan_nodes_graph_dtype(model, checker, graph_dtype):
            for node in model.nodes:
                if (
                    node.attr
                    and node.attr.get("output_dtype", "fp32") == graph_dtype
                    and node.name != "input_data"
                ):
                    dst_ops = _get_dst_ops(node, model)
                    for op in dst_ops:
                        if (
                            EXECUTOR_TYPE.get(op.op_type, op.op_type)
                            in checker[graph_dtype]
                            and op.attr.get("output_dtype", "fp32") != graph_dtype
                        ):
                            op.attr["output_dtype"] = graph_dtype
            ...
            for node in model.nodes:
                if node.op_type == "Quantize":
                    ...
                    if (
                        pre_node
                        and pre_node.attr
                        and pre_node.attr.get("output_dtype", None) == graph_dtype
                    ):
                        ...
                        if len(dst_ops) == 1 and dst_ops[0].op_type == "Quantize":
                            ...
                            if len(dst_ops) >= 2:
                                dst_ops_type = [
                                    EXECUTOR_TYPE.get(o.op_type, o.op_type)
                                    for o in dst_ops
                                ]
                                valid = True
                                for ot in dst_ops_type:
                                    if ot in checker[graph_dtype] or ot in [
                                        "Quantize",
                                        "Output",
                                    ]:
                                        continue
                                ...
                                if valid:
                                    ...
                                    for qdo in quant_dst_ops:
                                        r_tensor_idx = model.get_tensor_idx(
                                            qdo.name, qot_name, from_output=False
                                        )
                                        if r_tensor_idx != -1:
                                            pre_node.output_tensors[0].dest_op.append(
                                                qdo.name
                                            )
                                            qdo.input_tensors[
                                                r_tensor_idx
        ...
        def _check_append_sum_nodes(model, checker, graph_dtype):
            ...
            for node in model.nodes:
                if (
                    EXECUTOR_TYPE.get(node.op_type, node.op_type)
                    in ["InnerProduct", "Convolution"]
                    and node.attr
                    and node.attr.get("append_op") == "sum"
                    and graph_dtype == "bf16"
                ):
                    post_n = model.get_node_by_name(node.input_tensors[-1].source_op[0])
                    if (
                        post_n.attr
                        and post_n.attr.get("output_dtype", "fp32") != "bf16"
                    ):
        ...
        def _revert_logits_output_dtype(model, graph_dtype):
            for t in model.nodes[-1].input_tensors:
                ...
                ) in ["InnerProduct", "Softmax", "LogSoftmax", "Convolution"]:
        ...
        non_quantized_patterns = [
            ...
            [(), (5, "Range"), (6, "Reshape"), (7, ["Matmul", "BatchMatMul"])],
        ]
        ...
        if graph_dtype in ["bf16"]:
            for node in model.nodes:
                if (
                    EXECUTOR_TYPE.get(node.op_type, node.op_type) in ["Quantize"]
                    and node.attr["output_dtype"] == "bf16"
                ):
                    dst_ops = _get_dst_ops(node, model)
                    for op in dst_ops:
                        if (
            ...
            for t in model.nodes[0].output_tensors:
                if t.location and len(t.location) == 2:
                    ...
                    if (
                        match_ret
                        or model._framework_modeling_config.get("architecture", "None")
                        == "decoder_only"
                    ) and t.dtype == "fp32":
                        t.dtype = "bf16"
