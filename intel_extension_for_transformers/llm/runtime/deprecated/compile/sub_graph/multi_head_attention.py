    ...
    def __call__(self, model):
            ...
            "MultiHeadAttention": [
                    ...
                    "patterns": {
                        "in": [
                            [
                                ...
                                (1, ["Add", "AddV2", "BinaryAdd"]),
                                (2, ["Div", "BinaryOp"]),
                                (3, ["Add", "AddV2", "BinaryAdd"]),
                                (4, "Softmax"),
                                (
                                    5,
                                    [
                ...
                # GPT-J bf16 torch model
                {
                    "patterns": {
                        "in": [
                            [
                                (0, "MatmulwithTranspose"),
                                (1, "BinaryAdd"),
                                (2, "Div"),
                                (3, "Add"),
                                (4, "Softmax"),
                                (5, "Quantize"),
                                (6, "MatmulwithTranspose"),
                            ]
                        ],
                        "out": [[(0, "MultiHeadAttention")]],
                    },
            ...
            for i in range(len(new_node_names)):
                ...
                if len(ret_old_nodes[i]) == 3:
                    ...
                    if ret_old_nodes[i][2].attr.get("algorithm", None) == "div":
                        assert isinstance(
                            ret_old_nodes[i][2].input_tensors[1].data, np.ndarray
                        )
                        attr["output_scale"] = (
                            1 / ret_old_nodes[i][2].input_tensors[1].data.item()
                        )
                ...
                if len(new_node.input_tensors) == 15:
                    ...
                    if (
                        model.get_node_by_name(mask_1.source_op[0]).op_type
                        == "PaddingSequence"
                    ):
                        new_node.input_tensors[3], new_node.input_tensors[4] = (
                            new_node.input_tensors[4],
                            new_node.input_tensors[3],
                        )
        ...
        for i in range(len(pattern_mapping_config["MultiHeadAttention"])):
            if util.get_autocast_info()["cast_type"] == "bf16" and i in [0, 1]:
                continue
            search_pattern = pattern_mapping_config["MultiHeadAttention"][i][
                "patterns"
            ]["in"]
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "MultiHeadAttention", pattern_dict, model
            )
                    ...
                    for node in model.nodes:
                        if node.op_type == "PaddingSequence":
                            if all_fused:
                                ...
                                seq_len_output = Tensor(
                                    name="mha_seq_len_mask",
                                ...
                                for i in range(len(new_node_names)):
                                    mha_node = model.get_node_by_name(
                                        new_node_names[i][0]
                                    )
