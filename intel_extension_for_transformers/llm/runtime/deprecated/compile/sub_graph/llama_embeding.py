...
class LlamaEmbeddings(Pattern):
    ...
    def __call__(self, model):
        ...
        model, new_node_names, ret_old_nodes = util.pattern_mapping(
            "LlamaEmbeddings", pattern_dict, model
        )
        if len(new_node_names) != 0:
            for i in range(len(new_node_names)):
                ...
                reshape_node2.attr = OrderedDict(
                    {"dst_shape": "-1, -1, -1", "dims": "0, 1", "mul": "0, 1"}
                )
                gather_node = model.get_node_by_name(new_node_names[i][1])
                gather_node.attr = OrderedDict({"axis": "0", "batch_dims": "0"})
                binary_node = model.get_node_by_name(
                    ret_old_nodes[i][1].output_tensors[0].dest_op[0]
                )
        ...
        if len(new_node_names) != 0:
            for i in range(len(new_node_names)):
                ...
                padding_node.attr = OrderedDict(
                    {"dst_shape": "-1,1,1,-1", "dims": "1", "mode": "llama"}
                )
                ConstantOfShape = model.get_node_by_name(new_node_names[i][1])
                ConstantOfShape.attr = OrderedDict(
                    {"tensor": "1", "mode": "llama", "trilu": "2", "value": -10000}
                )
