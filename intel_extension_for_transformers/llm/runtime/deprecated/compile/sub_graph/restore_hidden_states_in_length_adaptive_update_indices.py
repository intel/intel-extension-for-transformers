        ...
        for i in range(
            ...
            if len(new_node_names) != 0:
                for i in range(len(new_node_names)):
                    ...
                    embedding_ln_out_tensor = copy.deepcopy(
                        reshape_3d_node.output_tensors[0]
                    )
                    scatter_node = model.get_node_by_name(new_node_names[i][2])
                    # check if one input tensor is from embedding_layer_norm node
                    if (
                        scatter_node.input_tensors[0].name
                        == reshape_3d_node.input_tensors[0].name
                    ):
                        model.change_node_input_tensors(
                            new_node_names[i][2],
                            0,
                            tensor=embedding_ln_out_tensor,
                            mode="modify",
                        )
                return model
