    ...
    def __call__(self, model):
        ...
        for pattern_key, keep_attr_start in _fusion_order:
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                pattern_key, pattern_dict, model
            )
                ...
                for j in range(len(new_node_names)):
                    ...
                    for n in ret_old_nodes[j][1:keep_attr_start]:
                        if n.output_tensors[0].name in tensor_info:
                            cast_dtype[tensor_info[n.output_tensors[0].name]] = n.attr[
                                "DstT"
                            ]
                        else:
                            cast_dtype[tensor_info[n.input_tensors[0].name]] = n.attr[
                                "DstT"
                            ]
                    attr_list = [node_attr]
                    attr_list.extend(
                        [n.attr for n in ret_old_nodes[j][keep_attr_start:]]
                    )
