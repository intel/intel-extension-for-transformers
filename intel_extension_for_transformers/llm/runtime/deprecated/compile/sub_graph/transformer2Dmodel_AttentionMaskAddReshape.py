...
class Transformer2Dmodel_AttentionMaskAddReshape(Pattern):
    ...
    def __call__(self, model):
            ...
            "Transformer2Dmodel_AttentionMaskAddReshape": [
                {
                    ...
                    "search_mode": "op_type",
                    "node_names": {0: 13, 1: 14},
                    "input_tensors": {
                        0: [[{13: [0]}, {"input_data": [0]}], [[0, 1], 2]],
                        1: [[], [[], 1]],
                    },
                    "output_tensors": {
                        0: [[{13: [0]}], [[0], 1]],
        ...
        for i in range(
            len(pattern_mapping_config["Transformer2Dmodel_AttentionMaskAddReshape"])
        ):
            ...
            if len(new_node_names) != 0:
                ...
                logger.debug(
                    "Transformer2Dmodel_AttentionMaskAddReshape = {}".format(
                        new_node_names
                    )
                )
                for j in range(len(new_node_names)):
                    ...
                    # the first new node
                    new_reshape_node = model.nodes[
                        model.get_node_id(new_node_names[j][0])
                    ]
                    add_node = model.get_node_by_name(
                        new_reshape_node.input_tensors[0].source_op[0]
                    )

                    matmulwithbias_node = model.get_node_by_name(
                        add_node.input_tensors[0].source_op[0]
                    )
                    # stable diffusion v2.1 is Add here.
                    if matmulwithbias_node.op_type == "MatmulWithBias":
                        output_channel = matmulwithbias_node.input_tensors[
                            1
                        ].data.shape[1]

                    # get H * W
                    div_node = model.get_node_by_name(
                        ret_old_nodes[j][3].input_tensors[0].source_op[0]
                    )
