    ...
    def __call__(self, model):
        ...
        def _set_attr(new_node_names, ret_old_nodes, model):
            for i in range(len(new_node_names)):
                ...
                concat_node = model.get_node_by_name(
                    model.nodes[mat_node_idx].input_tensors[0].source_op[0]
                )
                concat1_node = model.get_node_by_name(
                    model.nodes[mat_node_idx].input_tensors[1].source_op[0]
                )
        ...
        model, new_node_names, ret_old_nodes = util.pattern_mapping(
            "MatMulWithTranspose", pattern_dict, model
        )
        ...
        def _set_attr1(new_node_names, ret_old_nodes, model):
            for i in range(len(new_node_names)):
                transpose_a = ret_old_nodes[i][0].attr["dst_perm"]
                transpose_dims = util.str2list(
                    ret_old_nodes[i][1].attr.get("transpose_dims", "-1,-2")
                )
                ...
                b_dim_ori[transpose_dims[0]], b_dim_ori[transpose_dims[1]] = (
                    b_dim_ori[transpose_dims[1]],
                    b_dim_ori[transpose_dims[0]],
                )
                ...
                if concat1_node.op_type == "Concat":
                    ...
                    concat2 = model.get_node_by_name(
                        concat1_node.input_tensors[1].source_op[0]
                    )
