    ...
    def __call__(self, model):
        ...
        def CollectQDQInfo(model):
            ...
            for pattern_nodes_name in patterns_nodes_name:
                ...
                quant_min, quant_max = get_min_max_from_onnx(
                ...
                util.insert_quant_info(
                    quant_node.output_tensors[0].name, [quant_min, quant_max, dtype]
                )
                util.insert_quant_info(
                    dquant_node.input_tensors[0].name, [quant_min, quant_max, dtype]
                )
                util.insert_quant_info(
                    dquant_node.output_tensors[0].name, [quant_min, quant_max, dtype]
                )
                        ...
                        if input_tensor.name == dquant_output.name:
                            for pre_quant_name in quant_node.input_tensors[0].source_op:
                                ...
                                if pre_quant_node.op_type in EXECUTOR_TYPE and (
                                    ...
                                    in ["InnerProduct", "Matmul", "Convolution"]
                                ):
                                ...
                                if len(pre_quant_node.output_tensors) == 1:
                                    ...
                                    for idx, it in enumerate(
                                        pre_quant_node.output_tensors
                                    ):
                                        if it.name == quant_node.input_tensors[0].name:
                                            pre_quant_node.output_tensors[
                                                idx
                                            ].dest_op.append(dst_node.name)
            ...
            for pattern_nodes_name in patterns_nodes_name:
                if len(pattern_nodes_name[:-1]) == 3:
                    ...
                    dquant_node.input_tensors[2].data = copy.deepcopy(
                        model.get_node_by_name(pattern_nodes_name[0])
                        .input_tensors[0]
                        .data
                    )
                    cast_dtype = model.get_node_by_name(pattern_nodes_name[1]).attr[
                        "DstT"
                    ]
                    try:
                        dquant_node.input_tensors[2].data = dquant_node.input_tensors[
                            2
                        ].data.astype(cast_dtype)
                    except:
                ...
                if not is_bias:
                    ...
                    util.insert_quant_info(
                        dquant_node.output_tensors[0].name,
                        [quant_min, quant_max, dtype],
                    )
                    # transpose weight
                    for dst_op in dquant_output.dest_op:
                        ...
                        if (
                            EXECUTOR_TYPE.get(down_node.op_type, down_node.op_type)
                            == "Reorder"
                        ):
                            ...
                            dquant_node.input_tensors[0].shape = list(
                                dquant_node.input_tensors[0].data.shape
                            )
                            ...
                            util.insert_quant_info(
                                down_node.output_tensors[0].name,
                                [quant_min, quant_max, dtype],
                            )
                dquant_output = dquant_node.output_tensors[0]
        ...
        def UpdateQuantInfo(model):
            ...
            for node in model.nodes:
                if EXECUTOR_TYPE.get(node.op_type, node.op_type) in [
                    ...
                    int8_info = util.get_quant_info().get(
                        node.input_tensors[0].name, None
                    )
                    if not int8_info and node.input_tensors[1].data.dtype != "float32":
                        logger.warning(
                            "node {} 's input activation {} has no int8 min-max info".format(
                                node.name, node.input_tensors[0].name
                            )
                        )
                    else:
                    ...
                    int8_info = util.get_quant_info().get(
                        node.output_tensors[0].name, None
                    )
                    if (
                        not int8_info
                        and node.attr.get("output_dtype", "fp32") != "fp32"
                    ):
                        logger.warning(
                            "node {} 's output activation {} has no int8 min-max info".format(
                                node.name, node.output_tensors[0].name
                            )
                        )
                    else:
        ...
        if model.framework_modeling_config["framework"] == "torch":
            logger.error(
                "Neural Engine does not support Stable Diffusion PyTorch Model now."
            )
            return model
