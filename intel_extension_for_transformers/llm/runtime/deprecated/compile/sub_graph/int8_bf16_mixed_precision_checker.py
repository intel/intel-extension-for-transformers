    ...
    def __call__(self, model):
        ...
        def _all_dst_ops(node, model, checker, output_dtype):
            ...
            for op in dst_ops:
                valid_connections = (
                    False
                    if model.get_tensor_idx(
                        op.name, node.output_tensors[0].name, from_output=False
                    )
                    == -1
                    else True
                )
                o_type = EXECUTOR_TYPE.get(op.op_type, op.op_type)
                if not valid_connections or o_type in [
                    "Shape",
                    "Quantize",
                    "Output",
                    "Softmax",
                    ...
                    continue
                if (
                    o_type not in checker[output_dtype]
                    or op.attr.get("output_dtype", "fp32") != output_dtype
                ):
                    return False
            return True
        ...
        def _fall_back_to_fp32(model, checker):
            for node in model.nodes:
                ...
                if EXECUTOR_TYPE.get(node.op_type, node.op_type) in [
                    "Softmax"
                ] and node.attr.get("output_dtype", "fp32") in ["u8", "s8"]:
                    pre_node = None
                    try:
                ...
                if fall_back_node:
                    if (
                        EXECUTOR_TYPE.get(
                            fall_back_node.op_type, fall_back_node.op_type
                        )
                        in checker["bf16_fp32_mixed"]
                    ):
                        fall_back_node.attr["output_dtype"] = "fp32"
                    else:
                        logger.warning(
                            "{} can not fall back into fp32 dtype".format(
                                fall_back_node.name
                            )
                        )
                        return False
        ...
        checker = {
            "bf16": bf16_op,
            "int8": int8_op,
            "bf16_fp32_mixed": bf16_fp32_mixed_op,
        }
            ...
            if EXECUTOR_TYPE.get(
                ...
                for op in dst_ops:
                    if (
                        ...
                        and EXECUTOR_TYPE.get(op.op_type, op.op_type) in checker["bf16"]
                    ):
