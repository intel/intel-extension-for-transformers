...
class AddEmbeddings(Pattern):
    ...
    def __call__(self, model):
        ...
        def _set_attr(hidden_size, epsilon, node_names, model, is_vit=False):
            ...
            if is_vit:
                model.nodes[reshape_2_node_idx].input_tensors = model.nodes[
                    binary_add_node_idx
                ].output_tensors
        ...
        if model.framework_modeling_config["framework"] == "onnxruntime":
            ...
            for i in range(len(pattern_mapping_config["AddEmbeddings"]) - 1):
                ...
                model, new_node_names, ret_old_nodes = util.pattern_mapping(
                    "AddEmbeddings", pattern_dict, model
                )
                if len(new_node_names) != 0:
                    for j in range(len(new_node_names)):
                        ...
                        _set_attr(
                            hidden_size, epsilon, new_node_names[j], model, is_vit
                        )
                        if len(pattern_dict["patterns"]["in"][0]) == 2:
                            binary_add_node_idx = model.get_node_id(
                                new_node_names[j][0]
                            )
                ...
                if len(new_node_names) != 0:
                    for j in range(len(new_node_names)):
                        ...
                        for n in ret_old_nodes[j]:
                            if (
                                model.get_node_by_name(new_node_names[j][idx]).op_type
                                == "Reshape"
                            ):
                                reshape_idx = idx
                                idx += 1
                            model.get_node_by_name(
                                new_node_names[j][idx]
                            ).attr = copy.deepcopy(n.attr)
                            idx += 1
                        hidden_size = str(model.inquire_config_item("hidden_size"))
                        model.get_node_by_name(
                            new_node_names[j][reshape_idx]
                        ).attr = OrderedDict({"dst_shape": "-1," + hidden_size})
                    return model
