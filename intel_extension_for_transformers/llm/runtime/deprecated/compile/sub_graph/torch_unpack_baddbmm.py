    ...
    def __call__(self, model):
            ...
            "TorchUnpackBaddbmm": [
                {
                    ...
                    "search_mode": "op_type",
                    "node_names": {0: 0, 1: "Baddbmm_div", 2: "Baddbmm_add"},
                    "input_tensors": {
                        0: [[{0: [1]}, {0: [2]}], [[1, 2], 2]],
        ...
        def _set_attr(new_node_names, ret_old_nodes, model):
            for i in range(len(new_node_names)):
                ...
                mm_other_tensor = Tensor(
                    name=ret_old_nodes[i][0].name + "_mm_other",
                    source_op=[],
                    dest_op=[binary_node.name],
                    shape=[1],
                    data=np.array(1.0 / ret_old_nodes[i][0].attr["alpha"]).astype(
                        np.float32
                    ),
                    dtype="fp32",
                )
        ...
        model, new_node_names, ret_old_nodes = util.pattern_mapping(
            "TorchUnpackBaddbmm", pattern_dict, model
        )
