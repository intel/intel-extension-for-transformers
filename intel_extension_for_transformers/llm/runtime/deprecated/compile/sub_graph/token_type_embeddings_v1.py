...
class TokenTypeEmbeddingsV1(Pattern):
    ...
    def __call__(self, model):
        ...
        def _set_attr(hidden_size, axis, batch_dims, node_names, model):
            ...
            model.nodes[tokentype_node_idx].input_tensors[1].data = slice_data.astype(
                np.int32
            )
        ...
        for i in range(len(pattern_mapping_config["TokenTypeEmbeddingsV1"])):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "TokenTypeEmbeddingsV1", pattern_dict, model
            )
