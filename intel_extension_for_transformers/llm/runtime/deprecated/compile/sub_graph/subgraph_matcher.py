...
from .. import logger
from .pattern import PATTERNS, superbert_patterns, supported_patterns
...
class SubGraphMatcher(object):
    ...
    def _fuse_patterns(
        ...
        pattern_mask = (
            [True for _ in range(len(supported_patterns))]
    ...
    def _tune_patterns(self, model, iterations=10, warm_up=5):
        ...
        for idx in tqdm(range(len(supported_patterns), len(all_patterns))):
            ...
            try:
                ...
                off_result, off_latency = off_model._get_latency(
                    [], iterations, warm_up
                )
            except:
                logger.warning("Graph can not be inferenced, please check the graph!")
