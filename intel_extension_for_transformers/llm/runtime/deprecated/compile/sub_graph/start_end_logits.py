        ...
        for i in range(len(patterns["StartEndLogits"])):
            ...
            if len(match_result) != 0:
                for each_ret in match_result:
                    ...
                    if i == 1:
                        input_tensors = [
                            copy.deepcopy(first_node.input_tensors[0]),
                            copy.deepcopy(model.nodes[0].output_tensors[0]),
                        ]
                        output_tensors = [
                            Tensor(name="logits", source_op=[first_node.name])
                        ]
                        pre_first_node = model.get_node_by_name(
                            input_tensors[0].source_op[0]
                        )
                        assert pre_first_node.op_type == "MatMulWithBias"
                        last_dim = pre_first_node.input_tensors[1].shape[-1]
                        attr = OrderedDict(
                            {"dst_shape": "-1,-1," + str(last_dim), "dims": "0,1"}
                        )
