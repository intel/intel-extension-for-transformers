    ...
    def __call__(self, model):
        ...
        for i in range(len(pattern_mapping_config["AttentionBlock_QKVReshape"])):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "AttentionBlock_QKVReshape", pattern_dict, model
            )
                ...
                for j in range(len(new_node_names)):
                    ...
                    if concat_node_input_size == 4:
                        ...
                        if constant_weight_idx != False:
                            dim = int(
                                concat_node.input_tensors[constant_weight_idx].data
                            )
                        ...
                        matmul_node_name = (
                            model.nodes[reshape_node_idx].output_tensors[0].dest_op[0]
                        )
                        matmul_node = model.get_node_by_name(matmul_node_name)
                        if matmul_node.op_type == "MatMulWithBias":
