...
class CollectQuantInfo(Pattern):
    ...
    def __call__(self, model):
        ...
        pattern_mapping_config = {
            ...
            "RemoveQuantDequant": [
                {"patterns": {"in": [[(0, "Quantize")]]}},
                {"patterns": {"in": [[(0, "DequantizeLinear")]]}},
            ],
            "QLinearMatMul": [
                {
                    "patterns": {
        ...
        def CollectQDQInfo(model):
            ...
            for pattern_nodes_name in patterns_nodes_name:
                ...
                dtype = (
                    "s8" if dquant_node.input_tensors[2].data.dtype == "int8" else "u8"
                )
                max_range = 127 if dtype == "s8" else 255
                quant_max = (
                    max_range - dquant_node.input_tensors[2].data
                ) * dquant_node.input_tensors[1].data
                quant_min = quant_max - 255 * dquant_node.input_tensors[1].data

                dtype = dtype + "_insert"
                util.insert_quant_info(
                    quant_node.input_tensors[0].name, [quant_min, quant_max, dtype]
                )
                        ...
                        if input_tensor.name == dquant_output.name:
                            for pre_quant_name in quant_node.input_tensors[0].source_op:
                                ...
                                if pre_quant_node.op_type in EXECUTOR_TYPE and (
                                    EXECUTOR_TYPE[pre_quant_node.op_type]
                                    == "InnerProduct"
                                    or EXECUTOR_TYPE[pre_quant_node.op_type] == "Matmul"
                                ):
                                    ...
                                    if (
                                        ...
                                        origin_quant_min = quant_info[
                                            pre_quant_node.input_tensors[0].name
                                        ][0]
                                        ...
                                        input_tensor_name = (
                                            pre_quant_node.input_tensors[1].name
                                        )
                                        new_dict[input_tensor_name] = [
                                            quant_min,
                                            quant_max,
                                        ]
                                ...
                                pre_quant_node.output_tensors[0].dest_op.append(
                                    dst_node.name
                                )

                                dst_node.input_tensors[
                                    idx
            ...
            for pattern_nodes_name in patterns_nodes_name:
                ...
                quant_min = quant_max - 255 * dquant_node.input_tensors[1].data
                util.insert_quant_info(
                    dquant_node.input_tensors[0].name, [quant_min, quant_max, dtype]
                )
                if dquant_output.name in new_dict:
                    ...
                    dtype = "output_" + dtype
                    util.insert_quant_info(
                        dquant_node.input_tensors[0].name,
                ...
                for dst_op in dquant_output.dest_op:
                    ...
                    for idx, input_tensor in enumerate(matmul_node.input_tensors):
                        if input_tensor.name == dquant_output.name:
                            matmul_node.input_tensors[idx] = dquant_node.input_tensors[
                                0
                            ]
                ...
                for qnode_name in qnodes_name:
                    ...
                    for qnode_input in qnode.input_tensors:
                        if not isinstance(qnode_input.data, np.ndarray):
                            ...
                            qpre_node_out_dest_op = copy.deepcopy(
                                qpre_node.output_tensors[0].dest_op
                            )
                            ...
                            for qpre_node_dst_op in qpre_node_out_dest_op:
                                if qpre_node_dst_op == qnode.name:
                                    qpre_node.output_tensors[0].dest_op.remove(
                                        qnode.name
                                    )
                                    qnode_output = qnode.output_tensors[0]
                                    for qnext_node in qnode_output.dest_op:
                                        qpre_node.output_tensors[0].dest_op.append(
                                            qnext_node
                                        )
                            ...
                            for qnext_node_name in qnode_output.dest_op:
                                ...
                                for idx in range(len(qnext_node.input_tensors)):
                                    if (
                                        qnext_node.input_tensors[idx].name
                                        == qnode_output.name
                                    ):
                                        qnext_node.input_tensors[idx] = qnode_input
                                        qnext_node.input_tensors[idx].source_op = [
                                            qpre_node.name
                                        ]
        ...
        def QLinearMatMul(model):
            ...
            model, matmul_nodes_name, qmatmul_nodes = util.pattern_mapping(
                "QLinearMatMul", pattern_dict, model
            )
            util.quant_info_init()
            for qmatmul_node in qmatmul_nodes:
                ...
                input_qdtype = (
                    input_dtype + "_weight"
                    if type(input.data) == np.ndarray
                    else input_dtype + "_insert"
                )
                input_min, input_max = scale2minmax(
                    input_dtype, input_scale.data, input_zp.data
                )
                ...
                weight_qdtype = (
                    weight_dtype + "_weight"
                    if type(weight.data) == np.ndarray
                    else weight_dtype + "_insert"
                )
                weight_min, weight_max = scale2minmax(
                    weight_dtype, weight_scale.data, weight_zp.data
                )
                ...
                output_min, output_max = scale2minmax(
                    output_dtype, output_scale.data, output_zp.data
                )
                    ...
                    for pre_op_name in qmatmul_node.input_tensors[id].source_op:
                        ...
                        if pre_out_tensor_name == qmatmul_in_tensor_name:
                            ...
                            if pre_op.op_type == "Softmax":
                                ...
                                origin_min, origin_max = scale2minmax(
                                    origin_zp.data.dtype,
                                    origin_scale.data,
                                    origin_zp.data,
                                )
                                ...
                                output_min, output_max = scale2minmax(
                                    output_zp.data.dtype,
                                    output_scale.data,
                                    output_zp.data,
                                )
                                output_zp_dtype = (
                                    "s8" if output_zp.data.dtype == "int8" else "u8"
                                )
                            ...
                            # trick: pre transpose has the same min/max as matmul,
                                ...
                                qmatmul_in_min, qmatmul_in_max = scale2minmax(
                                    qmatmul_in_zp.data.dtype,
                                    qmatmul_in_scale.data,
                                    qmatmul_in_zp.data,
                                )
                                trans_in_tensor_name = pre_op.input_tensors[0].name
                                qmatmul_in_zp_dtype = (
                                    "s8" if qmatmul_in_zp.data.dtype == "int8" else "u8"
                                )
                                qmatmul_in_dtype = qmatmul_in_zp_dtype + "_insert"
                                util.insert_quant_info(
                                    trans_in_tensor_name,
        ...
        def CollectTorchInfo(model):
                    ...
                    for idx, input_tensor in enumerate(dst_node.input_tensors):
                        if input_tensor.name == dquant_output.name:
                            for pre_quant_name in quant_node.input_tensors[0].source_op:
                                pre_quant_node = model.get_node_by_name(pre_quant_name)
                                for (
                                    pre_quant_node_out_tensor
                                ) in pre_quant_node.output_tensors:
                                    if (
                                        pre_quant_node_out_tensor.name
                                        == quant_node.input_tensors[0].name
                                    ):
                                        if (
                                            quant_node.name
                                            in pre_quant_node_out_tensor.dest_op
                                        ):
                                            pre_quant_node_out_tensor.dest_op.remove(
                                                quant_node.name
                                            )
                                        pre_quant_node_out_tensor.dest_op.append(
                                            dst_node.name
                                        )
                                        dst_node.input_tensors[
                                            idx
            ...
            for pattern_nodes_name in patterns_nodes_name:
                ...
                for dst_op in dquant_output.dest_op:
                    ...
                    for idx, input_tensor in enumerate(dst_node.input_tensors):
                        if input_tensor.name == dquant_output.name:
                            dquant_node.input_tensors[0].dest_op.remove(
                                dquant_node.name
                            )
