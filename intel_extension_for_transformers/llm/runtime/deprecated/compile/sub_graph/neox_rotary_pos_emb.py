...
class NeoxRoraryPosEmb(Pattern):
    ...
    def __call__(self, model):
        ...
        def _set_attr(new_node_names, ret_old_nodes, model):
            ...
            for i in range(len(new_node_names)):
                ...
                if "ends_with_tensor" not in slice_node.attr:
                    slice_node.input_tensors[0].data = slice_node.input_tensors[
                        0
                    ].data.transpose(0, 2, 1, 3)
                    shape = slice_node.input_tensors[0].shape
                    slice_node.input_tensors[0].shape = [
                        shape[0],
                        shape[2],
                        shape[1],
                        shape[3],
        ...
        model, new_node_names, ret_old_nodes = util.pattern_mapping(
            "NeoxRoraryPosEmb", pattern_dict, model
        )
                ...
                for node in model.nodes:
                    for input_tensor in node.input_tensors:
                        if output_name == input_tensor.name:
                            is_remove = False
                if is_remove:
                    remove_node_list.extend(
                        [pattern_nodes_name[0], pattern_nodes_name[1]]
                    )
        ...
        for pattern_nodes_name in patterns_nodes_name:
            ...
            source_slice_node = model.get_node_by_name(
                slice1_node.input_tensors[0].source_op[0]
            )
            slice_sign = (
                source_slice_node.attr["ends"] - source_slice_node.attr["starts"]
            )
        ...
        for i in range(len(new_node_names)):
            ...
            add_node = model.get_node_by_name(
                ori_slice_node.input_tensors[1].source_op[0]
            )
            remove_node_list.append(add_node.name)

            repeat_node = ret_old_nodes[i][1]
            shape_node = model.get_node_by_name(
                repeat_node.input_tensors[1].source_op[0]
            )
            remove_node_list.append(shape_node.name)

            repeat2_node = ret_old_nodes[i][2]
            shape_node2 = model.get_node_by_name(
                repeat2_node.input_tensors[1].source_op[0]
            )
            shape_node3 = model.get_node_by_name(
                repeat2_node.input_tensors[2].source_op[0]
            )
