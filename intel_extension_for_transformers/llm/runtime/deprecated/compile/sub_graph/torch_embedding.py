    ...
    def __call__(self, model):
        ...
        for node in model.nodes:
            if node.op_type == "Gather":
                if (
                    len(node.input_tensors) >= 2
                    and len(node.input_tensors[1].source_op) == 1
                ):
                    view_node = model.get_node_by_name(
                        node.input_tensors[1].source_op[0]
                    )
                else:
                    continue
                if (
                    len(view_node.input_tensors) >= 2
                    and len(view_node.input_tensors[1].source_op) == 1
                ):
                    shape_node = model.get_node_by_name(
                        view_node.input_tensors[1].source_op[0]
                    )
                else:
                    continue
                ...
                node_idx.append(
                    (model.get_node_id(view_node.name), model.get_node_id(node.name))
                )
