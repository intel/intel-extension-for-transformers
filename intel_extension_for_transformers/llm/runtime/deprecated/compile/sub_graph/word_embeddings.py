    ...
    def __call__(self, model):
        ...
        for i in range(len(pattern_mapping_config["WordEmbeddings"])):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "WordEmbeddings", pattern_dict, model
            )
            if len(new_node_names) != 0:
                for j in range(len(new_node_names)):
                    ...
                    if len(new_node_names[j]) <= 3 or (
                        model.get_node_by_name(new_node_names[j][2]).op_type
                        == "Reshape"
                        and model.get_node_by_name(new_node_names[j][3]).op_type
                        != "Reshape"
                    ):
                        batch_idx = 1
                    _set_attr(
