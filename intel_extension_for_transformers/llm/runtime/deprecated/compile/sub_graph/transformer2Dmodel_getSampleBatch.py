...
class Transformer2Dmodel_GetSampleBatch(Pattern):
    ...
    def __call__(self, model):
            ...
            "Transformer2Dmodel_GetSampleBatch": [
                {
                    ...
                    "search_mode": "op_type",
                    "node_names": {0: "timestep/reshape", 1: 10, 2: 11},
                    "input_tensors": {
                        0: [[{"input_data": [1]}], [[0], 1]],
                        1: [[{10: [0]}], [[0], 2]],
                        2: [[], [[], 1]],
        ...
        for i in range(
            len(pattern_mapping_config["Transformer2Dmodel_GetSampleBatch"])
        ):
            pattern_dict = pattern_mapping_config["Transformer2Dmodel_GetSampleBatch"][
                i
            ]
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "Transformer2Dmodel_GetSampleBatch", pattern_dict, model
            )

            if len(new_node_names) != 0:
                logger.info("Transformer2Dmodel_GetSampleBatch matched...")
                logger.debug(
                    "Transformer2Dmodel_GetSampleBatch = {}".format(new_node_names)
                )
