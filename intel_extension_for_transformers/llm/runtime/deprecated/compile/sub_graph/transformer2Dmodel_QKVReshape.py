    ...
    def __call__(self, model):
                ...
                # v2-1 only
                {
                    "patterns": {
                        "in": [
                            ...
                            [(), (2, "Unsqueeze"), (4, "Concat")],
                            [(), (3, "Unsqueeze"), (4, "Concat")],
                        ],
                        "out": [[(0, "Reshape")]],
                    },
                    "search_mode": "op_type",
        ...
        for i in range(len(pattern_mapping_config["Transformer2Dmodel_QKVReshape"])):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "Transformer2Dmodel_QKVReshape", pattern_dict, model
            )
            weight = False
            if len(new_node_names) != 0:
                logger.info("Transformer2Dmodel_QKVReshape matched...")
                logger.debug(
                    "Transformer2Dmodel_QKVReshape = {}".format(new_node_names)
                )
                for j in range(len(new_node_names)):
                    ...
                    if matmul_node.op_type == "MatMulWithBias":
                        ...
                        shape_node = model.get_node_by_name(
                            gather_node.input_tensors[0].source_op[0]
                        )
                        matmul_node = model.get_node_by_name(
                            shape_node.input_tensors[0].source_op[0]
                        )
                        weight = matmul_node.input_tensors[1].data.shape[1]
                        attr = OrderedDict()
