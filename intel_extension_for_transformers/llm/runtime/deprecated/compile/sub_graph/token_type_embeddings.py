...
class TokenTypeEmbeddings(Pattern):
    ...
    def __call__(self, model):
        ...
        pattern_mapping_config = {
            "TokenTypeEmbeddings": [
                ...
                # geminet
                {
                    "patterns": {
                        "in": [
                            [(0, "Input"), (1, "Gather"), (2, "Add"), (3, "LayerNorm")]
                        ],
                        "out": [
                            [
                                (0, "Reshape"),
        ...
        model, new_node_names, ret_old_nodes = util.pattern_mapping(
            "TokenTypeEmbeddings", pattern_dict, model
        )
        if len(new_node_names) != 0:
            for i in range(len(new_node_names)):
                ...
                _set_attr_with_gather(
                    hidden_size, axis, batch_dims, new_node_names[i], model
                )

            return model

        def _set_attr_with_onehot(
            hidden_size, one_hot_attr, mat_attr, node_names, model
        ):
            attr1 = OrderedDict()
            attr1["dst_shape"] = -1
            attr2 = one_hot_attr
        ...
        if len(new_node_names) != 0:
            for i in range(len(new_node_names)):
                ...
                _set_attr_with_onehot(
                    hidden_size, onehot_node.attr, mat_attr, new_node_names[i], model
                )
