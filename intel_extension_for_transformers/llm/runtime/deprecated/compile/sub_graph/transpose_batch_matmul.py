    ...
    def __call__(self, model):
        ...
        pattern_mapping_config = {
            "TransposeBatchMatMul": [
                ...
                # opennmt encoder
                {
                    ...
                    "node_names": {0: 5},
                    "input_tensors": {
                        0: [[{0: [0]}, {2: [0]}, {5: [0, 1]}], [[0, 1, 2], 3]]
                    },
                    "output_tensors": {0: [[{5: [0]}], [[0], 1]]},
                    "returns": [0, 2, 3, 1],
                },
                # opennmt decoder
                {
                ...
                # transpose, transpose - BatchMatMul-transpose
                {
                    "patterns": {
                        "in": [
                            [
                                (0, "Transpose"),
                                (2, ["BatchMatMul", "BatchMatMulV2", "MatMul"]),
                                (3, "Transpose"),
                            ],
                            [
                                (),
                ...
                # remove one input_tensor after fusion
                {
                    "patterns": {
                        "in": [
                            [
                                (0, "Transpose"),
                                (1, ["BatchMatMul", "BatchMatMulV2", "MatMul"]),
                                (2, "Transpose"),
                            ]
                        ],
                        "out": [[(0, "TransposeBatchMatMul")]],
                ...
                # BatchMatMul-transpose
                {
                    "patterns": {
                        "in": [
                            [
                                (0, ["BatchMatMul", "BatchMatMulV2", "MatMul"]),
                                (1, "Transpose"),
                            ]
                        ],
                        "out": [[(0, "TransposeBatchMatMul")]],
        ...
        for i in range(0, len(pattern_mapping_config["TransposeBatchMatMul"]) - 5):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "TransposeBatchMatMul", pattern_dict, model
            )
            if len(new_node_names) != 0:
                for j in range(len(new_node_names)):
                    transpose_list = []
                    transpose_list.append(
                        util.str2list(ret_old_nodes[j][0].attr["dst_perm"])
                    )
                    transpose_list.append(
                        util.str2list(ret_old_nodes[j][1].attr["dst_perm"])
                    )
                    ...
                    if len(ret_old_nodes[j]) == 3 and ret_old_nodes[j][2].op_type in [
                        "FusedMatMul"
                    ]:
                        ...
                        if ret_old_nodes[j][3].op_type == "Div":
                            output_scale = (
                                1.0 / ret_old_nodes[j][3].input_tensors[1].data
                            )
                        else:
                ...
                for i in range(len(new_node_names)):
                    ...
                    if len(tb_input_tensors) == 3:
                        ...
                        if src_t.name == tb_input_tensors[1].name:
                            model.nodes[tb_node_idx].input_tensors = [
                                tb_input_tensors[0],
                                tb_input_tensors[2],
                            ]
                            perm_dict["src0_perm"] = ret_old_nodes[i][0].attr[
                                "dst_perm"
                            ]
                        else:
                            model.nodes[tb_node_idx].input_tensors = [
                            ...
                            perm_dict["src1_perm"] = ret_old_nodes[i][0].attr[
                                "dst_perm"
                            ]
                    else:
                        if ret_old_nodes[i][0].op_type == "Transpose":
                            ...
                            perm_dict["src1_perm"] = ret_old_nodes[i][1].attr[
                                "dst_perm"
                            ]
