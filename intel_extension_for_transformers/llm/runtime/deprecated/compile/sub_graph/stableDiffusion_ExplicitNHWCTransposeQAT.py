...
class ExplicitNHWCTransposeForConvQAT(Pattern):
    ...
    def __call__(self, model):
        ...
        model, new_node_names, ret_old_nodes = util.pattern_mapping(
            "ExplicitNHWCTransposeForConvQAT", pattern_dict, model
        )
            ...
            for i in range(len(new_node_names)):
                ...
                if "output_dtype" in conv_attr:
                    if "fp32" in conv_attr["output_dtype"]:
                        ...
                        reorder_post_node.attr["output_dtype"] = conv_attr[
                            "output_dtype"
                        ]
                        reorder_pre_node.attr["output_dtype"] = conv_attr[
                            "output_dtype"
                        ]
                else:
        ...
        if len(new_node_names) != 0:
            ...
            for transpose_node in model.nodes:
                if transpose_node.op_type == "Transpose":
                    ...
                    if (
                        second_transpose_node.op_type == "Transpose"
                        and second_transpose_node.input_tensors[0].name
                        == transpose_node.output_tensors[0].name
                    ):
                        if (
                            transpose_node.attr["dst_perm"] == "0,3,1,2"
                            ...
                            pre_node.output_tensors[0] = transpose_node.output_tensors[
                                0
                            ]
                            target_node.input_tensors[
                                0
                            ] = transpose_node.output_tensors[0]
            ...
            for node in model.nodes:
                if (
                    ...
                    activation_scale = ((activation_max - activation_min) / 255).astype(
                        float
                    )
                    weight_scale = (
                        np.maximum(abs(weight_max), abs(weight_min)) / 127
                    ).astype(float)
                    bias_fp32 = (bias_s32 * activation_scale * weight_scale).astype(
                        np.float32
                    )
                    if node.attr.get("src1_perm", "0,1") == "0,1":
                        compensation = (
                            activation_min
                            * weight_scale
                            * weight_s8.sum(-1).astype(np.float32)
                        )
                    else:
                        compensation = (
                            activation_min
                            * weight_scale
                            * weight_s8.sum(0).astype(np.float32)
                        )

                    node.input_tensors[2].data = copy.deepcopy(
                        (bias_fp32 + compensation).astype(np.float32)
                    )

                if (
                    node.op_type in EXECUTOR_TYPE
                    and EXECUTOR_TYPE[node.op_type] == "Convolution"
                ):
                    # convert s32 bias to fp32 bias due to ONEDNN 3.x required
                    weight_s8 = node.input_tensors[1].data
                    bias_s32 = node.input_tensors[2].data
                    ...
                    weight_scale = (
                        np.maximum(abs(weight_max), abs(weight_min)) / 128
                    ).astype(float)
                    bias_fp32 = (bias_s32 * activation_scale * weight_scale).astype(
                        np.float32
                    )
        ...
        def _mixed_bf16_precision(model):
            ...
            def _check_dst_op(start_node, model, checker, output_dtype):
                ...
                for op in dst_ops:
                    if (
                        EXECUTOR_TYPE.get(op.op_type, op.op_type)
                        in checker[output_dtype]
                    ):
            ...
            def _revert_logits_output_dtype(model, output_dtype):
                for t in model.nodes[-1].input_tensors:
                    ...
                    if pre_node and EXECUTOR_TYPE.get(
                        pre_node.op_type, pre_node.op_type
                    ) in ["InnerProduct", "Softmax", "LogSoftmax"]:
            ...
            for node in model.nodes:
                if EXECUTOR_TYPE.get(
                    node.op_type, node.op_type
                ) == "Quantize" and node.attr["output_dtype"] in ["u8", "s8", "bf16"]:
                    dst_ops = _get_dst_ops(node, model)
                    for op in dst_ops:
                        if (
                            op.attr.get("output_dtype", "fp32") == "fp32"
                            and EXECUTOR_TYPE.get(op.op_type, op.op_type)
                            in checker["bf16"]
                        ):
            ...
            for node in model.nodes:
                if node.op_type in EXECUTOR_TYPE and (
                    EXECUTOR_TYPE[node.op_type] == "BinaryAdd"
                    or EXECUTOR_TYPE[node.op_type] == "BinaryOp"
                ):
