    ...
    def __call__(self, model):
                ...
                # v2-1 only
                {
                    "patterns": {
                        "in": [
                            ...
                            [(), (2, "Unsqueeze"), (6, "Concat")],
                            [(), (3, "Unsqueeze"), (6, "Concat")],
        ...
        for i in range(len(pattern_mapping_config["Transformer2Dmodel_QKVPreReshape"])):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "Transformer2Dmodel_QKVPreReshape", pattern_dict, model
            )
            ...
            if len(new_node_names) != 0:
                ...
                logger.debug(
                    "Transformer2Dmodel_QKVPreReshape = {}".format(new_node_names)
                )
                for j in range(len(new_node_names)):
                    # the first new node
                    assert (
                        ret_old_nodes[j][0].op_type == "Conv"
                        or ret_old_nodes[j][0].op_type == "GroupNorm"
                    )
