...
class NeoxReorderChange(Pattern):
    ...
    def __call__(self, model):
        ...
        pattern_mapping_config = {
            "NeoxReorderChange": [
                ...
                },
                {
                    "patterns": {
                        "in": [
                            [(0, "Softmax"), (1, "Matmul"), (2, "Reorder"), (3, "View")]
                        ],
                    }
                },
                {
        ...
        remove_list = []
        patterns_nodes_name = util.search_pattern(
            pattern_mapping_config["NeoxReorderChange"][0]["patterns"]["in"], model
        )
        if len(patterns_nodes_name) != 0:
            for nodes_name in patterns_nodes_name:
                ...
                source_node.output_tensors[0] = copy.deepcopy(
                    reorder_node.output_tensors[0]
                )
                ...
                for dest_name in reorder_node.output_tensors[0].dest_op:
                    ...
                    for in_tensor in dest_node.input_tensors:
                        if reorder_node.output_tensors[0].name in in_tensor.source_op:
                            in_tensor.source_op.remove(
                                reorder_node.output_tensors[0].name
                            )
        ...
        patterns_nodes_name = util.search_pattern(
            pattern_mapping_config["NeoxReorderChange"][1]["patterns"]["in"], model
        )
        if len(patterns_nodes_name) != 0:
            for nodes_name in patterns_nodes_name:
                reorder_node = model.get_node_by_name(nodes_name[1])
        ...
        patterns_nodes_name = util.search_pattern(
            pattern_mapping_config["NeoxReorderChange"][2]["patterns"]["in"], model
        )
        if len(patterns_nodes_name) != 0:
            for nodes_name in patterns_nodes_name:
                reorder_node = model.get_node_by_name(nodes_name[1])
        ...
        remove_list.clear()
        patterns_nodes_name = util.search_pattern(
            pattern_mapping_config["NeoxReorderChange"][3]["patterns"]["in"], model
        )
        if len(patterns_nodes_name) != 0:
            for nodes_name in patterns_nodes_name:
        ...
        patterns_nodes_name = util.search_pattern(
            pattern_mapping_config["NeoxReorderChange"][4]["patterns"]["in"], model
        )
        if len(patterns_nodes_name) != 0:
            for nodes_name in patterns_nodes_name:
        ...
        patterns_nodes_name = util.search_pattern(
            pattern_mapping_config["NeoxReorderChange"][5]["patterns"]["in"], model
        )
        remove_list = []
        if len(patterns_nodes_name) != 0:
            for nodes_name in patterns_nodes_name:
                ...
                dest_node = model.get_node_by_name(
                    reshape_node.output_tensors[0].dest_op[0]
                )
                mm_node.attr["dst_perm"] = reorder_node.attr["dst_perm"]
                mm_node.attr["reshape"] = reshape_node.attr["shape"][3:]
                mm_node.output_tensors[0] = copy.deepcopy(
                    reshape_node.output_tensors[0]
                )
