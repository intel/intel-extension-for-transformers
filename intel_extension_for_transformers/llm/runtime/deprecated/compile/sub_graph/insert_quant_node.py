    ...
    def __call__(self, model):
        ...
        if model.framework_modeling_config["framework"] == "torch":
            ...
            while node_idx < len(model.nodes):
                ...
                if node.op_type in EXECUTOR_TYPE and (
                    ...
                    or EXECUTOR_TYPE[node.op_type] == "Matmul"
                    or EXECUTOR_TYPE[node.op_type] == "Convolution"
                ):
                    ...
                    while idx < input_size:
                        ...
                        if input_name in quant_info and idx < 3:
                            quant_min = Tensor(
                                ...
                                data=np.array(
                                    quant_info[input_name][0].astype("float32")
                                ),
                                dtype="fp32",
                            )
                            quant_max = Tensor(
                                ...
                                data=np.array(
                                    quant_info[input_name][1].astype("float32")
                                ),
                                dtype="fp32",
                            )
                            ...
                            if "insert" in quant_info[input_name][2]:
                                if input_name in unique_quant_nodes:
                                    ...
                                    quant_node = model.get_node_by_name(
                                        quantize_op_name
                                    )
                                    quant_node.output_tensors[0].dest_op.append(
                                        node.name
                                    )
                                    node.input_tensors[idx] = quant_node.output_tensors[
                                        0
                                    ]
                                else:
                                    quant_dtype = (
                                        "u8"
                                        if "u8" in quant_info[input_name][2]
                                        else "s8"
                                    )
                                    quant_dtype = (
                                        "s8"
                                        if EXECUTOR_TYPE[node.op_type] == "Matmul"
                                        else quant_dtype
                                    )
                                    quant_output = Tensor(
                                ...
                                model.change_node_input_tensors(
                                    node.name,
                                    insert_offset + 2 * idx + 0,
                                    quant_min,
                                    "insert",
                                )
                                model.change_node_input_tensors(
                                    node.name,
                                    insert_offset + 2 * idx + 1,
                                    quant_max,
                                    "insert",
                                )
                            ...
                            if "output" in quant_info[input_name][2]:
                                ...
                                quant_min = Tensor(
                                    ...
                                    data=np.array(
                                        quant_info[input_name][3].astype("float32")
                                    ),
                                    dtype="fp32",
                                )
                                quant_max = Tensor(
                                    ...
                                    data=np.array(
                                        quant_info[input_name][4].astype("float32")
                                    ),
                                    dtype="fp32",
                                )
                                # insert output min and max tensor
                                model.change_node_input_tensors(
                                    node.name, insert_offset + 4, quant_min, "insert"
                                )
                                model.change_node_input_tensors(
                                    node.name, insert_offset + 5, quant_max, "insert"
                                )
            ...
            for node in model.nodes:
                if node.name in quant_info:
                    ...
                    for remove_idx in remove_tensors_list:
                        model.change_node_input_tensors(
                            node.name, remove_tensors_list[0], None, "remove"
                        )
            ...
            for node in model.nodes:
                if node.op_type in EXECUTOR_TYPE and (
                    ...
                    for idx, input_tensor in enumerate(node.input_tensors):
                        ...
                        insert_offset = (
                            insert_offset - 2
                            if "append_op" not in node.attr and insert_offset == 1
                            else insert_offset
                        )
                        if input_name in quant_info and idx < 3:
                            quant_min = Tensor(
                            ...
                            if "insert" in quant_info[input_name][2]:
                                quant_dtype = (
                                    "u8" if "u8" in quant_info[input_name][2] else "s8"
                                )
                                quant_dtype = (
                                    "s8"
                                    if EXECUTOR_TYPE[node.op_type] == "Matmul"
                                ...
                                model.change_node_input_tensors(
                                    node.name,
                                    insert_offset + 2 * idx + 3,
                                    quant_min,
                                    "insert",
                                )
                                model.change_node_input_tensors(
                                    node.name,
                                    insert_offset + 2 * idx + 4,
                                    quant_max,
                                    "insert",
                                )
                            if "weight" in quant_info[input_name][2]:
                            ...
                            if "output" in quant_info[input_name][2]:
                                ...
                                # insert output min and max tensor
                                model.change_node_input_tensors(
                                    node.name, insert_offset + 7, quant_min, "insert"
                                )
                                model.change_node_input_tensors(
                                    node.name, insert_offset + 8, quant_max, "insert"
                                )
                                util.insert_quant_info(node.name, [])
            ...
            for node in model.nodes:
                if node.op_type in EXECUTOR_TYPE and (
                    ...
                    src0_dtype = (
                        node.input_tensors[0].dtype == "u8"
                        or node.input_tensors[0].dtype == "s8"
                    )
                    src1_dtype = (
                        node.input_tensors[1].dtype == "u8"
                        or node.input_tensors[1].dtype == "s8"
                    )
                    if src0_dtype ^ src1_dtype:
                        ...
                        if src0_dtype:
                            ...
                            node.input_tensors[0] = model.get_node_by_name(
                                src0_source_op[0]
                            ).input_tensors[0]
                            node.input_tensors[0].dest_op = [node.name]
                        else:
                            remove_list.append(src1_source_op[0])
                            node.input_tensors[1] = model.get_node_by_name(
                                src1_source_op[0]
                            ).input_tensors[0]
        ...
        for node in model.nodes:
            if (
                ...
                offset = 0
                if "append_op" in node.attr and node.attr["append_op"] in [
                    "binary_add",
                    "sum",
                ]:
                    offset = 1
                ...
                input_scale, input_zero_point = get_scale_zp(
                    node.input_tensors[offset + 3].data,
                    node.input_tensors[offset + 4].data,
                    dtype,
                )
                weight_scale, weight_zero_point = get_scale_zp(
                    node.input_tensors[offset + 5].data,
