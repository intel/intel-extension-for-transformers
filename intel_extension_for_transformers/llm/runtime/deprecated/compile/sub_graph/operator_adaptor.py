    ...
    def __call__(self, model):
        ...
        def _sweep_inputs(model, sweep_nodes_info):
            for name, target_pos in sweep_nodes_info.items():
                ...
                target_inputs = [
                    model.nodes[node_id].input_tensors[j] for j in target_pos
                ]
        ...
        def _reshape_non_2d_src_before_inner_product(model, node_names):
            for name in node_names:
                ...
                output_tensors = [
                    Tensor(
                        name=reshape_node_name + ":0",
                        source_op=[reshape_node_name],
                        dest_op=[name],
                    )
                ]
                ...
                reshape_node = util.construct_node(
                    ...
                    attr=OrderedDict({"dst_shape": "-1," + str(hidden_size)}),
                )
        ...
        def _remove_squeeze_after_inner_product(model, node_names):
            for name in node_names:
                ...
                pre_node = model.get_node_by_name(
                    squeeze_node.input_tensors[0].source_op[0]
                )
                assert (
                ...
                pre_node.output_tensors[0] = copy.deepcopy(
                    squeeze_node.output_tensors[0]
                )
        ...
        for n in model.nodes:
            if n.op_type in sweep_op.keys():
                ...
                if n.op_type == "Gather" and n.filling_method in [
                    "extract_from_onnxruntime",
                    "extract_from_torch",
                ]:
                ...
                if isinstance(n.input_tensors[1].data, np.ndarray):
                    ...
                    try:
                        pre_node = model.get_node_by_name(
                            n.input_tensors[0].source_op[0]
                        )
                    except:
                        pre_node = None
                    if (
                        pre_node
                        and pre_node.op_type == "Transpose"
                        and len(pre_node.attr["dst_perm"]) > 3
                    ):
                ...
                if (
                    pre_node
                    and EXECUTOR_TYPE.get(pre_node.op_type, pre_node.op_type)
                    == "InnerProduct"
                    and n.attr
                ):
