    ...
    def __call__(self, model):
        ...
        for i in range(len(pattern_mapping_config["InteractFeatures"])):
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "InteractFeatures", pattern_dict, model
            )
            ...
            if len(new_node_names) != 0 and i == 0:
                for j in range(len(new_node_names)):
                    ...
                    model.nodes[concat_node_idx].input_tensors = ret_old_nodes[j][
                        0
                    ].input_tensors
                ...
                for j in range(len(new_node_names)):
                    ...
                    gather_indices = (
                        concat_num * ret_old_nodes[j][0].input_tensors[0].data
                    ...
                    model.change_node_input_tensors(
                        new_node_names[j][2],
                        0,
                        ret_old_nodes[j][0].input_tensors[0],
                        mode="insert",
                    )
