    ...
    def __call__(self, model):
            ...
            "ArangewithReciprocal": [
                {
                    "patterns": {
                        ...
                        "out": [[(0, "Range"), (1, "Div"), (2, "Pow"), (3, "Div")]],
                    },
                    "search_mode": "op_type",
                    "node_names": {0: 1, 1: 2, 2: 3, 3: 5},
                    "input_tensors": {
                        0: [[{"input_data": [0]}], [[0], 1]],
                        1: [[], [[], 1]],
                        2: [[{3: [0]}], [[0], 2]],
        ...
        def _set_attr(new_node_names, ret_old_nodes, model):
            for i in range(len(new_node_names)):
                slice_node = model.get_node_by_name(
                ...
                div_dim_tensor = Tensor(
                    name=div_dim_node.input_tensors[0].name + "_dim",
                    source_op=[],
                ...
                model.get_node_by_name(new_node_names[i][3]).attr = OrderedDict(
                    {"algorithm": "div"}
                )
        ...
        if model.framework_modeling_config["framework"] == "torch":
            ...
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "ArangewithReciprocal", pattern_dict, model
            )
