...
class MatMulWithBiasAdd(Pattern):
    ...
    def __call__(self, model):
        ...
        for node in model.nodes:
            if (
                node.op_type in ["MatMulWithBias", "InnerProduct"]
                and node.name not in match_ret
            ):
                dst_ops = []
                for d in node.output_tensors[0].dest_op:
                    dst_ops.append(model.get_node_by_name(d))
                if (
                    node.output_tensors[0].name not in model.output_tensors_name
                    and len(dst_ops) == 1
                    and EXECUTOR_TYPE.get(dst_ops[0].op_type, dst_ops[0].op_type)
                    == "BinaryAdd"
                ):
                    add_s_ops = [
                        model.get_node_by_name(t.source_op[0])
                        for t in dst_ops[0].input_tensors
                    ]
        ...
        for k, v in match_ret.items():
            ...
            append_idx = (
                1
                if mat_node.output_tensors[0].name == a_node.input_tensors[0].name
                else 0
            )
            mat_node.input_tensors.append(
                copy.deepcopy(a_node.input_tensors[append_idx])
            )
