                ...
                for dest_op_name in node.output_tensors[0].dest_op:
                    ...
                    if dest_node.op_type == "BinaryAdd" or dest_node.op_type == "Add":
                        ...
                        for add_dest_op_name in dest_node.output_tensors[0].dest_op:
                            ...
                            for in_tensor_idx in range(
                                len(add_dest_node.input_tensors)
                            ):
                                if (
                                    add_dest_node.input_tensors[in_tensor_idx].name
                                    == dest_node.output_tensors[0].name
                                ):
                                    add_dest_node.input_tensors[
                                        in_tensor_idx
                                    ] = copy.deepcopy(ori_tensor)
