...
class Transformer2Dmodel_QKVReshapeTo4D(Pattern):
    ...
    def __call__(self, model):
        ...
        for i in range(
            len(pattern_mapping_config["Transformer2Dmodel_QKVReshapeTo4D"])
        ):
            pattern_dict = pattern_mapping_config["Transformer2Dmodel_QKVReshapeTo4D"][
                i
            ]
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "Transformer2Dmodel_QKVReshapeTo4D", pattern_dict, model
            )

            if len(new_node_names) != 0:
                logger.info("Transformer2Dmodel_QKVReshapeTo4D matched...")
                logger.debug(
                    "Transformer2Dmodel_QKVReshapeTo4D = {}".format(new_node_names)
                )
