...
class ONNXExtractor(object):
    ...
    def __call__(self, model):
        ...
        for graph_input in model.graph.input:
            ...
            new_node.extract(
                "onnxruntime", graph_input, model, graph_nodes_dict, new_graph
            )
                    ...
                    for inner_node in node.attribute[0].g.node:
                        ...
                        if op_type == "Constant":
                            ...
                            for input_tensor_name in input_tensor_names:
                                ...
                                if origin_tensor_name in graph_nodes_dict and hasattr(
                                    graph_nodes_dict[origin_tensor_name], "node"
                                ):
                                    ...
                                    for tensor in graph_node.input_tensors:
                                        if origin_tensor_name + ":0" == tensor.name:
                                            has_tensor = False
                                    if (
                                        pre_node in model.graph.initializer
                                        and has_tensor
                                    ):
                                        ...
                                        new_graph.change_node_input_tensors(
                                            node.name, 0, loop_tensor, mode="insert"
                                        )
