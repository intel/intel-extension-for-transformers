...
class Graph(object):
    ...
    def __init__(self):
        ...
        self._do_activation_mem_compression = (
            True if "ENGINE_ACTIVATION_MEM_COMPRESSION" in os.environ else False
        )
    ...
    def execution_options(self):
        ...
        if self._execution_options:
            values = [
                getattr(self._execution_options, option) for option in options_list
            ]
    ...
    def execution_options(self, options):
        ...
        if isinstance(options, ne.ExecutionOptions):
            ...
            def set_execution_mode(v):
                ...
                if v == "inference":
                    ...
                    logger.error(
                        "execution_mode value should be inference, debug or tuning."
                    )
            ...
            def set_activation_mem_compression(v):
                assert isinstance(
                    v, bool
                ), "activation_mem_compression should be True or False."
                ne_options.activation_mem_compression = v

            def set_dump_activation_dag(v):
                assert isinstance(
                    v, bool
                ), "dump_activation_dag should be True or False."
        ...
        self._do_activation_mem_compression = (
            self._execution_options.activation_mem_compression
        )
    ...
    def max_input_shapes_list(self, input_shapes_list):
        ...
        assert (
            isinstance(input_shapes_list, list)
            and isinstance(input_shapes_list[0], list)
            and isinstance(input_shapes_list[0][0], list)
        ) or input_shapes_list == None, (
    ...
    def get_node_id(self, node_name):
        ...
        try:
            ...
            raise ValueError(
                "There is no node named {}, please check the input name.".format(
                    node_name
                )
            )
    ...
    def rename_node(self, old_name, new_name):
        ...
        for i in range(len(self._nodes[index].input_tensors)):
            ...
            for pre_node_name in self._nodes[index].input_tensors[i].source_op:
                tensor_idx = self.get_tensor_idx(
                    pre_node_name, self._nodes[index].input_tensors[i].name
                )
                pre_node_idx = self._node_id[pre_node_name]
                self._nodes[pre_node_idx].output_tensors[tensor_idx].dest_op.remove(
                    old_name
                )
                self._nodes[pre_node_idx].output_tensors[tensor_idx].dest_op.append(
                    new_name
                )
        for i in range(len(self._nodes[index].output_tensors)):
            ...
            for next_node_name in self._nodes[index].output_tensors[i].dest_op:
                ...
                self._nodes[next_node_idx].input_tensors[tensor_idx].source_op = [
                    new_name
                ]
    ...
    def change_node_input_tensors(self, node_name, index, tensor=None, mode="modify"):
        ...
        if tensor.source_op != []:
            ...
            tensor_idx = self.get_tensor_idx(
                tensor.source_op[0], tensor.name, from_output=True
            )
        ...
        if mode == "insert":
            if source_node_idx is not None:
                if (
                    node_name
                    not in self._nodes[source_node_idx]
                    .output_tensors[tensor_idx]
                    .dest_op
                ):
                    self._nodes[source_node_idx].output_tensors[
                        tensor_idx
                    ].dest_op.append(node_name)
            ...
            self.change_node_input_tensors(
                node_name, index, tensor=tensor, mode="insert"
            )
        self._engine = None
            ...
            if t.source_op != [] and t.source_op[0] in self._node_id.keys():
                ...
                if mode == "insert":
                    if (
                        ...
                        self._nodes[source_node_idx].output_tensors[
                            tensor_idx
                        ].dest_op.append(node.name)
                if mode == "remove":
                    if (
                        node.name
                        in self._nodes[source_node_idx]
                        .output_tensors[tensor_idx]
                        .dest_op
                    ):
                        self._nodes[source_node_idx].output_tensors[
                            tensor_idx
                        ].dest_op.remove(node.name)
        ...
        for i in range(len(node.output_tensors)):
            ...
            for dest_op_name in node.output_tensors[i].dest_op:
                if dest_op_name in self._node_id.keys():
                    ...
                    tensor_idx = self.get_tensor_idx(
                        dest_op_name, t.name, from_output=False
                    )
                    if mode == "insert":
                        node.output_tensors[i].source_op = [node.name]
                        if tensor_idx != -1:
                            self._nodes[dest_node_idx].input_tensors[
                                tensor_idx
                            ].source_op = [node.name]
                    if mode == "remove":
    ...
    def weight_data(self):
        ...
        for i in range(len(self._nodes)):
            for j in range(len(self._nodes[i].input_tensors)):
                ...
                if (
                    t.source_op == [] or self._node_id.get(t.source_op[-1], None) == 0
                    ...
                    if t.name not in const_info:
                        ...
                        self._nodes[0].output_tensors.append(
                            self._nodes[i].input_tensors[j]
                        )
                    else:
                        if (
                            self._nodes[0].output_tensors[const_info[t.name]].data.dtype
                            != self._nodes[i].input_tensors[j].data.dtype
                        ):
                            logger.error(
                                "tensor {} has different dtypes in multi-nodes!".format(
                                    t.name
                                )
                            )
                        dest_op = set(
                            self._nodes[0].output_tensors[const_info[t.name]].dest_op
                            + self._nodes[i].input_tensors[j].dest_op
                        )
                        self._nodes[0].output_tensors[
                            const_info[t.name]
                        ].dest_op = list(dest_op)
            ...
            for node in net_info["model"]["operator"]:
                if "output" in net_info["model"]["operator"][node].keys():
                    for tensor in net_info["model"]["operator"][node]["output"]:
                        if (
                            "location"
                            in net_info["model"]["operator"][node]["output"][
                                tensor
                            ].keys()
                        ):
                            continue
                        net_info["model"]["operator"]["output_data"]["input"][
                            tensor
                        ] = {}
        else:
    ...
    def engine_init(self, net_info={}, weight_data=b"", refresh_model=True):
        ...
        if refresh_model:
            ...
            if not weight_data:
                weight_data = (
                    self._weight_root if self._weight_root else self.weight_data
                )
            if not net_info:
                ...
                for node in net_info["model"]["operator"]:
                    ...
                    if "output" in operator.keys():
                        for output_name, attrs in operator["output"].items():
                            ...
                            output_tensor = ne.tensor_config(
                                output_name,
                                tensor_shape,
                                tensor_dtype,
                                tensor_strides,
        ...
        if (
            self._do_activation_mem_compression
            and self._max_input_shapes_list
            and len(self._max_input_shapes_list) > 0
        ):
    ...
    def inference(self, input_data):
        ...
        if (
            self._refresh_max_input_shapes_list
            and self._do_activation_mem_compression
            and self._engine
        ):
    ...
    def graph_init(self, config, weight_data=None, load_weight=False):
        ...
        for node in d["model"]["operator"]:
            ...
            if optype == "Input":
                output_tensors = []
                for tensor_name, attrs in d["model"]["operator"][node][
                    "output"
                ].items():
                    tensor_strides = None
                    if "strides" in attrs.keys():
                    ...
                    if load_weight and "location" in attrs.keys():
                        ...
                        tensor_data = np.frombuffer(
                            tensor_data, dtype=DTYPES_DICT[tensor_dtype]
                ...
                op = util.construct_node(
                    node, "Input", [], copy.deepcopy(output_tensors)
                )
                ...
                op = util.construct_node(
                    node,
                    optype,
                    copy.deepcopy(input_tensors),
                    copy.deepcopy(output_tensors),
                    attr,
                )
        ...
        if onednn_graph_nodes_map == {"InnerProduct": [], "Softmax": []}:
            ...
            for onednn_graph_nodes_name in onednn_graph_nodes_name_list:
                ...
                try:
                    ...
                    logger.warning(
                        "Graph can not be inferenced, please check the graph!"
                    )
    ...
    def _get_onednn_graph_nodes(self):
        ...
        for node in self.nodes:
            if node.op_type == "InnerProduct":
                ...
                if type(weight.data) == np.ndarray and weight.data.dtype == "float32":
    ...
    def _generate_onednn_graph_nodes_name_list(self, onednn_graph_nodes_map):
        ...
        for ip_nodes_name in ip_nodes_name_list:
            ...
            onednn_graph_nodes_name_list.append(
                ip_nodes_name + onednn_graph_nodes_map["Softmax"]
            )
    ...
    def _tune_sparse_graph(self, inputs_shape=[], sparse_nodes_name=[]):
        ...
        if sparse_nodes_name == []:
            ...
            trans_nodes_name_list = self._generate_transpose_nodes_name_list(
                sparse_nodes_name
            )
    ...
    def get_sparse_nodes_name(self, threshold=0.7):
        ...
        for node in self.nodes:
            if node.op_type == "InnerProduct":
                ...
                # 4. post op != tanh
                if "append_op" not in node.attr or (
                    "append_op" in node.attr and node.attr["append_op"] != "tanh"
                ):
                    weight = node.input_tensors[1]
                    if (
                        type(weight.data) == np.ndarray
                        and (
                            weight.data.dtype == "int8" or weight.data.dtype == "uint8"
                        )
            ...
            if node.op_type == "Input":
                for tensor in node.output_tensors:
                    if not isinstance(tensor.data, np.ndarray):
                        ...
                        input = np.random.uniform(low=0, high=10, size=shape).astype(
                            dtype
                        )
                        inputs.append(input)
                        id += 1
    ...
    def transpose_mode_int8(self, node_name_list=None):
        ...
        def _modify_attr_perm(node):
            ...
            if (
                node.attr.get("src0_perm") == "0,2,1,3"
                and node.attr.get("src1_perm") == "0,2,3,1"
            ):
                node.attr["src0_perm"] = "2,0,3,1"
                node.attr["src1_perm"] = "2,0,1,3"
            if (
                node.attr.get("src1_perm") == "0,2,1,3"
                and node.attr.get("dst_perm") == "0,2,1,3"
            ):
                ...
                if "reshape" in node.attr and "reshape_dims" in node.attr:
                    ...
                    if len(value_list) == 4:
                        new_attr = OrderedDict(
                            {"src_perm": "0,1,2,3", "dst_perm": "2,3,0,1"}
                        )
                    else:
        ...
        def _reorder_node_insert(node, idx, insert_pos=None):
            ...
            reorder_output_tensor = Tensor(
                name=input_name + "_reorder",
            ...
            reorder_node = util.construct_node(
                node_name=reorder_node_name,
                op_type="Reorder",
        ...
        def _swap_innertproduct_input(
            node, data_swap_list=[0, 1], swap_list_2=[], swap_list_3=[]
        ):
            ...
            node.input_tensors[input_1].shape = list(
                node.input_tensors[input_1].data.shape
            )
        ...
        def _reorder_recover_node_insert(node, pre_node=None):
            ...
            reorder_output_tensor = Tensor(
                name=pre_node_output.name + "_recover",
            ...
            reorder_recover_node = util.construct_node(
                node_name=reorder_recover_node_name,
                op_type="Reorder",
        ...
        def _node_name_list_convert(node_name_list):
            ...
            for node_name in node_name_list:
                ...
                if (
                    node_type == "add_innerproduct_0"
                    or node_type == "mul_innerproduct_0"
                ):
        ...
        def _search_layernorm_fusion_node(
            ffn_lin_node_name_list, start_idx, range, node_name_list=[]
        ):
            ...
            for node_name in ffn_lin_node_name_list[start_idx:][::range]:
                ...
                if node_name_list != []:
                    split_node_name = (
                        self.get_node_by_name(node_name).output_tensors[0].dest_op[0]
                    )
                ...
                layernorm_node_name = (
                    self.get_node_by_name(node_name).input_tensors[3].source_op[0]
                )
        ...
        def _consecutive_reorder_fusion():
            ...
            for node in self._nodes:
                if (
                    node.op_type == "Reorder"
                    and "recover_reorder" in node.output_tensors[0].name
                ):
                    if "Reorder_Post" in node.name:
                        pre_node = self.get_node_by_name(
                            node.input_tensors[0].source_op[0]
                        )
                        if (
                            "Reorder_Recover" in pre_node.name
                            and pre_node.op_type == "Reorder"
                        ):
                            post_node = self.get_node_by_name(
                ...
                if node.op_type == "Reorder" and "Reorder_Post" in node.name:
                    ...
                    if pre_node.op_type == "Quantize":
                        ...
                        if layernorm_node.op_type != "LayerNorm":
                            logger.warning(
                                "The node.op_type = {} is not expected.".format(
                        ...
                        if (
                            "Reorder_Post"
                            in layernorm_node.output_tensors[0].dest_op[0]
                        ):
                            ...
                            layernorm_node.output_tensors[0].dest_op.append(
                                reorder_node.name
                            )
                            reorder_node.output_tensors[0].dest_op.append(
                                attention_out_node.name
                            )
        ...
        def _reorder_recover_fusion():
            ...
            for node in self._nodes:
                ...
                if (
                    node_type == "add_innerproduct_0"
                    and node.name in QKV_node_name_list
                ):
                    post_node = self.get_node_by_name(node.output_tensors[0].dest_op[0])
                    ...
                    if "Reorder_Recover" in post_node.name:
                        ...
                        if _innerproduct_type_check(target_node) == "matmul_0":
                            ...
                            def _transpose_and_matmul_nodes_modification(node):
                                ...
                                if (
                                    "Reorder_Post" in reorder_node.name
                                    and innerproduct_node.op_type == "InnerProduct"
                                ):
        ...
        def _layernorm_reorder_fusion():
            ...
            for node in self._nodes:
                if node.op_type == "LayerNorm" and node.name in layernorm_fusion_node:
                    ...
                    if (
                        "Reorder_Recover" in reorder_recover_node.name
                        and "Reorder_Post" in reorder_post_node.name
                    ):
        ...
        def _merged_matmul_fusion():
            ...
            def _transpose_matmul_modification(node):
                ...
                if attention_out_node.op_type == "InnerProduct":
                    ...
                    logger.warning(
                        "The attention_out_node.op_type = {} is not expected".format(
                ...
                if "Reorder_Recover" in reorder_recover_node.name:
                    ...
                    def mergedQK_modify(split_node):
                        ...
                        if v.op_type == "InnerProduct":
                            recover_node = self.get_node_by_name(
                                v.output_tensors[0].dest_op[0]
                            )
                        ...
                        post_node_of_v = self.get_node_by_name(
                            v.output_tensors[0].dest_op[0]
                        )
                            ...
                            if post_node.op_type == "Reshape":
                                ...
                                if matmul_node.op_type == "Matmul":
                                    ...
                                    if post_node_of_matmul.op_type == "Reorder":
                                        _del_current_node_and_modify_post_node(
                                            post_node_of_matmul
                                        )
                                else:
                                    logger.warning(
                                        "The post_node_of_reshape is not Matmul."
                                    )
        ...
        logger.debug(
            "merge_matmul_node_name_list = {}".format(merge_matmul_node_name_list)
        )
        ...
        layernorm_fusion_node = _search_layernorm_fusion_node(
            ffn_lin_node_name_list, 1, 2
        )
        layernorm_fusion_node += _search_layernorm_fusion_node(QKV_node_name_list, 3, 4)
        if merge_matmul_node_name_list != []:
            layernorm_fusion_node += _search_layernorm_fusion_node(
                merge_matmul_node_name_list, 0, 1, node_name_list
            )
    ...
    def _activation_mem_compression(self, input_shapes_list):
        if self._engine and input_shapes_list and len(input_shapes_list) > 0:
            ...
            logger.warning(
                "Skip activation_mem_compression due to empty engine model or "
                "empty input_shapes."
            )
