            ...
            if len(self._input_tensors) == 2:
                ...
                if const_idx != -1:
                    if len(self._input_tensors[const_idx].data.shape) == 0:
                        ...
                        self._input_tensors[const_idx].data = self._input_tensors[
                            const_idx
                        ].data.reshape(1)
                    ...
                    if const_data.dtype in [np.int64, np.int32]:
                        if all(const_data >= np.finfo(np.float32).min) and all(
                            const_data <= np.finfo(np.float64).max
                        ):
                            self._input_tensors[const_idx].data = const_data.astype(
                                np.float32
                            )
                        else:
                            logger.warning(
                                "Cannot convert const data into fp32 dtype in op type"
