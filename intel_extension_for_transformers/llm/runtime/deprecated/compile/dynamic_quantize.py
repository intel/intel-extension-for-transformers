...
def _insert_quantize(graph: Graph, per_token: bool):
    def quantize_src_tensor(input_tensor_idx: int, op_type: str):
        ...
        node_name = (
            node.input_tensors[input_tensor_idx].name + "_quant" + str(input_tensor_idx)
        )
        quant_output = Tensor(
            name=input_tensor.name + "_quant",
            source_op=[node_name],
            dest_op=[node.name],
            dtype=quantize_type,
        )
        quant_min = Tensor(
            name=input_tensor.name + "_min",
            source_op=[node_name],
            dest_op=[node.name],
            dtype="fp32",
        )
        quant_scale = Tensor(
            name=input_tensor.name + "_scale",
            source_op=[node_name],
            dest_op=[node.name],
            dtype="fp32",
        )
        per_token_ = node.op_type in ["InnerProduct", "Matmul"] and per_token
        quantize_op = construct_node(
            node_name=node_name,
            op_type="Quantize",
    ...
    def quantize_weight_tensor(weight_tensor: Tensor, op_type: str):
        ...
        tensor_min = Tensor(
            name=node.input_tensors[1].name + "_min",
            source_op=[],
            dest_op=[node.name],
            dtype="fp32",
        )
        tensor_scale = Tensor(
            name=node.input_tensors[1].name + "_scale",
            source_op=[],
            dest_op=[node.name],
            dtype="fp32",
        )
        ...
        ), "The graph to be dynamic quantized should be load with weight"
        if op_type == "Convolution":
            ...
            tensor_scale.data = np.where(
                tensor_scale.data == 0.0, 0.0, tensor_scale.data / 127.0
            )
            weight_tensor.data = np.round(
                weight_tensor.data / tensor_scale.data
            ).astype(np.int8)
        else:
            if weight_perm == "0,1":
                ...
                tensor_scale.data = np.maximum(
                    np.fabs(tensor_min.data), np.fabs(max_data)
                )
                tensor_scale.data = np.where(
                    tensor_scale.data == 0.0, 0.0, tensor_scale.data / 127.0
                )
                weight_tensor.data = (
                    np.round((weight_tensor.data).T / tensor_scale.data).astype(np.int8)
                ).T
            else:
    ...
    for node in filter(
        lambda x: x.op_type in ["InnerProduct", "Matmul", "Convolution"],
...
def _fuse_quatize(graph: Graph):
    pattern = {
        "patterns": {"in": [[(0, "ANY"), (1, "Quantize")]], "out": [[(0, "ANY")]]},
        "search_mode": "op_type",
        "node_names": {0: 0},
        "input_tensors": {0: [[{0: [0]}], [[0], 1]]},
        "output_tensors": {0: [[{1: [0]}, {1: [1]}, {1: [2]}], [[0, 1, 2], 3]]},
        "returns": [0, 1],
    }
    ...
    for any_op in ["InnerProduct", "Matmul", "Softmax", "Convolution"]:  # ,"LayerNorm"
        ...
        graph, new_node_names, ret_old_nodes = pattern_mapping(
            "fuse_quatize", now_pattern, graph
        )
        if len(new_node_names) != 0:
            for idx in range(len(new_node_names)):
                new_node_idx = graph.get_node_id(new_node_names[idx][0])
                graph.nodes[new_node_idx].input_tensors = ret_old_nodes[idx][
                    0
                ].input_tensors
...
def _fuse_mha(graph: Graph):
    pattern = {
        ...
        "node_names": {0: 0},
        "input_tensors": {0: [[{0: [0]}], [[0], 1]]},
        "output_tensors": {0: [[{2: [0]}, {2: [1]}, {2: [2]}], [[0, 1, 2], 3]]},
        "returns": [0, 2],
    }
    ...
    if len(new_node_names) != 0:
        for idx in range(len(new_node_names)):
            ...
            if (
                len(qkmatmul.input_tensors) > 2
                and qkmatmul.input_tensors[2].source_op
                and graph.get_node_by_name(
                    qkmatmul.input_tensors[2].source_op[0]
                ).op_type
                == "PaddingSequence"
            ):
                # graph.get_node_by_name(qkmatmul.input_tensors[2].source_op[0]).op_type = "SequenceLength"
                graph.get_node_by_name(qkmatmul.input_tensors[2].source_op[0]).attr = {
                    "dst_shape": "-1,-1"
                }
            ...
            if "reshape_dims" in avmatmul.attr:
                graph.nodes[new_node_idx].input_tensors.append(
                    avmatmul.input_tensors[-1]
                )
            # set attr
            ...
            if "output_scale" in qkmatmul.attr:
                graph.nodes[new_node_idx].attr["output_scale"] = qkmatmul.attr[
                    "output_scale"
                ]
...
def _quantize2bf16(graph):
    ...
    pattern = {
        ...
        "search_mode": "op_type",
        "node_names": {0: 0},
        "input_tensors": {0: [[{0: [0]}, {0: [1]}, {0: [2]}], [[0, 1, 2], 3]]},
...
def _dynamic_quantization(base_model, per_token=True):
    ...
    if isinstance(base_model, Graph):
        ...
        graph = Graph()
        graph.graph_init(
            base_model + "/conf.yaml", base_model + "/model.bin", load_weight=True
        )
