...
parser.add_argument(
    "--modeltype", type=int, default=0, help="modeltype: 0 pytorch model, 1 nn model"
)
parser.add_argument("--path", type=str, help="please input your path")
        ...
        for root, dirs, files in os.walk(args.path):
            ...
            with open(args.path + "output4x1.txt", "w") as f:
                ...
                for name in checkpoint:
                    if (
                        "weight" in name and len((checkpoint[name]).shape) > 1
                    ):  # and name.find("weight") > 0
                        ...
                        if len(tensor.size()) > 1:
                            if (
                                tensor.size()[0] % block[0] == 0
                                and tensor.size()[1] % block[1] == 0
                            ):
                                block_num = tensor_size // block[0] // block[1]
                                block_one, hotmap = cal_sparse_pytorch(tensor, block)
                                block_zero = (
                                    tensor_size // block[0] // block[1]
                                ) - block_one
                                block_sparse = block_zero / (
                                    tensor_size / block[0] / block[1]
                                )
                                print(
                                    name,
                                ...
                                f.write(
                                    "{}:{},{}\t{}".format(
                                        name, elt_zero, elt_sparse, tensor.size()
                                    )
                                )
        ...
        with open(os.path.join(ir_path, "conf.yaml")) as conf_file:
            ...
            with open(os.path.join(ir_path, "model.bin"), "rb") as weight_file:
                for _, value in filter(
                    lambda tmp: tmp[-1]["type"] == "InnerProduct", operators.items()
                ):
                    tensors = value["input"]
                    for tensor_name in filter(
                        lambda tensor_name: tensor_name in inputs, tensors.keys()
                    ):
                        ...
                        if (
                            len(shape) == 2
                            and shape[0] % block[0] == 0
                            and shape[1] % block[1] == 0
                        ):
                            size = 1
                            for i in shape:
                                size *= i
                            ...
                            for i in range(size):
                                tmp = struct.unpack_from(
                                    type_map[dtype][0], data_b, i * type_map[dtype][1]
                                )
                                data.append(tmp[0])
                            ...
                            print(
                                os.path.join(ir_path, tensor_name),
                                elt_zero,
                                size,
                                elt_sparse,
                                block_zero,
