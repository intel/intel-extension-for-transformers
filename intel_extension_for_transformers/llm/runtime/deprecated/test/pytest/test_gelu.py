...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.gelu import (
    Gelu,
)
...
class TestGelu(unittest.TestCase):
    ...
    def test_gelu_1(self):
        ...
        div_node.construct(
            "div", "Div", input_tensors=input_tensors, output_tensors=output_tensors
        )
        ...
        erf_node.construct(
            "erf", "Erf", input_tensors=input_tensors, output_tensors=output_tensors
        )
        ...
        output_tensors = [
            Tensor(name="mul_1:0", source_op=["mul_1"], dest_op=["mul_2"])
        ]
        ...
        graph.insert_nodes(
            len(graph.nodes),
            [input_data_node, div_node, erf_node, add_node, mul_1_node, mul_2_node],
        )
        graph = Gelu()(graph)
