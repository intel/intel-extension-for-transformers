...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.attention_reshape import (
    AttentionReshape,
)
...
class TestAttentionReshape(unittest.TestCase):
    ...
    def test_attention_reshape_0(self):
        ...
        input_tensors = [
            Tensor(name="b_src0", dest_op=["batchmatmul"]),
            Tensor(name="b_src1", dest_op=["batchmatmul"]),
        ]
        output_tensors = [
            Tensor(
                name="batchmatmul:0", source_op=["batchmatmul"], dest_op=["transpose"]
            )
        ]
        batchmatmul_node.construct(
            "batchmatmul",
            "BatchMatMul",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"src_perm": "0,1,2,3", "dst_perm": "0,2,1,3"}),
        )
        ...
        input_tensors = [
            Tensor(name="transpose:0", source_op=["transpose"], dest_op=["shape"])
        ]
        output_tensors = [
            Tensor(name="shape:0", source_op=["shape"], dest_op=["gather"])
        ]
        ...
        output_tensors = [
            Tensor(name="gather:0", source_op=["gather"], dest_op=["unsqueeze"])
        ]
        gather_node.construct(
            ...
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"batch_dims": "0", "axis": "0"}),
        )
        ...
        input_tensors = [
            Tensor(name="concat0", data=np.array(1), shape=[1, 384]),
            Tensor(name="unsqueeze:0", source_op=["unsqueeze"], dest_op=["concat"]),
            Tensor(name="concat2", shape=[1, 384], data=np.array(1)),
        ]
        output_tensors = [
        ...
        output_tensors = [
            Tensor(name="reshape:0", source_op=["reshape"], dest_op=["matmul"])
        ]
        ...
        input_tensors = [
            Tensor(name="reshape:0", source_op=["reshape"], dest_op=["matmul"]),
            Tensor(name="matmul1", data=np.array(1)),
            Tensor(name="matmul2", data=np.array(1)),
        ]
        ...
        matmul_node.construct(
            ...
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"src1_perm": "0,1"}),
        )
    ...
    def test_attention_reshape_1(self):
        ...
        reshape_node.construct(
            ...
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"dst_shape": "0,0,768"}),
        )
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, reshape_node, matmul_node]
        )
        graph = AttentionReshape()(graph)
