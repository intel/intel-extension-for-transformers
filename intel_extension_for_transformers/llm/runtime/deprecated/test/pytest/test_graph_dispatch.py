...
class TestGraphDispatch(unittest.TestCase):
    ...
    def test_graph_dispatch(self):
        ...
        int8_model_path = (
            "/tf_dataset2/inc-ut/nlptoolkit_ut_model/bert_mini_int8_original_IR"
        )
        if is_win():
            int8_model_path = (
        ...
        self.assertTrue(
            os.path.exists(int8_model_path),
            "INT8 IR model is not found, please set your own model path!",
        )
        int8_model = Graph()
        int8_model.graph_init(
            int8_model_path + "/conf.yaml",
            int8_model_path + "/model.bin",
            load_weight=True,
        )
        ...
        int8_dispatch_output = copy.deepcopy(
            list(int8_dispatch_output_dict.values())[0]
        )
        # compare outputs
        ...
        fp32_dispatch_output = copy.deepcopy(
            list(fp32_dispatch_output_dict.values())[0]
        )
        # compare outputs
