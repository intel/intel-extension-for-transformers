...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.dynamic_quantize import (
...
class TestDynamicQuantization(unittest.TestCase):
    ...
    def test_dynamic_quantization(self):
        ...
        output_tensors = [
            Tensor(
                name="qk", source_op=["qk_matmul"], dest_op=["softmax"], dtype="fp32"
            )
        ]
        qk_node.construct(
            "qk_matmul",
        ...
        input_tensors = [
            Tensor(
                name="a", source_op=["softmax"], dest_op=["av_matmul"], dtype="fp32"
            ),
            Tensor(name="v", source_op=["V"], dest_op=["av_matmul"], dtype="fp32"),
        ]
        output_tensors = [
        ...
        av_node.construct(
            ...
            output_tensors=output_tensors,
            attr=OrderedDict(
                {"src1_perm": "0,2,1,3", "dst_perm": "0,2,1,3", "reshape": "-1,256"}
            ),
        )
        ...
        output_tensors = [
            Tensor(name="att", source_op=["Att"], dest_op=["output_data"])
        ]
        Att_node.construct(
            "Att",
            "InnerProduct",
