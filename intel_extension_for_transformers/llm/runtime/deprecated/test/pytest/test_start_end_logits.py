...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.start_end_logits import (
    StartEndLogits,
)
...
class TestStartEndLogits(unittest.TestCase):
    ...
    def test_start_end_logits(self):
        ...
        output_tensors = [
            Tensor(name="transpose:0", source_op=["transpose"], dest_op=["unpack"])
        ]
        ...
        output_tensors = [
            Tensor(name="unpack:0", source_op=["unpack"], dest_op=["identity"])
        ]
        unpack_node.construct(
            "unpack",
            "Unpack",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
        ...
        graph.insert_nodes(
            len(graph.nodes),
            [input_data_node, transpose_node, unpack_node, identity_node],
        )
