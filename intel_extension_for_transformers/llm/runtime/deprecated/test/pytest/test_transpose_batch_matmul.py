...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.transpose_batch_matmul import (
...
class TestTransposeBatchMatMul(unittest.TestCase):
    ...
    def test_transpose_batch_matmul_1(self):
        ...
        output_tensors = [
            Tensor(
                name="transpose_1:0",
                source_op=["transpose_1"],
                dest_op=["fused_matmul"],
            )
        ]
        transpose_1_node.construct(
            "transpose_1",
            "Transpose",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"dst_perm": "0,2,1,3"}),
        )
        ...
        output_tensors = [
            Tensor(
                name="transpose_2:0",
                source_op=["transpose_2"],
                dest_op=["fused_matmul"],
            )
        ]
        transpose_2_node.construct(
            "transpose_2",
            "Transpose",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"dst_perm": "0,2,3,1"}),
        )
        ...
        output_tensors = [
            Tensor(name="fused_matmul:0", source_op=["fused_matmul"], dest_op=["add"])
        ]
        fused_matmul_node.construct(
            "fused_matmul",
            "FusedMatMul",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
    ...
    def test_transpose_batch_matmul_2(self):
        ...
        input_tensors = [Tensor(data=np.array(1))]
        output_tensors = [
            Tensor(name="transpose_1:0", source_op=["transpose_1"], dest_op=["matmul"])
        ]
        transpose_1_node.construct(
            "transpose_1",
            "Transpose",
        ...
        input_tensors = [
            Tensor(name="transpose_1:0", source_op=["transpose_1"], dest_op=["matmul"]),
            Tensor(name="m_src1:0"),
        ]
        output_tensors = [
            Tensor(name="matmul:0", source_op=["matmul"], dest_op=["transpose_2"])
        ]
        ...
        graph.insert_nodes(
            len(graph.nodes),
            [input_data_node, transpose_1_node, matmul_node, transpose_2_node],
        )
        graph = TransposeBatchMatMul()(graph)
        self.assertEqual(2, len(graph.nodes))
        self.assertEqual("0,2,1,3", graph.nodes[1].attr["src0_perm"])
    ...
    def test_transpose_batch_matmul_3(self):
        ...
        output_tensors = [
            Tensor(name="matmul:0", source_op=["matmul"], dest_op=["transpose"])
        ]
        matmul_node.construct(
            "matmul",
            "BatchMatMul",
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, matmul_node, transpose_node]
        )
        graph = TransposeBatchMatMul()(graph)
        self.assertEqual(2, len(graph.nodes))
        self.assertEqual("0,2,3,1", graph.nodes[1].attr["dst_perm"])
