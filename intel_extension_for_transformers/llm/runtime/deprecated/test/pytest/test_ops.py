...
class TestOps(unittest.TestCase):
    ...
    def test_fused_batch_matmul_v2(self):
        ...
        fused_batch_matmul_v2_node.attr["adj_x"].CopyFrom(
            attr_value_pb2.AttrValue(b=False)
        )
        fused_batch_matmul_v2_node.attr["adj_y"].CopyFrom(
            attr_value_pb2.AttrValue(b=True)
        )

        fused_batch_matmul_v2_node_test = OPERATORS["_FusedBatchMatMulV2"]()
        fused_batch_matmul_v2_node_test.set_attr(
            "tensorflow", fused_batch_matmul_v2_node
        )
    ...
    def test_fused_batch_norm_v3(self):
        ...
        fused_batch_norm_v3_node.attr["is_training"].CopyFrom(
            attr_value_pb2.AttrValue(b=True)
        )
        ...
        exponential_avg_factor = fused_batch_norm_v3_node_test.attr[
    ...
    def test_gather_onnx(self):
        ...
        gather_node_test.input_tensors = [
            Tensor(name="src"),
    ...
    def test_squeeze_tensorflow(self):
        ...
        squeeze_node.attr["squeeze_dims"].CopyFrom(
    ...
    def test_iter_dataset_related(self):
        ...
        attr = {"output_shapes": a(b(shape_list)), "output_types": a(e([3]))}
        fake_node = f(attr)
    ...
    def test_quantized_fused_matmul_and_dequantize(self):
        ...
        quantized_fused_matmul_and_dequantize_node.name = (
            "quantized_fused_matmul_and_dequantize"
        )
        quantized_fused_matmul_and_dequantize_node.op = (
        ...
        quantized_fused_matmul_and_dequantize_node_test.set_attr(
            "tensorflow", quantized_fused_matmul_and_dequantize_node
        )
        ...
        output_dtype = quantized_fused_matmul_and_dequantize_node_test.attr[
            "output_dtype"
        ]
        self.assertEqual("1,0", src0_perm)
    ...
    def test_quantized_matmul_with_bias_and_dequantize(self):
        ...
        quantized_matmul_with_bias_and_dequantize_node.op = (
        ...
        quantized_matmul_with_bias_and_dequantize_node_test.set_attr(
            "tensorflow", quantized_matmul_with_bias_and_dequantize_node
        )
        transpose_a = quantized_matmul_with_bias_and_dequantize_node_test.attr[
            "transpose_a"
        ]
        transpose_b = quantized_matmul_with_bias_and_dequantize_node_test.attr[
            "transpose_b"
        ]
        output_dtype = quantized_matmul_with_bias_and_dequantize_node_test.attr[
            "output_dtype"
        ]
        self.assertTrue(transpose_a)
        self.assertFalse(transpose_b)
    ...
    def test_layer_normalization(self):
        ...
        for op_type in op_type_list:
            ...
            layer_normalization_node_test.set_attr(
                "onnxruntime", layer_normalization_node
            )
    ...
    def test_one_hot(self):
        ...
        one_hot_node_test.input_tensors = [
            Tensor(),
            Tensor(data=0),
            Tensor(data=1),
            Tensor(data=2),
    ...
    def test_onnx_input(self):
        ...
        fake_node = a(
            ...
            type=b(
                c(
                    shape=d(
                        [
                            p(dim_param="seq_len", dim_value=None),
                            p(dim_param="", dim_value=32),
                        ]
                    ),
                    elem_type=7,
                )
        ...
        g = Graph()
        onnx_input_node_test.extract(
            "onnxruntime", onnx_input_node, None, fake_nodes_dict, g
        )
