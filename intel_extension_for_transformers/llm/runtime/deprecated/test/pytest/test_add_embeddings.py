...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.add_embeddings import (
    AddEmbeddings,
)
...
class TestAddEmbeddings(unittest.TestCase):
    ...
    def test_add_embeddings_with_seq_len_first(self):
        ...
        output_tensors = [
            Tensor(name="add:0", source_op=["add"], dest_op=["transpose"])
        ]
        add_node.construct(
        ...
        output_tensors = [
            Tensor(name="transpose:0", source_op=["transpose"], dest_op=["layernorm"])
        ]
        ...
        output_tensors = [
            Tensor(name="layernorm:0", source_op=["layernorm"], dest_op=[])
        ]
        ln_node.construct(
            "layernorm",
            "LayerNorm",
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, add_node, transpose_node, ln_node]
        )
