...
class TestMain(unittest.TestCase):
    ...
    def test_main(self):
        ...
        model.rename_node(
            "bert/encoder/layer_0/attention/self/query/BiasAdd", "test_name"
        )
        ...
        model.change_node_input_tensors(
            "test_name",
            0,
            tensor=Tensor(name="input_ids:0", source_op=["input_data"]),
            mode="insert",
        )
        self.assertEqual(4, len(model.nodes[new_name_idx].input_tensors))

        model.change_node_output_tensors(
            "test_name",
            1,
            tensor=Tensor(name="input_ids:0", source_op=["input_data"]),
            mode="insert",
        ...
        self.assertEqual(
            "input_ids:0", model.nodes[new_name_idx].output_tensors[0].name
        )
