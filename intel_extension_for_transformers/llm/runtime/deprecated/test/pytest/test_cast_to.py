...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.cast_to import (
    CastTo,
)
...
class TestCastTo(unittest.TestCase):
    ...
    def test_softmax_cast(self):
        ...
        output_tensors = [
            Tensor(name="softmax:0", source_op=["softmax"], dest_op=["cast"])
        ]
        softmax_node.construct(
            "softmax",
            "Softmax",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
        ...
        output_tensors = [Tensor(name="cast:0", source_op=["cast"], dest_op=[])]
        cast_node.construct(
            "cast",
            "Cast",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"DstT": "fp32"}),
        )
    ...
    def test_greater_cast_reducesum(self):
        ...
        input_tensors = [
            Tensor(name="g_src0"),
            Tensor(name="g_src1", data=np.array([1]).astype("int64"), shape=[1]),
        ]
        output_tensors = [
            Tensor(name="greater:0", source_op=["greater"], dest_op=["cast"])
        ]
        greater_node.construct(
            "greater",
            "Greater",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
        ...
        output_tensors = [
            Tensor(name="cast:0", source_op=["cast"], dest_op=["reducesum"])
        ]
        cast_node.construct(
            "cast",
            "Cast",
        ...
        output_tensors = [
            Tensor(name="reducesum:0", source_op=["reducesum"], dest_op=[])
        ]
        rs_node.construct(
            "reducesum",
            "ReduceSum",
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, greater_node, cast_node, rs_node]
        )
        graph = CastTo()(graph)
    ...
    def test_range_cast_less(self):
        ...
        input_tensors = [
            Tensor(name="range:0", source_op=["range"], dest_op=["less"]),
            Tensor(name="less_val", data=np.array([3]).astype("int64")),
        ]
        output_tensors = [Tensor(name="less:0", source_op=["less"], dest_op=[])]
        less_node.construct(
            "less", "Less", input_tensors=input_tensors, output_tensors=output_tensors
        )

        graph.insert_nodes(
            len(graph.nodes), [input_data_node, cast_node, range_node, less_node]
        )
        graph = CastTo()(graph)
        self.assertEqual(3, len(graph.nodes))
