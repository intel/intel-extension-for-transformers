...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.token_type_embeddings import (
    TokenTypeEmbeddings,
)
...
class TestTokenTypeEmbeddings(unittest.TestCase):
    ...
    def test_token_type_embeddings_1(self):
        ...
        output_tensors = [
            Tensor(name="reshape_1:0", source_op=["reshape_1"], dest_op=["one_hot"])
        ]
        reshape_1_node.construct(
            "reshape_1",
            "Reshape",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
        ...
        output_tensors = [
            Tensor(name="one_hot:0", source_op=["one_hot"], dest_op=["matmul"])
        ]
        one_hot_node.construct(
            "one_hot",
            "OneHot",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"test": 1}),
        )
        ...
        input_tensors = [
            Tensor(name="one_hot:0", source_op=["one_hot"], dest_op=["matmul"]),
            Tensor(
                name="embeddings", shape=[2, 768], dest_op=["matmul"], data=np.array(1)
            ),
        ]
        output_tensors = [
            Tensor(name="matmul:0", source_op=["matmul"], dest_op=["reshape_2"])
        ]
        matmul_node.construct(
            "matmul",
            "MatMul",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"transpose_a": False, "transpose_b": False}),
        )
        ...
        output_tensors = [
            Tensor(
                name="strided_slice:0", source_op=["strided_slice"], dest_op=["pack"]
            )
        ]
        ...
        pack_node.construct(
            "pack", "Pack", input_tensors=input_tensors, output_tensors=output_tensors
        )
    ...
    def test_token_type_embeddings_2(self):
        ...
        output_tensors = [
            Tensor(),
            Tensor(name="segment_ids", dest_op=["gather"]),
            Tensor(),
        ]
        input_data_node.construct(
            "input_data",
        ...
        input_tensors = [
            Tensor(shape=[2, 768], data=np.array(1)),
            Tensor(name="segment_ids", source_op=["input_data"], dest_op=["gather"]),
        ]
        output_tensors = [
            Tensor(name="gather:0", source_op=["gather"], dest_op=["add"])
        ]
        gather_node.construct(
            ...
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"axis": 0, "batch_dims": 0}),
        )
        ...
        output_tensors = [
            Tensor(name="add:0", source_op=["add"], dest_op=["layer_norm"])
        ]
        add_node.construct(
        ...
        output_tensors = [
            Tensor(name="layer_norm:0", source_op=["layer_norm"], dest_op=[])
        ]
        ln_node.construct(
            "layer_norm",
            "LayerNorm",
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, gather_node, add_node, ln_node]
        )
        graph = TokenTypeEmbeddings()(graph)
