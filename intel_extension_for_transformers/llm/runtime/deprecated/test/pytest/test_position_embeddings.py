...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.position_embeddings import (
    PositionEmbeddings,
)
...
class TestPositionEmbeddings(unittest.TestCase):
    ...
    def test_position_embeddings_1(self):
        ...
        input_tensors = [
            Tensor(shape=[512, 768], data=np.array(1)),
            Tensor(data=np.array(1)),
        ]
        output_tensors = [
            Tensor(name="slice:0", source_op=["slice"], dest_op=["reshape"])
        ]
        slice_node.construct(
            "slice", "Slice", input_tensors=input_tensors, output_tensors=output_tensors
        )
        ...
        output_tensors = [
            Tensor(name="less_equal:0", source_op=["less_equal"], dest_op=["all"])
        ]
        less_equal_node.construct(
            "less_equal",
            "LessEqual",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
        ...
        output_tensors = [Tensor(name="all:0", source_op=["all"], dest_op=["assert"])]
        all_node.construct(
            "all", "All", input_tensors=input_tensors, output_tensors=output_tensors
        )

        assert_node = OPERATORS["Assert"]()
        input_tensors = [Tensor(name="all:0", source_op=["all"], dest_op=["assert"])]
        output_tensors = [Tensor(name="assert:0", source_op=["assert"], dest_op=[])]
        assert_node.construct(
            "assert",
            "Assert",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
