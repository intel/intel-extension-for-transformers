...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.matmul_with_bias_sigmoid import (
...
class TestMatMulWithBiasSigmoid(unittest.TestCase):
    ...
    def test_matmul_with_bias_relu_1(self):
        ...
        output_tensors = [
            Tensor(name="matmul:0", source_op=["matmul"], dest_op=["sigmoid"])
        ]
        mat_node.construct(
            "matmul",
            "MatMulWithBias",
        ...
        output_tensors = [Tensor(name="sigmoid:0", source_op=["sigmoid"], dest_op=[])]
        tanh_node.construct(
            "sigmoid",
            "Sigmoid",
            input_tensors=input_tensors,
