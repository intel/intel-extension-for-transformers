...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.operator_adaptor import (
    OperatorAdaptor,
)
...
class TestOperatorAdaptor(unittest.TestCase):
    ...
    def test_gather_sweep(self):
        ...
        input_tensors = [
            Tensor(name="g2_idx:0", data=np.array(0).astype("int32")),
            Tensor(name="g2_src1"),
        ]
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, gather1_node, gather2_node]
        )
    ...
    def test_reshape_non_2d_src_before_inner_product(self):
        ...
        output_tensors = [
            Tensor(name="transpose:0", source_op=["transpose"], dest_op=["matmul"])
        ]
        transpose_node.construct(
            ...
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"dst_perm": "1,0,2"}),
        )
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, transpose_node, matmul_node]
        )
        graph = OperatorAdaptor()(graph)
