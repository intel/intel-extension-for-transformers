...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.last_layer_shape import (
    LastLayerShape,
)
...
class TestLastLayerShape(unittest.TestCase):
    ...
    def test_last_layer_shape_1(self):
        ...
        output_tensors = [
            Tensor(name="pack:0", source_op=["pack"], dest_op=["reshape"])
        ]
        pack_node.construct(
        ...
        output_tensors = [
            Tensor(name="reshape:0", source_op=["reshape"], dest_op=["strided_slice"])
        ]
        ...
        output_tensors = [
            Tensor(
                name="strided_slice:0", source_op=["strided_slice"], dest_op=["squeeze"]
            )
        ]
