...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.qkv_reshape import (
    QKVReshape,
)
...
class TestAttentionReshape(unittest.TestCase):
    ...
    def test_attention_reshape_1(self):
        ...
        output_tensors = [
            Tensor(name="matmul:0", source_op=["matmul"], dest_op=["reshape"])
        ]
        ...
        reshape_node.construct(
            ...
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"dst_shape": "0,0,12,64"}),
        )

        graph.insert_nodes(
            len(graph.nodes), [input_data_node, matmul_node, reshape_node]
        )
