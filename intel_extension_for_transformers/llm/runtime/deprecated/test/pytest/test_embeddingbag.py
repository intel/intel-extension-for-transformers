...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.embeddingbag import (
    EmbeddingBag,
)
...
class TestEmbeddingBag(unittest.TestCase):
    ...
    def test_embeddingbag_1(self):
        ...
        output_tensors = [
            Tensor(name="matmul:0", source_op=["matmul"], dest_op=["relu"])
        ]
        ...
        input_tensors = [
            Tensor(name="matmul:0", source_op=["matmul"], dest_op=["relu"]),
            Tensor(name="matmul1:0", source_op=["matmul"], dest_op=["relu"]),
            Tensor(name="matmul2:0", source_op=["matmul"], dest_op=["relu"]),
        ]
        output_tensors = [Tensor(name="relu:0", source_op=["relu"], dest_op=[])]
        embeddingbag_node.construct(
            "relu",
            "EmbeddingBag",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
        ...
        graph.insert_nodes(
            len(graph.nodes), [input_data_node, squeeze_node, embeddingbag_node]
        )
