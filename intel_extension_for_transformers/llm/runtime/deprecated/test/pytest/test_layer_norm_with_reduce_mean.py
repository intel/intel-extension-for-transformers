...
from intel_extension_for_transformers.llm.runtime.deprecated.compile.sub_graph.layer_norm_with_reduce_mean import (
...
class TestLayerNormWithReduceMean(unittest.TestCase):
    ...
    def test_layer_norm_with_reduce_mean_1(self):
        ...
        output_tensors = [
            Tensor(
                name="layer_norm:0", source_op=["layer_norm"], dest_op=["reduce_mean"]
            )
        ]
        ln_node.construct(
            "layer_norm",
        ...
        output_tensors = [
            Tensor(name="reduce_mean:0", source_op=["reduce_mean"], dest_op=[])
        ]
        reduce_mean_node.construct(
            "reduce_mean",
            "ReduceMean",
            input_tensors=input_tensors,
            output_tensors=output_tensors,
            attr=OrderedDict({"axis": 1, "keep_dims": False}),
        )

        graph.insert_nodes(
            len(graph.nodes), [input_data_node, ln_node, reduce_mean_node]
        )
