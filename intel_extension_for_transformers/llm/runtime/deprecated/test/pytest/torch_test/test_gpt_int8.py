...
class TestTorchModel(unittest.TestCase):
    ...
    def test_1(self):
        ...
        past_key_value_torch = tuple(
            [
                (torch.zeros([1, 16, 32, 256]), torch.zeros([1, 16, 32, 256]))
                for i in range(28)
            ]
        )
        past_key_value = tuple(
            [
                (torch.zeros([1, 32, 16, 256]), torch.zeros([1, 32, 16, 256]))
                for i in range(28)
            ]
        )
        ...
        ref_out = traced_model(
            input_ids, past_key_value_torch, attention_mask, position_ids
        )
