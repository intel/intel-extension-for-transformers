...
class SqrtTest : public testing::TestWithParam<TestParams> {
 protected:
  SqrtTest() {}
  ~SqrtTest() {}
  void SetUp() override {}
...
std::pair<OpArgs, OpArgs> GenerateFp32Case(const std::vector<std::vector<int64_t>>& input_shape) {
  ...
  auto op_config = std::make_shared<OperatorConfig>("sqrt", "Sqrt", input_config, output_config, op_attr);
...
INSTANTIATE_TEST_SUITE_P(Prefix, SqrtTest, CasesFp32());
