...
#include "../../include/operators/constantofshape.hpp"
...
bool CheckResult(const TestParams& t) {
  ...
  try {
    executor::ConstantOfShapeOperator ConstantOfShape(p.conf);
    ConstantOfShape.Reshape(p.input, p.output);
    ConstantOfShape.Forward(p.input, p.output);
...
class ConstantOfShapeTest : public testing::TestWithParam<TestParams> {
 protected:
  ConstantOfShapeTest() {}
  ~ConstantOfShapeTest() {}
  void SetUp() override {}
...
TEST_P(ConstantOfShapeTest, TestPostfix) {
...
std::pair<OpArgs, OpArgs> GenerateFp32Case(const std::vector<std::vector<int64_t>>& input_shape) {
  ...
  shared_ptr<OperatorConfig> op_config = std::make_shared<OperatorConfig>("cossin", "CosSin",
