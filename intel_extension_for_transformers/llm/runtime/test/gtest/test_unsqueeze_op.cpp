...
#include "../../include/operators/unsqueeze.hpp"
...
bool CheckResult(const TestParams& t) {
  ...
  try {
    executor::UnsqueezeOperator unsqueeze(p.conf);
    unsqueeze.Prepare(p.input, p.output);
    unsqueeze.Reshape(p.input, p.output);
    unsqueeze.Forward(p.input, p.output);
...
class UnsqueezeTest : public testing::TestWithParam<TestParams> {
 protected:
  UnsqueezeTest() {}
  ~UnsqueezeTest() {}
  void SetUp() override {}
...
TEST_P(UnsqueezeTest, TestPostfix) {
  TestParams t = testing::TestWithParam<TestParams>::GetParam();
  EXPECT_TRUE(CheckResult(t));
}

std::pair<OpArgs, OpArgs> GenerateFp32Case(const std::vector<std::vector<int64_t>>& input_shape, std::string axes) {
  ...
  auto op_config = std::make_shared<OperatorConfig>("unsqueeze", "Unsqueeze", input_config, output_config, op_attr);
...
static auto CasesFp32 = []() {
  ...
  // case: 1D
  src_shape = {3};
  cases.push_back({GenerateFp32Case({src_shape, {1, 3}}, "0")});

  src_shape = {3, 4, 5};
  cases.push_back({GenerateFp32Case({src_shape, {3, 4, 1, 5}}, "-2")});

  src_shape = {3, 4, 5};
  cases.push_back({GenerateFp32Case({src_shape, {1, 3, 4, 1, 5}}, "0,-2")});

  return ::testing::ValuesIn(cases);
};

INSTANTIATE_TEST_SUITE_P(Prefix, UnsqueezeTest, CasesFp32());
