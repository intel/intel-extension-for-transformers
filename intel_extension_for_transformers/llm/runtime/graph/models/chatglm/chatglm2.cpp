...
static bool chatglm_model_eval_internal(model_context* ctx, const model_input* inputs, const int n_input,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    cur = ne_mul(ctx0, ne_repeat(ctx0, model.layers[il].norm[0], cur), cur);
    {
      ...
      query_layer = ne_rope_inplace(ctx0, query_layer, std::max(n_cached - N, n_past), n_rot, 0, 0, hparams.freq_base, hparams.freq_scale);
      ...
      if (!run_mha_reordered) {
        ...
        if (is_ring_full) {
          ...
          key_layer = ne_rope_shift_inplace(ctx0, key_layer, -N, n_rot, 0, 0, n_keep, cossin_cache, hparams.freq_base, hparams.freq_scale);
