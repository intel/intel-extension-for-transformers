...
//   - weights
//
// see the convert-pt-to-ne.py script for details
//
...
static bool whisper_encode_internal(whisper_context& wctx, whisper_state& wstate, const int mel_offset,
  ...
  if (!use_coreml && !use_openvino) {
    // convolution + gelu
    {
      ...
      // cur = ne_conv_1d_ph(ctx0, model.e_conv_1_w, mel, 1, 1); TODO
      cur = ne_conv_1d_1s(ctx0, model.e_conv_1_w, mel);
      ...
      // cur = ne_conv_1d_ph(ctx0, model.e_conv_2_w, cur, 2, 1);  TODO
      cur = ne_conv_1d_1s(ctx0, model.e_conv_2_w, cur);
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // norm
      {
        ...
        // cur = ne_norm(ctx0, inpL, hparams.eps); TODO
      ...
      // feed-forward network
      {
        // norm
        {
          ...
          // cur = ne_norm(ctx0, inpFF, hparams.eps); TODO
        ...
        cur = ne_flash_ff(ctx0, ne_cpy(ctx0, cur, ne_new_tensor_2d(ctx0, wstate.itype, n_state, n_ctx, NE_SIZE_CALC)),
    ...
    // norm
    {
      ...
      // cur = ne_norm(ctx0, cur, hparams.eps); TODO
...
//   const int N = n_tokens;
//   const int M = 1500;  //     int32_t n_audio_ctx   = 1500;
//   ;

//   // WHISPER_PRINT_DEBUG("%s: n_past = %d, N = %d, M = %d, n_ctx = %d\n", __func__, n_past, N, M, n_ctx);

//   // size
//   // const size_t scale = 1;  // const size_t scale = ctx->model.hparams.ftype ? 1 : 2;

//   // // hparams.e_model_type = e_model::;
//   // e_model type = e_model::MODEL_TINY;
//   // const int buf_compute_size = scale * std::max(MEM_REQ_ENCODE.at(type), MEM_REQ_DECODE.at(type));

//   // struct ne_init_params params = {
//   //     /*.mem_size   =*/buf_compute_size,
//   //     /*.mem_buffer =*/wstate.buf_compute.data(),
//   //     /*.no_alloc   =*/false,
//   // };

//   auto& buf_compute = wctx.buf_compute;
//   struct ne_init_params params = {
...
//   // token encoding + position encoding
//   struct ne_tensor* cur =
//       ne_add(ctx0, ne_get_rows(ctx0, model.others[0], embd), ne_get_rows(ctx0, model.others[1], position));
...
//                    ne_repeat(ctx0, layer.norm[1], cur));
//     }
...
//       // note: no bias for Key
//       struct ne_tensor* Kcur = ne_mul_mat(ctx0, model.layers[il].attn[2], cur);

//       Kcur = ne_scale_inplace(ctx0, Kcur, ne_new_f32(ctx0, pow(float(n_state) / n_head, -0.25)));

//       // store key and value to memory
//       {
//         struct ne_tensor* Vcur = ne_mul_mat(ctx0, layer.attn[3], cur);

//         Vcur = ne_add(ctx0, ne_repeat(ctx0, layer.attn[4], Vcur), Vcur);
...
//       struct ne_tensor* Q = ne_permute(
//           ctx0, ne_cpy(ctx0, Qcur, ne_new_tensor_3d(ctx0, NE_TYPE_F32, n_state / n_head, n_head, N, NE_SIZE_CALC)),
//           0, 2, 1, 3);
...
//       cur = ne_mul_mat(ctx0, layer.attn[5], cur);
...
//       // cur = ne_norm(ctx0, inpCA, hparams.eps); TODO
//       cur = ne_norm(ctx0, inpCA);  // note: we use inpCA here
...
//     // cross-attention
//     {
//       struct ne_tensor* Qcur = ne_mul_mat(ctx0, layer.ffn[2], cur);

//       Qcur = ne_add(ctx0, ne_repeat(ctx0, layer.ffn[3], Qcur), Qcur);

//       Qcur = ne_scale_inplace(ctx0, Qcur, ne_new_f32(ctx0, pow(float(n_state) / n_head, -0.25)));

//       // Kcross is already scaled
//       struct ne_tensor* Kcross = ne_reshape_3d(
//           ctx0, ne_view_1d(ctx0, wstatekv_cross.k, M * n_state, il * M * ne_element_size(wstate.kv_cross.k) *
//           n_state), n_state / n_head, n_head, M);
...
//       //     ne_cpy(ctx0,
//       //             ne_permute(ctx0, Vcross, 1, 2, 0, 3),
//       //             ne_new_tensor_3d(ctx0, Vcross->type, M, n_state/n_head, n_head, NE_SIZE_CALC));
...
//         // cur = mlp_ln_w*cur + mlp_ln_b
//         cur =
//             ne_add(ctx0, ne_mul(ctx0, ne_repeat(ctx0, layer.mlp_ln_w, cur), cur), ne_repeat(ctx0, layer.mlp_ln_b,
//             cur));
//       }
...
static void log_mel_spectrogram_worker_thread(int ith, const std::vector<float>& hann,
                                              const std::vector<float>& samples, int n_samples, int frame_size,
                                              int frame_step, int n_threads, const whisper_filters& filters,
                                              whisper_mel& mel) {
...
static bool log_mel_spectrogram(whisper_state& wstate, const float* samples, const int n_samples,
...

  {
    ...
    for (int iw = 0; iw < n_threads - 1; ++iw) {
      workers[iw] =
          std::thread(log_mel_spectrogram_worker_thread, iw + 1, std::cref(hann), samples_padded,
...
int whisper_ctx_init_openvino_encoder(struct whisper_context* ctx, const char* model_path, const char* device,
  ...
  if (!model_path) {
    // if model_path is not set, attempt to find it in the same directory as ne-<model>.bin model
  ...
  if (!cache_dir) {
    // if cache_dir is not set, set it as a dir residing next to ne-<model>.bin
...
int whisper_decode(struct whisper_context* ctx, const whisper_token* tokens, int n_tokens, int n_past, int n_threads) {
  ...
  // if (!whisper_decode_internal(*ctx, *ctx->state, ctx->state->decoders[selected_decoder_id], tokens, n_tokens,
  // n_past,
  //                              n_threads)) {
  //   log("%s: failed to eval\n", __func__);
  //   return 1;
...
void whisper_print_timings(struct whisper_context* ctx) {
  ...
  if (ctx->state != nullptr) {
    ...
    log("%s:   sample time = %8.2f ms / %5d runs (%8.2f ms per run)\n", __func__, 1e-3f * ctx->state->t_sample_us,
        n_sample, 1e-3f * ctx->state->t_sample_us / n_sample);
    log("%s:   encode time = %8.2f ms / %5d runs (%8.2f ms per run)\n", __func__, 1e-3f * ctx->state->t_encode_us,
        n_encode, 1e-3f * ctx->state->t_encode_us / n_encode);
    log("%s:   decode time = %8.2f ms / %5d runs (%8.2f ms per run)\n", __func__, 1e-3f * ctx->state->t_decode_us,
        n_decode, 1e-3f * ctx->state->t_decode_us / n_decode);
  }
...
//
// Temporary interface needed for exposing ne interface
// Will be removed in the future when ne becomes a separate library
//
...
WHISPER_API const char* whisper_bench_ne_mul_mat_str(int n_threads) {
  ...
  for (int j = 0; j < (int)sizes.size(); j++) {
    ...
    for (int k = 0; k < 7; ++k) {
      const ne_type wtype = k == 0   ? NE_TYPE_Q4_0
                            : k == 1 ? NE_TYPE_Q4_1
                            : k == 2 ? NE_TYPE_Q5_0
                            : k == 3 ? NE_TYPE_Q5_1
                            : k == 4 ? NE_TYPE_Q8_0
      ...
      struct ne_tensor* a = ne_new_tensor_2d(ctx0, wtype, N, N, NE_SIZE_CALC);
      struct ne_tensor* b = ne_new_tensor_2d(ctx0, NE_TYPE_F32, N, N, NE_SIZE_CALC);
