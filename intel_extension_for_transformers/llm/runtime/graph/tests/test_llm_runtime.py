...
class TestLLMRUNTIME(unittest.TestCase):
    ...
    def test_llm_runtime(self):
        model_name = (
            "/tf_dataset2/models/nlp_toolkit/llama-2-7b-chat/Llama-2-7b-chat-hf"
        )
        ...
        pt_logits = torch.load(
            "/tf_dataset2/inc-ut/nlptoolkit_ut_model/llama2_pt_logits.pth"
        )[:, -1]
        pt_generate_ids = torch.load(
            "/tf_dataset2/inc-ut/nlptoolkit_ut_model/llama2_pt_generate_ids.pth"
        )[0].tolist()
        print(tokenizer.decode(pt_generate_ids))

        # check output ids
        woq_config = WeightOnlyQuantConfig(use_quant=False)
        itrex_model = AutoModel.from_pretrained(
            model_name,
        ...
        itrex_generate_ids = itrex_model.generate(
            inputs.input_ids, do_sample=False, max_new_tokens=100
        )[0]
        ...
        woq_configs = {
            ...
            "jblas_int4": WeightOnlyQuantConfig(
        ...
        for config_type in woq_configs:
            ...
            print(
                config_type,
                cmpData(pt_logits.detach().numpy().flatten(), itrex_logits.flatten()),
            )
    ...
    def test_beam_search(self):
        ...
        # pytorch fp32
        pt_generate_ids = torch.load(
            "/tf_dataset2/inc-ut/nlptoolkit_ut_model/beam_pt_generate_ids.pth"
        ).tolist()
