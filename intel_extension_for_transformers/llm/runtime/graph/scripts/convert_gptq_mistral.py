...
def convert_q4_1_tensor(src_name, dst_name, model, fout, n_head, n_head2=0, permute=False):
    qzeros = model[f"{src_name}.qzeros"]
    zeros = qzeros_to_zeros(qzeros)
    scales = model[f"{src_name}.scales"]
    # g_idx = model[f"{src_name}.g_idx"]
...
def convert_q4_f32_tensor(src_name, dst_name, model, fout, n_head, n_head2=0, permute=False):
    ...
    weight = weight.float()
    if permute:
        weight = permute_func(weight, n_head, n_head2).contiguous()
...
def convert_q4_jblas_tensor(src_name, dst_name, model, fout, n_head, n_head2=0, permute=False):
    ...
    if permute:
        int_weight = permute_func(int_weight.t(), n_head, n_head2).t().contiguous()
        gptq_scales = permute_func(gptq_scales.t(), n_head, n_head2).t().contiguous()
        gptq_zeros = permute_func(gptq_zeros.t(), n_head, n_head2).t().contiguous()
