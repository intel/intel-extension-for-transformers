...
## 2.4. Python bindings and scripts
...
# top_p, top_k sample
outputs = model.generate(inputs, streamer=streamer, max_new_tokens=300, do_sample=True, seed=12, top_k=40, top_p=0.95, temperature=0.8, repetition_penalty=1.1)
# beam search (only support GPT-J and GPT-NEOX model arch now)
outputs = model.generate(inputs, num_beams=4, max_new_tokens=128, min_new_tokens=30, early_stopping=True, length_penalty=1.0)
ans = tokenizer.batch_decode(outputs, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
print(ans)
```
