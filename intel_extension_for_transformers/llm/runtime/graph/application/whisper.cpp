...
static void byteswap_tensor(ne_tensor * tensor) {
    switch (tensor->type) {
        ...
        case NE_TYPE_F16: {
            byteswap_tensor_data<ne_fp16_t>(tensor);
            break;
        }
        case NE_TYPE_I32: {
...
static void ne_graph_compute_helper(std::vector<uint8_t> & buf, ne_cgraph * graph, int n_threads) {
    struct ne_cplan plan = ne_graph_plan(graph, n_threads);
...
//
static struct ne_tensor * ne_mul_mat_pad(struct ne_context * ctx, struct ne_tensor * x, struct ne_tensor * y, int pad = 32) {
    // use padding only if dimension 0 is at least 8 times larger than the padding
    // else we won't get much benefit from the optimization
    const int n_pad_req = 8;

    if (x->ne[0] % pad == 0 || x->ne[0] / pad < n_pad_req) {
        return ne_mul_mat(ctx, x, y);
    }

    struct ne_tensor * x_0 = ne_view_3d(ctx, x, (x->ne[0]/pad)*pad, x->ne[1], x->ne[2], x->nb[1], x->nb[2], 0);
    struct ne_tensor * x_1 = ne_view_3d(ctx, x,  x->ne[0]%pad,      x->ne[1], x->ne[2], x->nb[1], x->nb[2], x_0->ne[0]*x_0->nb[0]);

    struct ne_tensor * y_0 = ne_view_3d(ctx, y, (y->ne[0]/pad)*pad, y->ne[1], y->ne[2], y->nb[1], y->nb[2], 0);
    struct ne_tensor * y_1 = ne_view_3d(ctx, y,  y->ne[0]%pad,      y->ne[1], y->ne[2], y->nb[1], y->nb[2], y_0->ne[0]*y_0->nb[0]);

    return ne_add(ctx,
            ne_mul_mat(ctx, x_0, y_0),
            ne_mul_mat(ctx, x_1, y_1));
}

// TODO: check if other platforms can benefit from this optimization
#if defined(NE_USE_METAL)
#define ne_mul_mat ne_mul_mat_pad
#endif
...
struct whisper_kv_cache {
    ...
    // buf points to the memory allocated for both ne_tensor 'k' and 'v' (see kv_cache_init)
...
// ne_allocr wrapper for whisper usage
struct whisper_allocr {
    ne_allocr * alloc = nullptr;
...
static void whisper_allocr_graph_init(struct whisper_allocr & allocr, std::function<struct ne_cgraph *()> && get_graph) {
    ...
    meta.resize(ne_tensor_overhead()*NE_MAX_NODES + ne_graph_overhead());

    alloc = ne_allocr_new_measure(tensor_alignment);

    const size_t alloc_size = ne_allocr_alloc_graph(alloc, get_graph()) + tensor_alignment;

    ne_allocr_free(alloc);

    data.resize(alloc_size);

    alloc = ne_allocr_new(data.data(), data.size(), tensor_alignment);
}

static void whisper_allocr_free(struct whisper_allocr & allocr) {
    if (allocr.alloc) {
        ne_allocr_free(allocr.alloc);
...
struct whisper_state {
    ...
    // reusable buffer for `struct ne_graph_plan.work_data`
    ...
    struct ne_tensor * embd_conv = nullptr;
    struct ne_tensor * embd_enc  = nullptr;
...
#ifdef NE_USE_METAL
    ne_metal_context * ctx_metal = nullptr;
#endif
...
static bool kv_cache_init(
    ...
    const size_t mem_bytes = 2*(ne_type_size(wtype)*n_elements + ne_tensor_overhead());
...
static bool kv_cache_reinit(struct whisper_kv_cache & cache) {
    ...
    WHISPER_ASSERT(n_elements == ne_nelements(cache.v));

    const ne_type wtype = cache.k->type;
    WHISPER_ASSERT(wtype == cache.v->type);

    WHISPER_ASSERT(cache.buf.size() >= 2*n_elements*ne_type_sizef(wtype));
...
static bool whisper_model_load(struct whisper_model_loader * loader, whisper_context & wctx) {
    ...
    // prepare memory for the weights
    {
        ...
        // encoder
        {
            model.e_pe       = ne_new_tensor_2d(ctx, NE_TYPE_F32, n_audio_state, n_audio_ctx);

            model.e_conv_1_w = ne_new_tensor_3d(ctx, vtype,         3, n_mels, n_audio_state);
            model.e_conv_1_b = ne_new_tensor_2d(ctx, NE_TYPE_F32, 1, n_audio_state);

            model.e_conv_2_w = ne_new_tensor_3d(ctx, vtype,         3, n_audio_state, n_audio_state);
            model.e_conv_2_b = ne_new_tensor_2d(ctx, NE_TYPE_F32, 1, n_audio_state);

            model.e_ln_w     = ne_new_tensor_1d(ctx, NE_TYPE_F32, n_audio_state);
            model.e_ln_b     = ne_new_tensor_1d(ctx, NE_TYPE_F32, n_audio_state);
            ...
            for (int i = 0; i < n_audio_layer; ++i) {
                ...
                layer.mlp_ln_w    = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);
                layer.mlp_ln_b    = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);

                layer.mlp_0_w     = ne_new_tensor_2d(ctx, wtype,           n_audio_state, 4*n_audio_state);
                layer.mlp_0_b     = ne_new_tensor_1d(ctx, NE_TYPE_F32, 4*n_audio_state);

                layer.mlp_1_w     = ne_new_tensor_2d(ctx, wtype,         4*n_audio_state, n_audio_state);
                layer.mlp_1_b     = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);

                layer.attn_ln_0_w = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);
                layer.attn_ln_0_b = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);

                layer.attn_q_w    = ne_new_tensor_2d(ctx, wtype,           n_audio_state, n_audio_state);
                layer.attn_q_b    = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);

                layer.attn_k_w    = ne_new_tensor_2d(ctx, wtype,           n_audio_state, n_audio_state);

                layer.attn_v_w    = ne_new_tensor_2d(ctx, wtype,           n_audio_state, n_audio_state);
                layer.attn_v_b    = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);

                layer.attn_ln_1_w = ne_new_tensor_2d(ctx, wtype,           n_audio_state, n_audio_state);
                layer.attn_ln_1_b = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_audio_state);
        ...
        // decoder
        {
            model.d_pe   = ne_new_tensor_2d(ctx, NE_TYPE_F32, n_text_state, n_text_ctx);

            model.d_te   = ne_new_tensor_2d(ctx, wtype,         n_text_state, n_vocab);

            model.d_ln_w = ne_new_tensor_1d(ctx, NE_TYPE_F32, n_text_state);
            model.d_ln_b = ne_new_tensor_1d(ctx, NE_TYPE_F32, n_text_state);
            ...
            for (int i = 0; i < n_text_layer; ++i) {
                ...
                layer.mlp_ln_w          = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);
                layer.mlp_ln_b          = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.mlp_0_w           = ne_new_tensor_2d(ctx, wtype,           n_text_state, 4*n_text_state);
                layer.mlp_0_b           = ne_new_tensor_1d(ctx, NE_TYPE_F32, 4*n_text_state);

                layer.mlp_1_w           = ne_new_tensor_2d(ctx, wtype,         4*n_text_state, n_text_state);
                layer.mlp_1_b           = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.attn_ln_0_w       = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);
                layer.attn_ln_0_b       = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.attn_q_w          = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);
                layer.attn_q_b          = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.attn_k_w          = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);

                layer.attn_v_w          = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);
                layer.attn_v_b          = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.attn_ln_1_w       = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);
                layer.attn_ln_1_b       = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.cross_attn_ln_0_w = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);
                layer.cross_attn_ln_0_b = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.cross_attn_q_w    = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);
                layer.cross_attn_q_b    = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.cross_attn_k_w    = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);

                layer.cross_attn_v_w    = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);
                layer.cross_attn_v_b    = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);

                layer.cross_attn_ln_1_w = ne_new_tensor_2d(ctx, wtype,           n_text_state, n_text_state);
                layer.cross_attn_ln_1_b = ne_new_tensor_1d(ctx, NE_TYPE_F32,   n_text_state);
    ...
    // load weights
    {
        ...
        while (true) {
            ...
            //printf("%48s - [%5d, %5d, %5d], type = %6s, %6.2f MB\n", name.data(), ne[0], ne[1], ne[2], ne_type_name((ne_type) ttype), ne_nbytes(tensor)/1024.0/1024.0);
    ...
    wctx.t_load_us = ne_time_us() - t_start_us;
...
static struct ne_cgraph * whisper_build_graph_conv(
    ...
    const int n_state = hparams.n_audio_state; NE_UNUSED(n_state);
    ...
    ne_cgraph * gf = ne_new_graph(ctx0);

    ne_allocr * alloc = wstate.alloc_conv.alloc;

    struct ne_tensor * mel = ne_new_tensor_2d(ctx0, NE_TYPE_F32, 2*n_ctx, n_mels);
    ne_allocr_alloc(alloc, mel);

    assert(mel->type == NE_TYPE_F32);
    if (!ne_allocr_is_measure(alloc)) {
...
#ifdef WHISPER_USE_COREML
        cur = ne_new_tensor_2d(ctx0, NE_TYPE_F32, n_state, n_ctx);
        ne_allocr_alloc(alloc, cur);
...
static struct ne_cgraph * whisper_build_graph_encoder(
        whisper_context & wctx,
          whisper_state & wstate) {
    const auto & model   = wctx.model;
    const auto & hparams = model.hparams;
    ...
    ne_allocr * alloc = wstate.alloc_encode.alloc;

    struct ne_tensor * KQscale = ne_new_tensor_1d(ctx0, NE_TYPE_F32, 1);
    ne_allocr_alloc(alloc, KQscale);

    if (!ne_allocr_is_measure(alloc)) {
        ne_set_f32(KQscale, 1.0f/sqrt(float(n_state)/n_head));
    }

    struct ne_tensor * cur = ne_view_tensor(ctx0, wstate.embd_conv);
    ...
    cur = ne_add(ctx0, e_pe, ne_cont(ctx0, ne_transpose(ctx0, cur)));
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // norm
        {
            cur = ne_norm(ctx0, inpL, hparams.eps);

            // cur = ln_0_w*cur + ln_0_b
            cur = ne_add(ctx0,
                    ne_mul(ctx0, cur, layer.attn_ln_0_w),
                    layer.attn_ln_0_b);
        }

        // self-attention
        {
            struct ne_tensor * Qcur = ne_mul_mat(ctx0,
                    layer.attn_q_w,
                    cur);

            Qcur = ne_add(ctx0, Qcur, layer.attn_q_b);

            //Qcur = ne_scale(ctx0, Qcur, ne_new_f32(ctx0, pow(float(n_state)/n_head, -0.25)));

            // note: no bias for Key
            struct ne_tensor * Kcur = ne_mul_mat(ctx0,
                    layer.attn_k_w,
                    cur);

            //Kcur = ne_scale(ctx0, Kcur, ne_new_f32(ctx0, pow(float(n_state)/n_head, -0.25)));

            struct ne_tensor * Vcur = ne_mul_mat(ctx0,
                    layer.attn_v_w,
                    cur);

            Vcur = ne_add(ctx0, Vcur, layer.attn_v_b);
            ...
            struct ne_tensor * Q =
                ne_permute(ctx0,
                        ne_cpy(ctx0,
                            Qcur,
                            ne_new_tensor_3d(ctx0, wctx.itype, n_state/n_head, n_head, n_ctx)),
                        0, 2, 1, 3);
            ...
            struct ne_tensor * V =
                ne_cpy(ctx0,
                        ne_permute(ctx0,
                            ...
                            1, 2, 0, 3),
                        ne_new_tensor_3d(ctx0, wctx.itype, n_ctx, n_state/n_head, n_head));

            struct ne_tensor * KQV = ne_flash_attn(ctx0, Q, K, V, false);
#else
            struct ne_tensor * Q =
                ne_permute(ctx0,
                        ne_cpy(ctx0,
                            Qcur,
                            ne_new_tensor_3d(ctx0, NE_TYPE_F32, n_state/n_head, n_head, n_ctx)),
                        0, 2, 1, 3);
            ...
            struct ne_tensor * KQ_scaled = ne_scale(ctx0, KQ, KQscale);

            struct ne_tensor * KQ_soft_max = ne_soft_max(ctx0, KQ_scaled);
            ...
            cur = ne_cpy(ctx0,
                    KQV_merged,
                    ne_new_tensor_2d(ctx0, NE_TYPE_F32, n_state, n_ctx));
        }
        ...
        // projection
        {
            ...
            cur = ne_add(ctx0, cur, layer.attn_ln_1_b);
        }
        ...
        // feed-forward network
        {
            // norm
            {
                cur = ne_norm(ctx0, inpFF, hparams.eps);

                // cur = mlp_ln_w*cur + mlp_ln_b
                cur = ne_add(ctx0,
                        ne_mul(ctx0, cur, layer.mlp_ln_w),
                        layer.mlp_ln_b);
            }

#ifdef WHISPER_USE_FLASH_FF
            cur = ne_flash_ff(ctx0,
                    ne_cpy(ctx0, cur, ne_new_tensor_2d(ctx0, wstate.itype, n_state, n_ctx)),
            ...
            cur = ne_add(ctx0, cur, layer.mlp_0_b);
            ...
            cur = ne_add(ctx0, cur, layer.mlp_1_b);
#endif
        }
    ...
    // norm
    {
        cur = ne_norm(ctx0, cur, hparams.eps);

        // cur = ln_f_g*cur + ln_f_b
        cur = ne_add(ctx0,
                ne_mul(ctx0, cur, model.e_ln_w),
                model.e_ln_b);
    }
...
static struct ne_cgraph * whisper_build_graph_cross(
        whisper_context & wctx,
          whisper_state & wstate) {
    const auto & model   = wctx.model;
    const auto & hparams = model.hparams;
    ...
    ne_allocr * alloc = wstate.alloc_cross.alloc;

    struct ne_tensor * cur = ne_view_tensor(ctx0, wstate.embd_enc);

    struct ne_tensor * Kscale = ne_new_tensor_1d(ctx0, NE_TYPE_F32, 1);
    ne_allocr_alloc(alloc, Kscale);

    if (!ne_allocr_is_measure(alloc)) {
        ne_set_f32(Kscale, pow(float(n_state) / n_head, -0.25));
    }

    for (int il = 0; il < model.hparams.n_text_layer; ++il) {
        auto & layer = model.layers_decoder[il];

        struct ne_tensor* Kcross = ne_mul_mat(ctx0,
                layer.cross_attn_k_w,
                cur);

        Kcross = ne_scale(ctx0, Kcross, Kscale);

        struct ne_tensor* Vcross = ne_mul_mat(ctx0,
        ...
        struct ne_tensor * k = ne_view_1d(ctx0, wstate.kv_cross.k,
                n_state*n_ctx,
                (ne_element_size(wstate.kv_cross.k)*n_state)*(il*n_ctx));

        struct ne_tensor * v = ne_view_2d(ctx0, wstate.kv_cross.v, n_ctx, n_state,
                (   n_ctx)*ne_element_size(wstate.kv_cross.v),
...
static bool whisper_encode_internal(
    ...
    // conv
    {
        ...
        ne_cgraph * gf = whisper_build_graph_conv(wctx, wstate, mel_offset);

        ne_allocr_alloc_graph(alloc, gf);

        if (!whisper_encode_external(wstate)) {
            ne_graph_compute_helper(wstate.work_buffer, gf, n_threads);
        }
    }
    ...
    if (!whisper_encode_external(wstate)) {
        ...
        ne_cgraph * gf = whisper_build_graph_encoder(wctx, wstate);

        ne_allocr_alloc_graph(alloc, gf);

#ifdef NE_USE_METAL
        if (wstate.ctx_metal) {
            ne_metal_set_n_cb     (wstate.ctx_metal, n_threads);
            ne_metal_graph_compute(wstate.ctx_metal, gf);
        } else {
            ne_graph_compute_helper(wstate.work_buffer, gf, n_threads);
        }
#else
    ...
    // cross
    {
        ...
        ne_cgraph * gf = whisper_build_graph_cross(wctx, wstate);
    ...
    // ne_graph_compute_with_ctx(ctx0, &gf, n_threads);

    wstate.t_encode_us += ne_time_us() - t_start_us;
    wstate.n_encode++;

    return true;
}

static struct ne_cgraph * whisper_build_graph_decoder(
         whisper_context & wctx,
         whisper_state   & wstate,
    ...
    ne_allocr * alloc = wstate.alloc_decode.alloc;

    struct ne_tensor * embd = ne_new_tensor_1d(ctx0, NE_TYPE_I32, N);
    ne_allocr_alloc(alloc, embd);

    if (!ne_allocr_is_measure(alloc)) {
        memcpy(embd->data, tokens, N*ne_element_size(embd));
    }

    struct ne_tensor * position = ne_new_tensor_1d(ctx0, NE_TYPE_I32, N);
    ne_allocr_alloc(alloc, position);
    ...
    if (!ne_allocr_is_measure(alloc)) {
        ne_set_f32(KQscale, pow(float(n_state)/n_head, -0.25));
    }

    // token encoding + position encoding
    struct ne_tensor * cur =
        ne_add(ctx0,
                ne_get_rows(ctx0, model.d_te, embd),
                ne_get_rows(ctx0, model.d_pe, position));
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            Qcur = ne_scale(ctx0, Qcur, KQscale);
            ...
            Kcur = ne_scale(ctx0, Kcur, KQscale);
            ...
            // store key and value to memory
            {
                ...
                struct ne_tensor * k = ne_view_1d(ctx0, kv_self.k, N*n_state, (ne_element_size(kv_self.k)*n_state)*(il*n_ctx + n_past));
                struct ne_tensor * v = ne_view_2d(ctx0, kv_self.v, N, n_state,
                        (   n_ctx)*ne_element_size(kv_self.v),
            ...
            struct ne_tensor * Q =
                ne_permute(ctx0,
                        ne_reshape_3d(ctx0, Qcur, n_state/n_head, n_head, N),
                        0, 2, 1, 3);

            struct ne_tensor * K =
                ne_view_3d(ctx0, kv_self.k,
                        n_state/n_head, n_past + N, n_head,
                        ne_element_size(kv_self.k)*n_state,
                        ne_element_size(kv_self.k)*n_state/n_head,
                        ne_element_size(kv_self.k)*n_state*n_ctx*il);

            // K * Q
            struct ne_tensor * KQ = ne_mul_mat(ctx0, K, Q);

            //struct ne_tensor * KQ_scaled = ne_scale(ctx0, KQ, KQ_scale);

            struct ne_tensor * KQ_masked = ne_diag_mask_inf(ctx0, KQ, n_past);

            struct ne_tensor * KQ_soft_max = ne_soft_max(ctx0, KQ_masked);

            struct ne_tensor * V =
                ne_view_3d(ctx0, kv_self.v,
                        n_past + N, n_state/n_head, n_head,
                        n_ctx*ne_element_size(kv_self.v),
                        n_ctx*ne_element_size(kv_self.v)*n_state/n_head,
                        il*n_ctx*ne_element_size(kv_self.v)*n_state);
            ...
            cur = ne_cpy(ctx0,
                    KQV_merged,
                    ne_new_tensor_2d(ctx0, NE_TYPE_F32, n_state, N));
        }
        ...
        // norm
        {
            cur = ne_norm(ctx0, inpCA, hparams.eps); // note: we use inpCA here
        ...
        // cross-attention
        {
            ...
            struct ne_tensor * Kcross =
                ne_view_3d(ctx0, wstate.kv_cross.k,
                        n_state/n_head, M, n_head,
                        ne_element_size(wstate.kv_cross.k)*n_state,
                        ne_element_size(wstate.kv_cross.k)*n_state/n_head,
                        ne_element_size(wstate.kv_cross.k)*n_state*M*il);
            ...
            struct ne_tensor * V =
                ne_view_3d(ctx0, wstate.kv_cross.v,
                        M, n_state/n_head, n_head,
                        M*ne_element_size(wstate.kv_cross.v),
            ...
            // K * Q
            struct ne_tensor * KQ = ne_mul_mat(ctx0, Kcross, Q);
            ...
            struct ne_tensor * KQ_soft_max = ne_soft_max(ctx0, KQ);
...
static bool whisper_decode_internal(
    ...
    // decoder
    {
        ...
        ne_cgraph * gf = whisper_build_graph_decoder(wctx, wstate, decoder, tokens, n_tokens, n_past);
    ...
    //memcpy(logits_out.data(), ne_get_data(logits), sizeof(float)*n_tokens*n_vocab);
    ...
    if (n_tokens == 1) {
        ...
        wstate.t_prompt_us += ne_time_us() - t_start_us;
        wstate.n_prompt++;
    }
...
static bool log_mel_spectrogram(
    ...
    wstate.t_mel_us += ne_time_us() - t_start_us;
...
struct whisper_state * whisper_init_state(whisper_context * ctx) {
...
#ifdef NE_USE_METAL
    state->ctx_metal = ne_metal_init(1);
    if (!state->ctx_metal) {
        log("%s: ne_metal_init() failed\n", __func__);
        delete state;
        return nullptr;
    }
    ...
    //} else {
    //    data_ptr  = ne_get_mem_buffer(ctx->model.ctx);
    //    data_size = ne_get_mem_size  (ctx->model.ctx);
    //}

    data_ptr  = ne_get_mem_buffer(ctx->model.ctx);
    data_size = ne_get_mem_size  (ctx->model.ctx);

    const size_t max_size = ne_get_max_tensor_size(ctx->model.ctx);
...
#define WHISPER_METAL_CHECK_BUF(result)              \
    ...
    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "data", data_ptr, data_size, max_size));

    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "meta_conv",   state->alloc_conv.meta.data(),   state->alloc_conv.meta.size(),   0));
    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "meta_encode", state->alloc_encode.meta.data(), state->alloc_encode.meta.size(), 0));
    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "meta_cross",  state->alloc_cross.meta.data(),  state->alloc_cross.meta.size(),  0));
    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "meta_decode", state->alloc_decode.meta.data(), state->alloc_decode.meta.size(), 0));

    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "data_conv",   state->alloc_conv.data.data(),   state->alloc_conv.data.size(),   0));
    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "data_encode", state->alloc_encode.data.data(), state->alloc_encode.data.size(), 0));
    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "data_cross",  state->alloc_cross.data.data(),  state->alloc_cross.data.size(),  0));
    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "data_decode", state->alloc_decode.data.data(), state->alloc_decode.data.size(), 0));

    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "kv_cross",  state->kv_cross.buf.data(), state->kv_cross.buf.size(), 0));

    WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, "kv_self_0", state->decoders[0].kv_self.buf.data(), state->decoders[0].kv_self.buf.size(), 0));
...
const char * whisper_print_system_info(void) {
    ...
    s += "NEON = "      + std::to_string(ne_cpu_has_neon())      + " | ";
    s += "ARM_FMA = "   + std::to_string(ne_cpu_has_arm_fma())   + " | ";
    s += "METAL = "     + std::to_string(ne_cpu_has_metal())     + " | ";
    s += "F16C = "      + std::to_string(ne_cpu_has_f16c())      + " | ";
    s += "FP16_VA = "   + std::to_string(ne_cpu_has_fp16_va())   + " | ";
    s += "WASM_SIMD = " + std::to_string(ne_cpu_has_wasm_simd()) + " | ";
    s += "BLAS = "      + std::to_string(ne_cpu_has_blas())      + " | ";
    s += "SSE3 = "      + std::to_string(ne_cpu_has_sse3())      + " | ";
    s += "SSSE3 = "     + std::to_string(ne_cpu_has_ssse3())     + " | ";
...
static bool whisper_kv_swap_fast(
    ...
    for (int i = 0; i < n_decoders; i++) {
        kv_swap_bufs[i].k.resize(ne_nbytes(src[i].kv_self.k));
        kv_swap_bufs[i].v.resize(ne_nbytes(src[i].kv_self.v));
    }
    ...
    for (auto & i : two_copy) {
        ...
        if (two_copy.find(view[i]) != two_copy.end()) {
            ...
            memcpy(src[i].kv_self.k->data, src[view[i]].kv_self.k->data, ne_nbytes(src[view[i]].kv_self.k));
            memcpy(src[i].kv_self.v->data, src[view[i]].kv_self.v->data, ne_nbytes(src[view[i]].kv_self.v));
        }
    }
...
#define WHISPER_METAL_CHECK_BUF(result)              \
            ...
            WHISPER_METAL_CHECK_BUF(ne_metal_add_buffer(state->ctx_metal, kv_name.c_str(), kv_self.buf.data(), kv_self.buf.size(), 0));
#undef WHISPER_METAL_CHECK_BUF
#endif
        }
    }
        ...
        for (int it = 0; it < (int) temperatures.size(); ++it) {
            ...
            // run whisper_decoder() only for decoder 0 and copy the results for the other decoders
            {
...

                {
                    ...
                    state->t_sample_us += ne_time_us() - t_start_sample_us;
                }
            }
...
WHISPER_API const char * whisper_bench_memcpy_str(int n_threads) {
    ...
    // single-thread
    {
        ...
        for (size_t i = 0; i < n; i++) {
            const int64_t t0 = ne_time_us();

            memcpy(dst, src, size);

            const int64_t t1 = ne_time_us();
...
WHISPER_API int whisper_bench_ne_mul_mat(int n_threads) {
    fputs(whisper_bench_ne_mul_mat_str(n_threads), stderr);
    return 0;
}

WHISPER_API const char * whisper_bench_ne_mul_mat_str(int n_threads) {
    static std::string s;
    s = "";
    char strbuf[256];
    ...
    std::vector<uint8_t> buf(3llu*N_max*N_max*sizeof(float) + 3*ne_tensor_overhead());
    ...
    for (int j = 0; j < (int) sizes.size(); j++) {
        ...
        for (int k = 0; k < 7; ++k) {
            ...
            struct ne_init_params gparams = {
                /*.mem_size   =*/ buf.size(),
                /*.mem_buffer =*/ buf.data(),
                /*.no_alloc   =*/ false,
            };

            struct ne_context * ctx0 = ne_init(gparams);

            struct ne_tensor * a = ne_new_tensor_2d(ctx0, wtype,         N, N);
            struct ne_tensor * b = ne_new_tensor_2d(ctx0, NE_TYPE_F32, N, N);

            struct ne_tensor * c = ne_mul_mat(ctx0, a, b);

            struct ne_cgraph gf = ne_build_forward(c);

            double tsum = 0.0;

            // heat-up
            ne_graph_compute_helper(work, &gf, n_threads);
