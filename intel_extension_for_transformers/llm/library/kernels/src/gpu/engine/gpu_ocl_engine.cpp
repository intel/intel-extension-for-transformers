...
#define DECLARE_IMPL_LIST(kind)                                                \
  const std::vector<impl_list_item_t> *get_gpu_##kind##_impl_list(             \
...
gpu_ocl_engine_t::get_implementation_list(const operator_desc &op_desc) const {
  // Call C API.
#define CASE(kind)                                                             \
...
gpu_ocl_engine_t::gpu_ocl_engine_t()
    : engine_t(engine_kind::gpu, runtime_kind::opencl) {
  cl_int err;
  device_ = findGPU();
...
bool gpu_ocl_engine_t::create_kernel(const operator_desc &op_desc,
  ...
  const auto &callback = std::bind(&kernel_desc_t::create_primitive, result_kd,
                                   std::placeholders::_1,
                                   result_kd); // k_t->create() + k_t->init()
  std::shared_ptr<const kernel_t> value =
      global_primitive_cache->find_or_construct(result_kd->get_operator_desc(),
                                                callback);
  if (value == nullptr) {
    return false;
  }
