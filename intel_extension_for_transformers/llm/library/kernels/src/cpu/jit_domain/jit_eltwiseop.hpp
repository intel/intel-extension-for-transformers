...
public:
  explicit jit_eltwiseop_t(const ssd::eltwiseop_param_t &param)
...
private:
  ...
  size_t load_offset() {
    if (param_.postop_attrs[0].op_alg == postop_alg::eltop_int_lut &&
        param_.postop_attrs[0].alpha == 8) {
      return 64u; // special case:bit8_lut
    }
    if (param_.postop_attrs[0].op_alg == postop_alg::eltop_int_lut &&
        param_.postop_attrs[0].alpha == 16) {
    ...
    case data_type::s8:
      return 16u;
    default:
      SPARSE_LOG(ERROR) << "wrong head data type, expect fp32/bf16/u8"
                        << std::endl;
      return 0u;
    }
  }
  ...
  size_t store_offset() {
    // todo:except dequantize case, our zmm always full of result data and needs
    // to be stored.
    if (param_.postop_attrs.front().op_alg == postop_alg::eltop_int_lut)
      return 64u; // lut case;
    if (param_.postop_attrs.back().op_alg == postop_alg::quantize)
      return 16u; // quantize case.
    if (param_.postop_attrs.back().op_alg == postop_alg::dequantize)
      return 64u; // dequantize case.
    switch (param_.out_dt) {
    case data_type::fp32:
    case data_type::bf16:
      return 64u;
    default:
      SPARSE_LOG(ERROR) << "wrong output data type, expect fp32/bf16"
                        << std::endl;
      return 0u;
    }
  }
  ...
  size_t process_element_num() {
    ...
    if (front_attr.op_alg == postop_alg::eltop_int_lut && front_attr.alpha == 8)
      return 64; // special case:bit8_lut
    if (front_attr.op_alg == postop_alg::eltop_int_lut &&
        front_attr.alpha == 16)
      return 32; // sepcial case:bit16_lut
    switch (param_.in_dt) {
    case data_type::fp32:
      return 16;
    case data_type::bf16:
    ...
    case data_type::s8:
      return 16;
    default:
      SPARSE_LOG(ERROR) << "wrong input data type, expect fp32/bf16/u8"
                        << std::endl;
      return 0u;
    }
  }
