...
void jit_amx_s8s8_dynamic_dequant_matmul_t::generate() {
  ...
  inLocalLabel();
  {
    ...
    auto ip_Mx16 = [&](int M, int block_num, bool need_mask = false,
                       int no_mask_tail_n = 0) {
      ...
      // build block
      {
        ...
        imul(reg_tmp, reg_n_loop,
             param_.align_build_block_num * trans_block_col *
                 reorder_block_col_eltnum * (param_.tile_k / 4));
        ...
        for (int k_loop = 0; k_loop < param_.k / param_.tile_k; k_loop++) {
          ...
          for (int idx = 0; idx < block_num; idx++) {
            int offset = (idx + no_mask_tail_n) * trans_block_col *
                             reorder_block_col_eltnum * (param_.tile_k / 4) +
      ...
      {
        for (int idx = 0; idx < block_num; idx++)
          tilestored(ptr[reg_tmpbuf + reg_strideTmpbuf +
                         idx * align_build_N * sizeof(int)],
                     Tmm(idx));
        // dequant + add_bias
        ...
        imul(reg_tmp, reg_n_loop,
             align_build_N * param_.align_build_block_num *
                 sizeof(int)); // offset of scale_w & bias
        imul(reg_tmp2, reg_m_loop,
             align_build_M * sizeof(float)); // offset of scale_a
        for (int idx = 0; idx < block_num; idx++) {
          vmovups(zmms[0], ptr[reg_scale_w + reg_tmp +
                               (idx + no_mask_tail_n) * 16 * sizeof(float)]);
          if (param_.add_bias)
            vmovups(
                zmms[1],
                ptr[reg_bias + reg_tmp +
                    (idx + no_mask_tail_n) * align_build_N * sizeof(float)]);
          for (int row_loop = 0; row_loop < M; row_loop++) {
            vcvtdq2ps(zmms[2],
                      ptr[reg_tmpbuf +
                          (idx + row_loop * param_.align_build_block_num) *
                              align_build_N * sizeof(float)]);
            vbroadcastss(
                zmms[3],
            ...
            if (param_.dst_dt != data_type::fp32) {
              ...
              RegExp write_back_addr = reg_dst +
                                       reg_dst_offset * sizeof(bfloat16_t) +
                                       ((no_mask_tail_n + idx) * align_build_N +
                                        row_loop * dst_n_dim_) *
                                           sizeof(bfloat16_t);
              vmovdqu16(need_mask ? ptr[write_back_addr] | matC_n_mask
                                  : ptr[write_back_addr],
                        Ymm(zmms[2].getIdx()));
            } else {
              RegExp write_back_addr = reg_dst +
                                       reg_dst_offset * sizeof(float) +
                                       ((no_mask_tail_n + idx) * align_build_N +
                                        row_loop * dst_n_dim_) *
                                           sizeof(float);
              // append sum
              ...
              vmovups(need_mask ? ptr[write_back_addr] | matC_n_mask
                                : ptr[write_back_addr],
                      zmms[2]);
            }
          }
      ...
      if (calc_abs_max_) {
        ...
        for (int block_idx = 0; block_idx < block_num; block_idx++) {
          for (int row_idx = 0; row_idx < M; row_idx++) {
            vrangeps(
                need_mask ? zmms[row_idx] | matC_n_mask : zmms[row_idx],
                zmms[row_idx],
                ptr[reg_tmpbuf +
                    align_build_N * sizeof(float) *
                        (block_idx + row_idx * param_.align_build_block_num)],
                11U);
          }
        }
    ...
    auto align_loop_ip_Mx16 = [&](int M, int loop_num,
    ...
    auto build_MxN_tile = [&](int M, std::string label_prefix = ".") {
      if (calc_abs_max_) {
        auto clear_zmm =
            rp.reg<Zmm>(); // init abs max (equal to 0) in scale_dst.
        ...
        vmovups(M == 16 ? ptr[scale_wirte_back_offset]
                        : ptr[scale_wirte_back_offset] | scaleC_mask,
                clear_zmm);
        mov(ptr[rsp + 8],
            reg_scale_offset);     // store the scaleC offset to the stack.
        mov(reg_m_loop, ptr[rsp]); // restore m_loop value.
      }
      xor_(reg_n_loop, reg_n_loop);
      if (param_.write_mask == 0) {
        if (param_.align_n_loop > 0)
          align_loop_ip_Mx16(M, param_.align_n_loop, label_prefix);
        if (param_.tail_n_loop != 0)
          ip_Mx16(M, param_.tail_n_loop);
      } else {
        int no_mask_align_n_loop =
            (param_.n - align_build_N) /
            (align_build_N * param_.align_build_block_num);
        int no_mask_tail_n = ((param_.n - align_build_N) %
                              (align_build_N * param_.align_build_block_num)) /
                             align_build_N;
        if (no_mask_align_n_loop > 0)
          align_loop_ip_Mx16(M, no_mask_align_n_loop, label_prefix);
        if (no_mask_tail_n != 0)
          ip_Mx16(M, no_mask_tail_n);
    ...
    mov(reg_strideTmpbuf,
        align_build_N * param_.align_build_block_num * sizeof(int));
