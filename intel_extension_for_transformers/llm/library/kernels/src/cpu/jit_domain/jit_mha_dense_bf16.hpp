...
/// @note jit classes in this file may not meet the coding convention and is to
/// be refactored.
...
public:
  ...
  explicit jit_padding_interleave4b_n(int _ntile, int _srcbytes)
    ...
    SPARSE_LOG_IF(FATAL, SrcBytes != 1 && SrcBytes != 2)
        << "Unexpected SrcBytes!";
  }

private:
  void process_row_tile(regs_pool *const rp, const Reg64 src_ptr,
                        const Reg64 dst_ptr, const Reg64 src_stride,
                        const Reg64 dst_stride, const Reg64 iter_row,
                        const Reg64 col_size, bool is_row_tail) {
    ...
    for (int i = 0; i < ZMM_PerNTile; i++) {
      auto curr_itercol =
          rp->reg<Reg64>(); // curr_itercol = reg_itercol + i * NPerLoop;
      lea(curr_itercol, ptr[reg_itercol + i * NPerLoop]);
      generate_Nbitsmask(masks[i], curr_itercol, col_size, rp->reg<Reg64>(),
                         rp->reg<Reg64>(), NPerLoop);
    }
    ...
    for (int i = 0; i < NTile; i += NPerLoop) {
      ...
      if (is_row_tail) {
        for (int j = 0; j < RowTile; j++)
          vxorps(reg_srcs[j], reg_srcs[j]);
      }

      const auto curr_src = rp->reg<Reg64>(); // curr_src := reg_itercol *
                                              // SrcBytes + j * reg_srcstride
      Xbyak::Label l_skip_row;
      for (int j = 0; j < RowTile; j++) {
        if (is_row_tail) {
          const auto &curr_iter_row = rp->reg<Reg64>();
          lea(curr_iter_row, ptr[iter_row + j]);
          cmp(curr_iter_row.cvt32(),
              dword[rp->p[0] + offsetof(rt_data_t, row)]);
          jge(l_skip_row, T_NEAR);
        }

        (j == 0) ? lea(curr_src, ptr[src_ptr + reg_itercol * SrcBytes])
                 : lea(curr_src, ptr[curr_src + src_stride]);
        if (SrcBytes == 1) {
          vmovdqu8(reg_srcs[j] | masks[i / NPerLoop] | T_z,
                   zword[curr_src + i * SrcBytes]);
        } else if (SrcBytes == 2) {
          vmovdqu16(reg_srcs[j] | masks[i / NPerLoop] | T_z,
                    zword[curr_src + i * SrcBytes]);
        }
      }
      if (is_row_tail)
      ...
      for (int j = 0; j < Valid_NReg; j++)
        vmovaps(ptr[curr_dst + j * BYTES_ZMM + i * 4], reg_srcs[j]);
    }
  ...
  inline void generate() override {
    ...
    process_row_tile(&rp, reg_srcptr, reg_dstptr, reg_srcstride, reg_dststride,
                     reg_iterrow, reg_colsize, false);
  ...
  void interleave_2rows_4regs(std::vector<Xbyak::Zmm> src_2regs,
                              std::vector<Xbyak::Zmm> tmp_2reg) {
...
protected:
  inline void generate_mask(const Opmask &_msk, const Reg64 &_pos,
                            const Reg64 &_total, const Reg64 &_tmp,
...
public:
  ...
  explicit jit_mha_bf16_row_amx_32x32_softmax(bool has_badd,
      ...
      : jit_generator(), binary_add(has_badd), stable_softmax(stable_softmax),
        pre_amx_cfg_(pre_amx_cfg), required_amx_cfg_{16, 16, KTile, true, 2},
...
private:
  inline void generate() override {
    ...
    const int TmpSpace =
        amx_config_size + TmmReserve + ExpSumReserve + MaxValReserve;
    ...
    {
      regs_pool rp(this, 1, {9, 32, 0}, TmpSpace, regs_pool::DefaultFlags,
                   64); // align 64
      ...
      L(".mloop");
      {
        ...
        for (int i = 0; i < MTile; i++)
          vmovaps(ptr[rsp + TExpsumStart + i * 64], vreg_tmp);
        if (stable_softmax) {
          vpbroadcastd(vreg_tmp, dword[rip + l_neginf]);
          for (int i = 0; i < MTile; i++)
            vmovaps(ptr[rsp + TMaxValStart + i * 64], vreg_tmp);
        }
      }

      {
        ...
        { // tile product
          ...
          tile_product_amx_bf16ps(reg_ksize, reg_matA, reg_matB, reg_astep,
                                  stride_bc, rp.reg<Reg64>(), rp.reg<Reg64>(),
                                  [&](int i, int j) {
                                    return ptr[rsp + stride_bc + TTmmStart +
                                               i * 16 * 64 * 2 + j * 64];
                                  });
        }
        ...
        imul(reg_cstep.cvt32(), dword[rp.p[0] + offsetof(rt_data_t, n)],
             sizeof(bfloat16_t));
        ...
        for (int in = 0; in < NTile; in += 16) {
          ...
          for (int i = 0; i < MTile; i += 16) {
            ...
            for (int ii = 0; ii < 16; ii++) {
              (i == 0 && ii == 0)
                  ? lea(curr_matC, ptr[reg_matC + in * sizeof(bfloat16_t)])
                  : lea(curr_matC, ptr[curr_matC + reg_cstep]);
              const auto vreg_badd = rp.reg<Zmm>();
              if (binary_add) {
                (i == 0 && ii == 0)
                    ? xor_(badd_ptr.cvt32(), badd_ptr.cvt32())
                    : add(badd_ptr.cvt32(),
                          dword[rp.p[0] + offsetof(rt_data_t, dstep)]);
                vmovups(vreg_badd, zword[reg_matD + badd_ptr + in * 4]);
              }

              vmovaps(vregs_xs[ii],
                      zword[rsp + TTmmStart + in * 4 + (i + ii) * NTile * 4]);
              !binary_add // (optionally) add mask and scale
                  ? vmulps(vregs_xs[ii], vregs_xs[ii],
                           zword_b[rp.p[0] + offsetof(rt_data_t, scaleAB)])
                  : vfmadd132ps(
                        vregs_xs[ii], vreg_badd,
                        zword_b[rp.p[0] + offsetof(rt_data_t, scaleAB)]);
              if (!stable_softmax)
                exp_approx_f32(vregs_xs[ii], vregs_xs[ii], vreg_log2e, vreg_ln2,
                               c, rp.regs<Zmm, 2>());
              vcvtneps2bf16(Ymm(vreg_badd.getIdx()), vregs_xs[ii]);
              vmovaps(yword[curr_matC], Ymm(vreg_badd.getIdx()));
              (!stable_softmax)
                  ? vaddps(vregs_xs[ii], vregs_xs[ii],
                           ptr[rsp + TExpsumStart + (i + ii) * 64])
                  : vmaxps(vregs_xs[ii], vregs_xs[ii],
                           ptr[rsp + TMaxValStart + (i + ii) * 64]);
              vmovaps(
                  zword[rsp + (stable_softmax ? TMaxValStart : TExpsumStart) +
                        (i + ii) * 64],
                  vregs_xs[ii]);
            }
          }
      ...
      if (stable_softmax) {
        for (int i = 0; i < MTile;
             i += 16) { // round max to bf16 representable values
          const auto vregs_max = rp.regs<Zmm, 16>();
          for (int ii = 0; ii < 16; ++ii)
            vmovaps(vregs_max[ii],
                    zword[rsp + TMaxValStart + (i + ii) * BYTES_ZMM]);
        ...
        for (int in = 0; in < NTile; in += 16) {
          ...
          for (int i = 0; i < MTile; i += 16) {
            ...
            for (int ii = 0; ii < 16; ii++) {
              ...
              vsubps(vregs_xs[ii], vregs_xs[ii],
                     zword_b[rsp + TMaxValStart + i * BYTES_ZMM +
                             ii * sizeof(float)]);
              exp_approx_f32(vregs_xs[ii], vregs_xs[ii], vreg_log2e, vreg_ln2,
                             c, rp.regs<Zmm, 2>());
              ...
              vaddps(vregs_xs[ii], vregs_xs[ii],
                     zword[rsp + TExpsumStart + (i + ii) * 64]);
      ...
      for (int im = 0; im < MTile; im += 16) {
        const auto vregs_xs = rp.regs<Zmm, 16>();
        for (int j = 0; j < 16; j++)
          vmovaps(vregs_xs[j], ptr[rsp + TExpsumStart + (im + j) * BYTES_ZMM]);
      ...
      L(".nwrloop");
      const auto reg_matC =
          rp.reg<Reg64>(); // reg_matC := matC + itern * bfloat16_t + reg_cstep
                           // * reg_iterm
      {
        mov(reg_matC, ptr[rp.p[0] + offsetof(rt_data_t, matC)]);
        lea(reg_matC, ptr[reg_matC + reg_itern * sizeof(bfloat16_t)]);
    ...
    if (need_cfg_amx) {
      ...
      L(l_amx_cfg);
      db(reinterpret_cast<const uint8_t *>(&required_tile_cfg_),
         sizeof(tileconfig_t));
    }
  }
...
public:
  ...
  explicit jit_mha_bf16_row_amx_32x32(const tile_param_t *pre_amx_cfg)
      : jit_generator(), pre_amx_cfg_(pre_amx_cfg),
...
private:
  inline void generate() override {
    ...
    {
      regs_pool rp(this, 1, {9, ZMM_PerROW, 1}, TmpSpace,
                   regs_pool::DefaultFlags, 64); // align 64
      std::shared_ptr<void> local_cfg;
      if (need_cfg_amx) { // create a local amx config environment
        local_cfg = {
            (sttilecfg(ptr[rsp]), ldtilecfg(ptr[rip + l_amx_cfg]), nullptr),
      ...
      for (int i = 0; i < MTile; i++) {
        if (i != 0)
          lea(reg_matC, ptr[reg_matC + reg_cstep]);
