...
public:
  ...
  void eltwise_injector_init(jit_generator *ptr,
  ...
  inline typename std::enable_if<std::is_base_of<Xbyak::Xmm, reg_t>::value,
                                 void>::type
  escape_regs(const reg_t &reg) {
    escape_regs(reg_type::zmm, reg.getIdx());
  }
  template <typename reg_t>
  inline typename std::enable_if<std::is_base_of<Xbyak::Reg32e, reg_t>::value,
                                 void>::type
  escape_regs(const reg_t &reg) {
    escape_regs(reg_type::reg64, reg.getIdx());
  }
  template <typename reg_t>
  inline typename std::enable_if<std::is_base_of<Xbyak::Opmask, reg_t>::value,
                                 void>::type
  escape_regs(const reg_t &reg) {
...
private:
  ...
  uint32_t get_bit8_lut_term(int integer,
                             const std::vector<postop_attr> &postop_attrs,
                             data_type output_dt);
  uint32_t get_bit16_lut_term(int integer,
                              const std::vector<postop_attr> &postop_attrs,
                              data_type output_dt);
  std::string get_attr_idx_key(
      const postop_attr
          &attr); // for get the key of alpha_idx,beta_idx,scale_idx map.
