 ...
 * dst(m, 48) = exp( scale(1,) * ( src0(m, K) x src1(K, 48) - bias(1, 48) <<
 * bias_lshift ) + b0(m, 1) ) dst_scale(1, 48) = 255 / sum( dst(m, 48) ) src0:
 * u8 | ABa8b4 | (M/8)x(K/4)x8x4;  src1: s8 | BAb48a4 | (K/4)x48x4 dst:
 * bf16/fp32 | ab
 */
...
public:
  ...
  template <typename dst_t> struct rt_data_t {
    const uint8_t *src0;
    const int8_t *src1;
    const int32_t *bias;
    const float *src_b0;
...
private:
  ...
  Xbyak::Zmm
  TW_Vmm(int j); // Register allocator of load activation. 1D shape=(TW)
  Xbyak::Zmm
  dst_scale_Vmm(int j); // Register allocator of load activation. 1D shape=(TW)
  Xbyak::Zmm
  dst_tile_Vmm(int i,
  ...
  static constexpr size_t dsize_src_b0 =
      sizeof(decltype(*rt_data_t<void>::src_b0));
