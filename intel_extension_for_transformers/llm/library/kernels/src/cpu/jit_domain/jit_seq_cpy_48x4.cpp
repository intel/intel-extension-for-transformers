...
void jit_seq_cpy_48x4::trans_4x16(const Zmm &zreg_res, const Zmm &zreg_sum,
                                  const Xbyak::Operand &op0,
                                  const Xbyak::Operand &op1,
                                  const Xbyak::Operand &op2,
                                  const Xbyak::Operand &op3, bool is_tail_x) {
  if (!is_tail_x) {
    ...
    vshuff32x4(Xbyak::Ymm(zreg_res.getIdx()), Xbyak::Ymm(zreg_res.getIdx()),
               Xbyak::Ymm(vreg_t2.getIdx()), 0b00);
    vmovdqu8(Xbyak::Xmm(vreg_t1.getIdx()) | reg_k_tail | T_z, op2);
    vmovdqu8(Xbyak::Xmm(vreg_t2.getIdx()) | reg_k_tail | T_z, op3);
    vshuff32x4(Xbyak::Ymm(vreg_t1.getIdx()), Xbyak::Ymm(vreg_t1.getIdx()),
               Xbyak::Ymm(vreg_t2.getIdx()), 0b00);
    vpermt2d(zreg_res, vpermt2d_arg_idx, vreg_t1);
    vpshufb(zreg_res, zreg_res, vpshufb_arg_b);
  }
...
void jit_seq_cpy_48x4::generate() {
  ...
  if (sum_m)
    mov(reg_sum_append, byte[parambase + GET_OFF(sum_append)]);
  ...
  // if (N > 48)
  // div: Unsigned divide EDX:EAX by r/m32, with result stored in EAX :=
  ...
  je(l_n_loop_end, T_NEAR);
  {
    ...
    trans_4x16(v_res0, v_sum0, ptr[s_base], ptr[s_base + reg_ld_src],
               ptr[s_base + reg_ld_src * 2], ptr[s_base + reg_3ld_src]);
    vmovdqa32(zword[reg_dst + BYTES_ZMM * 0],
              v_res0); // store with non-temp mem hint

    // second 16x4
    s_base = s_base + 16;
    trans_4x16(v_res1, v_sum1, ptr[s_base], ptr[s_base + reg_ld_src],
               ptr[s_base + reg_ld_src * 2], ptr[s_base + reg_3ld_src]);
    vmovdqa32(zword[reg_dst + BYTES_ZMM * 1], v_res1);

    // third 16x4
    s_base = s_base + 16;
    trans_4x16(v_res2, v_sum2, ptr[s_base], ptr[s_base + reg_ld_src],
               ptr[s_base + reg_ld_src * 2], ptr[s_base + reg_3ld_src]);
  ...
  je(l_ntail_end[0], T_NEAR);
  {
    ...
    L(l_tail_1);
    trans_4x16(v_res0, v_sum0, ptr[s_base], ptr[s_base + reg_ld_src],
               ptr[s_base + reg_ld_src * 2], ptr[s_base + reg_3ld_src], true);
  ...
  vmovdqa32(zword[reg_sum], vreg_sum_pad_val);
  // lea(reg_sum, ptr[reg_sum + BYTES_ZMM]);  // update omitted as it will not
  // be used later
