...
#define SHUFFLE(fp3, fp2, fp1, fp0)                                            \
  (((fp3) << 6) | ((fp2) << 4) | ((fp1) << 2) | (fp0))
namespace jd {

void jit_softmax_Ab16a::scaleph2i16(const Zmm &zmm1, const Zmm &zmm_scale,
                                    const Zmm &zmm_max, const Zmm &zmm_min) {
...
void jit_softmax_Ab16a::cvtepi16_epi8_shuffle_storeu(
    RegExp dst, const Zmm &zmm0, const Zmm &zmm1, const Zmm &zmm2,
...
void jit_softmax_Ab16a::generate() {
  ...
  for (int i = 1; i < 16; ++i)
    vmovaps(Zmm(i), zmm0); // zmm0~15 is vmax
  mov(ebx, 0);
  mov(r13, src_addr);
  ...
  if (param_.has_badd)
    mov(r12d, dword[reg_param + GET_OFF(ld_badd)]);
  if (param_.has_badd)
    lea(r12d, ptr[r12d * sizeof(float)]);
  cmp(att_tile, 0);
  ...
  for (int i = 16; i < 24; ++i)
    vpxorq(Zmm(i), Zmm(i), Zmm(i)); // zmm16~23 is vsum
  ...
  for (int i = 0; i < 16; i += 2) {
    ...
    exp_approx_f16(zmm25, zmm25, zmm26, zmm27, {zmm28, zmm29, zmm30},
                   {zmm24, zmm31});
  ...
  if (param_.att_tail) {
    ...
    for (int i = 0; i < 16; i += 2) {
      ...
      vaddps(zmm25 | mask, Zmm(i + 1),
             zword[r11 + (i + 1) * 16 * 4]); // subtract max
      vcvtps2ph(ymm25, zmm25, 0x08);
      vshufi64x2(zmm25, zmm24, zmm25, SHUFFLE(1, 0, 1, 0));
      exp_approx_f16(zmm25, zmm25, zmm26, zmm27, {zmm28, zmm29, zmm30},
                     {zmm24, zmm31});
  ...
  for (int i = 0; i < 16; i += 2) {
    ...
    cvtepi16_epi8_shuffle_storeu(r13 + i * 4 * 16, zmm0, zmm1, zmm2, zmm3, zmm4,
                                 zmm5, zmm6, zmm7);
  }
  add(ebx, 1);
  add(r13, 16 * 4 * 16);
  ...
  if (param_.att_tail) {
    for (int i = 0; i < 16; i += 2) {
      ...
      cvtepi16_epi8_shuffle_storeu(r13 + i * 4 * 16, zmm0, zmm1, zmm2, zmm3,
                                   zmm4, zmm5, zmm6, zmm7);
    }
  }
  ...
  db(reinterpret_cast<const uint8_t *>(exp_approx_f16_coeff.data()),
     sizeof(exp_approx_f16_coeff));
}
