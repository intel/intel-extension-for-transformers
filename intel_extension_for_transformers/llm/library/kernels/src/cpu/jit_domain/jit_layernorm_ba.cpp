...
void jit_layernorm_ba_t::direct_gen() {
  ...
  while ((unroll_degree > 0) && ((align_col / (unroll_degree * 16) == 0) ||
                                 (align_col % (unroll_degree * 16) != 0)))
    unroll_degree--;
  // reg alisa
  ...
  int col_loop = 0;
  if (unroll_degree != 0)
    col_loop = param_.col_num / (unroll_degree * 16);
  // set mask
  if (tail > 0) {
    unsigned int mask = 0;
  ...
  if (col_loop > 0) {
    ...
    if (param_.split_output)
      add(dst2_addr_cp, param_.col_num * get_data_size(param_.output2_dt));
    cmp(Xbyak::Reg32(reg_row.getIdx()),
        ptr[reg_param + LNBA_GET_OFF(process_row)]);
    ...
    if (param_.split_output)
      add(dst2_addr, unroll_degree * 16 * get_data_size(param_.output2_dt));
...
void jit_layernorm_ba_t::direct_handel_norm(int degree, Reg64 src_addr,
                                            Reg64 dst_addr, Reg64 reg_mean,
                                            bool tail) {
  vbroadcastss(Zmm(2 * degree),
               dword[reg_alpha + get_data_size(data_type::fp32) * reg_row]);
  for (int i = 0; i < degree; i++)
    vdivps(Zmm(i + degree), Zmm(degree * 2), Zmm(i));
  for (int i = 0; i < degree; i++) {
    vmovups(Zmm(i + degree * 2), dword[src_addr + i * zmm_byte_size]);
    // dt convert when input is int32
    if (param_.input_dt == data_type::s32)
      vcvtdq2ps(Zmm(i + degree * 2), Zmm(i + degree * 2));
    vsubps(Zmm(i + degree * 2), Zmm(i + degree * 2),
           dword[reg_mean + i * zmm_byte_size]);
    vfmadd213ps(Zmm(i + degree * 2), Zmm(i + degree),
                ptr_b[reg_beta + get_data_size(data_type::fp32) * reg_row]);
    if (param_.split_output) {
      vmovups(tail ? ptr[dst_addr + i * zmm_byte_size] | tail_mask
                   : ptr[dst_addr + i * zmm_byte_size],
              Zmm(degree * 2 + i));
      eltwise_injector.vector_compute(Zmm(degree * 2 + i), param_.postop_attrs);
      switch (param_.output2_dt) {
      case data_type::s8:
        vpmovsdb(tail ? ptr[dst2_addr_cp + i * xmm_byte_size] | tail_mask
                      : ptr[dst2_addr_cp + i * xmm_byte_size],
                 Zmm(degree * 2 + i));
        break;

      case data_type::u8:
        vpmovusdb(tail ? ptr[dst2_addr_cp + i * xmm_byte_size] | tail_mask
                       : ptr[dst2_addr_cp + i * xmm_byte_size],
                  Zmm(degree * 2 + i));
        break;
      default:
      ...
      // apply postops.
      if (!param_.postop_attrs.empty())
        eltwise_injector.vector_compute(Zmm(degree * 2 + i),
                                        param_.postop_attrs);
      // store data
      ...
      case data_type::fp32:
        ...
        break;
      case data_type::s8:
        vpmovsdb(tail ? ptr[dst_addr + i * xmm_byte_size] | tail_mask
                      : ptr[dst_addr + i * zmm_byte_size],
                 Zmm(degree * 2 + i));
        break;
      case data_type::u8:
        vpmovusdb(tail ? ptr[dst_addr + i * xmm_byte_size] | tail_mask
                       : ptr[dst_addr + i * xmm_byte_size],
                  Zmm(degree * 2 + i));
        break;
      default:
        SPARSE_LOG(FATAL)
            << "unsupported output data type in direct translnorm.";
        break;
      }
    }
  }
...
void jit_layernorm_ba_t::normal_gen() {
  ...
  add(reg_src_offset,
      unroll_degree * param_.col_num * get_data_size(param_.input_dt));
  add(reg_dst_offset,
      unroll_degree * param_.col_num * get_data_size(param_.output_dt));
  ...
  mov(reg_affine_offset,
      (param_.row_num - unroll_degree) *
          get_data_size(data_type::fp32)); // dt(affine)==fp32
  L(norm_loop_start);
  ...
  for (int k = 0; k < unroll_degree; k++) {
    ...
    vbroadcastss(
        zmm_alpha,
        dword[reg_alpha + reg_affine_offset +
              k * get_data_size(data_type::fp32)]); // dt(aplha)==dt(beta)==fp32
    vbroadcastss(zmm_beta, dword[reg_beta + reg_affine_offset +
                                 k * get_data_size(data_type::fp32)]);
  ...
  for (int k = 0; k < unroll_degree; k++) {
    ...
    for (auto &&attr : param_.binaryop_attrs) {
      if (attr.op_alg == binaryop_alg::per_channel_quant ||
          attr.op_alg == binaryop_alg::per_channel_dequant) {
    ...
    if (param_.output_dt == data_type::fp32) {
      ...
      if (param_.output_dt == data_type::u8)
        vpmovusdb(ptr[dst_addr + reg_dst_offset + dst_load_offset[k]], Zmm(k));
      if (param_.output_dt == data_type::s8)
        vpmovsdb(ptr[dst_addr + reg_dst_offset + dst_load_offset[k]], Zmm(k));
    }
  }
  ...
  add(reg_batch, 1);
  add(src_addr,
      param_.col_num * param_.row_num * get_data_size(param_.input_dt));
  add(dst_addr,
      param_.col_num * param_.row_num * get_data_size(param_.output_dt));
...
void jit_layernorm_ba_t::escape_regs(int degree) {
  ...
  for (int i = 0; i < degree; i++)
    eltwise_injector.escape_regs(reg_type::zmm, i);
}
...
void jit_layernorm_ba_t::assign_regs() {
  ...
  reg_map.insert(
      std::pair<reg_type, std::set<int>>(reg_type::zmm, {zmm_one.getIdx()}));
  ...
  reg_batch = r14;
  dst2_addr =
      rsi; // Don't need to notify injector cause injector will skip these
           // special usage registers(e.g. rsi,rdx) when reg allocating.
  dst2_addr_cp = rdx;
  reg_map.insert(std::pair<reg_type, std::set<int>>(
      reg_type::reg64,
      {reg_affine_offset.getIdx(), reg_row.getIdx(), reg_col.getIdx(),
       src_addr.getIdx(), reg_alpha.getIdx(), reg_src_offset.getIdx(),
       reg_dst_offset.getIdx(), reg_beta.getIdx(), dst_addr.getIdx(),
       reg_batch.getIdx()}));

  tail_mask = Opmask(1);
  reg_map.insert(
      std::pair<reg_type, std::set<int>>(reg_type::mask, {tail_mask.getIdx()}));
}

void jit_layernorm_ba_t::normal_gen_load_offset() {
  size_t src_offset = param_.thread_elt_offset * get_data_size(param_.input_dt);
  size_t dst_offset =
      param_.thread_elt_offset * get_data_size(param_.output_dt);
...
void jit_layernorm_ba_t::normal_reset_unroll_reg_idxs(int degree) {
  ...
  for (int i = 0; i < degree; i++)
    unroll_reg_idxs.push_back(true);
}
...
std::pair<int, int> jit_layernorm_ba_t::normal_get_unroll_add_idx(int begin) {
  ...
  while (first_idx == -1 || second_idx == -1) {
    ...
    if (unroll_reg_idxs[iter] == true && first_idx == -1) {
      first_idx = iter;
    } else if (unroll_reg_idxs[iter] == true && first_idx != -1 &&
               second_idx == -1) {
      second_idx = iter;
