...
void jit_matmul_avx512f_p2031_p2013_t::calc_THxkxTW() {
  ...
  if (param_.beta != 0.f) {
    ...
    for (int i = 0; i < TH_; i++)
      for (int j = 0; j < TW_; ++j)
        vmulps(dst_tile_Vmm(i, j), vreg_beta,
               zword[reg_src2 + i * ld_src2 + j * BYTES_ZMM]);
  } else {
  ...
  for (int kk = 0; kk < UNROLL_K; kk++) {
    for (int i = 0; i < TH_; i++) {
      vpbroadcastd(
          vreg_temp,
          dword[reg_src0 + ld_src0 * kk + dsize_src0 * i]); // load src0
      for (int j = 0; j < TW_; ++j) {
        // load src1
        if (i == 0)
          vmovups(TW_Vmm(j), ptr[reg_src1 + ld_src1 * kk +
                                 BYTES_ZMM * j]); // 64 is #bytes of a zmm
...
void jit_matmul_avx512f_p2031_p2013_t::generate() {
  ...
  add(reg_src0, TH_ * dsize_src0); // notice that src0 is transposed to it
                                   // multiplies dsize rather than ld
