...
public:
  explicit jit_groupnorm_t(const groupnorm_param_t &param)
    ...
    for (auto &i : param_.postop_attrs) {
      if (i.op_alg != postop_alg::quantize &&
          i.op_alg != postop_alg::dequantize &&
  ...
  void sum_code_gen(regs_pool *rp, Reg64 reg_src, Reg64 reg_sum_x,
                    Reg64 reg_sum_powx, Opmask sun_write_mask,
                    const Xbyak::Label &data_label, size_t sum_dim);
  void calc_scale_and_norm(regs_pool *rp, Reg64 reg_src, Reg64 reg_dst,
                           Reg64 reg_sum_x, Reg64 reg_sum_powx, Reg64 reg_gamma,
                           Reg64 reg_beta, const Xbyak::Label &div_const_label,
...
public:
  explicit jit_channelwise_sum_t(const groupnorm_param_t &param)
      : jit_groupnorm_t(param) {}
...
public:
  explicit jit_channelwise_norm_t(const groupnorm_param_t &param)
      : jit_groupnorm_t(param) {}
