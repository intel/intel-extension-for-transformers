...
/**
 * @brief jit_trans_AB16a4b_16x transpose a matrix of bytes of size 16xn (ab) to
 * (1)x(n/4)x16x4 (AB16a4b). Paddings are set to zero.
 *
 * e.g. 384x64 => 24x16x16x4; 384x32 => 24x8x16x4
 *
 * @note Comparing to jit_trans_AB16a4b: it supports dynamic shape & loop along
 * N
 */
class jit_trans_AB16a4b_16x : public jit_generator {
public:
  struct param_t {
    int pad_n; // pad the tail along N; should be no greater then 64 and be a
               // multiple of 4
  ...
  explicit jit_trans_AB16a4b_16x(const jit_trans_AB16a4b_16x::param_t &param)
      : jit_generator(), pad_n(param.pad_n), cvt_s8u8(param.cvt_s8u8),
        tile_m(param.tile_m) {
    SPARSE_LOG_IF(FATAL, pad_n % 4 != 0 || pad_n > 64) << "Improper pad_n!";
    SPARSE_LOG_IF(FATAL, tile_m <= 0) << "Improper tile_m!";
    SPARSE_LOG_IF(WARNING, pad_n % 16 != 0)
        << "pad_n value of " << pad_n << " may lead to suboptimal performance";
  }
...
private:
  ...
  void transpose_16x16_ps( //
      ...
      const Xbyak::Reg64 &ld_src, const Xbyak::Reg64 &reg_tmp, const int M,
