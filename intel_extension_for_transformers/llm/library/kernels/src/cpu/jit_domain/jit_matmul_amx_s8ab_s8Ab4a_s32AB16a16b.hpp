...
/**
 * @brief jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b does matrix multiplication to a
 * non-transposed and a transposed matrix, saving result in blocked format
 * AB16a16b with 0 padding.
 *
...
public:
  ...
  explicit jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b(
      const jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b::param_t &param)
      : jit_generator(), M(param.M), K(param.K), N(param.N),
        ld_src0(param.ld_src0), pre_amx_cfg_(param.pre_amx_cfg),
        required_amx_cfg_(16, 16, std::min(64, K), false, 4) {
    SPARSE_LOG_IF(ERROR, K > ld_src0)
        << "K > ld_src0 which may lead to unexpected behavior!";
    SPARSE_LOG_IF(FATAL, K > 64 && K % 64 != 0)
        << "Currently only support K <= 64 or K as a multiple of 64";
    SPARSE_LOG_IF(FATAL, K % 4 != 0)
        << "Currently only support K as a multiple of 4";
