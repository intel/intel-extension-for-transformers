...
#define GET_OFF(field)                                                         \
  offsetof(jit_mm_exp_vnni_mxkx48_t::rt_data_t<void>, field)

namespace jd {
jit_mm_exp_vnni_mxkx48_t::jit_mm_exp_vnni_mxkx48_t(const param_t &param)
    : jit_generator(), N_(param.dst_N), TW_(ceil_div(N_, 16L)),
      bias_lshift(param.bias_lshift), binary_add(param.binary_add),
      dt_dst(param.dt_dst), dsize_dst(get_data_size(dt_dst)) {
...
Xbyak::Zmm jit_mm_exp_vnni_mxkx48_t::dst_scale_Vmm(int j) {
  return Xbyak::Zmm(3 + j);
}
...
void jit_mm_exp_vnni_mxkx48_t::generate() {
    ...
    for (int j = 0; j < TW_; ++j) {
      ...
      if (binary_add) {              // f32 x scale + badd
        vfmadd213ps(dst_tile_Vmm(i, j), vreg_scale,
                    zword_b[reg_src_b0 + i * sizeof(float)]);
      } else {
        vmulps(dst_tile_Vmm(i, j), dst_tile_Vmm(i, j), vreg_scale);
      }
      exp_approx_f32(dst_tile_Vmm(i, j), dst_tile_Vmm(i, j),
                     dword[rip + l_log2e], dword[rip + l_ln2], vreg_c,
                     vreg_exp_temp);
      vaddps(dst_scale_Vmm(j), dst_scale_Vmm(j), dst_tile_Vmm(i, j));

      const bool needs_mask = N_ % VEC != 0 && j == TW_ - 1;
      const Xbyak::Address out =
          ptr[reg_dst + j * VEC * dsize_dst] | (needs_mask ? mask_n : k0);
  ...
  for (int j = 0; j < TW_; ++j) {
    const bool needs_mask = N_ % VEC != 0 && j == TW_ - 1;
    const Xbyak::Address out =
        ptr[reg_dst + j * VEC * sizeof(float)] | (needs_mask ? mask_n : k0);
