...
jit_matmul_vnni_Ba4b_Ab4a_ba_t::get_Ymm(int start, int num) const {
...
Xbyak::Zmm jit_matmul_vnni_Ba4b_Ab4a_ba_t::src0_tile_Vmm(int i) {
  return Xbyak::Zmm(i);
}
...
void jit_matmul_vnni_Ba4b_Ab4a_ba_t::generate() {
  ...
  for (int ik = 0; ik < TK_; ++ik) {
    ...
    for (int i = 0; i < TH_; ++i) {
      vmovups(src0_tile_Vmm(i * 2 + (1 - src0_group)),
              zword[reg_src0 + ((ik + 1) * TH_ + i) * BYTES_ZMM]);
    }

    for (int j = 0; j < TW_; ++j) {
      // load src1 for next run
      vpbroadcastd(vreg_temp[1 - src1_group],
                   dword[reg_src1 + (j + 1) * VNNI_ADJ + ik * TW_ * VNNI_ADJ]);

      // inner prod
      for (int i = 0; i < TH_; ++i) {
        vpdpbusds(dst_tile_Vmm(i, j), src0_tile_Vmm(i * 2 + src0_group),
                  vreg_temp[src1_group]);
      }
  ...
  for (int j = 0; j < TW_; ++j) {
    for (int i = 0; i < TH_; ++i) {
      ...
      vfmadd132ps(dst_tile_Vmm(i, j), vreg_zp,
                  vreg_scale); // multiplies scaler and add zp
      vcmpleps(reg_k1, vreg_zero,
               dst_tile_Vmm(i, j)); // mask of everything greater than 0
      vcvtps2udq(dst_tile_Vmm(i, j) | T_z | reg_k1,
                 dst_tile_Vmm(i, j)); // fp32->u32
      vpmovusdb(ptr[reg_dst + ld_dst * j + VNNI_GROUPS * dsize_dst * i],
                dst_tile_Vmm(i, j)); // store result
    }
  }
