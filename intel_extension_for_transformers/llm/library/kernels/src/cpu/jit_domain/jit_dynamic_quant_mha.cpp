...
void jit_mmsoftmax_batch_amx_s8_ab_BA16b4a_u8_16x::mm_exp_sum( //
    regs_pool *const rp, const Reg64 &reg_tmpf32,
  ...
  for (int i = 1; i < 16; ++i)
    vmovdqa32(zmm_expsum[i], zmm_expsum[0]);
  ...
  const auto mm_exp_sum_16xkx16tw = [&](int tw) {
    { // k loop part
      ...
      add(reg_src1,
          qword[rp->p[0] +
                PARAM_OFF(src1)]); // reg_src1 = src1 + reg_ld_src1 * reg_nbiter
      if (runtime_kloop)
      ...
      { // k loop body
        for (int j = 0; j < tw; ++j) {
          // load src0
          if (runtime_kloop && j == 0)
            tileloadd(tmm_src0[0], ptr[reg_src0 + reg_ld_src0]);
    ...
    { // tile epilogue: exp & sum & store
      ...
      for (int ii = 0; ii < 16; ++ii) {
        for (int j = 0; j < tw; ++j) {
          ...
          if (j == 0)
            vmulps(vreg_src0scale, zmm_scale,
                   zword_b[reg_src0scale + ii * sizeof(float)]);
          if (ii == 0)
            vmovups(vreg_src1scale[j], zword[reg_src1scale + j * BYTES_ZMM]);
          if (ii == 0 && has_bias)
            vmovups(vreg_bias[j], zword[reg_bias + j * BYTES_ZMM]);

          const auto tmp_dst_addr =
              ptr[reg_tmpf32 + j * BYTES_TMM + ii * BYTES_ZMM];
          ...
          exp_approx_f32(zmm_x, zmm_x, zword_b[rip + l_log2ef],
                         zword_b[rip + l_ln2], vreg_c, rp->regs<Zmm, 2>());
...
void jit_mmsoftmax_batch_amx_s8_ab_BA16b4a_u8_16x::quant255_store(
    regs_pool *const rp, const Xbyak::Reg64 &reg_tmpf32,
  ...
  { // n loop body
    ...
    for (int ii = 0; ii < 16; ++ii) {
      ...
      vmulps(zmm_x, zmm_expscale[ii],
             zword[reg_tmpf32 + ii * BYTES_ZMM]); // quant
      vcvtps2udq(zmm_x, zmm_x);
      vpmovusdb(ptr[reg_dst + reg_dst_off], zmm_x);
      if (ii != 15)
        lea(reg_dst_off, ptr[reg_dst_off + reg_tmp_ld_dst]);
    }
  }
...
void jit_mmsoftmax_batch_amx_s8_ab_BA16b4a_u8_16x::generate() {
                                           ...
                                           [&](...) { outLocalLabel(); }};
  {
    ...
    const auto stack_size =
        (need_cfg_amx ? sizeof(tileconfig_t) : 0) + tmp_mem_size;
    ...
    // stacksize = (N + 16) * M * sizeof(float) + 64  // 16 as enough padding;
    // 64 for tmp memory
    mov(reg_stacksize.cvt32(), dword[rp.p[0] + PARAM_OFF(N)]);
    imul(reg_stacksize, reg_stacksize,
         M * sizeof(float)); // (N) * M * sizeof(float)
    lea(reg_stacksize, ptr[reg_stacksize + 16 * M * sizeof(float) +
                           64]); // (+16) * M * sizeof(float)
    std::shared_ptr<void> use_dyn_stack = {
        (sub(rsp, reg_stacksize), nullptr),
    ...
    L(l_bloop);
    {
      ...
      for (int ii = 0; ii < 16; ++ii)
        vpbroadcastd(zmm_expscale[ii], dword[m512_tmp + ii * sizeof(float)]);
    ...
    { // tile epilogue: exp & sum & store
      ...
      for (int ii = 0; ii < 16; ++ii) {
        for (int j = 0; j < tw; ++j) {
          ...
          const auto curr_dst_addr =
              ptr[reg_dst + j * BYTES_TMM + ii * BYTES_ZMM];
...
void jit_mmexp_amx_s8_ab_BA16b4a_u8_16x::generate() {
                                           ...
                                           [&](...) { outLocalLabel(); }};
  {
    ...
    { // init max exp; 0 should be small enough as the range of exp greater than
      // 0
      const auto vreg_zero = rp.reg<Zmm>();
      vxorps(vreg_zero, vreg_zero, vreg_zero);
      for (int i = 0; i < 16; ++i)
        vmovaps(zword[addr_max + i * BYTES_ZMM], vreg_zero);
    }
    ...
    }
    {
      ...
      for (int i = 0; i < 16; ++i)
        vmovaps(zmm_expmax[i], zword[addr_max + i * BYTES_ZMM]);
...
#define PARAM_OFF(field) offsetof(rt_data_t, field)
void jit_mm_batch_amx_u8s8_ab_AB16a4b_dynamic_quant_16x::mm_absmax(
    regs_pool *const rp, const Xbyak::Reg64 &reg_tmpf32,
    ...
    { // tile epilogue: scale & absmax & store
      ...
      for (int ii = 0; ii < 16; ++ii) {
        for (int j = 0; j < tw; ++j) {
          ...
          vmovaps(tmp_dst_addr, zmm_x);
          vrangeps(zmm_absmax[ii], zmm_absmax[ii], zmm_x,
                   0b1011); // 1011 for absmax
        }
      }
    }
  };
  ...
  jz(l_end, T_NEAR);
  {
    ...
    kmovd(mask_tail16, reg_tmp.cvt32());
    {
      ...
      for (int ii = 0; ii < 16; ++ii) {
        ...
        if (ii !=
            15) // omit update the last reg_dst_off to save one instruction
          lea(reg_dst_off, ptr[reg_dst_off + reg_ld_dst]);
      }
    }
  }
...
void jit_mm_batch_amx_u8s8_ab_AB16a4b_dynamic_quant_16x::generate() {
                                           ...
                                           [&](...) { outLocalLabel(); }};
  {
    ...
    // stacksize = (N + 16) * M * headsize * sizeof(float) + 64  // 16 as enough
    // padding; 64 for tmp memory
    mov(reg_headstacksize.cvt32(), dword[rp.p[0] + PARAM_OFF(N)]);
    imul(reg_headstacksize.cvt32(), reg_headstacksize.cvt32(),
         M * sizeof(float)); // (N) * M * sizeof(float)
    lea(reg_headstacksize.cvt32(),
        ptr[reg_headstacksize.cvt32() + 16 * M * sizeof(float) +
            64]); // (+16) * M * sizeof(float)
    mov(reg_stacksize.cvt32(), reg_headstacksize.cvt32());
    imul(reg_stacksize.cvt32(),
         dword[rp.p[0] + PARAM_OFF(batch_size)]); // (N + 16) * M *
    ...
    for (int i = 1; i < 16; ++i)
      vmovdqa32(zmm_absmax[i], zmm_absmax[0]);
    ...
    const auto &zmm_rcpscale = zmm_absmax;
    {
      ...
      reduce_vmms(zmm_absmax, [this](auto &_0, auto &_1, auto &_2) {
        vrangeps(_0, _1, _2, 0b1011);
      });
      ...
      vaddps(zmm_absmax[0], zmm_absmax[0],
             zword_b[rip + l_float_epsilon]);  // avoid zero division
      vdivps(zmm_tmp, zmm_127, zmm_absmax[0]); // 127 / max
      vmovdqa32(zword[m512_tmp], zmm_tmp);     // rcpscale
      for (int ii = 0; ii < 16; ++ii)
        vpbroadcastd(zmm_rcpscale[ii], dword[m512_tmp + ii * sizeof(float)]);
    }
...
#define PARAM_OFF(field) offsetof(rt_data_t, field)
void jit_scale_mm_amx_u8s8_ab_BA16b_16x::mm_absmax(
    regs_pool *const rp, std::array<Xbyak::Zmm, 16UL> zmm_absmax) {
  const auto reg_nbiter = rp->reg<Reg64>(); // iterations of blocks of size 16
  xor_(reg_nbiter.cvt32(), reg_nbiter.cvt32());
  const auto reg_dst = rp->reg<Reg64>();
  mov(reg_dst, qword[rp->p[0] + PARAM_OFF(dst)]);
  ...
  const auto mm_absmax_16xkxtw = [&](int tw) {
    ...
    { // k loop part
      ...
      lea(reg_src0, ptr[reg_src0 + BYTES_TMM * sizeof(float)]); // 4 float tile
                                                                // => 1 u8 tile
      lea(reg_src1, ptr[reg_src1 + BYTES_TMM]);
      lea(reg_kiter, ptr[reg_kiter + 64]);
      cmp(reg_kiter.cvt32(), dword[rp->p[0] + PARAM_OFF(K)]);
    ...
    { // tile epilogue: scale & absmax & store
      ...
      for (int ii = 0; ii < 16; ++ii) {
        ...
        (ii == 0) ? mov(reg_curr_dst, reg_dst)
                  : lea(reg_curr_dst, ptr[reg_curr_dst + reg_lb_dst]);
        for (int j = 0; j < tw; ++j) {
          if (ii == 0) {
...
void jit_scale_mm_amx_u8s8_ab_BA16b_16x::generate() {
                                           ...
                                           [&](...) { outLocalLabel(); }};
  {
    ...
    regs_pool rp(this, 1, {10, 32, 0},
                 (need_cfg_amx ? sizeof(tileconfig_t) : 0) + tmp_mem_size, 0,
                 64);
    std::shared_ptr<void> local_cfg;
    if (need_cfg_amx) { // create a local amx config environment
      sttilecfg(ptr[rsp]);
