...
public:
  struct params {
    ...
    params() = default;
    params(int att_tail_, int sl_pad64_, bool has_badd_ = false,
           std::string output_type_ = "s8")
        : att_tail(att_tail_), sl_pad64(sl_pad64_), has_badd(has_badd_),
  ...
  explicit jit_softmax_Ab16a(const params &param)
...
private:
  void cvtepi16_epi8_shuffle_storeu(RegExp dst, const Zmm &zmm0,
                                    const Zmm &zmm1, const Zmm &zmm2,
                                    const Zmm &zmm3, const Zmm &tmp0,
                                    const Zmm &tmp1, const Zmm &tmp2,
                                    const Zmm &tmp3);
  void exp_ph_0_1(const Zmm &src, const Zmm &dst, const Zmm &magic_number0,
                  const Zmm &magic_number1, const Zmm &magic_number2,
                  const Zmm &magic_number3, const Zmm &magic_number4,
                  const Reg64 bd);
  void scaleph2i16(const Zmm &zmm, const Zmm &zmm_scale, const Zmm &tmp1,
                   const Zmm &tmp2);
  void assign_regs() {
    src_addr = r8;
    dst_addr = r9;
  ...
  float16_t min;
  float16_t max;
  const int16_t perm_data[32] = {1,  0,  3,  2,  5,  4,  7,  6,  9,  8,  11,
