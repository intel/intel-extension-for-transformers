...
void jit_softmax_t::generate() {
  ...
  if (param_.sepc_type == ssd::spec_softmax_type::lut)
    get_lut_exp_injector.prepare_table();
  eltwise_injector.prepare_table();
}

void jit_softmax_t::lut_softmax_kernel_gen() {
  // for eltwise injector regs allocating, we need to escape the regs which we
  // care.
  for (auto &&i : reg_map) {
    for (auto &&j : i.second) {
  ...
  vshuff32x4(zmm_tmp, zmm_exp_neg_max, zmm_exp_neg_max,
             0x4E); //  horizontal max
  vpmaxsw(zmm_exp_neg_max, zmm_exp_neg_max,
          zmm_tmp); // ymm_exp_neg_max contain the max value(s16)
  vpmovsxwd(zmm_exp_neg_max,
            ymm_exp_neg_max); // s16 cvt s32 and store into the zmm_exp_neg_max
  ...
  vpbroadcastb(zmm_exp_neg_max,
               xmm_exp_neg_max); // zmm_exp_neg_max contain all max int8 value
  ...
  lut_handle_exp();
  add(src_addr_volatile,
      isa_available(avx512_core_bf16) ? ymm_byte_size : xmm_byte_size);
  add(dst_addr_volatile, zmm_byte_size);
  add(vec_offset, isa_available(avx512_core_bf16) ? process_element_16bit
                                                  : process_element_32bit);
  cmp(vec_offset, param_.vec_align_len);
  ...
  reduce_dwords(zmm_denominator, zmm_tmp,
                &CodeGenerator::vaddps); // horizontal sum
  vdivps(zmm_denominator, zmm_one_fp32,
         zmm_denominator); // calculate denominator
  ...
  if (isa_available(avx512_core_bf16)) {
    for (int i = 0; i < unroll; i++)
      vmovups(Ymm(10 + i),
              ptr[src_addr_volatile + i * ymm_byte_size]); // exp value(bf16)
    for (int i = 0; i < unroll; i++)
      bf16_cvt_fp32(Zmm(10 + i));
  } else {
    for (int i = 0; i < unroll; i++)
      vmovups(Zmm(10 + i),
              ptr[src_addr_volatile + i * zmm_byte_size]); // exp value(bf16)
  }
  for (int i = 0; i < unroll; i++)
    vmulps(Zmm(10 + i), Zmm(10 + i), zmm_denominator);

  // apply postops
  if (param_.postop_attrs.size() != 0) {
    for (int i = 0; i < unroll; i++)
      eltwise_injector.vector_compute(Zmm(10 + i), param_.postop_attrs);
  }

  // store data.
  for (int i = 0; i < unroll; i++)
    lut_store_data(10 + i, dst_addr_volatile, i);
  add(src_addr_volatile, isa_available(avx512_core_bf16)
                             ? unroll * ymm_byte_size
                             : unroll * zmm_byte_size);
  ...
  add(dst_addr, get_data_size(param_.output_dt) *
                    (param_.vec_align_len + param_.vec_tail_len));
  if (isa_available(avx512_core_bf16)) {
    add(reg_tmp, get_data_size(data_type::bf16) *
                     (param_.vec_align_len + param_.vec_tail_len));
  } else {
    add(reg_tmp, get_data_size(data_type::fp32) *
                     (param_.vec_align_len + param_.vec_tail_len));
  }
  inc(vec_num);
...
void jit_softmax_t::lut_handle_exp(bool tail) {
  ...
  get_lut_exp_injector.vector_compute(zmm_vec,
                                      param_.get_lut_exp_attrs); // e^(x-max)
  if (!tail) {
    if (isa_available(avx512_core_bf16)) {
    ...
    vmovups(ptr[dst_addr_volatile],
            zmm_vec); // now tmp store the bf16/fp32 exp value
  } else {
    if (isa_available(avx512_core_bf16))
    ...
    if (param_.vec_tail_len < 16) {
      ...
      if (!isa_available(avx512_core_bf16))
        vmovups(ptr[dst_addr_volatile] | bit16_mask, zmm_vec);
    } else {
...
void jit_softmax_t::lut_store_data(int simd_idx, Reg64 dst, int offset,
                                   bool mask) {
  if (param_.output_dt == data_type::u8) {
    vpmovusdb(mask ? ptr[dst + offset * xmm_byte_size] | bit32_mask
                   : ptr[dst + offset * xmm_byte_size],
              Zmm(simd_idx));
  } else if (param_.output_dt == data_type::s8) {
    vpmovsdb(mask ? ptr[dst + offset * xmm_byte_size] | bit32_mask
                  : ptr[dst + offset * xmm_byte_size],
             Zmm(simd_idx));
  } else if (param_.output_dt == data_type::bf16) {
    fp32_cvt_bf16(Zmm(simd_idx));
    vmovdqu16(mask ? ptr[dst + offset * ymm_byte_size] | bit32_mask
                   : ptr[dst + offset * ymm_byte_size],
              Ymm(simd_idx));
  } else {
    vmovups(mask ? ptr[dst + offset * zmm_byte_size] | bit32_mask
                 : ptr[dst + offset * zmm_byte_size],
            Zmm(simd_idx));
  }
}
...
void jit_softmax_t::get_unroll() {
  unroll = 8;
  while (param_.vec_align_len % (unroll * process_element_32bit) != 0)
    unroll--;
  if (param_.vec_tail_len >= 16)
    unroll += 1;
}
...
void jit_softmax_t::prepare_mask() {
  int mask = 0x0;
  for (size_t i = 0; i < param_.vec_tail_len; i++)
    mask = (mask << 1) + 1;
  mov(reg_tmp.cvt32(), mask);
  kmovd(bit16_mask, reg_tmp.cvt32());
  mask = 0x0;
  for (size_t i = 0; i < param_.vec_tail_len % 16; i++)
    mask = (mask << 1) + 1;
  mov(reg_tmp.cvt32(), mask);
...
void jit_softmax_t::assign_regs() {
  ...
  reg_map.insert(std::pair<reg_type, std::set<int>>(
      reg_type::reg64,
      {src_addr.getIdx(), dst_addr.getIdx(), vec_num.getIdx(),
       vec_offset.getIdx(), reg_tmp.getIdx(), src_addr_volatile.getIdx(),
       dst_addr_volatile.getIdx()}));

  bit16_mask = Opmask(6);
  bit32_mask = Opmask(7);
  reg_map.insert(std::pair<reg_type, std::set<int>>(
      reg_type::mask, {bit16_mask.getIdx(), bit16_mask.getIdx()}));
  ...
  reg_map.insert(std::pair<reg_type, std::set<int>>(
      reg_type::zmm,
      {zmm_vec.getIdx(), zmm_denominator.getIdx(), zmm_exp_neg_max.getIdx(),
       zmm_one_bf16.getIdx(), zmm_one_fp32.getIdx()}));
}
} // namespace jd
