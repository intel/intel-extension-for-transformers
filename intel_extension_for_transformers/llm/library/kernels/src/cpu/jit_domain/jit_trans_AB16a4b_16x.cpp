...
void jit_trans_AB16a4b_16x::transpose_16x16_ps( //
    regs_pool *const rp, const Reg64 &src, const Reg64 &dst,
    const Reg64 &ld_src, const Reg64 &reg_tmp, const int M,
    const Xbyak::Opmask &mask) {
  const auto mat = rp->regs<Xbyak::Zmm, 16>();
  const auto tmp = rp->regs<Xbyak::Zmm, 16>();
  if (cvt_s8u8)
    vmovdqa32(tmp[0], zword[rip + l_128x64]);
  ...
  for (int i = 0; i < 16; ++i) {
    if (i < M) {
      ...
      if (mask != k0)
        ...
        !cvt_s8u8 ? vmovdqu8(mat[i], zword[src + reg_tmp])
                  : vpaddb(mat[i], tmp[0], zword[src + reg_tmp]);
    } else {
  ...
  // transpose 16 zmms
  if (M > 0)
    jit_generator::transpose_16x16_ps(mat, tmp, 16);

  // move out
  for (int i = 0; i < (mask == k0 ? 16 : pad_n / 4); ++i)
    vmovdqa32(ptr[dst + i * tile_m * BYTES_ZMM], mat[i]);
}

void jit_trans_AB16a4b_16x::generate() {
  regs_pool rp(this, 1, {7 + (tile_m > 1 ? 2 : 0), 32, 1}, 0,
               regs_pool::DisableEpilog);
  ...
  for (int i = 0; i <= 16 * tile_m; ++i) { // case(0) case(1) ... case(16)
    ...
    { // n loop
      ...
      for (int ii = 0; ii < tile_m * 16; ii += 16) {
        ...
        transpose_16x16_ps(&rp, reg_src_tmp, reg_dst_tmp, reg_ld_src, reg_tmp,
                           std::max(0, std::min(16, i - ii)), k0);
      }
    ...
    for (int ii = 0; ii < tile_m * 16; ii += 16) {
      ...
      transpose_16x16_ps(&rp, reg_src_tmp, reg_dst_tmp, reg_ld_src, reg_tmp,
                         std::max(0, std::min(16, i - ii)), mask_n);
    }
    L(l_end);
    rp.close();
  }
  ...
  align(sizeof(void *));
  L(l_msize_tbl);
  for (int i = 0; i <= 16 * tile_m; ++i)
    putL(l_msize_case[i]);
  align(BYTES_ZMM);
  L(l_128x64);
  for (uint64_t i = 0; i < BYTES_ZMM; ++i)
    db(128);
}
