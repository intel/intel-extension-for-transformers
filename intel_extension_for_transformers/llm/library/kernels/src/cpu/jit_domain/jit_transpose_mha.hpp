...
public:
  ...
  MHA_kernel(size_t size, int m, int n, int k, int npacked)
      : jit_generator(size, nullptr), MTile(m), NTile(n), KTile(k),
        NPacked(npacked) {}
  virtual ~MHA_kernel() {}
...
public:
  MHA_stage1_kernel(size_t size, int m, int n, int k, int npacked)
      : MHA_kernel(size, m, n, k, npacked) {}
...
public:
  MHA_stage2_kernel(size_t size, int m, int n, int k, int npacked)
      : MHA_kernel(size, m, n, k, npacked) {}
  virtual ~MHA_stage2_kernel() {}
  virtual void generate() {}

protected:
  void loadbf16_norm_rows(const Zmm &x, const RegExp &load_addr,
                          const Zmm &scale);
  void loadbf16_norm_rows(int idx, const RegExp &addr, const Zmm &scale) {
    loadbf16_norm_rows(Zmm(idx), addr, scale);
  }
...
public:
  explicit TransposeCopy8x8_1B_kernel(size_t size = 16 * 1024)
      : MHA_kernel(size, 0, 0, 0, 0) {}
...
public:
  explicit MHA_s8s8s8_row_amx_32x32_batchk_binary_exp(int k,
                                                      size_t size = 32 * 1024)
...
public:
  explicit MHA_s8s8s8_row_vnni_8x32_batchk_binary_exp(size_t size = 32 * 1024)
      : MHA_stage1_kernel(size, 8, 32, 8, 0) {}
...
public:
  MHA_norm_quantize_reorder_prescale_packed(int npacked, int tile,
                                            size_t size = 16 * 1024)
...
class MHA_norm_quantize_reorder_vnnib_prescale_packed
    : public MHA_stage2_kernel {
public:
  explicit MHA_norm_quantize_reorder_vnnib_prescale_packed(int NTile,
                                                           size_t size = 16 *
                                                                         1024)
...
class MHA_norm_quantize_reorder_vnniw_prescale_packed
    : public MHA_stage2_kernel {
public:
  MHA_norm_quantize_reorder_vnniw_prescale_packed(int npacked, int tile,
                                                  size_t size = 16 * 1024)
      : MHA_stage2_kernel(size, 0, tile, 0, npacked) {}
...
public:
  explicit MHA_Matmul_s8u8u8_amx_32x32(int k, size_t size = 32 * 1024)
      : MHA_kernel(size, 32, 32, k, 0) {}
...
public:
  explicit MHA_Matmul_s8u8u8_vnni_word_8x32(size_t size = 32 * 1024)
      : MHA_kernel(size, 8, 32, 8, 0) {}
...
public:
  explicit SeqCopy_1B_avx512_Nx4_Temp(int N = 32, size_t size = 16 * 1024)
      : MHA_kernel(size, 0, N, 0, 0) {}
...
public:
  explicit SeqCopy_1B_avx512_Nx2_Temp(int N = 32, size_t size = 16 * 1024)
      : MHA_kernel(size, 0, N, 0, 0) {}
