...
/**
 * @brief jit_transpose_nx8_4b transpose a matrix of bytes and write it to
 * memory with tile size of nx8 in terms of #elements: mk ==> Km4k
 *
 * @tparam tile_m The n in nx8_4b in terms of #elements
 */
template <int tile_m> class jit_transpose_nx8_4b : public jit_generator {
public:
  struct params {
    int K;
    int ld_src;
  ...
  explicit jit_transpose_nx8_4b(const jit_transpose_nx8_4b::params &param)
      : jit_generator(), param_(param) {
    if (tile_m % dim_transpose != 0) {
      SPARSE_LOG(ERROR) << "tile_m must ve divided by dim_transpose"
                        << std::endl;
    }
  }
