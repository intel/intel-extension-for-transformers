...
private:
  ...
  std::array<int, reg_kind_size>
      ridx_next; // The index of next register to be allocated
  const std::array<int, reg_kind_size>
      ridx_max; // max number of registers  to be allocated
  std::array<int, reg_kind_size>
      ridx_touched; // (max register indexever allocated) + 1
  std::unordered_map<std::string, int>
      reg_name_idx_map; // The index of specific reg_name

  template <typename Base, typename R>
  using enable_if_reg_kind_t =
      std::enable_if_t<std::is_base_of<Base, R>::value, reg_kind>;
  FOREACH_REG static constexpr enable_if_reg_kind_t<Xbyak::Reg32e, R>
  get_reg_kind() {
    return reg_kind::gpr;
  }
  FOREACH_REG static constexpr enable_if_reg_kind_t<Xbyak::Xmm, R>
  get_reg_kind() {
    return reg_kind::xmm;
  }
  FOREACH_REG static constexpr enable_if_reg_kind_t<Xbyak::Opmask, R>
  get_reg_kind() {
    return reg_kind::mask;
  }
  FOREACH_REG static constexpr size_t get_reg_kind_i() {
    return static_cast<size_t>(get_reg_kind<R>());
  }

  FOREACH_REG inline int &get_next() { return ridx_next[get_reg_kind_i<R>()]; }
  FOREACH_REG inline int get_next() const {
    return ridx_next[get_reg_kind_i<R>()];
  }
  FOREACH_REG inline int get_max() const {
    return ridx_max[get_reg_kind_i<R>()];
  }
  FOREACH_REG inline int &get_touched() {
    return ridx_touched[get_reg_kind_i<R>()];
  }
  FOREACH_REG inline int get_touched() const {
    return ridx_touched[get_reg_kind_i<R>()];
  }

  /**
   * @brief map idx-th allocated register of kind rt to Xbyak's index used as
  ...
  FOREACH_REG class shared_reg_vec : public std::vector<R>,
                                     private std::shared_ptr<void> {
    friend regs_pool;
    shared_reg_vec(regs_pool *const s, const size_t n)
        : std::vector<R>(), //
          std::shared_ptr<void>{nullptr,
                                [s, n](...) { s->get_next<R>() -= n; }} {
      for (auto _ = n; _-- > 0;)
        this->emplace_back(s->get_next_reg<R>());
    }
  };

  FOREACH_REG_N class shared_reg_arr : public std::array<R, N>,
                                       private std::shared_ptr<void> {
    friend regs_pool;
    using arr_t = std::array<R, N>;
    explicit shared_reg_arr(regs_pool *const s)
        : arr_t(sequence_map_([s](...) { return s->get_next_reg<R>(); },
...
public:
  static const int UseRCX =
      Xbyak::util::UseRCX; // RCX|reg_num[0] to enable usage(reserved but not
                           // alloc) of RCX
  static const int UseRDX =
      Xbyak::util::UseRDX; // RDX|reg_num[0] to enable usage(reserved but not
                           // alloc) of RDX
  const decltype(Xbyak::util::StackFrame::p) &p; // alias of sf.p

  static constexpr int DefaultFlags = 0; // All flags off by default
  static constexpr int DisableEpilog =
      1 << 0; // disable epilog generation during deconstruction
  static constexpr int DisableEvex =
      1
      << 1; // disable the use of EVEX encoded instructions (including zmm16-31)
  static constexpr int IgnoreWaste =
      1
      << 2; // surpress warnings raised when not all registers applied are used
   ...
   * @param flags controlling flags including DisableEpilog / DisableEvex /
   * IgnoreWaste
   * @param stack_align alignment of stack space
   */
  regs_pool( //
      Xbyak::CodeGenerator *const code, const int pNum,
      const std::array<int, 3UL> reg_num = zero_x3, const size_t stack_size = 0,
      const int flags = DefaultFlags, const size_t stack_align = 8)
      : sf_(code, pNum, reg_num[get_reg_kind_i<Reg64>()], 0,
            false), // stack memory and epilogue managed here
        code_(code), stack_size_(stack_size), stack_align_(stack_align),
        make_epilog_(!(flags & DisableEpilog)), evex_(!(flags & DisableEvex)),
        warn_waste_(!(flags & IgnoreWaste)), ridx_next(zero_x3),
        ridx_max({reg_num[0] & (~UseRCX) & (~UseRDX), reg_num[1], reg_num[2]}),
        ridx_touched(zero_x3), p(sf_.p) {
    // availability check
    SPARSE_LOG_IF(FATAL, get_max<Reg64>() + pNum >
                             15) // #{pNum + num_gpr [+rcx] + [rdx]} <= 14
        << "No more GPR registers!";
    SPARSE_LOG_IF(FATAL, get_max<Zmm>() > (evex_ ? 32 : 16))
        << "No more XMM registers!";
    SPARSE_LOG_IF(FATAL, get_max<Opmask>() > 7) << "No more mask registers!";
    SPARSE_LOG_IF(FATAL,
                  stack_align < 8 || (stack_align & (stack_align - 1)) != 0)
    ...
    if (num_to_save > 0) {
      ...
      for (int i = 0; i < num_to_save; ++i)
        code_->movdqu(code_->xword[code_->rsp + i * 16],
    ...
    if (stack_size) {
      if (stack_align <= 8) { // stack is aligned to 8 by default
        ...
        code_->lea(code_->rsp,
                   code_->ptr[tmp - pad_to(stack_size_, stack_align)]);
      }
    }
  }
  ...
  inline void close(const bool call_ret = true) {
    ...
    SPARSE_DLOG_IF(WARNING,
                   warn_waste_ && get_max<Reg64>() > get_touched<Reg64>())
        << "Asked too many GPRs! Actually used: " << get_touched<Reg64>() << "/"
        << get_max<Reg64>();
    SPARSE_DLOG_IF(WARNING, warn_waste_ && get_max<Zmm>() > get_touched<Zmm>())
        << "Asked too many XMMs! Actually used: " << get_touched<Zmm>() << "/"
        << get_max<Zmm>();
    SPARSE_DLOG_IF(WARNING,
                   warn_waste_ && get_max<Opmask>() > get_touched<Opmask>())
        << "Asked too many masks! Actually used: " << get_touched<Opmask>()
        << "/" << get_max<Opmask>();
    ...
    if (stack_size_) {
      if (stack_align_ <= 8) {
        ...
        code_->mov(code_->rsp,
                   code_->ptr[code_->rsp + pad_to(stack_size_, stack_align_)]);
      }
    }
    ...
    if (num_to_save > 0) {
      for (int i = 0; i < num_to_save; ++i)
        code_->movdqu(Xbyak::Xmm(xmm_to_preserve_start + i),
                      code_->xword[code_->rsp + i * 16]);
  ...
  /** @warning Use `auto` hold the result as the referring count will be
   * stripped otherwise. */
  ...
  /** @warning Use `auto` hold the result as the referring count will be
   * stripped otherwise. */
  FOREACH_REG inline const shared_reg_vec<R> regs(size_t n) {
    return shared_reg_vec<R>{this, n};
  }
  /** @warning Use `auto` hold the result as the referring count will be
   * stripped otherwise. */
  FOREACH_REG_N inline const shared_reg_arr<R, N> regs() {
    return shared_reg_arr<R, N>{this};
  }
  inline int get_idx_by_name(std::string reg_name) {
    return reg_name_idx_map[reg_name];
  }
};
#undef FOREACH_REG
#undef FOREACH_REG_N
