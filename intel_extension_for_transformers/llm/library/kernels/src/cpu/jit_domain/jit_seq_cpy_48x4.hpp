...
public:
  ...
  struct param_t {
    ...
    bool is_unsigned; // datatype of either signed or unsigned; only useful when
                      // calculating sum
   ...
   * +------------------------------------ dst_sum
   * ---------------------------------------+ | +=0+48+96+144   +=1+49+97+145
   * +=2+50+98+146   +=3+51+99+147 ...  +=47+95+143+191 |
   * +------------------------------------------------------------------------------------+
   */
  explicit jit_seq_cpy_48x4(const jit_seq_cpy_48x4::param_t &param)
      : jit_generator(), sum_m(param.sum_m), is_unsigned(param.is_unsigned),
        sum_pad_val(param.sum_pad_val) {}
  virtual ~jit_seq_cpy_48x4() {}

private:
  void generate() override;
  void trans_4x16(const Zmm &zreg_res, const Zmm &zreg_sum,
                  const Xbyak::Operand &op0, const Xbyak::Operand &op1,
                  const Xbyak::Operand &op2, const Xbyak::Operand &op3,
                  bool is_tail = false);
