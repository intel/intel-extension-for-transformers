...
/**
 * @brief jit_matmul_amx_u8AB16a64b_s8BA16b4a_ab does matrix multiplication to a
 * non-transposed and a transposed matrix, saving result in plain format of f32
 * or any other data output by the last postop.
 *
...
public:
  ...
  explicit jit_matmul_amx_u8AB16a64b_s8BA16b4a_ab(
      const jit_matmul_amx_u8AB16a64b_s8BA16b4a_ab::param_t &param)
      : jit_generator(), M(param.M), K_pad(param.K_pad), N(param.N),
        dt_dst(param.dst_dt), ld_dst(param.ld_dst),
        lb_dst(ld_dst * type_size.at(dt_dst)) {
    SPARSE_LOG_IF(FATAL, is_all_of({data_type::u8, data_type::s8,
                                    data_type::fp32, data_type::bf16},
                                   [dst = dt_dst](auto t) { return t != dst; }))
        << "Unexpected dt_dst";
    SPARSE_LOG_IF(ERROR, N > ld_dst)
        << "N > ld_dst which may lead to unexpected behavior!";
    SPARSE_LOG_IF(FATAL, K_pad % 64 != 0)
        << "Currently only support K as a multiple of 64";
    SPARSE_LOG_IF(FATAL, M % 16 != 0)
        << "Currently only support M as a multiple of 16";
    SPARSE_LOG_IF(FATAL, N % 16 != 0)
        << "Currently only support N as a multiple of 16";
  }
