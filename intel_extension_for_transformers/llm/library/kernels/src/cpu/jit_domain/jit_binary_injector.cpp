...
void jit_binary_injector::compute_vector(const Xmm &zmm_src1,
                                         const RegExp &src2,
...
void jit_binary_injector::per_channel_quant(const Xmm &src1, const RegExp &src2,
                                            const binaryop_attr &op_attr) {
  ...
  if (op_attr.op_dt == data_type::s8) {
    ...
    SPARSE_LOG(FATAL)
        << "per_channel_quant op only support int8 as input data_type";
  }
}

void jit_binary_injector::per_channel_dequant(const Xmm &src1,
                                              const RegExp &src2,
                                              const binaryop_attr &op_attr) {
  if (op_attr.op_dt == data_type::u8)
    ...
    SPARSE_LOG(FATAL)
        << "per_channel_dequant op only support int8 as input data_type";
...
void jit_binary_injector::mul(const Xmm &src1, const RegExp &src2,
                              data_type op_dt, bool enable_mask,
                              bool broadcast) {
  if (op_dt == data_type::fp16) {
    h->vmulph(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  } else if (op_dt == data_type::fp32) {
    h->vmulps(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  } else {
    SPARSE_LOG(FATAL)
        << "mul op in binary injector only support floating point arithmetic";
  }
}

void jit_binary_injector::add(const Xmm &src1, const RegExp &src2,
                              data_type op_dt, bool enable_mask,
                              bool broadcast) {
  if (op_dt == data_type::fp32)
    h->vaddps(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  if (op_dt == data_type::s32)
    h->vpaddd(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  if (op_dt == data_type::s8)
    h->vpaddb(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  if (op_dt == data_type::u8)
    h->vpaddusb(enable_mask ? src1 | mask : src1, src1,
                broadcast ? h->ptr_b[src2] : h->ptr[src2]);
}

void jit_binary_injector::sub(const Xmm &src1, const RegExp &src2,
                              data_type op_dt, bool enable_mask,
                              bool broadcast) {
  if (op_dt == data_type::fp32)
    h->vsubps(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  if (op_dt == data_type::s32)
    h->vpsubd(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  if (op_dt == data_type::s8)
    h->vpsubb(enable_mask ? src1 | mask : src1, src1,
              broadcast ? h->ptr_b[src2] : h->ptr[src2]);
  if (op_dt == data_type::u8)
    h->vpsubusb(enable_mask ? src1 | mask : src1, src1,
                broadcast ? h->ptr_b[src2] : h->ptr[src2]);
}
...
void jit_binary_injector::get_addr(const Reg64 &reg, binaryop_attr op_attr,
                                   addr_type type) {
  int64_t addr;
  switch (type) {
