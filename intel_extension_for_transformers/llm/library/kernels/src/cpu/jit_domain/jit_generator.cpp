...
bool jit_generator::create_kernel() {
  generate();
  if (std::getenv("SPARSE_LIB_DUMP") != nullptr)
    dump_asm_flag = true;
  if (dump_asm_flag)
    dump_asm();
  jit_ker_ = get_code();
...
template <typename T>
Xbyak::Address jit_generator::EVEX_compress_addr(Xbyak::Reg64 base, T raw_offt,
                                                 bool bcast) {
  using Xbyak::Address;
  using Xbyak::Reg64;
  using Xbyak::RegExp;
...
Xbyak::Address jit_generator::make_safe_addr(const Xbyak::Reg64 &reg_out,
                                             size_t offt,
...
Xbyak::Address jit_generator::EVEX_compress_addr_safe(
    const Xbyak::Reg64 &base, size_t raw_offt, const Xbyak::Reg64 &reg_offt,
    bool bcast) {
  if (raw_offt > INT_MAX) {
...
void jit_generator::transpose_16x16_ps(const std::array<Xbyak::Zmm, 16UL> &src,
...
void jit_generator::tile_product_amx_bf16ps(
  ...
  for (int i = 0; i < TH; i++)
    for (int j = 0; j < TW; j++)
      tilestored(dst_addr(i, j), Tmm(i * 2 + j));
}

const std::array<float16_t, 3> exp_approx_f16_coeff = {
    float16_t(exp_approx_f32_coeff[0]), float16_t(exp_approx_f32_coeff[1]),
    float16_t(exp_approx_f32_coeff[2])};

void jit_generator::vmov_avx2(const RegExp &dst, const RegExp &src,
                              const int bytes, const Xmm &tmp_xmm,
                              const Reg64 &tmp_r64) {
  SPARSE_LOG_IF(FATAL, bytes <= 0 || bytes >= 32)
      << "AVX2 tail length between 1 and 31";
  if (bytes >= 16) {
    vmovups(tmp_xmm, xword[src]);
    vmovups(xword[dst], tmp_xmm);
    if (bytes > 16)
      vmovups(tmp_xmm, xword[src + (bytes - 16)]);
    if (bytes > 16)
      vmovups(xword[dst + (bytes - 16)], tmp_xmm);
  } else if (bytes >= 8) {
    mov(tmp_r64, qword[src]);
    mov(qword[dst], tmp_r64);
    if (bytes > 8)
      mov(tmp_r64, qword[src + (bytes - 8)]);
    if (bytes > 8)
      mov(qword[dst + (bytes - 8)], tmp_r64);
  } else if (bytes >= 4) {
    const auto tmp_r32 = tmp_r64.cvt32();
    mov(tmp_r32, dword[src]);
    mov(dword[dst], tmp_r32);
    if (bytes > 4)
      mov(tmp_r32, dword[src + (bytes - 4)]);
    if (bytes > 4)
      mov(dword[dst + (bytes - 4)], tmp_r32);
  } else if (bytes >= 2) {
    const auto tmp_r32 = tmp_r64.cvt32();
    movzx(tmp_r32, word[src]);
    mov(word[dst], tmp_r32.cvt16());
    if (bytes > 2)
      movzx(tmp_r32, word[src + (bytes - 2)]);
    if (bytes > 2)
      mov(word[dst + (bytes - 2)], tmp_r32.cvt16());
  } else if (bytes == 1) {
    const auto tmp_r32 = tmp_r64.cvt32();
