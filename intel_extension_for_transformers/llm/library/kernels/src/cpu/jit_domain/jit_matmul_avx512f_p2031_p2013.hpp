...
namespace jd {
/**
 * @brief jit_matmul_avx512f_p2031_p2013_t calculates this kind matmul: alpha *
 * src0 x src1 + beta * src2 = dst. alpha * src0(M, K) x src1(K, N) + beta *
 * scr2(M, N) = dst(M, N)
 */
...
public:
  explicit jit_matmul_avx512f_p2031_p2013_t(const ssd::matmul_param_t &param)
      : jit_generator(), param_(param), TH_(param.m_tile), TW_(param.n_tile),
        ...
        ld_src2(param.N * dsize_src2), ld_dst(param.N * dsize_dst),
...
private:
  ...
  Xbyak::Zmm dst_tile_Vmm(
      int i = 0,
      int j = 0); // Reg alloc of DST tile. 2D shape=(TH,TW), stride=(TW,1)
  ...
  static constexpr size_t dsize_src0 =
      sizeof(decltype(*ssd::matmul_data_t::src0));
  static constexpr size_t dsize_src1 =
      sizeof(decltype(*ssd::matmul_data_t::src1));
  static constexpr size_t dsize_src2 =
      sizeof(decltype(*ssd::matmul_data_t::src2));
  static constexpr size_t dsize_dst =
      sizeof(decltype(*ssd::matmul_data_t::dst));
