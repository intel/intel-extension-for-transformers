...
namespace jd {
/**
 * @brief jit_matmul_vnni_noperm_p2031_p1302_t calculates this kind matmul:
 * scale * src0 x src1 + zp = dst. scale(1) * src0(M, K) x src1(K, N) + zp(1) =
 * dst(M, N)
 */
class jit_matmul_vnni_noperm_p2031_p1302_t : public jit_generator {
public:
  explicit jit_matmul_vnni_noperm_p2031_p1302_t(
      const ssd::matmul_param_t &param)
      : jit_generator(), param_(param), TH_(param.m_tile), TW_(param.n_tile),
        ld_src0(param.K * dsize_src0),
        ld_src1(param.K * param.batch *
                dsize_src1), // stride to load next row (before transpose)
        ld_dst(param.M * param.batch * dsize_dst), start_transpose_src0(0),
        size_transpose_src0((TH_ * BYTES_ZMM / VNNI_ADJ) * (TK_ * VNNI_ADJ) *
                            dsize_src0),
...
private:
  ...
  // transpose a 8x8 matrix of 4-byte-group in Ymm mat with help of 8 tmp vector
  // registers
  void transpose8_ps(const Xbyak::Ymm mat[8], const Xbyak::Ymm tmp[8]);
  void calc_THxTKxTW();

  std::vector<Xbyak::Ymm> get_Ymm(int start,
                                  int num) const; // generate a group of tmp YMM
  ...
  static constexpr int TK_ =
      8; // tile size in reduction dim in terms of (unrolled) iterations
  static constexpr size_t dsize_src0 =
      sizeof(decltype(*ssd::matmul_u8_data_t::src0));
  static constexpr size_t dsize_src1 =
      sizeof(decltype(*ssd::matmul_u8_data_t::src1));
  static constexpr size_t dsize_dst =
      sizeof(decltype(*ssd::matmul_u8_data_t::dst));
  // leading dimension in #bytes
  const int ld_src0, ld_src1, ld_dst;
  const int start_transpose_src0, size_transpose_src0, start_transpose_src1,
      size_transpose_src1, stack_size;
  ...
  static constexpr int VNNI_GROUPS =
      BYTES_ZMM / VNNI_ADJ; // spacial dim of VPDPBUSD
#ifdef _WIN32
