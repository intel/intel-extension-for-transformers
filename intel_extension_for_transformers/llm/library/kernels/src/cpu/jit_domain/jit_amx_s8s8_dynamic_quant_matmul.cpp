...
#define N_DIM_IP(label_name, ip_func)                                          \
  ...
  if (param_.tail_n_loop != 0)                                                 \
    ip_func(param_.tail_n_loop);
...
void jit_amx_s8s8_dynamic_quant_matmul_t::generate() {
  ...
  inLocalLabel();
  {
    ...
    regs_pool rp(this, 1, {does_calc ? 12 : 6, does_calc ? 32 : 0, 0},
                 BYTES_ZMM, regs_pool::IgnoreWaste);
    ...
    auto amx_ip_16x16 = [&](int block_num) {
      ...
      // build block
      {
        ...
        imul(reg_tmp, reg_n_loop,
             param_.align_build_block_num * trans_block_col * 64 *
                 (param_.tile_k / 4));
        add(reg_matB_addr, reg_tmp);
        for (int k_loop = 0; k_loop < param_.k / param_.tile_k; k_loop++) {
          tileloadd(Tmm(3),
                    ptr[reg_matA_addr + reg_strideA + k_loop * param_.tile_k]);
          for (int idx = 0; idx < block_num; idx++) {
            int offset =
                idx * trans_block_col * 64 * (param_.tile_k / 4) + k_loop * 64;
      ...
      // store block to tmp_buf & dequant+add_bias
      {
        ...
        for (int idx = 0; idx < block_num; idx++) {
          vmovups(zmms[0],
                  ptr[reg_scale_w + reg_tmp + idx * 16 * sizeof(float)]);
          if (param_.add_bias)
            vmovups(zmms[1],
                    ptr[reg_bias + reg_tmp + idx * 16 * sizeof(float)]);
          for (int row_loop = 0; row_loop < 16; row_loop++) {
            vcvtdq2ps(
                zmms[2],
                ptr[reg_tmp_buf + reg_tmp +
                    (idx * 16 + row_loop * param_.pad_n) * sizeof(float)]);
            vbroadcastss(
                zmms[3],
                dword[reg_scale_a + reg_tmp2 + row_loop * sizeof(float)]);
            ...
            vmovups(ptr[reg_tmp_buf + reg_tmp +
                        (idx * 16 + row_loop * param_.pad_n) * sizeof(float)],
                    zmms[2]);
          }
        }
    ...
    auto vnni_ip_1xN = [&](int N) {
      ...
      // vnni_gemm
      {
        ...
        mov(reg_matB_addr, ptr[rp.p[0] + GET_OFF(reordered_weight)]);
        for (int i = 0; i < 2 * N; i++)
          vxorps(zmms[1 + i], zmms[1 + i], zmms[1 + i]);
        imul(reg_tmp, reg_m_loop, 16 * param_.k);
        ...
        for (int k_loop = 0; k_loop < param_.k; k_loop += param_.tile_k) {
          imul(reg_wei_offset, reg_n_loop,
               param_.align_build_block_num * 16 * param_.tile_k *
                   trans_block_col);
          auto wei_base_offset = k_loop / param_.tile_k * 64;
          for (int tile_k_loop = 0; tile_k_loop < param_.tile_k;
               tile_k_loop += 4) {
            auto activation_offset = k_loop + tile_k_loop;
            vbroadcastss(zmms[0], dword[reg_matA_addr + reg_activation_offset +
                                        activation_offset]);
            vpaddb(zmms[0], zmms[0], neg_128_zmm);
            for (int n_loop = 0; n_loop < N; n_loop++) {
              auto fin_wei_offset =
                  wei_base_offset +
                  (tile_k_loop + n_loop * param_.tile_k) * 16 * trans_block_col;
              vpdpbusds(zmms[1 + n_loop], zmms[0],
                        ptr[reg_matB_addr + reg_wei_offset + fin_wei_offset]);
              vpdpbusds(zmms[1 + N + n_loop], neg_128_zmm,
                        ptr[reg_matB_addr + reg_wei_offset + fin_wei_offset]);
            }
          }
        }
      ...
      // store to tmpbuf
      {
        ...
        for (int store_loop = 0; store_loop < N; store_loop++) {
          vpsubd(zmms[1 + store_loop], zmms[1 + store_loop],
                 zmms[1 + N + store_loop]);
          vcvtdq2ps(zmms[1 + store_loop], zmms[1 + store_loop]);
          imul(reg_tmp, reg_m_loop, 16);
          add(reg_tmp, vnni_m_loop);
          vmulps(zmms[1 + store_loop], zmms[1 + store_loop],
                 zword_b[reg_scale_a + sizeof(float) * reg_tmp]);
          auto zmm_scale_w = rp.reg<Zmm>();
          imul(reg_tmp, reg_n_loop, param_.align_build_block_num * 16);
          vmovups(zmm_scale_w, ptr[reg_scale_w + sizeof(float) * reg_tmp +
                                   store_loop * 16 * sizeof(float)]);
          if (param_.add_bias) {
            vfmadd213ps(zmms[1 + store_loop], zmm_scale_w,
                        ptr[reg_bias + sizeof(float) * reg_tmp +
                            store_loop * 16 * sizeof(float)]);
          } else {
          ...
          if (param_.postop_attrs.size() != 0) {
            eltwise_injector_.escape_rp_all_type(&rp);
            eltwise_injector_.vector_compute(zmms[1 + store_loop],
                                             param_.postop_attrs);
          }
          imul(reg_tmp2, vnni_m_loop, param_.n);
          add(reg_tmp2, reg_tmp);
          vmovups(ptr[reg_tmp_buf + sizeof(float) * reg_tmp2 +
                      store_loop * 16 * sizeof(float)],
                  zmms[1 + store_loop]);
        }
      }
    ...
    auto tmp_buf_load_M_row = [&](const int M, const Reg64 &offset) {
      ...
      for (int i = 0; i < M; i++)
        vmovups(Zmm(i),
                ptr[reg_tmp_buf + offset + (i * param_.pad_n) * sizeof(int)]);
    };

    auto get_16_abs_max_zmm = [&](const std::array<Zmm, 16> &zmms,
                                  const Reg64 &reg_max_abs_loop,
      ...
      for (int i = 0; i < 16; i++)
        vrangeps(need_mask ? zmms[i] | matC_n_mask : zmms[i], zmms[i],
                 ptr[reg_tmp_buf + reg_tmp + i * param_.pad_n * sizeof(int)],
                 11U);
    };
    ...
    auto log2n_max_reduce_16x16 = [&](const std::array<Zmm, 16> &zmms) {
      ...
      while (i != 0) {
        for (int ii = 0; ii < i; ii++)
          vmaxps(zmms[ii], zmms[ii], zmms[ii + i]);
        i /= 2;
      }
    };
    ...
    auto write_back_scale = [&](const Zmm &scale, const int M) {
      ...
      vmovups(M == 16 ? ptr[reg_scale_dst + reg_tmp]
                      : ptr[reg_scale_dst + reg_tmp] | scaleC_mask,
              scale);
    ...
    auto quant_write_back_Mx16 = [&](const int M, const Reg64 &store_n_loop,
                                     const bool need_mask = false) {
      const auto reg_tmp = rp.reg<Reg64>();
      const auto reg_tmp2 = rp.reg<Reg64>();
      ...
      for (int i = 0; i < M; i++) {
        ...
        vpmovsdb(need_mask ? ptr[reg_dst + reg_tmp + i * param_.n] | matC_n_mask
                           : ptr[reg_dst + reg_tmp + i * param_.n],
                 Zmm(i));
      }
    };

    auto build_MxN_tile = [&](int M, std::string label_prefix = ".") {
      auto gemm_isa = GEMM_ISA::amx; // vnni show no perf benefit in SD, but we
                                     // also want to keep the vnni-jit-path
      ...
      if (param_.write_mask)
        quant_write_back_Mx16(M, store_n_loop, true);
    };
    ...
    mov(reg_strideC,
        param_.pad_n * sizeof(int)); // strideC has same value as strideB.
