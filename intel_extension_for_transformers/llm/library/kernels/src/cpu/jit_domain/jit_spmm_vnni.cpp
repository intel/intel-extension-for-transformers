...
void jit_spmm_vnni_t::load_intermediate_dst(dim_t m_start) {
  for (int i = 0; i < TH(); ++i) {
    for (int j = 0; j < TW(); ++j) {
      ...
      vmovdqu32(dst_tile_Vmm(i, j),
                ptr[reg_dst + reg_n_idx * BYTE4 + sliced_dst_idx * BYTE4]);
    }
  }
}
...
void jit_spmm_vnni_t::load_dense(const std::vector<int64_t> &k_indices) {
  // e.g.: when tile_shape = {4, 4}, zmm24 = b[[0, 3, 4, 9], 0:16], zmm23 =
  // b[[0, 3, 4, 9], 16:32],
  // ..., zmm21 = b[[0, 3, 4, 9], 48:64]. zmm24 is shared by each row in a TH,
  // so the TH's blocked.
  for (int j = 0; j < TW(); ++j) {
    ...
    if (k_indices.size() == spns::ADJ) {
      vmovdqu8(TW_xmm, ptr[reg_dense + reg_n_idx * BYTE1 +
                           k_indices[0] * ld_dst() + j * VEC]);
      vbroadcasti32x4(TW_ymm, ptr[reg_dense + reg_n_idx * BYTE1 +
                                  k_indices[1] * ld_dst() + j * VEC]);
      vmovdqu8(temp_xmm, ptr[reg_dense + reg_n_idx * BYTE1 +
                             k_indices[2] * ld_dst() + j * VEC]);
      vbroadcasti32x4(temp_ymm, ptr[reg_dense + reg_n_idx * BYTE1 +
                                    k_indices[3] * ld_dst() + j * VEC]);
    } else {
...
void jit_spmm_vnni_t::load_sparse(const Xbyak::Reg64 &reg_addr,
                                  uint64_t offset) {
  for (int i = 0; i < TH(); ++i) {
    vpbroadcastd(TH_Vmm(i),
                 ptr[reg_addr + offset +
                     i * spns::ADJ * sizeof(decltype(*param_.weight))]);
  }
}

void jit_spmm_vnni_t::repeat_THx4xTW_matmal(dim_t m_start) {
  int need_regs = TH() + TW() + TH() * TW() + USED_VREGS;
  SPARSE_LOG_IF(FATAL, need_regs >= VREG_NUMS)
      << "loading weight's REGs (TH=" << TH()
      << "), loading "
  ...
  const dim_t imb =
      (param_.im_start + m_start) / TH(); // global index of m-block

  // ADJ=4 means 4 S8 combine a DST_S32. As ADJ repeats in K-dim, a DST_S32 also
  // accumulates. Note that a whole k-dim(segment) is processed. Terminology:
  const dim_t indptr_kernel_start =
      param_.indptr[param_.im_start / TH()] * spns::ADJ;
  const dim_t indptr_lo =
      param_.indptr[imb] * spns::ADJ; // min offset of index pointer
  const dim_t indptr_hi =
      param_.indptr[imb + 1] * spns::ADJ; // max offset of index pointer
  const dim_t nnz = indptr_hi - indptr_lo;

  auto idx_begin = param_.indices.begin();
  const std::vector<dim_t> k_indices(idx_begin + indptr_lo,
                                     idx_begin + indptr_hi);
  ...
  case ssd::subfunc_level::kdims:
    mov(reg_seq_indices,
        reinterpret_cast<uint64_t>(dense_load_offsets.data() + indptr_lo -
                                   indptr_kernel_start));
    mov(reg_wei, reinterpret_cast<uint64_t>(
                     param_.weight +
                     param_.blocksize[0] * param_.blocksize[1] * indptr_lo));
    break;
  default:
    break;
  }
  ...
  case ssd::subfunc_level::non_kdims:
    ...
    for (int64_t kp_lo = 0; kp_lo < nnz; kp_lo += spns::ADJ) {
      const int64_t kp_hi = std::min(
          kp_lo + spns::ADJ, nnz); // end of k-index pointer (noninclusive)
      dim_t element_offset =
          param_.blocksize[0] * param_.blocksize[1] * indptr_lo + kp_lo * TH();

      // Step 1: load dense (activation). Note that k_indices length is
      // processed.00-00 Step 2: load sparse (weight) and reorder data for that.
      ...
      // Min tile calculation: Tile width/height is 1, compute (1, ADJ) x (ADJ,
      // 16) = (1, 16) matmul.
      switch (param_.sub_func) {
      case ssd::subfunc_level::none:
...
void jit_spmm_vnni_t::store_intermediate_dst(dim_t m_start) {
  ...
  for (int i = 0; i < TH(); ++i) {
    for (int j = 0; j < TW(); ++j) {
      int sliced_dst_idx = (m_start + i) * ld_dst() + j * VEC;
      vmovdqu32(ptr[reg_dst + reg_n_idx * BYTE4 + sliced_dst_idx * BYTE4],
                dst_tile_Vmm(i, j));
    }
  }
}
...
void jit_spmm_vnni_t::handle_dst_buffer_epilogue_sub(bool set_zero) {
  ...
  for (int i = 0; i < TH(); ++i) {
    for (int j = 0; j < TW(); ++j) {
      ...
      vmulps(dst_tile_Vmm(i, j), dst_tile_Vmm(i, j),
             zword_b[reg_scale + reg_m_idx * BYTE4 + i * BYTE4]); // *= scale.
      if (output_type() == data_type::fp32 && param_.append_sum) {
        vaddps(dst_tile_Vmm(i, j), dst_tile_Vmm(i, j),
               ptr[reg_dst + reg_n_idx * BYTE4 + i * ld_dst() * BYTE4 +
                   j * VEC * BYTE4]);
      }
      if (param_.postop_attrs.size() != 0) {
        eltwise_injector_.vector_compute(dst_tile_Vmm(i, j),
                                         param_.postop_attrs);
      }
      ...
      if (output_type() == data_type::u8) {
        if (param_.postop_attrs.size() == 0 ||
            param_.postop_attrs.back().op_alg != postop_alg::quantize)
          vcvtps2udq(dst_tile_Vmm(i, j), dst_tile_Vmm(i, j));
        vpmovusdb(ptr[reg_dst + reg_n_idx * BYTE1 + i * ld_dst() * BYTE1 +
                      j * VEC * BYTE1],
                  dst_tile_Vmm(i, j));
        ...
        vpmovsdb(ptr[reg_dst + reg_n_idx * BYTE1 + i * ld_dst() * BYTE1 +
                     j * VEC * BYTE1],
                 dst_tile_Vmm(i, j));
      } else if (output_type() == data_type::fp32) {
        vmovups(ptr[reg_dst + reg_n_idx * BYTE4 + i * ld_dst() * BYTE4 +
                    j * VEC * BYTE4],
                dst_tile_Vmm(i, j));
      }
...
void jit_spmm_vnni_t::calc_mean_variance(int i, int j, bool set_zero) {
   ...
   */
  const Xbyak::Address &zword_dst_m1 =
      ptr[reg_dst_m1 + reg_n_idx * BYTE4 + j * VEC * BYTE4];
  const Xbyak::Address &zword_dst_m2 =
      ptr[reg_dst_m2 + reg_n_idx * BYTE4 + j * VEC * BYTE4];
  if (j == 0)
    vaddps(vreg_m_idx, vreg_m_idx,
           zword_b[rip + L_m512_1f]); // ({m+1}_idx_, n) := m_idx + 1
  if (j == 0)
    vrcp14ps(vreg_temp2, vreg_m_idx); // get 1/n
  if (i == 0) {
    if (set_zero == true) {
  ...
  vmulps(dst_tile_Vmm(i, j), dst_tile_Vmm(i, j),
         dst_tile_Vmm(i, j)); // A2 = A * A
  vfnmadd231ps(dst_tile_Vmm(i, j), dst_tile_Vmm(i, j),
               vreg_temp2); // A2 - A2 / N

  if (i == 0) {
    if (set_zero == true) {
      vxorps(dst_tile_Vmm(0, j), dst_tile_Vmm(0, j),
             dst_tile_Vmm(0, j)); // update m2
    } else {
      vaddps(dst_tile_Vmm(0, j), dst_tile_Vmm(i, j), zword_dst_m2); // update m2
    }
  } else {
    vaddps(dst_tile_Vmm(0, j), dst_tile_Vmm(i, j),
           dst_tile_Vmm(0, j)); // update m2
  }
 ...
 *  reg_seq_indices - the start of offset for each TW, it will be updated after
 * read reg_wei - the start of weight matrix, it will be updated after read
 */
void jit_spmm_vnni_t::load_dense_sparse_prod() {
  ...
  for (int j = 0; j < TW(); ++j) {
    ...
    for (int i = 0; i < TH(); ++i) {
      // load sparse
      if (j == 0)
        vpbroadcastd(TH_Vmm(i), ptr[reg_wei + i * spns::ADJ * wei_size]);
    ...
    if (j == TW() / 2)
      add(reg_wei, TH() * spns::ADJ * wei_size);
  }
}
...
void jit_spmm_vnni_t::generate() {
  ...
  case ssd::subfunc_level::non_kdims:
    ...
    break;
  default:
    SPARSE_LOG(FATAL) << "Unexpected subfunc_level: "
                      << static_cast<uint8_t>(param_.sub_func);
    break;
  }
...

  {
    ...
    // initialize the control reg which we are going to use for permutes and
    // shuffles.
    vpmovzxbd(vpermt2d_arg_idx, ptr[rip + L_vpermt2d_arg]);
    vbroadcasti32x4(vpshufb_arg_b, ptr[rip + L_vpshufb_arg]);
    ...
    // When K-dim is cut into k_blocks parts, it'll produce same number of DST
    // intermediate results. Loop-M2: CPP loop for each blocked row. Asm code
    // unroll.
    for (dim_t im = 0; im < param_.BM; im += mt_size()) {
      xor_(reg_n_idx, reg_n_idx); // reg_n_idx = 0
      mov(reg_m_idx, im);         // reg_m_idx = im

      // n_blocks and n_tiles are N-dim loops, and can be used for N-dim
      // multithread. n_blocks is asm outer loop, with longer assembly context
      // span. n_tiles is inner loop. Loop-N2: Assembly loop at "n_tiles". Asm
      // code fold.
      Xbyak::Label L_nt_loop;
      L(L_nt_loop);
      // init dst buffer, like init the value to bias or the previous
      // intermediate result.
      handle_dst_buffer_init(0, im);
      repeat_THx4xTW_matmal(im);
      // generate the epilogue logic. This is different depending on B_blocks
      // value (should we cache intermediate results or write results with
      // post-op to output)
      if (im == 0) {
        call(func_dst_epilogue_start_); // zeroing registers for the first
                                        // iteration
      } else {
  ...
  int word_size = 1;
  int num_size = 16;
  const uint8_t vpermt2d_control[16] = {0, 4, 16, 20, 1, 5, 17, 21,
                                        2, 6, 18, 22, 3, 7, 19, 23};
  const uint8_t vpshufb_control[16] = {0, 4, 8,  12, 1, 5, 9,  13,
                                       2, 6, 10, 14, 3, 7, 11, 15};
  L(L_vpermt2d_arg);
