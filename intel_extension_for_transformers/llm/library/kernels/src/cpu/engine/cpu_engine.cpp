...
#define DECLARE_IMPL_LIST(kind)                                                \
  const std::vector<impl_list_item_t> *get_##kind##_impl_list(                 \
...
cpu_engine_t::get_implementation_list(const operator_desc &op_desc) const {
  // Call C API.
#define CASE(kind)                                                             \
...
bool cpu_engine_t::create_kernel(const operator_desc &op_desc,
