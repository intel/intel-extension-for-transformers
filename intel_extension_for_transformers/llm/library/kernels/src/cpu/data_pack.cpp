...
void reference(
    dst_t *output, src_t *input, int row, int col, int rowpad, int colpad,
  ...
  for (int irow = 0; irow < rowpad; irow += NTile) {
    for (int icol = 0; icol < colpad; icol += 1) {
      for (int iin = 0; iin < NTile; iin++) {
        if (irow + iin < row) {
          if (icol < col) {
            *(output + irow * dststride + icol * NTile + iin) =
                cast_func(*(input + (irow + iin) * srcld + icol));
          } else {
            *(output + irow * dststride + icol * NTile + iin) =
                cast_func(static_cast<src_t>(0));
          }
        } else {
          *(output + irow * dststride + icol * NTile + iin) =
              cast_func(static_cast<src_t>(0));
...
template <typename dst_t, typename src_t>
void pack(dst_t *output, src_t *input, dim_t N, dim_t K,
          std::function<dst_t(src_t)> cast_func) {
...
#pragma omp parallel
  {
    ...
    if (rowsize > 0 && colsize > 0) {
      ...
      reference<dst_t, src_t>(output + rowidx * kpad + colidx * 16,
                              input + rowidx * K + colidx, rowremain, colremain,
...
#define DECLARE_DATA_PACK(dst_type, src_type)                                  \
  template SPARSE_API_ void pack<dst_type, src_type>(                          \
      dst_type *, src_type *, dim_t, dim_t,                                    \
