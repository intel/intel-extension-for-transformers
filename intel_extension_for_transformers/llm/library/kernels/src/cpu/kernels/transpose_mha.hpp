...
public:
  ...
  explicit transpose_mha_k_t(const std::shared_ptr<const kd_t> &kd)
      : kernel_t(kd) {
    ...
    MHA_vnni_step2.reset(
        new MHA_norm_quantize_reorder_vnniw_prescale_packed(8, 32));
...
private:
  ...
  std::shared_ptr<MHA_norm_quantize_reorder_vnniw_prescale_packed>
      MHA_vnni_step2;
  ...
  std::unique_ptr<MHA_norm_quantize_reorder_vnnib_prescale_packed>
      ker_scale_trans;
