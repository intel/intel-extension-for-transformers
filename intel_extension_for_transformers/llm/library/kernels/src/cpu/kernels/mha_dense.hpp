...
public:
  ...
  virtual ~mha_dense_k_t() {
    for (int j = 1; j <= MAX_SL_N / 64; j++) {
      for (int i = 0; i < 16; i++)
        safe_delete(ker_softmax_[j][i]);
      safe_delete(ker_av_gemm_16x_[j]);
      safe_delete(ker_av_gemm_32x_[j]);
    }
    for (int j = 1; j <= 16; j++)
      safe_delete(ker_trans_k_[j]);
    for (int j = 0; j <= 4; j++)
      safe_delete(ker_trans_v_[j]);
  ...
  size_t get_workspace_size() const override {
    return omp_get_max_threads() * thread_workspace_size_;
  }
  bool init() override;
...
private:
  bool execute_tiny(void **src_data, void **dst_data, void *workspace,
                    dim_t *shape_data) const;
  const std::vector<tensor_desc> &ts_desc;
  const data_type dst_dt_;
  const format_type kv_ft_;
  const int src_bs_, src_sl_m_, src_sl_n_, head_num_, head_size_, ld_q_, ld_kv_,
      ld_dst_;
