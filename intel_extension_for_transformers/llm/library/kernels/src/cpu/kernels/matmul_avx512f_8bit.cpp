...
bool matmul_avx512f_8bit_kd_t::params_init() {
  ...
  jit_param_.has_scale0 =
      shapes.size() > io::SCALE0 && !shapes[io::SCALE0].empty();
  ...
  if (has_bias) {
    ...
    jit_param_.beta =
        0.f; // set beta to 0 to avoid generate unnecessary asm ascode
  }
  ...
  jit_param_.weight_8bit =
      reinterpret_cast<uint8_t *>(str_to_num<intptr_t>(attrs["weight_8bit"]));

  if (dtypes[io::SRC1] == data_type::bf16) {
    jit_param_.weight_bf16 = reinterpret_cast<bfloat16_t *>(
        str_to_num<intptr_t>(attrs["weight_bf16"]));
    ...
    if (jit_param_.weight_type == data_type::f8_e4m3) {
      std::function<float8_e4m3_t(bfloat16_t)> cast_func =
          [&](bfloat16_t bf16) -> float8_e4m3_t {
        float fp32 = static_cast<float>(bfloat16_t(bf16));
        return float8_e4m3_t(fp32);
      };
      pack<float8_e4m3_t, bfloat16_t>(jit_param_.weight_f8_e4m3,
                                      jit_param_.weight_bf16, N, K, cast_func);
    } else if (jit_param_.weight_type == data_type::f8_e5m2) {
      std::function<float8_e5m2_t(bfloat16_t)> cast_func =
          [&](bfloat16_t bf16) -> float8_e5m2_t {
        float fp32 = static_cast<float>(bfloat16_t(bf16));
        return float8_e5m2_t(fp32);
      };
      pack<float8_e5m2_t, bfloat16_t>(jit_param_.weight_f8_e5m2,
                                      jit_param_.weight_bf16, N, K, cast_func);
    }
  } else if (dtypes[io::SRC1] == data_type::s8 ||
             dtypes[io::SRC1] == data_type::f8_e4m3 ||
...
matmul_avx512f_8bit_k_t::matmul_avx512f_8bit_k_t(
    const std::shared_ptr<const kd_t> &kd)
    : kernel_t(kd), t_shapes_(kd->get_operator_desc().tensor_shapes()),
      M_(t_shapes_[io::SRC0][0]), K_(t_shapes_[io::SRC0][1]),
...
bool matmul_avx512f_8bit_k_t::init() {
  ...
  mParallel.update(M_, N_, jit_ker_->MTile, jit_ker_->NTile,
                   cpudevice->getThreads(), mCacheAdapter);
...
bool matmul_avx512f_8bit_k_t::execute(
    const std::vector<const void *> &rt_data) const {
  bfloat16_t *matA = const_cast<bfloat16_t *>(
      reinterpret_cast<const bfloat16_t *>(rt_data[io::SRC0]));
  bfloat16_t *matC = const_cast<bfloat16_t *>(
      reinterpret_cast<const bfloat16_t *>(rt_data[io::DST0]));
  bfloat16_t *matD = const_cast<bfloat16_t *>(
      reinterpret_cast<const bfloat16_t *>(rt_data[io::SRC2]));
  float *scale =
      const_cast<float *>(reinterpret_cast<const float *>(rt_data[io::SCALE0]));
  bfloat16_t *matE =
      derived_kd()->jit_param().has_append_sum
          ? const_cast<bfloat16_t *>(
                reinterpret_cast<const bfloat16_t *>(rt_data[io::APPEND_SUM]))
          : nullptr;
#pragma omp parallel
  {
    ...
    if (rowsize > 0 && colsize > 0) {
      ...
      auto eptr = derived_kd()->jit_param().has_append_sum
                      ? matE + colidx + rowidx * ldc
                      : nullptr;
      ...
      for (int iterk = 0; iterk < K_; iterk += kbatch) {
        ...
        auto bptr =
            derived_kd()->jit_param().weight_8bit + colidx * K_ + iterk * 16;
          ...
          for (int i = 0; i < rowremain; i += jit_ker_->MTile) {
            for (int in = 0; in < mParallel.mNStep; in += jit_ker_->NTile) {
              ...
              if (j + in < colremain) {
                ...
                ssd::matmul_fp8_data_t parm = ssd::matmul_fp8_data_t{
                    aptr + i * lda,
                    bptr + tmpcol * K_,
                    cptr + i * ldc + tmpcol,
                    dptr + i * ldd + tmpcol,
...
bool matmul_avx512f_8bit_k_t::execute(const exec_context_t &context) const {
  ...
  dim_t M = context.get_dynamic_shape().empty()
                ? M_
  ...
  if (derived_kd()->jit_param().has_append_sum) {
    context.input(input::APPEND_SUM)
        ->get_handle(reinterpret_cast<void **>(&matE));
  }
#pragma omp parallel
  {
    int tidx = omp_get_thread_num();
