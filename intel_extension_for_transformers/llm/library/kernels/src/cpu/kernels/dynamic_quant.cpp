...
bool dynamic_quant_k_t::init() {
  ...
  for (int i = 0; i < enable_thr; i++) {
    auto process_channel =
        i < remain_channel ? channel_per_thr + 1 : channel_per_thr;
...
bool dynamic_quant_k_t::execute(
...
#pragma omp parallel for
  for (int i = 0; i < enable_thr; i++) {
    ...
    data.src = reinterpret_cast<char *>(const_cast<void *>(rt_data[io::SRC])) +
               offset * get_data_size(param.input_dt);
    data.mat_dst =
        reinterpret_cast<char *>(const_cast<void *>(rt_data[io::MAT_DST])) +
        offset * sizeof(int8_t);
    data.scale =
        reinterpret_cast<char *>(const_cast<void *>(rt_data[io::SCALE_DST])) +
