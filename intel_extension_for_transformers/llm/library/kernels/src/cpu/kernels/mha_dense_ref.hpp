...
public:
  ...
  inline dim_t bs() const {
    return op_desc_.tensor_descs()[io::SRC_Q].shape()[0];
  }
  inline dim_t sl_m() const {
    return op_desc_.tensor_descs()[io::SRC_Q].shape()[1];
  }
  inline dim_t sl_n() const {
    return op_desc_.tensor_descs()[io::SRC_K].shape()[1];
  }
  inline dim_t head_num() const {
    return op_desc_.tensor_descs()[io::SRC_Q].shape()[2];
  }
  inline dim_t head_size() const {
    return op_desc_.tensor_descs()[io::SRC_Q].shape()[3];
  }
  ...
  inline format_type kv_ft() const {
    return op_desc_.tensor_descs()[io::SRC_K].ftype();
  }
...
public:
  bool init() override;
  [[deprecated("Please use exec_context_t instead of rt_data")]] bool
  execute(const std::vector<const void *> &rt_data) const override;
