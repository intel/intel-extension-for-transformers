...
public:
  ...
  DECLARE_COMMON_PD_T(matmul_vnni_noperm_p2031_p1302_k_t,
  ...
  inline std::vector<dim_t> shape() const {
    std::vector<dim_t> result(
        op_desc_.tensor_descs()[io::SRC0].shape()); // bs0 bs1 M K
...
public:
  ...
  explicit matmul_vnni_noperm_p2031_p1302_k_t(
  ...
  virtual ~matmul_vnni_noperm_p2031_p1302_k_t() {
    ...
    if (src0_tmp_ != nullptr)
      aligned_free(src0_tmp_);
    if (src1_tmp_ != nullptr)
      aligned_free(src1_tmp_);
  }

  // Delete move constructor and move operator
  matmul_vnni_noperm_p2031_p1302_k_t(
      matmul_vnni_noperm_p2031_p1302_k_t &&other) = delete;
  matmul_vnni_noperm_p2031_p1302_k_t &
  operator=(matmul_vnni_noperm_p2031_p1302_k_t &&other) = delete;
  // Delete copy constructor and copy operator
  matmul_vnni_noperm_p2031_p1302_k_t(
      const matmul_vnni_noperm_p2031_p1302_k_t &other) = delete;
  matmul_vnni_noperm_p2031_p1302_k_t &
  operator=(const matmul_vnni_noperm_p2031_p1302_k_t &other) = delete;
...
private:
  void thread_exec(const std::vector<const void *> &rt_data, const dim_t ibs0,
                   const dim_t ibs1) const;
  bool matmul_kernel_create(jit_matmul_vnni_noperm_p2031_p1302_t **ker_pp,
