...
bool spmm_amx_bf16_x16_kd_t::init() {
  ...
  bool is_supported =
      ...
      is_any_of({data_type::bf16, data_type::fp32},
...
bool spmm_amx_bf16_x16_kd_t::spmm_params_init(
    std::vector<ssd::amx_bf16_params_t> &param_ref,
  ...
  const auto &all_bsr_data =
      reinterpret_cast<std::vector<bsr_data_t<bfloat16_t> *> *>(temp_addr);
...
bool spmm_amx_bf16_x16_k_t::spmm_kernel_create(
    jit_spmm_amx_bf16_x16_t **ker_pp, const ssd::amx_bf16_params_t &param) {
...
bool spmm_amx_bf16_x16_k_t::execute(
    ...
    for (dim_t micro_oc = 0; micro_oc < num_tileOC; micro_oc++) {
      for (dim_t micro_bs = 0; micro_bs < num_tileBS; micro_bs++) {
        ...
        inputs.dst = static_cast<float *>(const_cast<void *>(rt_data[3])) +
                     micro_bs * tileBS * OC + micro_oc * tileOC * tileBS;
...
#pragma omp parallel for collapse(2)
    for (dim_t micro_oc = 0; micro_oc < num_tileOC; micro_oc++) {
      for (dim_t micro_bs = 0; micro_bs < num_tileBS; micro_bs++) {
        ...
        inputs.bias = static_cast<float *>(const_cast<void *>(rt_data[2])) +
                      micro_oc * tileOC;
        inputs.dst = static_cast<bfloat16_t *>(const_cast<void *>(rt_data[3])) +
                     micro_bs * tileBS * OC + micro_oc * tileOC * tileBS;
        (*jit_kers_[micro_oc])(&inputs);
      }
    }
