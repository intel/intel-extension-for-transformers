...
bool eltwiseop_kd_t::init() {
  ...
  for (auto &postop_attr : params_.postop_attrs) {
    if (postop_attr.op_alg == postop_alg::eltop_int_lut &&
        !isa_available(avx512_core_vbmi))
      return false;
    if (postop_attr.dt == data_type::bf16 && !isa_available(avx512_core_bf16))
      return false;
  }
  int nthr = op_desc_.impl_nthr();
  params_.element_num_each_th = params_.element_num / nthr;
  params_.remain_element =
      params_.element_num - (nthr - 1) * params_.element_num_each_th;
...
bool eltwiseop_k_t::init() {
  ...
  for (int i = 0; i < nthr; i++)
    td.push_back(new ssd::eltwiseop_data_t());
...
bool eltwiseop_k_t::eltwiseop_kernel_create(
    jit_eltwiseop_t **ker_pp, const ssd::eltwiseop_param_t &param) {
...
bool eltwiseop_k_t::execute(const std::vector<const void *> &rt_data) const {
  ...
  auto dst_offset = [&] {
    return get_data_size(derived_kd()->params().out_dt);
  };
