...
public:
  ...
  const std::vector<ssd::avx512_fp32_params_t> &params() const {
    return params_;
  }
...
public:
  ...
  explicit spmm_avx512f_k_t(const std::shared_ptr<const kd_t> &kd)
      : kernel_t(kd) {}
...
private:
  bool spmm_kernel_create(jit_spmm_avx512f_t **ker_pp,
                          const ssd::avx512_fp32_params_t &param);
