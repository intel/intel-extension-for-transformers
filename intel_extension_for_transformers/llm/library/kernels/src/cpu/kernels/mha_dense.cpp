...
#pragma GCC diagnostic ignored                                                 \
    "-Wmaybe-uninitialized" // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=105593
...
bool mha_dense_kd_t::init() {
  ...
  merged_ = (op_attrs.find("merged_QKV") != op_attrs.end() &&
             op_attrs["merged_QKV"] == "True");
  ...
  KERNEL_INIT_CHECK(op_attrs.find("stable_softmax") != op_attrs.end() &&
                    op_attrs.at("stable_softmax") == "True");
  KERNEL_INIT_CHECK(op_attrs.find("softmax_rescale") != op_attrs.end());
  KERNEL_INIT_CHECK(
      std::all_of(op_attrs.cbegin(), op_attrs.cend(), [](auto &&kv) {
        return kv.first == "merged_QKV" || kv.first == "approx_exp" ||
               kv.first == "stable_softmax" || kv.first == "softmax_rescale";
      }))
  ...
  KERNEL_INIT_CHECK(tensor_desc[io::SRC_V].ftype() ==
                    tensor_desc[io::SRC_K].ftype())
  ...
  KERNEL_INIT_CHECK(is_any_of(
      {data_type::u8, data_type::s8, data_type::fp32, data_type::bf16},
      [dst_dt](auto t) { return dst_dt == t; }))

  KERNEL_INIT_CHECK((tensor_desc[io::ATT_SCALE] ==
                     jd::tensor_desc{{1}, data_type::fp32, format_type::a}));
  KERNEL_INIT_CHECK((tensor_desc[io::Q_SCALE] ==
                     jd::tensor_desc{{1}, data_type::fp32, format_type::a}));
  KERNEL_INIT_CHECK((tensor_desc[io::K_SCALE] ==
                     jd::tensor_desc{{1}, data_type::fp32, format_type::a}));
  KERNEL_INIT_CHECK((tensor_desc[io::V_SCALE] ==
                     jd::tensor_desc{{1}, data_type::fp32, format_type::a}));
  KERNEL_INIT_CHECK((tensor_desc[io::SRC_DST_SCALE] ==
                     jd::tensor_desc{{1}, data_type::fp32, format_type::a}));
  KERNEL_INIT_CHECK((tensor_desc[io::SRC_DST_ZP] ==
                     jd::tensor_desc{{1}, data_type::s32, format_type::a}));
  ...
  if (has_badd) {
    ...
    KERNEL_INIT_CHECK(tensor_desc[io::BINARY_ADD].ftype() ==
                      plain_format(badd_dim));
    switch (badd_dim) {
    case 4:
      KERNEL_INIT_CHECK(badd_shape[badd_dim - 4] == 1 ||
                        badd_shape[badd_dim - 4] == src_bs);
      [[fallthrough]];
    case 3:
      KERNEL_INIT_CHECK(badd_shape[badd_dim - 3] == 1 ||
                        badd_shape[badd_dim - 3] == head_num);
      [[fallthrough]];
    case 2:
      KERNEL_INIT_CHECK(badd_shape[badd_dim - 2] == 1 ||
                        badd_shape[badd_dim - 2] == src_sl_m);
      [[fallthrough]];
    case 1:
      KERNEL_INIT_CHECK(badd_shape[badd_dim - 1] ==
                        src_sl_n); // the last dim can not be broadcasted
      break;
    default:
...
mha_dense_k_t::mha_dense_k_t(const std::shared_ptr<const kernel_desc_t> &kd)
    : kernel_t(kd), ts_desc(derived_kd()->get_operator_desc().tensor_descs()),
      dst_dt_(ts_desc[io::DST].dtype()), kv_ft_(ts_desc[io::SRC_K].ftype()),
      ...
      ld_kv_(kv_ft_ == format_type::abcd ? ld_q_
             : kv_ft_ == format_type::acbd
                 ? head_size_ * (derived_kd()->merged() ? 3 : 1)
                 : 0),
      ld_dst_(head_size_ * head_num_ * get_data_size(dst_dt_)),
      softmax_rescale_f32_(str_to_num<float>(
          derived_kd()->get_operator_desc().attrs().at("softmax_rescale"))),
      ...
      thread_workspace_size_(
          sizeof(int8_t) * head_size_ * pad_to(src_sl_n_, 16) + // k
...
#define SET_NULL(arr)                                                          \
  std::fill_n(reinterpret_cast<void **>(arr), sizeof(arr) / sizeof(nullptr),   \
              nullptr)
...
bool mha_dense_k_t::init() {
  ...
  for (int i = 1; i <= 16; ++i) {
    ker_trans_k_[i] = new jit_trans_AB16a4b(
        {/*.M = */ i, /*.N = */ hs_max64, /*.ld_src = */ ld_kv_,
         /*.pad_n = */ pad_to(hs_max64, 16)});
    if (!ker_trans_k_[i]->create_kernel())
      return false;
  }

  // init ker_qk_gemm
  for (int i = 16; i <= MAX_SL_N; i += 16) {
    ker_qk_gemm_32x_[i / 16] = new jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b(
        jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b::param_t{
            32,
            head_size_,
            i,
            ld_q_,
    ...
    if (!ker_qk_gemm_32x_[i / 16]->create_kernel())
      return false;
    ker_qk_gemm_16x_[i / 16] = new jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b(
        jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b::param_t{
            16,
            head_size_,
            i,
    ...
    if (!ker_qk_gemm_16x_[i / 16]->create_kernel())
      return false;
  }
  ...
  for (int i = 64; i <= MAX_SL_N; i += 64) {
    for (int j = 0; j < 16; j++) {
      ker_softmax_[i / 64][j] =
          new jit_softmax_Ab16a({j, i, has_binary_add, "u8"});
      if (!ker_softmax_[i / 64][j]->create_kernel())
        return false;
    }
  }
  ...
  // init ker_trans_v
  for (int i = 0; i <= 4; i++) {
    ker_trans_v_[i] = new jit_trans_BA16b4a(
        {/*.M = */ i, /*.N = */ hs_max64, /*.ld_src = */ ld_kv_});
    if (!ker_trans_v_[i]->create_kernel())
      return false;
  }
  ...
  for (int i = 64; i <= MAX_SL_N; i += 64) {
    ker_av_gemm_32x_[i / 64] = new jit_matmul_amx_u8AB16a64b_s8BA16b4a_ab(
        {/*.M = */ 32, /*.K_pad = */ i, /*.N = */ head_size_,
         /*.ld_dst = */ ld_dst, /*.dst_dt = */ dst_dt_});
    if (!ker_av_gemm_32x_[i / 64]->create_kernel())
      return false;
    ker_av_gemm_16x_[i / 64] = new jit_matmul_amx_u8AB16a64b_s8BA16b4a_ab(
        {/*.M = */ 16, /*.K_pad = */ i, /*.N = */ head_size_,
         /*.ld_dst = */ ld_dst, /*.dst_dt = */ dst_dt_});
    if (!ker_av_gemm_16x_[i / 64]->create_kernel())
      return false;
  }
  return true;
}

inline void mha_dense_k_t::mha_per_head_32x(
    const jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b::rt_data_t &rt_data_qk,
...
inline void mha_dense_k_t::mha_per_head_16x(
    const jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b::rt_data_t &rt_data_qk,
    const jit_softmax_Ab16a::rt_data_t &rt_data_softmax1,
    const jit_matmul_amx_u8AB16a64b_s8BA16b4a_ab::rt_data_t &rt_data_av,
    const int att_tail, const int col_tile, const int att_tile) const {
...
bool mha_dense_k_t::execute(const exec_context_t &ctx) const {
  ...
  for (auto i = 0; i < io_src::SIZE; ++i)
    ctx.input(i)->get_handle(&src_data[i]);
  for (auto i = 0; i < io_dst::SIZE; ++i)
    ctx.output(i)->get_handle(&dst_data[i]);
  for (auto i = 0; i < io_shape::SIZE; ++i)
    shape_data[i] = ctx.get_dynamic_shape()[i];
  ctx.workspace()->get_handle(&workspace);

  const auto src_sl_m =
      shape_data[io_shape::M] ? shape_data[io_shape::M] : src_sl_m_;
  const auto src_sl_n =
      shape_data[io_shape::N] ? shape_data[io_shape::N] : src_sl_n_;
  assert(src_sl_n <= MAX_SL_N);

  // dynamic BATCH_SIZE / HEAD_SIZE / HEAD_NUM not supported
  assert(shape_data[io_shape::BATCH_SIZE] <= 0 ||
         shape_data[io_shape::BATCH_SIZE] == src_bs_);
  assert(shape_data[io_shape::HEAD_SIZE] <= 0 ||
         shape_data[io_shape::HEAD_SIZE] == head_size_);
  assert(shape_data[io_shape::HEAD_NUM] <= 0 ||
         shape_data[io_shape::HEAD_NUM] == head_num_);
  ...
  if (has_binary_add) {
    ...
    if (badd_shape_prepad[2] > 1)
      badd_shape_prepad[2] = src_sl_m;
    if (badd_shape_prepad[3] > 1)
      badd_shape_prepad[3] = src_sl_n;
    const auto tmp_stride_ = dim2step(badd_shape_prepad);
    for (int i = 0; i < 4; ++i)
      badd_stride[i] = tmp_stride_[i];
  }
...
#pragma omp parallel for collapse(2)
  for (int ibs = 0; ibs < bs; ibs++) {
    for (int ihn = 0; ihn < head_num_; ihn++) {
      ...
      const auto thread_workspace = reinterpret_cast<char *>(workspace) +
                                    thread_id * thread_workspace_size_;

      const auto k_scrach = reinterpret_cast<int8_t *>(thread_workspace);
      const auto v_scrach_p64 =
          reinterpret_cast<int8_t *>(k_scrach + head_size_ * sl_n_pad16);
      const auto qk_scrach =
          reinterpret_cast<int32_t *>(v_scrach_p64 + head_size_ * sl_n_pad64);
      const auto softmax_scrach_p64 =
          reinterpret_cast<uint8_t *>(qk_scrach + 32 * sl_n_pad16);
      // softmax_scrach_p64_size = 32 * sl_n_pad64

      const int src_q_offset = ibs * src_sl_m * ld_q_ + ihn * head_size_;
      const int src_kv_offset =
          kv_ft_ == format_type::abcd
              ? ibs * src_sl_n * ld_kv_ + ihn * head_size_
          : kv_ft_ == format_type::acbd
              ? (ibs * head_num_ + ihn) * src_sl_n * ld_kv_
              : 0;
      const int dst_offset =
          ibs * src_sl_m * ld_dst_ + ihn * head_size_ * get_data_size(dst_dt_);
      // init amx for each omp thread
      ker_amx_cfg_(&amx_full_tile_cfg_);

      const auto curr_q =
          reinterpret_cast<const int8_t *>(src_data[io_src::SRC_Q]) +
          src_q_offset;
      const auto curr_dst =
          reinterpret_cast<char *>(dst_data[io_dst::DST]) + dst_offset;
      const auto curr_k =
          reinterpret_cast<const int8_t *>(src_data[io_src::SRC_K]) +
          src_kv_offset;
      const auto curr_v =
          reinterpret_cast<const int8_t *>(src_data[io_src::SRC_V]) +
          src_kv_offset;

      const int badd_offset = ibs * badd_stride[0] + ihn * badd_stride[1];
      const auto badd_f32 =
          has_binary_add
              ? reinterpret_cast<const float *>(src_data[io_src::BINARY_ADD]) +
                    badd_offset
              : nullptr;

      const auto src_mask =
          reinterpret_cast<const int32_t *>(src_data[io_src::MASK]);
      const auto att_scale =
          reinterpret_cast<const float *>(src_data[io_src::ATT_SCALE])[0];
      const auto q_scale =
          reinterpret_cast<const float *>(src_data[io_src::Q_SCALE])[0];
      const auto k_scale =
          reinterpret_cast<const float *>(src_data[io_src::K_SCALE])[0];
      const auto v_scale =
          reinterpret_cast<const float *>(src_data[io_src::V_SCALE])[0];
      const auto dst_scale =
          reinterpret_cast<const float *>(src_data[io_src::SRC_DST_SCALE])[0];
      const auto dst_zp = static_cast<float>(
          reinterpret_cast<const int32_t *>(src_data[io_src::SRC_DST_ZP])[0]);
      ...
      for (int j = 0; j < head_size_; j += 64)
        for (int i = 0; i < sl_n_pad64; i += 4) {
          ...
          (*ker_trans_v_[std::max(dim_t(0), std::min(dim_t(4), src_sl_n - i))])(
              &rt_data_tr_v);
        }
      ...
      jit_softmax_Ab16a::rt_data_t rt_data_softmax2{
          ...
          /*.dst = */ softmax_scrach_p64 +
              sl_n_pad64 * 16, // sl_pad64_ / 64 * 16 * 64
          /*.att_tile = */ padding_mask / 16,
          /*.softmax_rescale = */ softmax_rescale_,
          /*.src_badd = */ nullptr,
          /*.ld_badd = */ badd_stride[2],
      ...
      for (int j = 0; j < row_loop - 1; j++, cur_r_pos += 32) {
        ...
        if (has_binary_add)
          rt_data_softmax1.src_badd = badd_f32 + cur_r_pos * badd_stride[2];
        if (has_binary_add)
          rt_data_softmax2.src_badd =
              badd_f32 + (cur_r_pos + 16) * badd_stride[2];
        mha_per_head_32x(rt_data_qk, rt_data_softmax1, rt_data_softmax2,
                         rt_data_av, att_tail, col_tile, att_tile);
      }
      ...
      if (is_even) {
        if (rollback == 0) {
          ...
          if (has_binary_add)
            rt_data_softmax1.src_badd = badd_f32 + cur_r_pos * badd_stride[2];
          mha_per_head_16x(rt_data_qk, rt_data_softmax1, rt_data_av, att_tail,
                           col_tile, att_tile);
...
static inline __m512 snd_order_poly_exp(__m512 z, __m512 f, const float c[]) {
  ...
  auto y = _mm512_fmadd_ps(_mm512_fmadd_ps(f, c0, c1), f,
                           c2); // auto y = (f * c0 + c1) * f + c2;
...
static inline __m512 exp_ps_0_1(__m512 x) {
  ...
  auto x1 = _mm512_fmadd_ps(
      x, log2e,
      _mm512_set1_ps(.5f)); // auto x1 = x * log2e + _mm512_set1_ps(.5f);
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored                                                 \
    "-Wignored-attributes" // https://stackoverflow.com/a/49216021
#endif
template <int tail, int off>
static inline std::array<__m512i, 16> tr_add_vnni_x64(const int8_t *a,
                                                      size_t lda) {
  ...
  // for (int i = tail; i < 16; ++i) dst[i] = _mm512_setzero_epi32();  // don't
...
static const decltype(tr_add_vnni_x64<1, 128>) *tr_add128_vnni_x64_tbl[] = {
    tr_add_vnni_x64<1, 128>,  tr_add_vnni_x64<1, 128>,
    tr_add_vnni_x64<2, 128>,  tr_add_vnni_x64<3, 128>,
    tr_add_vnni_x64<4, 128>,  tr_add_vnni_x64<5, 128>,
    tr_add_vnni_x64<6, 128>,  tr_add_vnni_x64<7, 128>,
    tr_add_vnni_x64<8, 128>,  tr_add_vnni_x64<9, 128>,
    tr_add_vnni_x64<10, 128>, tr_add_vnni_x64<11, 128>,
    tr_add_vnni_x64<12, 128>, tr_add_vnni_x64<13, 128>,
    tr_add_vnni_x64<14, 128>, tr_add_vnni_x64<15, 128>,
    tr_add_vnni_x64<16, 128>,
};

template <int tail>
static inline __m512i load_interleave_vnni(const int8_t *src, const size_t lda,
                                           const __m512i &vperm_ctl,
                                           const __m512i &vpshuf_ctl);
template <> // TODO(Yi): better impl?
inline __m512i load_interleave_vnni<4>(const int8_t *src, const size_t lda,
                                       const __m512i &vperm_ctl,
                                       const __m512i &vpshuf_ctl) {
  ...
  __m512i abcd = _mm512_permutex2var_epi32(
      _mm512_castsi256_si512(ab), vperm_ctl, _mm512_castsi256_si512(cd));

  return _mm512_shuffle_epi8(abcd, vpshuf_ctl);
}
template <> // TODO(Yi): better impl?
inline __m512i load_interleave_vnni<3>(const int8_t *src, const size_t lda,
                                       const __m512i &vperm_ctl,
                                       const __m512i &vpshuf_ctl) {
  __m128i a = _mm_loadu_si128(reinterpret_cast<const __m128i *>(src));
  __m128i b = _mm_loadu_si128(reinterpret_cast<const __m128i *>(src + lda));
  __m256i ab = _mm256_mask_broadcast_i32x4(_mm256_castsi128_si256(a), 0xf0, b);
  __m128i c = _mm_loadu_si128(reinterpret_cast<const __m128i *>(src + lda * 2));
  __m256i cd = _mm256_mask_broadcast_i32x4(_mm256_castsi128_si256(c), 0xf0,
                                           _mm_setzero_si128());
  __m512i abcd = _mm512_permutex2var_epi32(
      _mm512_castsi256_si512(ab), vperm_ctl, _mm512_castsi256_si512(cd));
...
template <> // TODO(Yi): better impl?
inline __m512i load_interleave_vnni<2>(const int8_t *src, const size_t lda,
                                       const __m512i &vperm_ctl,
                                       const __m512i &vpshuf_ctl) {
  __m128i a = _mm_loadu_si128(reinterpret_cast<const __m128i *>(src));
  __m128i b = _mm_loadu_si128(reinterpret_cast<const __m128i *>(src + lda));
  __m256i ab = _mm256_mask_broadcast_i32x4(_mm256_castsi128_si256(a), 0xf0, b);
  __m512i abcd = _mm512_permutex2var_epi32(_mm512_castsi256_si512(ab),
                                           vperm_ctl, _mm512_setzero_si512());

  return _mm512_shuffle_epi8(abcd, vpshuf_ctl);
}
template <> // TODO(Yi): better impl?
inline __m512i load_interleave_vnni<1>(const int8_t *src, const size_t,
                                       const __m512i &vperm_ctl,
                                       const __m512i &vpshuf_ctl) {
  __m128i a = _mm_loadu_si128(reinterpret_cast<const __m128i *>(src));
  __m256i ab = _mm256_mask_broadcast_i32x4(_mm256_castsi128_si256(a), 0xf0,
                                           _mm_setzero_si128());
  __m512i abcd = _mm512_permutex2var_epi32(_mm512_castsi256_si512(ab),
                                           vperm_ctl, _mm512_setzero_si512());
...
static const decltype(load_interleave_vnni<1>) *load_interleave_vnni_tbl[] = {
    nullptr,
    load_interleave_vnni<1>,
    load_interleave_vnni<2>,
    load_interleave_vnni<3>,
    load_interleave_vnni<4>,
};
bool mha_dense_k_t::execute_tiny(void **src_data, void **dst_data,
                                 void *workspace, dim_t *shape_data) const {
  const auto src_sl_m =
      shape_data[io_shape::M] ? shape_data[io_shape::M] : src_sl_m_;
  const auto src_sl_n =
      shape_data[io_shape::N] ? shape_data[io_shape::N] : src_sl_n_;
  ...
  std::array<int, 4> badd_stride{0, 0, 0, 0};
  if (has_binary_add) {
    const auto tmp_stride_ =
        dim2step(pre_pad1(4, ts_descs_[io::BINARY_ADD].shape()));
    for (int i = 0; i < 4; ++i)
      badd_stride[i] = tmp_stride_[i];
  }
  const int sl_n_pad16 = pad_to(src_sl_n, 16);
  ...
  for (int ibs = 0; ibs < bs; ibs++) {
    for (int ihn = 0; ihn < head_num_; ihn++) {
      ...
      const auto curr_v =
          ...
          src_kv_offset;
      const auto curr_dst =
          reinterpret_cast<char *>(const_cast<void *>(dst_data[io_dst::DST])) +
          dst_offset;
      ...
      const auto qk_scrach =
          reinterpret_cast<float *>(v_scrach_p64 + head_size_ * sl_n_pad64);
      const auto softmax_scrach_p64 =
          reinterpret_cast<uint8_t *>(qk_scrach + 32 * sl_n_pad16);
      ...
      for (int j = 0; j < pmask_floor16; j += VEC) {
        ...
        for (int k = 0; k < head_size_; k += 4 * VEC) {
          auto v_src1 =
              tr_add_vnni_x64<16, 128>(curr_k + j * ld_kv_ + k, ld_kv_);
          v_src0_sum128 = _mm512_dpbusds_epi32(v_src0_sum128, v_128u,
                                               _mm512_loadu_si512(curr_q + k));
#pragma GCC unroll VEC
          for (int kk = 0; kk < VEC; ++kk) {
            const auto v_src0 = _mm512_set1_epi32(
                *reinterpret_cast<const int32_t *>(curr_q + k + kk * 4));
        ...
        auto dstf32 = _mm512_fmadd_ps(_mm512_cvt_roundepi32_ps(v_dst, rn_sae),
                                      scales, _mm512_loadu_ps(badd_f32 + j));
      ...
      { // QxK tail
        ...
        __m512i v_src0_sum128 = _mm512_setzero_epi32();
        for (int k = 0; k < head_size_; k += 4 * VEC) {
          auto v_src1 = tr_add128_vnni_x64_tbl[pmask_tail16](
              curr_k + pmask_floor16 * ld_kv_ + k, ld_kv_);
          v_src0_sum128 = _mm512_dpbusds_epi32(v_src0_sum128, v_128u,
                                               _mm512_loadu_si512(curr_q + k));
#pragma GCC unroll VEC
          for (int kk = 0; kk < VEC; ++kk) {
            const auto v_src0 = _mm512_maskz_set1_epi32(
                pmask_tail,
                *reinterpret_cast<const int32_t *>(curr_q + k + kk * 4));
            v_dst = _mm512_dpbusds_epi32(v_dst, v_src1[kk], v_src0);
          }
        ...
        const int32_t src0_sum128 = _mm512_reduce_add_epi32(v_src0_sum128);
        v_dst = _mm512_sub_epi32(v_dst, _mm512_set1_epi32(src0_sum128));
        auto dstf32 = _mm512_fmadd_ps(
            _mm512_cvt_roundepi32_ps(v_dst, rn_sae), scales,
      ...
      { // exp tail
        __m512 xs =
            _mm512_sub_ps(_mm512_load_ps(qk_scrach + pmask_floor16), v_max);
        xs = exp_ps_0_1(_mm512_max_ps(xs, _mm512_set1_ps(-1000.f)));
...
#pragma omp simd
      ...
      for (int j = 0; j < pmask_floor16; j += VEC) {
        ...
        _mm512_mask_cvtepi32_storeu_epi8(softmax_scrach_p64 + j, 0xffff,
                                         _mm512_cvt_roundps_epu32(xs, rn_sae));
      }
      {
        auto xs = _mm512_maskz_load_ps(pmask_tail, qk_scrach + pmask_floor16);
        xs = _mm512_mul_ps(xs, scales);
        _mm512_mask_cvtepi32_storeu_epi8(softmax_scrach_p64 + pmask_floor16,
                                         0xffff,
                                         _mm512_cvt_roundps_epu32(xs, rn_sae));
      }

      // A x V
      alignas(16) const uint8_t vpermt2d_control[16] = {
          0, 4, 16, 20, 1, 5, 17, 21, 2, 6, 18, 22, 3, 7, 19, 23};
      alignas(16) const uint8_t vpshufb_control[16] = {
          0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15};
      __m512i vperm_ctl = _mm512_cvtepu8_epi32(
          _mm_load_si128(reinterpret_cast<const __m128i *>(vpermt2d_control)));
      __m512i vpshuf_ctl = _mm512_broadcast_i32x4(
          _mm_load_si128(reinterpret_cast<const __m128i *>(vpshufb_control)));
      scales = _mm512_set1_ps(v_scale / softmax_rescale_f32_ / dst_scale);
      auto v_zp = _mm512_set1_ps(dst_zp);
      for (int j = 0; j < head_size_;
           j += VEC) { // head_size_ must be a multiple of 16
        __m512i v_dst = _mm512_setzero_epi32();
        for (int k = 0; k < pmask_floor4; k += 4) {
          __m512i v_src0 = _mm512_set1_epi32(
              *reinterpret_cast<const int32_t *>(softmax_scrach_p64 + k));
          __m512i v_src1 = load_interleave_vnni<4>(
              curr_v + j + k * ld_kv_, ld_kv_, vperm_ctl, vpshuf_ctl);
          v_dst = _mm512_dpbusds_epi32(v_dst, v_src0, v_src1);
        }
        { // tail
          __m512i v_src0 = _mm512_set1_epi32(*reinterpret_cast<const int32_t *>(
              softmax_scrach_p64 + pmask_floor4));
          __m512i v_src1 = load_interleave_vnni_tbl[pmask_tail4]( //
              curr_v + j + pmask_floor4 * ld_kv_, ld_kv_, vperm_ctl,
              vpshuf_ctl);
          v_dst = _mm512_dpbusds_epi32(v_dst, v_src0, v_src1);
        }
        auto xs = _mm512_fmadd_ps(_mm512_cvt_roundepi32_ps(v_dst, rn_sae),
                                  scales, v_zp);
        switch (dst_dt_) {
        case data_type::u8:
          _mm512_mask_cvtusepi32_storeu_epi8(
              curr_dst + j, 0xffff,
              _mm512_max_epi32(_mm512_cvt_roundps_epu32(xs, rn_sae),
        ...
        case data_type::bf16:
          _mm256_storeu_si256(reinterpret_cast<__m256i *>(
                                  reinterpret_cast<bfloat16_t *>(curr_dst) + j),
                              _mm512_cvtepi32_epi16(_mm512_srli_epi32(
                                  _mm512_castps_si512(xs), 16)));
          break;
        default:
          break;
        }
