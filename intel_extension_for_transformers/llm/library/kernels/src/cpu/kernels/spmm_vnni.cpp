...
void auto_blocking(dim_t &BM, dim_t BN, const dim_t M,
                   const dim_t N) { // NOLINT
  if (BM > M) {
    BM = M;
    ...
    SPARSE_LOG(INFO)
        << "BM (micro output channel) automatically configured: BM=" << BM;
  }
}
 ...
 *   append_sum: if the kenrel add the result to the destination tensor instead
 * of overwriting it. Set to "true" to enable. tile_n: n-size of a tile in terms
 * of #registers; default is 4 sub_func: use -1 / 0 / 1 / 2 to specify sub_func
 * folding level -1. use max available value 0. no subfunction
...
bool spmm_vnni_kd_t::init() {
  ...
  bool is_supported =
      ...
      is_any_of({data_type::s8, data_type::fp32},
                [&](const data_type &a) { return wei_desc.dtype() == a; }) &&
      is_any_of({data_type::u8, data_type::fp32},
                [&](const data_type &a) { return src_desc.dtype() == a; }) &&
      (!has_bias ||
       is_any_of({data_type::s32, data_type::fp32},
                 [&](const data_type &a) { return bias_desc.dtype() == a; })) &&
  ...
  bool shape_matched =
      wei_desc.shape().size() == 2 &&
      (src_desc.shape().size() == 2 || src_desc.shape().size() == 3) &&
  ...
  SPARSE_LOG_IF(FATAL, BM_ % TILE_SIZE_M != 0)
      << "BM must be a multiple of TILE_SIZE_M";
  if (op_attrs["welford"] == "true") {
    ...
    for (size_t welford_idx : {ssd::DST_M1, ssd::DST_M2}) {
      ...
      KERNEL_INIT_CHECK(op_desc_.tensor_descs()[welford_idx].dtype() ==
                        data_type::fp32);
      if (op_desc_.tensor_descs()[welford_idx].shape().size() != 0) {
        if (ds_src.size() == 3) {
          KERNEL_INIT_CHECK((op_desc_.tensor_descs()[welford_idx].shape() ==
                             std::vector<dim_t>{ds_src[0], ds_src[2]}));
        } else {
          KERNEL_INIT_CHECK(op_desc_.tensor_descs()[welford_idx].shape() ==
                            std::vector<dim_t>{N()});
        }
...
#ifndef WORKSPACE
    tmp_mem_mean_ =
        aligned_allocator_t<float>::allocate(ceil_div(M_, BM_) * N_);
...
bool spmm_vnni_k_t::execute_(const std::vector<const void *> &rt_data) const {
  ...
  if (derived_kd()->welford()) {
#ifdef WORKSPACE
    tmp_mem_mean = const_cast<float *>(
        static_cast<const float *>(rt_data[ssd::WORK_SPACE]));
    tmp_mem_var = const_cast<float *>(
                      static_cast<const float *>(rt_data[ssd::WORK_SPACE])) +
                  ceil_div(M_, BM_) * N_;
#else
...
#pragma omp parallel for collapse(2)
  for (dim_t im = 0; im < M_; im += BM_) {
    for (dim_t in = 0; in < N_; in += BN_) {
      ...
      data.ptr_dense =
          static_cast<const uint8_t *>(rt_data[ssd::SRC]) + in * K_;
      data.ptr_bias = static_cast<const int32_t *>(rt_data[ssd::BIAS]) + im;
      data.ptr_scales = static_cast<const float *>(rt_data[ssd::SCALES]) + im;
      data.ptr_dst =
          const_cast<dst_t *>(static_cast<const dst_t *>(rt_data[ssd::DST])) +
          in * M_ + im * BN_;
      if (derived_kd()->welford()) {
        data.ptr_dst_m1 =
            tmp_mem_mean + in * ceil_div(M_, BM_) + im / BM_ * BN_;
...
#pragma omp parallel for collapse(2)
    for (dim_t idx_mbs = 0; idx_mbs < N_ / BN_; ++idx_mbs) {
      for (dim_t j = 0; j < BN_; j += 16) {
        ...
        const jit_mean_var_reduce_t *jit_impl =
            jit_mean_var_reduce_kers_[index];
        ...
        data.mean_out = reinterpret_cast<float *>(
                            const_cast<void *>(rt_data[ssd::DST_M1])) +
                        idx_mbs * BN_ + j;
        data.var_out = reinterpret_cast<float *>(
                           const_cast<void *>(rt_data[ssd::DST_M2])) +
                       idx_mbs * BN_ + j;
        (*jit_impl)(&data);
      }
    }
...
bool spmm_vnni_k_t::execute(const std::vector<const void *> &rt_data) const {
  ...
  default:
    SPARSE_LOG(ERROR) << "Unexpected dst_type: "
                      << static_cast<uint8_t>(dst_type());
    break;
  }
  return false;
}
