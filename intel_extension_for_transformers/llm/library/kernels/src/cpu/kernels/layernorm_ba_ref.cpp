...
namespace jd {
bool layernorm_ba_ref_k_t::execute(
  ...
  int batch =
      tensor_desc[0].shape().size() == 2 ? 1 : tensor_desc[0].shape()[0];
  int row = tensor_desc[0].shape().size() == 2 ? tensor_desc[0].shape()[0]
                                               : tensor_desc[0].shape()[1];
  ...
  auto normal_translnorm = [&]() {
    ...
    for (int k = 0; k < batch; k++) {
      for (int i = 0; i < col; i++) {
        ...
        float mean = 0;
        for (int j = 0; j < row; j++)
          mean += src[k * col * row + j * col + i];
        mean /= row;
        // calculate var
        float var = 0;
        for (int j = 0; j < row; j++)
          var += (src[k * col * row + j * col + i] - mean) *
                 (src[k * col * row + j * col + i] - mean);
        var /= row;
        var += 1e-5;
        var = sqrt(var);
        var = 1 / var;
        ...
        for (int j = 0; j < row; j++) {
          ...
          if (!binary_op_list.empty()) {
            value = get_quantize(value, binary_op_list[0].zp[j],
                                 1 / binary_op_list[0].scale[j],
                                 binary_op_list[0].op_dt);
          }
    ...
    for (int i = 0; i < batch; i++) {
      for (int j = 0; j < row; j++) {
        for (int k = 0; k < col; k++) {
          ...
          // check whether enable split-output, if true then stroe twice, one
          // time is fp32, the other is dt(quant)
