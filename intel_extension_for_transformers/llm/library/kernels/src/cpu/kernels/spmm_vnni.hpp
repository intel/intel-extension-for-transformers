...
public:
  ...
  inline dim_t M() const {
    return op_desc_.tensor_descs()[ssd::WEI].shape()[0];
  }
  inline dim_t K() const {
    return op_desc_.tensor_descs()[ssd::WEI].shape()[1];
  }
  ...
  inline bool has_bias() const {
    return !op_desc_.tensor_descs()[ssd::BIAS].shape().empty();
  }
  inline data_type dst_type() const {
    return op_desc_.tensor_descs()[ssd::DST].dtype();
  }
...
public:
  ...
  explicit spmm_vnni_k_t(const std::shared_ptr<const kd_t> &kd)
      : kernel_t(kd), M_(derived_kd()->M()), N_(derived_kd()->N()),
        K_(derived_kd()->K()), BM_(derived_kd()->BM()),
        BN_(derived_kd()->BN()) {}
  virtual ~spmm_vnni_k_t() {
    for (auto &kernel : jit_spmm_kers_)
      safe_delete(kernel);
    for (auto &kernel : jit_mean_var_reduce_kers_)
      safe_delete(kernel);
...
private:
  bool spmm_kernel_create(jit_spmm_vnni_t **ker_pp,
                          const ssd::vnni_param_t &param);
