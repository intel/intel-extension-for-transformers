...
bool groupnorm_ref_k_t::execute(
  ...
  bool affine = true;
  int64_t map_size = std::accumulate(shape.begin() + 2, shape.end(), 1,
                                     std::multiplies<int>());
  ...
  float *beta_data =
      static_cast<float *>(const_cast<void *>(rt_data[idx::BETA]));
...
#pragma omp simd
    for (int64_t g = 0; g < groups; g++) {
      const float *src_group_data =
          src_single_data + g * channels_per_group * map_size;
      float *dst_group_data =
          dst_single_data + g * channels_per_group * map_size;
      // mean and var
      float sum = 0.f;
  ...
  if (src_desc.dtype() == data_type::fp32) {
    cast_from_float_array<float>(
        fp32_dst, const_cast<void *>(rt_data[idx::DST]), src_desc.size());
  } else {
    cast_from_float_array<bfloat16_t>(
        fp32_dst, const_cast<void *>(rt_data[idx::DST]), src_desc.size());
  }
