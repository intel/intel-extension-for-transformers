...
dynamic_quant_matmul_ref_kd_t::dynamic_quant_matmul_ref_kd_t(
...
bool dynamic_quant_matmul_ref_kd_t::init() {
  ...
  if (ts_desc[0].dtype() != data_type::s8 ||
      ts_desc[1].dtype() != data_type::s8)
    SPARSE_LOG(FATAL)
...
#pragma omp parallel for
    for (int i = 0; i < M; i++) {
      for (int j = 0; j < N; j++) {
        ...
        for (int k = 0; k < K; k++) {
          tmp +=
              (DST) * (a + batch * M * K + i * K + k) * (DST) * (b + k * N + j);
        }
...
void dequant_add_bias(float *mat, const float *scale_a, const float *scale_w,
                      int b, int m, int n, bool add_bias, const float *bias,
                      const std::vector<postop_attr> &postop_list) {
  for (int batch = 0; batch < b; batch++) {
#pragma omp parallel for
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < n; j++) {
        mat[batch * m * n + i * n + j] = mat[batch * m * n + i * n + j] *
                                         scale_a[batch * m + i] * scale_w[j];
        if (add_bias)
          mat[batch * m * n + i * n + j] += bias[j];
        mat[batch * m * n + i * n + j] =
            apply_postop_list(mat[batch * m * n + i * n + j], postop_list);
      }
    }
  }
}
...
#pragma omp parallel for
    for (int i = 0; i < m; i++) {
      float max = 0.f;
      for (int j = 0; j < n; j++)
        max = max < abs(mat[batch * m * n + i * n + j])
                  ? abs(mat[batch * m * n + i * n + j])
                  : max;
...
void s8_quant_mat(int8_t *dst_mat, const std::vector<float> &src_mat,
                  float *scale, int b, int m, int n) {
  for (int batch = 0; batch < b; batch++) {
#pragma omp parallel for
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < n; j++) {
        int ans = nearbyint(src_mat[batch * m * n + i * n + j] /
                            scale[batch * m + i]);
        ans = ans > 127 ? 127 : ans;
        ans = ans < -128 ? -128 : ans;
...
void fp32_append_sum(const float *src_mat, std::vector<float> *dst_mat, int b,
                     int m, int n) {
  for (int batch = 0; batch < b; batch++) {
#pragma omp parallel for
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < n; j++) {
        (*dst_mat)[batch * m * n + i * n + j] +=
            src_mat[batch * m * n + i * n + j];
      }
    }
  }
}
  ...
  for (int n_loop = 0; n_loop < trans_block_row; n_loop++)
    for (int k_loop = 0; k_loop < trans_block_col; k_loop++)
      for (int i = 0; i < tile_k / 4; i++)
        for (int j = 0; j < 64; j++)
          trans_back_buf[k_loop * trans_block_row * block_size + n_loop * 64 +
                         i * pad_n * 4 + j] =
              reorder_mat[n_loop * trans_block_col * block_size + k_loop * 64 +
                          i * trans_block_col * 64 + j];
  // step2. reorder back.
...
#pragma omp parallel for
  for (int i = 0; i < k / 4; i++)
    for (int j = 0; j < n * 4; j++)
      reorder_back_mat[j % 4 * n + j / 4 + i * 4 * n] =
          trans_back_buf[i * 4 * pad_n + j];
  return reorder_back_mat;
}

bool dynamic_quant_matmul_ref_k_t::execute(
  ...
  std::vector<int8_t> reorder_back_mat =
      reorder_back(r_mat, prob_size[k], prob_size[n]);
  ...
  std::vector<float> fp32_dst_mat(
      prob_size[batch] * prob_size[m] * prob_size[n], 0);
  gemm(l_mat, const_cast<const int8_t *>(reorder_back_mat.data()),
       fp32_dst_mat.data(), prob_size[batch], prob_size[m], prob_size[n],
       prob_size[k]);
  dequant_add_bias(fp32_dst_mat.data(), scale_a, scale_w, prob_size[batch],
                   prob_size[m], prob_size[n], add_bias, bias, postop_list);
  if (append_sum) {
    float *append_value;
    ...
    if (derived_kd()->check_dst_dt() == data_type::fp32) {
      ...
      cast_to_float_array<bfloat16_t>(dst_mat, &cvt_append_value,
                                      cvt_append_value.size());
      append_value = cvt_append_value.data();
    }
    fp32_append_sum(append_value, &fp32_dst_mat, prob_size[batch], prob_size[m],
                    prob_size[n]);
  }
  if (derived_kd()->check_dst_dt() == data_type::s8) {
    get_dynamic_quant_scale(fp32_dst_mat.data(), scale_dst, prob_size[batch],
                            prob_size[m], prob_size[n]);
    s8_quant_mat(dst_mat, fp32_dst_mat, scale_dst, prob_size[batch],
                 prob_size[m], prob_size[n]);
  } else {
    if (derived_kd()->check_dst_dt() == data_type::fp32)
      cast_from_float_array<float>(fp32_dst_mat, dst_mat,
                                   prob_size[m] * prob_size[n] *
                                       prob_size[batch]);
    else
      cast_from_float_array<bfloat16_t>(fp32_dst_mat, dst_mat,
                                        prob_size[m] * prob_size[n] *
                                            prob_size[batch]);
  }
  return true;
