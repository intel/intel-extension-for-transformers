...
bool spmm_ref_kd_t::init() {
  ...
  if (wei_desc.dtype() != data_type::fp32) {
    shape_matched =
        ...
        wei_desc.shape().back() !=
            src_desc.shape()[src_desc.shape().size() - 2];
  } else {
  ...
  if (BM_ == 0 &&
      wei_desc.dtype() != data_type::fp32) { // try to get optimized block size
    ...
    SPARSE_LOG(INFO)
        << "BM (micro output channel) automatically configured: BM=" << BM_;
  }
...
bool spmm_ref_k_t::execute_s8_(const std::vector<const void *> &rt_data) const {
  ...
  if ((dst_dt == data_type::s8 || dst_dt == data_type::u8) &&
      (postop_list.size() == 0)) {
    if (postop_list.size() == 0 ||
        postop_list.back().op_alg != postop_alg::quantize)
      postop_list.emplace_back(dst_dt, postop_type::eltwise,
                               postop_alg::quantize, 0., 0., 1.);
  }
...
#pragma omp simd
        for (dim_t k = 0; k < K_; ++k) {
          ...
          dim_t r_idx = idx_mbs * right_stride[0] + k * right_stride[1] +
                        j * right_stride[2];
        ...
        dim_t dst_idx =
            idx_mbs * dst_stride[0] + i * dst_stride[1] + j * dst_stride[2];
        dim_t scale_idx = i;
  ...
  if (rt_data.size() >= ssd::DST_M2 + 1 && dst_dt == data_type::fp32 &&
      welford) {
    auto mean_fp32 =
        static_cast<float *>(const_cast<void *>(rt_data[ssd::DST_M1]));
    auto var_fp32 =
        static_cast<float *>(const_cast<void *>(rt_data[ssd::DST_M2]));
    ...
    for (dim_t idx_mbs = 0; idx_mbs < num_BN; ++idx_mbs) {
      for (dim_t j = 0; j < BN_; ++j) {
        double M2 = 0.f;
        for (dim_t i = 0; i < M_; ++i) {
          M2 += pow(dst_fp32[idx_mbs * BN_ * M_ + i * BN_ + j] -
                        mean_fp32[idx_mbs * BN_ + j],
                    2);
        }
...
bool spmm_ref_k_t::execute_bf16_(
...
#pragma omp parallel for
      for (int n = 0; n < BN_; ++n) {
        auto dst_idx =
            num_n * dst_stride[0] + m * dst_stride[1] + n * dst_stride[2];
        for (int k = 0; k < K_; ++k) {
          float_dst_data[dst_idx] +=
              static_cast<float>(
                  wei_data[m * wei_stride[0] + k * wei_stride[1]]) *
              static_cast<float>(
                  src_data[num_n * src_stride[0] + k * src_stride[1] +
                           n * src_stride[2]]);
        }
        if (has_bias) {
        ...
        float_dst_data[dst_idx] =
            apply_postop_list(float_dst_data[dst_idx], postop_list);
...
bool spmm_ref_k_t::execute_f32_(
    const std::vector<const void *> &rt_data) const {
  const auto &ts_descs = derived_kd()->get_operator_desc().tensor_descs();
  const auto &postops_list =
      derived_kd()->get_operator_desc().apply_postops_list();
...
#pragma omp parallel for
    for (int j = 0; j < N; ++j) {
      float value = 0; // Consistent with the actual precision (float or double)
                       // of cpu instructions.
#pragma omp simd
      for (int k = 0; k < K; ++k) {
      ...
      dst_fp32[i * dst_stride[0] + j * dst_stride[1]] =
