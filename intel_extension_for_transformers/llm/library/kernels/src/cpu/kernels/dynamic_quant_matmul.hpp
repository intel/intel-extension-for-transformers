...
public:
  ...
  const std::vector<ssd::dynamic_quant_matmul_param_t> &params() const {
    return params_;
  }
  const std::vector<dynamic_quant_matmul_reduce_scale_quant_param_t> &
  reduce_scale_quant_params() const {
...
private:
  ...
  std::vector<dynamic_quant_matmul_reduce_scale_quant_param_t>
      reduce_scale_quant_params_;
...
public:
  ...
  explicit dynamic_quant_matmul_k_t(const std::shared_ptr<const kd_t> &kd)
      : kernel_t(kd) {}
  virtual ~dynamic_quant_matmul_k_t() {
    for (auto &&ker : jit_kers_)
      safe_delete(ker);
  ...
  dynamic_quant_matmul_k_t &
  operator=(dynamic_quant_matmul_k_t &&other) = delete;
  // Delete copy constructor and copy operator
  dynamic_quant_matmul_k_t(const dynamic_quant_matmul_k_t &other) = delete;
  dynamic_quant_matmul_k_t &
  operator=(const dynamic_quant_matmul_k_t &other) = delete;
...
private:
  ...
  std::vector<jit_amx_s8s8_dynamic_dequant_matmul_t *>
      jit_s8s8_dynamic_dequant_kers_;
  std::vector<jit_dynamic_quant_matmul_reduce_scale_quant_t *>
      jit_reduce_scale_quant_kers_;
