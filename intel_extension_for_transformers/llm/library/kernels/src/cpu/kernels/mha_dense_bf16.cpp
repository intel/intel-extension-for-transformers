...
bool mha_dense_bf16_kd_t::init() {
  if (!isa_available(avx512_core_bf16_amx_bf16))
    return false;
  const auto attrs = op_desc_.attrs();
  KERNEL_INIT_CHECK(attrs.find("approx_exp") != attrs.end() &&
                    attrs.at("approx_exp") == "True");
  KERNEL_INIT_CHECK(attrs.find("stable_softmax") != attrs.end() &&
                    (attrs.at("stable_softmax") == "False" ||
                     attrs.at("stable_softmax") == "True"));
  KERNEL_INIT_CHECK(attrs.find("merged_QKV") == attrs.end() ||
                    attrs.at("merged_QKV") != "True"); // no merge support
  KERNEL_INIT_CHECK(std::all_of(
      attrs.cbegin(), attrs.cend(), [](auto &&kv) { // no unrecognized attr
        return is_any_of({"approx_exp", "stable_softmax", "merged_QKV"},
                         [&kv](auto &&k) { return kv.first == k; });
      }))
  ...
  KERNEL_INIT_CHECK(
      (desc[io::SRC_Q].shape() ==
       std::vector<dim_t>{batch_size, sl_m, head_num, head_size}));
  KERNEL_INIT_CHECK(
      (desc[io::SRC_K].shape() ==
       std::vector<dim_t>{batch_size, sl_n, head_num, head_size}));
  KERNEL_INIT_CHECK(
      (desc[io::SRC_V].shape() ==
       std::vector<dim_t>{batch_size, sl_n, head_num, head_size}));
  KERNEL_INIT_CHECK(
  ...
  if (!badd_shape.empty()) {
    ...
    KERNEL_INIT_CHECK(desc[io::BINARY_ADD].ftype() ==
                      plain_format(badd_shape.size()));
    const auto &badd_pad1 = pre_pad1(4, badd_shape);
    KERNEL_INIT_CHECK(
        (badd_pad1 ==
         std::vector<dim_t>{1, 1, sl_m,
                            sl_n})); // TODO(Yi): tmp restriction to removed
  ...
  KERNEL_INIT_CHECK(
      (desc[io::MASK].shape().empty() ||
       desc[io::MASK] == tensor_desc{{batch_size}, data_type::s32, ft::a}));
  KERNEL_INIT_CHECK(
      (desc[io::ATT_SCALE] == tensor_desc{{1}, data_type::fp32, ft::a}));
...
inline V get_entry(const std::unordered_map<K, V> &map, const K2 &k,
                   const V2 &v_default) {
...
mha_dense_bf16_k_t::mha_dense_bf16_k_t(
    const std::shared_ptr<const kernel_desc_t> &kd)
    : kernel_t(kd), ts_descs_(derived_kd()->get_operator_desc().tensor_descs()),
      has_pmask(!ts_descs_[io::MASK].shape().empty()),
      has_badd(!ts_descs_[io::BINARY_ADD].shape().empty()),
      dt_dst(ts_descs_[io::DST].dtype()), bs_(ts_descs_[io::SRC_Q].shape()[0]),
      ...
      ld_src_(head_size_ * head_num_), ld_dst_(head_size_ * head_num_),
      ...
      workspace_size_(sizeof(bfloat16_t) * reo_k_size_ +            //
                      ...
                      sizeof(float) * pad_to(sl_m_, PAD_SIZE) *
                          sl_n_pad_), // extra space for badd to read
      amx_full_tile_param_(16, 16, 32, true, 2),
      amx_full_tile_cfg_(amx_full_tile_param_),
      kern_tr_k({/*.pad_n = */ 64, /*.cvt_s8u8 = */ false, 2}),
      kern_tr_v(32, sizeof(bfloat16_t)), kern_tr_q(),
      kern_qksoftmax(has_pmask || has_badd ||
                         sl_n_pad_ != sl_n_, // pmask is applied via binary add
                     derived_kd()->get_operator_desc().attrs().at(
                         "stable_softmax") == "True",
...
bool mha_dense_bf16_k_t::init() {
  ...
  if (!kern_tr_k.create_kernel())
    return false;
  if (!kern_tr_v.create_kernel())
    return false;
  if (!kern_tr_q.create_kernel())
    return false;
  if (!kern_qksoftmax.create_kernel())
    return false;
  if (!kern_mmav.create_kernel())
    return false;
...
bool mha_dense_bf16_k_t::execute(const exec_context_t &ctx) const {
  ...
  const auto sl_n = shape_data[io_shape::N]
                        ? shape_data[io_shape::N]
                        : sl_n_; // TODO(Yi): sl_n affects kern_qksoftmax
  assert(shape_data[io_shape::BATCH_SIZE] <=
         0); // dynamic BATCH_SIZE not supported
  assert(shape_data[io_shape::HEAD_SIZE] <=
         0); // dynamic HEAD_SIZE not supported
  ...
  SPARSE_DLOG_IF(FATAL, has_badd && !src_data[io_src::BINARY_ADD])
      << "Binary-add operand expected but not passed!";
  SPARSE_DLOG_IF(FATAL, has_pmask && !src_data[io_src::MASK])
      << "Padding mask expected but not passed!";
  ...
  const auto badd =
      has_badd ? reinterpret_cast<const float *>(src_data[io_src::BINARY_ADD])
               : nullptr;
  const auto dst =
      reinterpret_cast<bfloat16_t *>(const_cast<void *>(dst_data[io_dst::DST]));
  ...
  const auto tmp_mask = reinterpret_cast<float *>(
      tmp_thread + omp_get_max_threads() * thread_total_bytes_);
...
#pragma omp parallel for collapse(2)
  for (int ibat = 0; ibat < bs_ * head_num_; ibat++) {
    for (int i_n = 0; i_n < sl_n; i_n += PAD_SIZE) {
      ...
      const auto bat_reo_offset =
          (ibs * head_num_ + ihn) * sl_n_pad * head_size_pad_;
      ...
      { // Reorder V
        ...
        const jit_padding_interleave4b_n::rt_data_t rtdata_reorder_v{
            ...
            /*.dststride = */ curr_pmask_pad *
                static_cast<int>(sizeof(bfloat16_t)),
        };
      ...
      if (i_n == 0 && ihn == 0 && (has_pmask || sl_n_pad != sl_n)) {
        for (int ii = 0; ii < sl_m; ++ii) {
          ...
          std::fill(curr_tmp_mask + curr_pmask, curr_tmp_mask + sl_n_pad,
                    -INFINITY);
        }
      }
    }
...
#pragma omp parallel for collapse(2)
  for (int ibat = 0; ibat < bs_ * head_num_; ibat++) {
    for (int i_m = 0; i_m < sl_m; i_m += m_tile) {
      ...
      const auto curr_reo_q = reinterpret_cast<bfloat16_t *>(
          tmp_thread + thread_num * thread_total_bytes_);
      const auto curr_softmax =
          reinterpret_cast<bfloat16_t *>(curr_reo_q + thread_reo_q_size_);
      assert(dt_dst == data_type::bf16);
      const auto curr_tmp_dst =
          reinterpret_cast<bfloat16_t *>(curr_softmax + thread_softmax_size_);
      ...
      const int curr_pmask_pad = pad_to(curr_pmask, PAD_SIZE);
      auto ioffset =
          ibs * sl_m * ld_src_ + ihn * head_size_; // offset for Q & DST
      ...
      const auto qksoftmax_badd =
          (has_pmask || sl_n_pad != sl_n) ? curr_tmp_mask : curr_badd;
      ...
      const jit_mha_bf16_row_amx_32x32::rt_data_t rtdata_mmav{
          ...
          /*.cstep = */ (is_tail_m ? head_size_pad_ : ld_dst_) *
              static_cast<int>(sizeof(bfloat16_t)),
      ...
      if (is_tail_m) {
        for (int i = 0; i < sl_m - i_m; ++i)
          memcpy(curr_dst + i * ld_dst_, mmav_dst + i * head_size_pad_,
                 head_size_ * sizeof(bfloat16_t));
      }
    }
  }
  return true;
...
bool mha_dense_bf16_k_t::execute(
