...
template <typename T> void binary_add(void *dst, void *append_src) {
    ...
    for (int j = 0; j < src0_shape[1]; ++j) {
      memcpy(dst_data + (i * src0_shape[1] + j) * get_data_size(input_dt),
             reinterpret_cast<const char *>(src0_data) +
                 (indices_val * src0_shape[1] + j) * get_data_size(input_dt),
...
#pragma omp simd
    for (int j = 0; j < dst_shape[1]; ++j) {
      ...
      for (size_t k = 3; k < ts_descs.size(); k++) {
        int broad_cast_i = i;
        if (ts_descs[k].shape()[0] == 1)
          broad_cast_i = 0;
        if (input_dt == data_type::s8) {
          binary_add<int8_t>(
              dst_data + (i * dst_shape[1] + j) * get_data_size(input_dt),
