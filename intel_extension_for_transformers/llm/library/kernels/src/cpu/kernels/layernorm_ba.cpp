...
void layernorm_ba_kd_t::handle_3D() {
  ...
  SPARSE_LOG_IF(FATAL, !(input_tensor_shape.size() >= 2 &&
                         input_tensor_shape.size() <= 3))
      << "only support 2D/3D input.";
  if (input_tensor_shape.size() == 2)
    input_tensor_shape.insert(input_tensor_shape.begin(), 1);
...
void layernorm_ba_kd_t::direct_translnorm_init() {
  ...
  SPARSE_LOG_IF(FATAL,
                input_dt != data_type::fp32 && input_dt != data_type::s32)
  ...
  if (op_attrs["split_output"] == "true") {
    SPARSE_LOG_IF(FATAL, op_desc_.apply_postops_list().size() == 0 ||
                             op_desc_.apply_postops_list().back().op_alg !=
                                 postop_alg::quantize ||
                             tensor_desc[1].dtype() != data_type::fp32)
        << "split output feature need quant op and fp32 data type in dst "
           "tensor descriptor";
...
void layernorm_ba_kd_t::normal_translnorm_init() {
  auto tensor_desc = op_desc_.tensor_descs();
  auto op_attr = op_desc_.attrs();
  SPARSE_LOG_IF(FATAL, op_attr["split_output"] == "true")
      << "split output feature only for directlnorm";
  auto input_dt = tensor_desc[0].dtype();
  auto output_dt = tensor_desc[1].dtype();
  SPARSE_LOG_IF(FATAL, input_dt != data_type::fp32)
      << "only support fp32 input.";
  handle_3D();
  ...
  for (int i = 0; i < ker_num; i++) {
    ...
    param.process_batch_per_ker =
        omp_get_max_threads() >= batch_num ? 1 : batch_num;
...
void layernorm_ba_k_t::direct_execute(
    ...
    for (int j = 0; j < ker_num; j++) {
      ...
      data_param.src =
          const_cast<char *>(reinterpret_cast<const char *>(rt_data[0]) +
                             (i * col_num * row_num + row_offset * col_num) *
                                 get_data_size(src_dt));
      data_param.dst =
          const_cast<char *>(reinterpret_cast<const char *>(rt_data[1]) +
                             (i * col_num * row_num + row_offset * col_num) *
                                 get_data_size(dst_dt));
      data_param.alpha =
          reinterpret_cast<float *>(const_cast<void *>(rt_data[2])) +
          row_offset;
      data_param.beta =
          reinterpret_cast<float *>(const_cast<void *>(rt_data[3])) +
          row_offset;
      data_param.mean =
          reinterpret_cast<float *>(const_cast<void *>(rt_data[4])) +
          i * col_num;
      data_param.var =
          reinterpret_cast<float *>(const_cast<void *>(rt_data[5])) +
          i * col_num;
      if (split_output)
        data_param.dst2 =
            const_cast<char *>(reinterpret_cast<const char *>(rt_data[6]) +
                               (i * col_num * row_num + row_offset * col_num) *
                                   get_data_size(dst2_dt));
...
void layernorm_ba_k_t::normal_execute(
    const std::vector<const void *> &rt_data) const {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < batch_loop; i++) {
    for (int j = 0; j < ker_num; j++) {
      ...
      data_param.src =
          const_cast<char *>(reinterpret_cast<const char *>(rt_data[0]) +
                             i * row_num * col_num * get_data_size(src_dt));
      data_param.dst =
          const_cast<char *>(reinterpret_cast<const char *>(rt_data[1]) +
                             i * row_num * col_num * get_data_size(dst_dt));
      data_param.alpha =
          reinterpret_cast<float *>(const_cast<void *>(rt_data[2]));
      data_param.beta =
          reinterpret_cast<float *>(const_cast<void *>(rt_data[3]));
