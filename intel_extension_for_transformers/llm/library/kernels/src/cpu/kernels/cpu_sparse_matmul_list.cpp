...
static const std::map<map_key_t, std::vector<impl_list_item_t>> impl_list_map =
    {
        {{kernel_prop::forward_inference, data_type::bf16, data_type::bf16,
          data_type::fp32},
         {CPU_INSTANCE(spmm_amx_bf16_x16_k_t), CPU_INSTANCE(spmm_ref_k_t),
          NULL_INSTANCE()}},
        {{kernel_prop::forward_inference, data_type::bf16, data_type::bf16,
          data_type::bf16},
         {CPU_INSTANCE(spmm_amx_bf16_x16_k_t), CPU_INSTANCE(spmm_ref_k_t),
          NULL_INSTANCE()}},
        {{kernel_prop::forward_inference, data_type::s8, data_type::u8,
          data_type::s8},
         {CPU_INSTANCE(spmm_vnni_k_t), CPU_INSTANCE(spmm_ref_k_t),
          NULL_INSTANCE()}},
        {{kernel_prop::forward_inference, data_type::s8, data_type::u8,
          data_type::fp32},
         {CPU_INSTANCE(spmm_vnni_k_t), CPU_INSTANCE(spmm_ref_k_t),
        ...
        {{kernel_prop::forward_inference, data_type::fp32, data_type::fp32,
          data_type::fp32},
         {CPU_INSTANCE(spmm_avx512f_k_t), CPU_INSTANCE(spmm_ref_k_t),
          NULL_INSTANCE()}},
};

const std::vector<impl_list_item_t> *
get_sparse_matmul_impl_list(const operator_desc &op_desc) {
