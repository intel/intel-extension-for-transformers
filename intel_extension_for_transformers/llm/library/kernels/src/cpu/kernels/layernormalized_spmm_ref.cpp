...
bool layernormalized_spmm_ref_kd_t::init() {
  ...
  tensor_desc mean_desc = {
      {num_mbs, micro_bs}, data_type::fp32, format_type::a};
...
bool layernormalized_spmm_ref_k_t::init() {
  ...
  if (!kernel_desc_t::create<spmm_ref_kd_t>(spmm_ker_desc, *(kd->spmm_desc)))
    return false;
  if (!create<spmm_ref_k_t, spmm_ref_kd_t>(kernels_[0], spmm_ker_desc))
    return false;
  if (!kernel_desc_t::create<layernorm_ba_ref_kd_t>(lnorm_ker_desc,
                                                    *(kd->lnorm_desc)))
    return false;
  if (!create<layernorm_ba_ref_k_t, layernorm_ba_ref_kd_t>(kernels_[1],
                                                           lnorm_ker_desc))
    return false;
  return true;
}

bool layernormalized_spmm_ref_k_t::execute(
