...
attention_ref_kd_t::~attention_ref_kd_t() {
  ...
  if (qk_weight_addr_ != nullptr)
    aligned_allocator_t<char>::deallocate(qk_weight_addr_);
}

template <typename T_kd>
inline bool attention_ref_kd_t::add_kernel_desc(const operator_desc &op_desc,
                                                const char *name) {
...
bool attention_ref_kd_t::init() {
  ...
  qk_weight_addr_ = reinterpret_cast<void *>(
  ...
  void *qk_weight_addr_offset = reinterpret_cast<void *>(
      reinterpret_cast<intptr_t>(qk_weight_addr_) + wei_bytes);
  ...
  qk_sparse_ptr_ = new bsr_data_t<int8_t>(spns::reorder_to_bsr_group<int8_t, 4>(
      ...
      reinterpret_cast<int8_t *>(qk_weight_addr_)));
  ...
  operator_desc spmm_qk_desc = operator_desc(
      ...
      {{"weight_ptr",
        std::to_string(reinterpret_cast<uint64_t>(qk_weight_addr_))},
       {"sparse_ptr",
  ...
  if (!add_kernel_desc<spmm_ref_kd_t>(spmm_qk_desc, "spmm_qk"))
    return false;
  ...
  if (!add_kernel_desc<matmul_ref_kd_t>(matmul_qk_desc, "matmul_qk"))
    return false;
  ...
  operator_desc softmax_desc =
      operator_desc(kernel_kind::softmax, op_desc_.kernel_prop(),
                    op_desc_.engine_kind(), {qk_out_desc, softmax_output},
                    {{"spec_type", "lut"},
                     {"vec_len", std::to_string(seq_len)},
                     {"postop_list", "attention_dequantize+quantize"}},
                    {dequantize_s8_attr, quant_u8_attr});
  if (!add_kernel_desc<softmax_ref_kd_t>(softmax_desc, "softmax"))
    return false;
  ...
  operator_desc spmm_v_desc = operator_desc(
      ...
      {{"weight_ptr",
        std::to_string(reinterpret_cast<uint64_t>(v_weight_addr))},
       {"sparse_ptr",
  ...
  if (!add_kernel_desc<spmm_ref_kd_t>(spmm_v_desc, "spmm_v"))
    return false;
  ...
  operator_desc qk_v_desc = operator_desc(
      ...
      {});
  if (!add_kernel_desc<matmul_ref_kd_t>(qk_v_desc, "qk_matmul_v"))
    return false;
  return true;
}
...
bool attention_ref_k_t::setup_kernel() {
  ...
  if (!create<spmm_ref_k_t, spmm_ref_kd_t>(kernels_[SubKernel::QK_SPMM],
                                           qk_spmm_desc))
    return false;
  ...
  if (!create<matmul_ref_k_t, matmul_ref_kd_t>(kernels_[SubKernel::Q_K_GEMM],
                                               q_k_gemm_desc))
    return false;
  ...
  if (!create<spmm_ref_k_t, spmm_ref_kd_t>(kernels_[SubKernel::V_SPMM],
                                           v_spmm_desc))
    return false;
  ...
  if (!create<matmul_ref_k_t, matmul_ref_kd_t>(kernels_[SubKernel::QK_V_MATMUL],
                                               qk_v_matmul))
    return false;
  return true;
}
void attention_ref_k_t::setup_memory() {
  ...
  mem_[SubKernel::QK_SPMM][ssd::WEI] =
      reinterpret_cast<char *>(str_to_num<intptr_t>(
          ker_opdesc(SubKernel::QK_SPMM).attrs().at("weight_ptr")));
  ...
  mem_[SubKernel::V_SPMM][ssd::WEI] =
      reinterpret_cast<char *>(str_to_num<intptr_t>(
          ker_opdesc(SubKernel::V_SPMM).attrs().at("weight_ptr")));
...
bool attention_ref_k_t::execute(
...
std::vector<const void *> attention_ref_k_t::set_input_output(
