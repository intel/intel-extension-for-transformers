...
std::vector<bsr_data_t<T> *> *reorder_to_bsr_amx(dim_t rows, dim_t cols,
                                                 dim_t micro_rows,
                                                 const void *uncoded_ptr) {
  const dim_t blk_row = 16;
  const dim_t blk_col = 1;
  SPARSE_LOG_IF(FATAL, rows % micro_rows != 0)
      << "rows should divded by micro_rows";
  ...
  for (int i = 0; i < num_micro_rows; ++i) {
    const T *uncoded_data =
        static_cast<const T *>(uncoded_ptr) + i * micro_rows * cols;
    const auto bsr_data = reorder_to_bsr_group<T, group>(
        micro_rows, cols, blk_row, blk_col, uncoded_data);
    sparse_data->push_back(
        new bsr_data_t<T>({blk_row, blk_col}, {rows, cols}, bsr_data.indptr(),
                          bsr_data.indices(), bsr_data.data(), group));
  }
  return sparse_data;
}
template std::vector<bsr_data_t<int8_t> *> *
reorder_to_bsr_amx<int8_t, 64>(dim_t, dim_t, dim_t, const void *);
template std::vector<bsr_data_t<bfloat16_t> *> *
reorder_to_bsr_amx<bfloat16_t, 32>(dim_t, dim_t, dim_t, const void *);

template <typename T>
bsr_data_t<T> tobsr(dim_t rows, dim_t cols, dim_t blk_row, dim_t blk_col,
                    const T *uncoded_data) {
  ...
  for (dim_t b_row = 0; b_row < rows / blk_row; b_row++) {
    ...
    for (dim_t b_col = 0; b_col < cols / blk_col; b_col++) {
      bool is_zero = true;
      const T *dense_start =
          uncoded_data + b_row * blk_row * cols + b_col * blk_col;
  ...
  for (dim_t b_row = 0; b_row < rows / blk_row; b_row++) {
    for (dim_t b_col_idx = rowptr[b_row]; b_col_idx < rowptr[b_row + 1];
         b_col_idx++, nnz_idx++) {
  ...
  return (
      bsr_data_t<T>({blk_row, blk_col}, {rows, cols}, rowptr, colidxs, data));
}
...
bsr_data_t<T> reorder_to_bsr_group(dim_t rows, dim_t cols, dim_t blk_row,
  ...
  for (dim_t i = 0; i < num_row; ++i) { // for each row of blocks
    ...
    for (dim_t col_idx = col_idx_lo; col_idx < col_idx_hi;
         col_idx += group) { // for each group
      for (dim_t bi = 0; bi < blk_row;
           ++bi) { // for each row (in terms of dense matrix) in a group
          ...
          if (col_idx + ig < col_idx_hi) {
            ...
            if (bi == 0) // colidx only need to be added once for each block
              padded_indices.push_back(
                  padded_indices.back()); // padd index with last one
            for (dim_t bj = 0; bj < blk_col; ++bj)
              padded_data.push_back(static_cast<T>(0)); // pad data with zeros
          }
        }
      }
    }
  ...
  return bsr_data_t<T>({blk_row, blk_col}, {rows, cols}, padded_indptr,
                       padded_indices, padded_data, group);
}
template bsr_data_t<int8_t>
reorder_to_bsr_group<int8_t, 4>(dim_t rows, dim_t cols, dim_t blk_row,
                                dim_t blk_col, const int8_t *uncoded_data);

template <typename T>
bsc_data_t<T> tobsc(dim_t rows, dim_t cols, dim_t blk_row, dim_t blk_col,
                    const T *uncoded_data) {
  ...
  for (dim_t ib_col = 0; ib_col < cols / blk_col; ib_col++) {
    ...
    for (dim_t ib_row = 0; ib_row < rows / blk_row; ib_row++) {
      const T *blk_start =
          uncoded_data + ib_row * blk_row * cols + ib_col * blk_col;
  ...
  for (dim_t ib_col = 0; ib_col < cols / blk_col; ib_col++) {
    for (dim_t ib_row_idx = colptr[ib_col]; ib_row_idx < colptr[ib_col + 1];
         ib_row_idx++) {
...
template bsc_data_t<float> tobsc<float>(dim_t rows, dim_t cols, dim_t blk_row,
                                        dim_t blk_col,
