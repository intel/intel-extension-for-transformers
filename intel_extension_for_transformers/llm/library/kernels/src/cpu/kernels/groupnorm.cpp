...
bool groupnorm_kd_t::init() {
  ...
  param_.HW = std::accumulate(src_shape.begin() + 2, src_shape.end(), 1,
                              std::multiplies<int>());
...
size_t groupnorm_k_t::get_workspace_size() const {
  ...
  if (mode_ == parallelC)
    buf_size = 2 * channels_ * sizeof(float);
  return buf_size;
}

void groupnorm_k_t::parallelC_execute(
    const std::vector<const void *> &rt_data) const {
  float *sum_x =
      static_cast<float *>(const_cast<void *>(rt_data[idx::WORKSPACE]));
  float *sum_powx =
      static_cast<float *>(const_cast<void *>(rt_data[idx::WORKSPACE])) +
      channels_;
...
#pragma omp parallel for
    for (int channel = 0; channel < channels_; channel++) {
      ...
      data.src =
          static_cast<char *>(const_cast<void *>(rt_data[idx::SRC])) + offset;
...
#pragma omp parallel for
    for (int channel = 0; channel < channels_; channel++) {
      ...
      data.src =
          static_cast<char *>(const_cast<void *>(rt_data[idx::SRC])) + offset;
      data.dst =
          static_cast<char *>(const_cast<void *>(rt_data[idx::DST])) + offset;
      data.gamma =
          static_cast<float *>(const_cast<void *>(rt_data[idx::GAMMA])) +
          channel;
      data.beta = static_cast<float *>(const_cast<void *>(rt_data[idx::BETA])) +
                  channel;
...
void groupnorm_k_t::parallelG_execute(
    const std::vector<const void *> &rf_data) const {
  float *sum_x =
      static_cast<float *>(const_cast<void *>(rf_data[idx::WORKSPACE]));
  float *sum_powx =
      static_cast<float *>(const_cast<void *>(rf_data[idx::WORKSPACE])) +
      batchs_ * groups_;
#pragma omp parallel for collapse(2)
  for (int64_t batch = 0; batch < batchs_; batch++) {
    for (int64_t group = 0; group < groups_; group++) {
      groupnorm_data_t data;
      auto offset = (batch * channels_ + group * channels_per_group_) * HW_ *
                    dt_bytewidth_;
      data.src =
          static_cast<char *>(const_cast<void *>(rf_data[idx::SRC])) + offset;
      data.dst =
          static_cast<char *>(const_cast<void *>(rf_data[idx::DST])) + offset;
      data.sum_x_ptr = sum_x + batch * groups_ + group;
      data.sum_powx_ptr = sum_powx + batch * groups_ + group;
      data.gamma =
          static_cast<float *>(const_cast<void *>(rf_data[idx::GAMMA])) +
          group * channels_per_group_;
      data.beta = static_cast<float *>(const_cast<void *>(rf_data[idx::BETA])) +
                  group * channels_per_group_;
