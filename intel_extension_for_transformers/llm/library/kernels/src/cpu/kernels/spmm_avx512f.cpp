...
bool spmm_avx512f_kd_t::init() {
  ...
  if (wei_desc.shape().front() != src_desc.shape().back()) {
    SPARSE_LOG(WARNING) << "Skip as weight shape (" << wei_desc.shape().front()
                        << ") and source shape (" << wei_desc.shape().back()
                        << ") don't match!";
    return false;
  }
...
#pragma omp parallel for
  for (int i = 0; i < static_cast<int>(jit_kers_.size()); ++i) {
    ...
    rt_param.dst =
        const_cast<float *>(reinterpret_cast<const float *>(rt_data[ssd::DST]));
    (*jit_impl)(&(rt_param));
  }
  return true;
}
