 ...
 *   = ((src0p128_u8 - 128) / src0_scale) * (src1_s8 / src1_scale) * out_scale +
 * src2_f32 = (src0p128_u8 * src1_s8 - 128 * sum(src1_s8, axis=K)) / (src0_scale
 * * src1_scale) * out_scale + src2_f32 where src0p128_u8 = src0_s8 + 128
 */
...
bool matmul_vnni_p2031_p2013_kd_t::init() {
  ...
  if (!is_supported) {
    SPARSE_LOG(WARNING)
        << "Argument type not supported by matmul_vnni_p2031_p2013_kd_t!";
    return false;
  }

  auto &postops = op_desc_.apply_postops_list();
  if (dtypes[io::DST0] !=
      (postops.size() != 0 ? postops.back().dt : data_type::fp32)) {
  ...
  if (attrs.find("out_scale") != attrs.end())
    out_scale_ = str_to_num<float>(attrs.at("out_scale"));
  return true;
}

// Part2: class matmul_vnni_p2031_p2013_k_t

matmul_vnni_p2031_p2013_k_t::matmul_vnni_p2031_p2013_k_t(
    ...
    : kernel_t(kd), t_shapes_(kd->get_operator_desc().tensor_shapes()),
      ...
      M_pad_(pad_to(M_, 8L)), K_pad_(pad_to(K_, 8L)), N_pad_(pad_to(N_, 48L)),
...
bool matmul_vnni_p2031_p2013_k_t::init() {
  ...
  try {
    ...
    if (!add128_2x8x8_ker_->create_kernel())
      return false;

    // The kernel for preprocessing src1
    constexpr bool src1_is_unsigned = false;
    cpy_48x4_ker_ = new jit_seq_cpy_48x4(
        jit_seq_cpy_48x4::param_t{true, src1_is_unsigned, 0});
    if (!cpy_48x4_ker_->create_kernel())
      return false;

    // The kernel for compute matmul
    if (N_ >= 48) {
      matmul_ker_ =
          new jit_matmul_vnni_8xkx48_t(jit_matmul_vnni_8xkx48_t::param_t{
              K_,
              N_,
    ...
    if (N_ % 48 != 0) {
      ...
      if (!matmul_tile_n_ker_->create_kernel())
        return false;
    }
    if (!use_thread_exec_) {
      src0_tmp_ =
          aligned_allocator_t<uint8_t>::allocate(bs0_ * bs1_ * M_pad_ * K_pad_);
      src1_tmp_ =
          aligned_allocator_t<int8_t>::allocate(bs0_ * bs1_ * N_pad_ * K_pad_);
      sum_tmp_ = aligned_allocator_t<int32_t>::allocate(bs0_ * bs1_ * N_pad_ *
                                                        max_cpy_nthr);
    }
...
inline bool matmul_vnni_p2031_p2013_k_t::thread_exec(
    const std::vector<const void *> &rt_data, const dim_t ibs0,
    const dim_t ibs1) const {
  const auto curr_src0 = reinterpret_cast<const uint8_t *>(rt_data[io::SRC0]) +
                         ibs0 * M_ + ibs1 * bs0_ * M_ * K_;
  const auto curr_src1 = reinterpret_cast<const int8_t *>(rt_data[io::SRC1]) +
                         ibs0 * N_ + ibs1 * bs0_ * N_ * K_;
  const auto curr_dst_offset = (ibs0 * bs1_ + ibs1) * M_ * N_;
  const auto curr_dst = const_cast<dt_dst *>(reinterpret_cast<const dt_dst *>(
                            rt_data[io::DST0])) +
                        curr_dst_offset;
  const auto curr_src2 =
      reinterpret_cast<const float *>(rt_data[io::SRC2]) + curr_dst_offset;
  ...
  const auto curr_src1_tmp = reinterpret_cast<int8_t *>(
      reinterpret_cast<char *>(mem_tmp) + tmp0_bytes);
  const auto curr_sum_tmp = reinterpret_cast<int32_t *>(
      reinterpret_cast<char *>(mem_tmp) + tmp0_bytes + tmp1_bytes);
...
template <typename dt_dst>
inline bool matmul_vnni_p2031_p2013_k_t::execute_(
...
#pragma omp parallel for collapse(2)
    for (dim_t ibs1 = 0; ibs1 < bs1_; ++ibs1)
      for (dim_t ibs0 = 0; ibs0 < bs0_; ++ibs0)
        thread_exec<dt_dst>(rt_data, ibs0, ibs1);
    return succeed;
  }
  ...
  const auto base_dst =
      const_cast<dt_dst *>(static_cast<const dt_dst *>(rt_data[io::DST0]));
  ...
  for (dim_t ibs1 = 0; ibs1 < bs1_; ++ibs1)
    for (dim_t ibs0 = 0; ibs0 < bs0_; ++ibs0) {
      for (dim_t k = 0; k < K_; k += 8) {
        ...
        const auto curr_src0_tmp =
            src0_tmp_ + (ibs0 * bs1_ + ibs1) * M_pad_ * K_pad_;
        rt_cpy_src0.src = curr_src0 + k * bs0_ * M_;
        rt_cpy_src0.dst = curr_src0_tmp + k * 8;
        (*add128_2x8x8_ker_)(&rt_cpy_src0);
      }
  ...
  for (dim_t ibs1 = 0; ibs1 < bs1_; ++ibs1)
    for (dim_t ibs0 = 0; ibs0 < bs0_; ++ibs0) {
      for (dim_t k = 0; k < K_; k += 4) {
        ...
        const auto curr_src1_tmp =
            src1_tmp_ + (ibs0 * bs1_ + ibs1) * N_pad_ * K_pad_;
        const auto curr_sum_tmp =
            sum_tmp_ + (ibs0 * bs1_ + ibs1) * N_pad_ * max_cpy_nthr;
        rt_cpy_src1.src = curr_src1 + k * bs0_ * N_;
        rt_cpy_src1.dst = curr_src1_tmp + k * 48;
...
bool matmul_vnni_p2031_p2013_k_t::execute(
    const std::vector<const void *> &rt_data) const {
  switch (dst_type) {
  case data_type::fp32:
