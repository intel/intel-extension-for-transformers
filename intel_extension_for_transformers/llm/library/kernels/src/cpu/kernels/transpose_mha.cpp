...
bool transpose_mha_k_t::init() {
  ...
  if (impl_ == impl::vnni_b) {
    ker_seq_cpy_k_.reset(
        new jit_seq_cpy_2x8x8(jit_seq_cpy_2x8x8::param_t{128}));
    if (!ker_seq_cpy_k_->create_kernel())
      return false;
    ker_seq_cpy_q_.reset(new jit_seq_cpy_48x4(
        jit_seq_cpy_48x4::param_t{true, false, INT32_MAX}));
    if (!ker_seq_cpy_q_->create_kernel())
      return false;
    ker_kxq_.reset(
        new jit_mm_exp_vnni_mxkx48_t({7, true, data_type::bf16, 48}));
    if (!ker_kxq_->create_kernel())
      return false;
    ker_scale_trans.reset(
        new MHA_norm_quantize_reorder_vnnib_prescale_packed(48));
    if (!ker_scale_trans->create_kernel())
      return false;
    ker_vxa_.reset(new MHA_Matmul_s8u8u8_vnni_byte_8x48());
    if (!ker_vxa_->create_kernel())
      return false;
  }
  return true;
}

inline bool transpose_mha_k_t::execute_vnnib(
  ...
  const auto dst =
      reinterpret_cast<uint8_t *>(const_cast<void *>(rt_data[io::DST]));
  const auto mTmp =
      reinterpret_cast<uint8_t *>(const_cast<void *>(rt_data[io::TMP2M]));
  ...
  const auto scale_dst =
      *reinterpret_cast<const float *>(rt_data[io::SCALE_DST]);
...
#pragma omp parallel for collapse(2)
  for (int ibs = 0; ibs < batch_size; ++ibs) {
    for (int ihn = 0; ihn < head_num; ++ihn) {
      ...
      const auto curr_dst =
          dst + src_offset; // src & dst shape should be identical
      ...
      const auto expsumbuf =
          reinterpret_cast<float *>(expoutbuf + expoutbuf_size);
      const size_t expsumbuf_size = exp_nstep;
      const auto scaletrbuf = reinterpret_cast<uint8_t *>(
          expsumbuf + expsumbuf_size); // quantized & transposed exp
      const size_t scaletrbuf_size = seq_pad * seq_pad;
      const auto tmp_k = reinterpret_cast<uint8_t *>(
          scaletrbuf + scaletrbuf_size); // dst of trans_cpy_src0
      ...
      if (thread_idx == 0) {
        const auto total_size =
            reinterpret_cast<uint8_t *>(tmp_q_sum + tmp_q_sum_size) - curr_tmp;
...
bool transpose_mha_k_t::execute(
    const std::vector<const void *> &rt_data) const {
  if (impl_ == impl::vnni_b)
    return execute_vnnib(rt_data);
  ...
  MHA_kernel *MHA_step1 = k == 64
                              ? kernel_set[mha_amx_step1_k64].get()
                              : kernel_set[ker_idx::mha_amx_step1_k32].get();
  MHA_kernel *MHA_step2 = kernel_set[ker_idx::mha_amx_step2].get();
  MHA_kernel *MHA_step3 =
      seq_len % 64 == 0 ? kernel_set[ker_idx::mha_amx_step3_ktile64].get()
...
#pragma omp parallel for collapse(1)
  for (int ibat = 0; ibat < totalbatch; ibat++) {
    ...
    int thread_idx = 0;
#endif
    const auto expoutbuf =
        reinterpret_cast<bfloat16_t *>(mTmp + thread_idx * Size2M);
    ...
    const auto out0buf = reinterpret_cast<uint8_t *>(
        expsumbuf + expsumbuf_size); // buf for normalized & quantized exp
    const auto reAbuf = reinterpret_cast<int8_t *>(out0buf); // trans_copied K
    const size_t reAbuf_size = batchk * k * m;

    const auto reBbuf =
        reinterpret_cast<int8_t *>(reAbuf + reAbuf_size); // trans_copied Q
    const size_t reBbuf_size = reAbuf_size;
    if (thread_idx == 0)
      SPARSE_LOG_IF(FATAL,
                    reinterpret_cast<uint8_t *>(reBbuf + reBbuf_size) - mTmp >
                        Size2M)
          << "Buffer size too samll";
    ...
    const auto cbatchptr =
        matC + _batch * m; // mask tensor for current "batchk"

    // reorder K
    for (int i = 0; i < batchk * k; i += 8) {
      ssd::transpose_copy_params p{abatchptr + i * seq_pad, reAbuf + i, seq_pad,
                                   batchk * k, m};
      (*transpose_cpy)(&p);
    }
    ...
    for (int in = 0; in < batchk * k; in += vnni_cpy_inc_var) {
      ssd::seq_vnni_copy_params p{bbatchptr + in * seq_pad,
                                  reBbuf + in * vnni_cpy->NTile, seq_pad,
    ...
    for (int i = 0; i < batchk; i++) { // for each head in batchk of heads
      ...
      for (int in = 0; in < m; in += MHA_step2->NPacked) {
        ssd::transpose_mha_step2_params p{
            expoutptr + in * n,
      ...
      for (int im = 0; im < k; im += MHA_step3->MTile) {
        ...
        for (int in = 0; in < n; in += MHA_step3->NTile) {
          ...
          ssd::transpose_mha_step3_params p{
              aptr,     bptr,
              cptr,     reinterpret_cast<uint8_t *>(&(MHA_step3->tc)),
              m,        seq_pad,
              seq_pad,  scaleV * sotmax_scale,
