...
float time(const std::string &state) {
  ...
  if (state == "start") {
    ...
    return (micros.count() - time_axis.count()) /
           1e3; // Displayed in milliseconds.
  }
  return -1;
}
...
template <>
std::vector<std::string> split_str<std::string>(const std::string &s,
                                                const char &delim) {
  std::stringstream ss(s);
  std::string temp;
...
template std::vector<int64_t> split_str<int64_t>(const std::string &,
                                                 const char &);
...
float get_gelu(float x) {
  ...
  return 0.5 * x *
         (1 +
          tanhf(0.7978845834732056 * (x + 0.044714998453855515 * x * x * x)));
}
...
float get_swish(float x, float alpha) {
  return x / (1.f + get_exp(-1 * alpha * x));
}
  ...
  for (auto &&i : attrs) {
    if (i.op_type == postop_type::eltwise) {
      ...
      if (i.op_alg == postop_alg::gelu)
        value = get_gelu(value);
      if (i.op_alg == postop_alg::relu)
        value = get_relu(value, i.alpha);
      if (i.op_alg == postop_alg::quantize)
        value = get_quantize(value, i.alpha, i.scale, i.dt);
      if (i.op_alg == postop_alg::dequantize)
        value = get_dequantize(value, i.alpha, i.scale);
      if (i.op_alg == postop_alg::tanh)
        value = tanh(value);
      if (i.op_alg == postop_alg::linear)
        value = get_linear(value, i.alpha, i.beta);
      if (i.op_alg == postop_alg::swish)
        value = get_swish(value, i.alpha);
...
bool set_once_before_first_get_setting_t<T>::set(T new_value) {
  if (state_.load() == locked)
    return false;

  while (true) {
    unsigned expected = idle;
    if (state_.compare_exchange_weak(expected, busy_setting))
      break;
    if (expected == locked)
      return false;
  }
...
template void cast_to_float_array<float>(const void *, std::vector<float> *,
                                         int);
template void cast_to_float_array<int>(const void *, std::vector<float> *, int);
template void cast_to_float_array<int8_t>(const void *, std::vector<float> *,
                                          int);
template void cast_to_float_array<uint8_t>(const void *, std::vector<float> *,
                                           int);
template void cast_to_float_array<bfloat16_t>(const void *,
...
template void cast_from_float_array<float>(const std::vector<float> &, void *,
                                           int);
template void cast_from_float_array<int>(const std::vector<float> &, void *,
                                         int);
template void cast_from_float_array<int8_t>(const std::vector<float> &, void *,
                                            int);
template void cast_from_float_array<uint8_t>(const std::vector<float> &, void *,
                                             int);
template void cast_from_float_array<bfloat16_t>(const std::vector<float> &,
                                                void *, int);
