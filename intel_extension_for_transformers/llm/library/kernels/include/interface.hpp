 ...
 */
template <typename T, typename arg_t = void> class SPARSE_API_ proxy_base {
public:
  proxy_base() {}
  virtual ~proxy_base() {}
...
protected:
  ...
  virtual bool create_proxy_object(std::shared_ptr<const T> &result_ref,
                                   const arg_t &arg) = 0; // NOLINT
 ...
 */
class SPARSE_API_ kernel_desc_proxy
    : public proxy_base<kernel_desc_t, operator_desc> {
public:
  kernel_desc_proxy() {}
...
protected:
  bool create_proxy_object(std::shared_ptr<const kernel_desc_t> &result_ref,
                           const operator_desc &op_desc) override;

public:
  inline const jd::kernel_kind &kernel_kind() const {
    return get_sp()->kernel_kind();
  }
 ...
 */
class SPARSE_API_ kernel_proxy
    : public proxy_base<kernel_t, std::shared_ptr<const kernel_desc_t>> {
public:
  kernel_proxy() {}
...
protected:
  bool
  create_proxy_object(std::shared_ptr<const kernel_t> &result_ref,
                      const std::shared_ptr<const kernel_desc_t> &kd) override;

public:
  inline const jd::kernel_kind &kernel_kind() const {
    return get_sp()->kd()->kernel_kind();
  }
  void execute(const std::vector<const void *> &rt_data)
      const; // TODO(Jiwei): depredate it when most kernels ready
  void execute(const exec_context_t &ctx) const;
  size_t get_workspace_size() const;
};

//// The following paragraphs are the various derived kernels and its
///descriptors.
/**
 * @brief Derived proxy class, interfacing to the real/cached
 * sparse_matmul_desc_t.
 */
class SPARSE_API_ sparse_matmul_desc : public kernel_desc_proxy {
public:
  sparse_matmul_desc() {}
  explicit sparse_matmul_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  ...
  explicit transpose_matmul_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  ...
  explicit dynamic_quant_matmul_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  dynamic_quant_desc() {}
  explicit dynamic_quant_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  eltwiseop_desc() {}
  explicit eltwiseop_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  groupnorm_desc() {}
  explicit groupnorm_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  layernorm_ba_desc() {}
  explicit layernorm_ba_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  ...
  explicit layernormalized_spmm_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  gather_desc() {}
  explicit gather_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
  virtual ~gather_desc() {}
};
...
public:
  softmax_desc() {}
  explicit softmax_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  logsoftmax_desc() {}
  explicit logsoftmax_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  attention_desc() {}
  explicit attention_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  transpose_mha_desc() {}
  explicit transpose_mha_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  mha_dense_desc() {}
  explicit mha_dense_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
...
public:
  slice_desc() {}
  explicit slice_desc(const operator_desc &op_desc)
      : kernel_desc_proxy(op_desc) {}
  virtual ~slice_desc() {}
};
...
public:
  ...
  explicit dynamic_quant_matmul(const kernel_desc_proxy &kdp)
      : kernel_proxy(kdp) {}
...
public:
  ...
  explicit layernormalized_spmm(const kernel_desc_proxy &kdp)
      : kernel_proxy(kdp) {}
