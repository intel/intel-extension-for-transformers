 ...
 * compress/uncompress.
 */
template <typename T> class sparse_data_t {
public:
  sparse_data_t() {}
  sparse_data_t(const std::vector<int64_t> &indptr,
                const std::vector<int64_t> &indices, const std::vector<T> &data)
...
public:
   ...
   * groups (for grouped data format) of the matrix.
   */
  virtual uint64_t getnnz(int idx = -1) const {
    if (indptr_.empty()) {
      return 0;
    }
    return (idx == -1) ? (indptr_.back() - indptr_[0])
                       : (indptr_[idx + 1] - indptr_[idx]);
  }
...
template <typename T> class csr_data_t : public sparse_data_t<T> {
public:
  explicit csr_data_t(const format_type &encode_fmt = format_type::csr)
      : sparse_data_t<T>(), encode_fmt_(encode_fmt) {}
  explicit csr_data_t(const sparse_data_t<T> &spdata,
                      const format_type &encode_fmt = format_type::csr)
...
template <typename T> class csrp_data_t : public csr_data_t<T> {
public:
  explicit csrp_data_t(const format_type &encode_fmt = format_type::csrp)
      : csr_data_t<T>(encode_fmt) {}
  csrp_data_t(const sparse_data_t<T> &spdata,
              const format_type &encode_fmt = format_type::csrp,
              const std::vector<int64_t> &iperm = {},
              const std::vector<int64_t> &xgroup = {})
...
protected:
  ...
  std::vector<int64_t>
      xgroup_; // xgroup points to beginning indices of groups in iperm.
};

template <typename T> class bsr_data_t : public sparse_data_t<T> {
public:
  explicit bsr_data_t(const std::vector<dim_t> block_size,
                      const std::vector<dim_t> shape,
                      const std::vector<dim_t> &indptr,
                      const std::vector<dim_t> &indices,
                      const std::vector<T> &data, const dim_t group = 1)
      : sparse_data_t<T>(indptr, indices, data), shape_(shape), group_(group),
...
template <typename T> class bsc_data_t : public sparse_data_t<T> {
public:
  explicit bsc_data_t(const std::vector<dim_t> block_size,
                      const std::vector<dim_t> shape,
                      const std::vector<dim_t> &indptr,
                      const std::vector<dim_t> &indices,
                      const std::vector<T> &data)
      : sparse_data_t<T>(indptr, indices, data), shape_(shape),
        block_size_(block_size) {}
  virtual ~bsc_data_t() {}
...
namespace spns {
static constexpr int ADJ =
    4; // 4 is that "Multiply groups of 4 adjacent pairs..."(vpdpbusd).

inline int align_nnz(const int &a_nnz);

template <typename T, dim_t group>
SPARSE_API_ std::vector<bsr_data_t<T> *> *
reorder_to_bsr_amx(dim_t rows, dim_t cols, dim_t micro_rows,
                   const void *uncoded_ptr);
#ifdef _WIN32
template SPARSE_API_ std::vector<bsr_data_t<bfloat16_t> *> *
reorder_to_bsr_amx<bfloat16_t, 32>(dim_t rows, dim_t cols, dim_t micro_rows,
                                   const void *uncoded_ptr);
#endif

template <typename T>
bsr_data_t<T> SPARSE_API_ tobsr(dim_t rows, dim_t cols, dim_t blk_row,
                                dim_t blk_col, const T *uncoded_data);

#ifdef _WIN32
template bsr_data_t<signed char>
    SPARSE_API_ tobsr<signed char>(dim_t rows, dim_t cols, dim_t blk_row,
                                   dim_t blk_col,
                                   const signed char *uncoded_data);

template bsr_data_t<float> SPARSE_API_ tobsr<float>(dim_t rows, dim_t cols,
                                                    dim_t blk_row,
                                                    dim_t blk_col,
...
/**
 * @brief Reorder a dense matrix to BSR encoded sparse format, grouping a
 * certain number of blocks in a row and reorder each groups in row-major,
 * adding padding if no enough blocks left in a row. Note that colidxs will be
 * padded with last value of the same row, while data will be padded with 0.
 *
 ...
 */
template <typename T, dim_t group>
bsr_data_t<T> reorder_to_bsr_group(dim_t rows, dim_t cols, dim_t blk_row,
                                   dim_t blk_col, const T *uncoded_data);
#ifdef _WIN32
template bsr_data_t<signed char> SPARSE_API_
reorder_to_bsr_group<signed char, 4>(dim_t rows, dim_t cols, dim_t blk_row,
                                     dim_t blk_col,
                                     const signed char *uncoded_data);
#endif

template <typename T>
bsc_data_t<T> SPARSE_API_ tobsc(dim_t rows, dim_t cols, dim_t blk_row,
                                dim_t blk_col, const T *uncoded_data);

#ifdef _WIN32
template bsc_data_t<float> SPARSE_API_ tobsc<float>(dim_t rows, dim_t cols,
                                                    dim_t blk_row,
                                                    dim_t blk_col,
                                                    const float *uncoded_data);
#endif
