...
public:
  operator_desc()
      : ker_kind_(kernel_kind::undef), ker_prop_(kernel_prop::undef),
        engine_kind_(engine_kind::undef), runtime_kind_(runtime_kind::undef),
        impl_nthr_(0), ts_descs_({}), attrs_({}), apply_postops_list_({}) {}
  operator_desc(const kernel_kind &ker_kind, const kernel_prop &ker_prop,
                const engine_kind &eng_kind,
                const std::vector<tensor_desc> &ts_descs,
                const std::unordered_map<std::string, std::string> &attrs,
                const std::vector<postop_attr> &apply_postops_list = {})
      : ker_kind_(ker_kind), ker_prop_(ker_prop), engine_kind_(eng_kind),
        runtime_kind_(runtime_kind::undef), impl_nthr_(omp_get_max_threads()),
        ts_descs_(ts_descs), attrs_(attrs),
        apply_postops_list_(apply_postops_list) {}
  operator_desc(const kernel_kind &ker_kind, const kernel_prop &ker_prop,
                const engine_kind &eng_kind, const runtime_kind &runtime_kind,
                const std::vector<tensor_desc> &ts_descs,
                const std::unordered_map<std::string, std::string> &attrs,
                const std::vector<postop_attr> &apply_postops_list = {})
      : ker_kind_(ker_kind), ker_prop_(ker_prop), engine_kind_(eng_kind),
        runtime_kind_(runtime_kind), impl_nthr_(omp_get_max_threads()),
        ts_descs_(ts_descs), attrs_(attrs),
        apply_postops_list_(apply_postops_list) {}
...
public:
  bool operator==(const operator_desc &rhs) const {
    return (ker_kind_ == rhs.ker_kind_) && (ker_prop_ == rhs.ker_prop_) &&
           ...
           (impl_nthr_ == rhs.impl_nthr_) && (ts_descs_ == rhs.ts_descs_) &&
           (attrs_ == rhs.attrs_);
  }

  void set_binaryop_list(const std::vector<binaryop_attr> &binaryop_list) {
    this->binaryop_list_ = binaryop_list;
  }
