...
private:
  ...
  static void hash_combine(size_t &seed, const T &v) { // NOLINT
    seed ^= static_cast<size_t>(std::hash<T>()(v)) +
            static_cast<size_t>(0x9e3779b9) + (seed << 6) + (seed >> 2);
  }

private:
  uint64_t
  get_tensor_descs_hash(const std::vector<tensor_desc> &ts_descs) const {
    uint64_t seed = 0;
...
#pragma GCC diagnostic ignored "-Wimplicit-fallthrough"
  uint64_t
  get_attr_hash(const std::unordered_map<std::string, std::string> &attrs,
                const kernel_kind &ker_kind) const {
    auto op_attrs = attrs;
    uint64_t seed = 0;
    // if front op want to apply postop-fusion,they should add a filed named
    // postop_list in op_attr for distinguishing.
