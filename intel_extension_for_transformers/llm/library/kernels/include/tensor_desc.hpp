...
public:
  tensor_desc()
      : shape_({}), dtype_(data_type::undef), ftype_(format_type::undef) {}
  tensor_desc(const std::vector<int64_t> &shape, const data_type &dtype,
              const format_type &ftype);
  virtual ~tensor_desc() {}

public:
  bool operator==(const tensor_desc &rhs) const {
    return (shape_ == rhs.shape_) && (dtype_ == rhs.dtype_) &&
           (ftype_ == rhs.ftype_);
  }
...
public:
  ...
  inline int64_t size() const {
    return shape_.size() == 0
               ? 0
               : std::accumulate(shape_.cbegin(), shape_.cend(), int64_t{1},
                                 std::multiplies<int64_t>());
  }

  inline int64_t reduce_rows() const {
    return std::accumulate(shape_.begin(), shape_.end() - 1, int64_t{1},
                           std::multiplies<int64_t>());
  }
