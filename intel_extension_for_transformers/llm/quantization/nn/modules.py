    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.bias is not None:
            torch.ops.weight_only_jblasop.qbits_f32in_f32out_linear_with_bias(
                x, self.weight.data, self.bias.data, out, 
                self.in_features, self.out_features, self.compute_dtype, self.weight_dtype
            )
        else:
            torch.ops.weight_only_jblasop.qbits_f32in_f32out_linear_without_bias(
                x, self.weight.data, out, self.out_features,
                self.in_features, self.out_features, self.compute_dtype, self.weight_dtype
            )
...
class QuantizedLinearINT4(QuantizedLinearQBits, LoraLayer):
    ...
    def set_weights_bias(self, weight_data, bias=None):
        self.weight = ParamsQBits(
            weight_data, requires_grad=False, compress_statistics=self.compress_statistics, quant_dtype=self.weight_dtype
        )
