...
class QuantizedLinearGPU(torch.nn.Linear):
    ...
    def forward(self, x: torch.Tensor):
        ...
        gbits.linear(
            ...
            self.out_features, self.bias is not None, self.compute_dtype, self.weight_dtype, self.blocksize)
    ...
    def set_weights_bias(self, weight_data, bias=None):
        ...
        if weight_data.is_meta:
            weight = torch.ones(shape, dtype=torch.int8).to(self.device)
        else:
...
#         lora_dropout=0.01,
#     ):
#         QuantizedLinearGPU.__init__(self, input_features, output_features, bias, compute_dtype, compress_statistics,
...
# class QuantizedLinearINT8(QuantizedLinearGPU):
#     def __init__(
#         self,
#         input_features,
#         output_features,
