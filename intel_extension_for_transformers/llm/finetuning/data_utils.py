    ...
    def tokenize_func(self, tokenizer, data_args, finetune_args):
        ...
        def preprocess_function(examples):
            ...
            for instruction, response in zip(instructions, responses):
                header = re.findall(
                    r"### System.*?{}".format(self.end), instruction, re.DOTALL
                )[0]
                convs = re.findall(
                    r"### User.*?{0}|### Assistant.*?{0}".format(self.end),
                    instruction,
                    re.DOTALL,
                )
                convs_tokens = [
                ...
                if len(truncated_convs) == 0:
                    truncated_convs = [
                        convs_tokens[-1][: max_input - 3] + convs_tokens[-1][-3:]
                    ]

                prompt_tokens = [header_tokens] + truncated_convs + [assistant_tokens]
                prompt_ids = [
                    tokenizer.convert_tokens_to_ids(prompt_token)
                    for prompt_token in prompt_tokens
                ]
                prompt_ids = list(chain(*prompt_ids))

                resp_ids = tokenizer.convert_tokens_to_ids(
                    tokenizer.tokenize(response.strip())
                )
...
class SummarizationDataPreprocess:
    ...
    def tokenize_func(self, tokenizer, data_args, finetune_args):
        template_ids = tokenizer.convert_tokens_to_ids(
            tokenizer.tokenize(self.prompt_template)
        )
        ...
        def preprocess_function(examples):
            ...
            for article, highlight in zip(articles, highlights):
                ...
                prompt_ids = (
                    tokenizer.convert_tokens_to_ids(article_tokens) + template_ids
                )
                ...
                max_resp = data_args.max_seq_length - len(prompt_ids) - 1
                resp_ids = tokenizer.convert_tokens_to_ids(
                    tokenizer.tokenize(highlight)
                )[:max_resp] + [tokenizer.eos_token_id]

                # for inference
                max_decoder_labels_len = (
                    data_args.max_seq_length - data_args.max_source_length - 1
                )
                decoder_labels = tokenizer.convert_tokens_to_ids(
                    tokenizer.tokenize(highlight)
                ...
                decoder_input_ids = [
                    tokenizer.eos_token_id
                ] * pad_len + decoder_input_ids
                decoder_attention_mask = [0] * pad_len + [1] * input_len

                input_len = len(decoder_labels)
                pad_len = (
                    data_args.max_seq_length - data_args.max_source_length - input_len
                )
...
def preprocess_dataset(raw_datasets, tokenizer, data_args, finetune_args):
    if data_args.dataset_name == "Intel/orca_dpo_pairs":
        ...
        raise NotImplementedError(
            "finetune task data preprocessing is not support currently."
        )
