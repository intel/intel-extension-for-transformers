...
void xetla_linear(sycl::queue queue, T *A, CompressWei4Bit *B, T *C,
  ...
  data_type_acc *D_d = static_cast<data_type_acc *>(aligned_alloc_device(
      DEVICE_MEM_ALIGNMENT, size_d * sizeof(data_type_acc), device, context));
  if (with_bias) {
  ...
  using bias_op_t =
      gpu::xetla::subgroup::bias_add_op_t<float, gpu::xetla::gpu_arch::Xe>;
  ;
  using tile_op_t =
      gpu::xetla::subgroup::chained_tile_op_t<bias_op_t // apply elementwise
                                                        // BiasAdd
                                              >;
  using epilogue_t = gpu::xetla::group::epilogue_t<
      gpu::xetla::group::epilogue_policy_unaligned<tile_op_t,
                                                   gpu::xetla::gpu_arch::Xe>,
  ...
  typename gemm_op_t::arguments_t gemm_arg(
      matrix_m, matrix_k, matrix_n, A_d, matrix_k, B_d, matrix_n, C_d, matrix_n,
...
void xetla_linear_fp16(sycl::queue queue, fp16 *A, CompressWei4Bit *B, fp16 *C,
  ...
  return xetla_linear(queue, A, B, C, matrix_m, matrix_n, matrix_k, with_bias,
                      bias);
}
