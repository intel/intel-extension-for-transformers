...
void compress_s8_s4(const int8_t* srcptr, gblas::int4x2* dstptr, int row, int col, int ld_src, int ld_dst) {
  for (int j = 0; j < row; j++) {
...
torch::Tensor quantize(float* weight, int k, int n, int blksize, bool transpose, std::string weight_type,
  ...
  if (weight_type == "s4fullrange_scalef32") {
    ...
    gblas::int4x2* wei = reinterpret_cast<gblas::int4x2*>(compress_wei.get_4bit_wei_ptr());
...
void gpu_dequant_s4fullrange_f32_KxN(queue& q, buffer<int8_t, 2>& src, buffer<DST_T, 2>& dst, buffer<float, 1>& scale,
...
void dequant_dispatch(queue& q, CompressWei4Bit* compress_wei, torch::Tensor& dequant_weight, bool transpose,
                 const std::string& compute_type, const std::string& weight_type) {
  if (compute_type == "fp32")  {
      gpu_dequant<float>(q, compress_wei, dequant_weight.data_ptr<float>(), transpose, compute_type, weight_type);
  }
  //else if (compute_type == "bf16")  {
  //    gpu_dequant<__bf16>(q, compress_wei, dequant_weight.data_ptr<__bf16>(), transpose, compute_type, weight_type);
  //}
  else {
      gpu_dequant<__fp16>(q, compress_wei, dequant_weight.data_ptr<__fp16>(), transpose, compute_type, weight_type);
  }
}

template<typename T1, typename T2>
void gpu_linear(queue& queue, const torch::Tensor& activation, const T1* weight, const torch::Tensor& bias,
...
void linear_dispatch(queue& q, const torch::Tensor& activation, const torch::Tensor weight, const torch::Tensor& bias,
                         torch::Tensor& output, int64_t ldo, bool with_bias, const std::string& compute_type,
                         const std::string& weight_type) {
  if (compute_type == "fp32")  {
    gpu_linear<float, gpu::xetla::tf32>(q, activation, weight.data_ptr<float>(), bias, output, ldo, with_bias, compute_type, weight_type);
  }
  else {
    gpu_linear<__fp16, sycl::half>(q, activation, weight.data_ptr<__fp16>(), bias, output, ldo, with_bias, compute_type, weight_type);
  }
}