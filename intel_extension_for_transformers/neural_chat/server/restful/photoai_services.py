...
def check_user_ip(user_ip: str) -> bool:
    ...
    mysql_db = MysqlDb()
    user_list = mysql_db.fetch_one(
        sql=f'select * from user_info where user_id = "{user_ip}";'
    )
    logger.info(f"[Check IP] user list: {user_list}")
    if user_list == None:
        logger.info("[Check IP] no current user, add into db.")
        cur_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with mysql_db.transaction():
            mysql_db.insert(
                sql=f"insert into user_info values('{user_ip}', '{cur_time}', null, 1);",
                params=None,
            )
    mysql_db._close()
    return True
...
def delete_single_image(user_id, image_id):
    ...
    image_path = mysql_db.fetch_one(
        sql=f"SELECT image_path FROM image_info WHERE image_id={image_id}", params=None
    )
    if image_path == None:
...
def process_images_in_background(user_id: str, image_obj_list: List[Dict]):
    try:
        logger.info(
            f"[background] ======= processing image list for user {user_id} in background ======="
        )
        for i in range(len(image_obj_list)):
            ...
            try:
                ...
                logger.error("[background] error occurred, delete image.")
...
def process_single_image(img_id, img_path, user_id):
    logger.info(
        f"[background - single] ----- processing image {img_path} in background -----"
    )
    ...
    logger.info(
        f"[background - single] Image is captured at: {captured_time},"
        + "latitude: {latitude}, longitude: {longitude}, altitude: {altitude}"
    )
    if latitude:
        update_image_attr(
            image={"image_id": img_id, "latitude": latitude}, attr="latitude"
        )
    if longitude:
        update_image_attr(
            image={"image_id": img_id, "longitude": longitude}, attr="longitude"
        )
    if altitude:
        update_image_attr(
            image={"image_id": img_id, "altitude": altitude}, attr="altitude"
        )
    if captured_time:
        update_image_attr(
        ...
        update_image_attr(
            image={"image_id": img_id, "captured_time": beijing_time},
            attr="captured_time",
        )
    ...
    if address:
        ...
        update_image_attr(
            image={"image_id": img_id, "address": formatted_address}, attr="address"
        )
    else:
        address = None
        logger.info("[background - single] Can not get address from image.")
    ...
    if result_caption:
        ...
        update_image_attr(
            image={"image_id": img_id, "caption": result_caption}, attr="caption"
        )
    else:
        logger.info("[background - single] Can not generate caption for image.")
    ...
    try:
        process_face_for_single_image(
            image_id=img_id, image_path=img_path, db_path=db_path, user_id=user_id
        )
    except Exception as e:
        logger.error("[background - single] Error occurred while processing face.")
    ...
    try:
        ...
        with mysql_db.transaction():
            mysql_db.update(
                sql=f"UPDATE image_info SET process_status='ready' WHERE image_id={img_id}",
                params=None,
            )
    except Exception as e:
        logger.error("[background - single] " + str(e))
...
def process_face_for_single_image(image_id, image_path, db_path, user_id):
    logger.info(
        f"[background - face] ### processing face for {image_path} in background ###"
    )
    ...
    logger.info(
        f"[background - face] face xywh list of image {image_id} is: {face_xywh_list}"
    )
    ...
    if os.path.exists(pkl_path):
        logger.info("[background - face] pkl file already exists, delete it.")
        os.remove(pkl_path)
    dfs = DeepFace.find(
        img_path=image_path,
        db_path=db_path,
    ...
    logger.info("[background - face] Finding match faces in image database.")
    ...
    for df in dfs:
        ...
        if len(df) <= 1:
            logger.info(
                "[background - face] length of current df less than 1, continue"
            )
            continue
        # findd ref image
        ref_image_path = None
        ...
        if not ref_image_path:
            logger.info("[background - face] no other reference image found, continue")
            continue
        ...
        try:
            ...
            raise Exception(
                f"Exception occurred while selecting info from image_face: {e}"
            )
        ...
        obj = DeepFace.verify(
            img1_path=image_path, img2_path=ref_image_path, model_name="Facenet512"
        )
        ...
        if face_id == -1 and face_tag == None:
            raise Exception(
                "Error occurred when verifying faces for reference image: Inconsistent face information."
            )
        ...
        try:
            ...
            logger.error("[background - face] " + str(e))
            raise Exception(
                f"Exception occurred while inserting info into image_face: {e}"
            )
        # current face matched and saved into db, delete from face_xywh_list
        logger.info(
            f"[background - face] image_face data inserted: {insert_img_face_sql}"
        )
    ...
    for cur_xywh in face_xywh_list:
        face_cnt = mysql_db.fetch_one(sql="SELECT COUNT(*) AS cnt FROM face_info;")[
            "cnt"
        ]
        ...
        try:
            ...
            logger.error("[background - face] " + str(e))
            raise Exception(
                f"Exception occurred while inserting new face into face_info: {e}"
            )
        logger.info(f"[background - face] face {tag} inserted into db.")
        face_id = mysql_db.fetch_one(
            f"SELECT * FROM face_info WHERE face_tag='{tag}';"
        )["face_id"]
        ...
        try:
            ...
            logger.error("[background - face] " + str(e))
            raise Exception(
                f"Exception occurred while inserting new face into image_face: {e}"
            )
...
def get_type_obj_from_attr(attr, user_id):
    ...
    if simplify_flag:
        logger.info("address need to be simplified")
        new_result = {}
...
def get_process_status(user_id):
    ...
    total_cnt = mysql_db.fetch_one(
        ...
        user_id='{user_id}' AND exist_status='active';"""
    )["cnt"]
    processing_cnt = mysql_db.fetch_one(
        sql=f"""SELECT COUNT(*) AS cnt FROM image_info WHERE
        user_id='{user_id}' AND exist_status='active' AND process_status='processing';"""
    )["cnt"]
    mysql_db._close()
    result = {}
...
def get_image_list_by_ner_query(
    ner_result: Dict, user_id: str, query: str
) -> List[Dict]:
    logger.info("[NER query] start query from ner results")
    ...
    if face_list:
        ...
        if sql_conditions != []:
            ...
            logger.info("[NER query] no person name in ner query")
    else:
        logger.info("[NER query] no person name in ner query")
    ...
    if location_list:
        ...
        logger.info("[NER query] no location in query")
    ...
    if ner_result["time"]:
        ...
        logger.info("[NER query] no time in query")
    ...
    if ner_result["period"]:
        ...
        logger.info("[NER query] no time period in query")

    if not query_flag:
        logger.info("[NER query] no compatible data for current query")
        return []
    query_sql += (
        f' AND ( image_info.user_id="{user_id}" ) AND ( exist_status="active" ) ;'
    )
...
def delete_user_infos(user_id: str):
    logger.info("[delete user] start delete user info")
    ...
    try:
        ...
        with mysql_db.transaction():
            ...
            mysql_db.delete(
                sql=f"""DELETE FROM image_face WHERE user_id='{user_id}'""", params=None
            )

            # delete face_info
            logger.info(f"[delete user] delete face_info of user {user_id}.")
            mysql_db.delete(
                sql="""DELETE face_info FROM face_info LEFT JOIN image_face
            ...
            mysql_db.delete(
                sql=f"DELETE FROM image_info WHERE user_id='{user_id}'", params=None
            )

            # delete user_info
            logger.info(f"[delete user] delete user_info of user {user_id}.")
            mysql_db.delete(
                sql=f"DELETE FROM user_info WHERE user_id='{user_id}'", params=None
            )
    except Exception as e:
        raise Exception(e)
    finally:
