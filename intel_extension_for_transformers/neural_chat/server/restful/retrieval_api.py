...
from ...server.restful.request import FeedbackRequest, RetrievalRequest
...
def check_retrieval_params(request: RetrievalRequest) -> Optional[str]:
    if request.params is not None and (not isinstance(request.params, Dict)):
        return (
            f"Param Error: request.params {request.params} is not in the type of Dict"
        )
...
class RetrievalAPIRouter(APIRouter):
    ...
    def set_chatbot(
        self, bot, use_deepspeed=False, world_size=1, host="0.0.0.0", port="80"
    ) -> None:
        self.chatbot = bot
...
RETRIEVAL_FILE_PATH = (
    os.getenv("RETRIEVAL_FILE_PATH", default="./photoai_retrieval_docs") + "/"
)
...
async def retrieval_upload_link(request: Request):
    ...
    if "knowledge_base_id" in params.keys():
        print("[askdoc - upload_link] append")
        knowledge_base_id = params["knowledge_base_id"]
        persist_path = (
            RETRIEVAL_FILE_PATH + user_id + "-" + knowledge_base_id + "/persist_dir"
        )
        ...
        try:
            ...
            instance.append_localdb(
                append_path=link_list, persist_directory=persist_path
            )
            print("[askdoc - upload_link] kb appended successfully")
        except Exception as e:  # pragma: no cover
            logger.info(f"[askdoc - upload_link] create knowledge base fails! {e}")
            return Response(
                content="Error occurred while uploading links.", status_code=500
            )
        return {"Succeed"}
    # create new kb with link
    else:
        print("[askdoc - upload_link] create")
        import uuid
        ...
        try:
            ...
            instance = plugins["retrieval"]["instance"]
            instance.create(
                input_path=link_list, persist_directory=str(user_persist_dir)
            )
            logger.info("[askdoc - upload_link] kb created successfully")
        except Exception as e:  # pragma: no cover
            logger.info(f"[askdoc - upload_link] create knowledge base fails! {e}")
...
async def retrieval_create(request: Request, file: UploadFile = File(...)):
    global plugins
    ...
    try:
        ...
        instance = plugins["retrieval"]["instance"]
        instance.create(
            input_path=str(user_upload_dir), persist_directory=str(user_persist_dir)
        )
        logger.info("[askdoc - create] kb created successfully")
    except Exception as e:  # pragma: no cover
...
async def retrieval_append(
    request: Request, file: UploadFile = File(...), knowledge_base_id: str = Form(...)
):
    global plugins
    filename = file.filename
    if "/" in filename:
        filename = filename.split("/")[-1]
    logger.info(
        f"[askdoc - append] received file: {filename}, kb_id: {knowledge_base_id}"
    )
    ...
    try:
        ...
        instance = plugins["retrieval"]["instance"]
        instance.append_localdb(
            append_path=save_file_name, persist_directory=persist_path
        )
        logger.info("[askdoc - append] new file successfully appended to kb")
    except Exception as e:  # pragma: no cover
...
async def retrieval_chat(request: Request):
    ...
    logger.info(
        f"[askdoc - chat] kb_id: '{kb_id}', query: '{query}', \
    ...
    if not stream:
        response = chatbot.predict(
            query=query, origin_query=origin_query, config=config
        )
        ...
        def stream_generator():
            for output in generator:
                ...
                if res != None:
                    ...
                    formatted_str = formatted_str.replace("**:", "</b>:").replace(
                        "**", "<b>"
                    )
                    logger.info(f"[askdoc - chat] formatted: {formatted_str}")
                    yield f"data: {formatted_str}\n\n"
            if return_link and link != []:
                yield "data: <hr style='border: 1px solid white; margin:0.5rem 0; '>\n\n"
                for single_link in link:
                    ...
                    if isinstance(single_link, str):
                        ...
                        logger.info("[askdoc - chat] wrong link format")
                        continue
...
def save_chat_feedback_to_db(request: FeedbackRequest) -> None:
    logger.info("[askdoc - feedback] fastrag feedback received.")
    mysql_db = MysqlDb()
    mysql_db._set_db("fastrag")
    question, answer, feedback, comments = (
        request.question,
        request.answer,
        request.feedback,
        request.comments,
    ...
    logger.info(
        f"""[askdoc - feedback] feedback question: [{question}],
    ...
    try:
        ...
        raise Exception(
            """Exception occurred when inserting data into MySQL,
...
def get_feedback_from_db():
    ...
    try:
        ...
        raise Exception(
            """Exception occurred when querying data from MySQL, \
                        please check the db session and your syntax."""
        )
    else:
        mysql_db._close()
        ...
        def data_generator():
            ...
            for row in feedback_list:
                ...
                row["feedback_result"] = (
                    "like" if (row["feedback_result"] == 0) else "dislike"
                )
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment;filename=feedback{cur_time_str}.csv"
            },
        )
