...
from intel_extension_for_transformers.neural_chat.pipeline.plugins.image2image.instructpix2pix_pipeline import \
    StableDiffusionInstructPix2PixPipeline

pipe_img2img = StableDiffusionInstructPix2PixPipeline.from_pretrained(
    "timbrooks/instruct-pix2pix", torch_dtype=torch.float32, use_auth_token=True
)

neural_engine_graph = diffusion_utils.neural_engine_init("./bf16_ir")
...
@route('/v1/image2image', method=['POST'])
def do_inference():
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                image = pipe_img2img(prompt=task.prompt, image=init_image, engine_graph=neural_engine_graph,
...
if __name__ == "__main__":
    ...
    worker.start()
    run(host='0.0.0.0', port=8888, debug=True, server='gevent')
