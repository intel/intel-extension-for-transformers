...
def latitude_and_longitude_convert_to_decimal_system(*arg):
    ...
    return float(arg[0]) + (
        (
            float(arg[1])
            + (float(arg[2].split("/")[0]) / float(arg[2].split("/")[-1]) / 60)
        )
        / 60
    )
        ...
        for tag, value in tags.items():
            if re.match("GPS GPSLatitudeRef", tag):
                ...
                try:
                    ...
                    deg, min, sec = [
                        x.replace(" ", "") for x in str(value)[1:-1].split(",")
                    ]
                    GPS[
                        "GPSLatitude"
                    ] = latitude_and_longitude_convert_to_decimal_system(deg, min, sec)
        ...
        for component in address_components:
            if "country" in component["types"]:
                ...
                result["administrative_area_level_1"] = (
                    component["long_name"].replace(" ", "").capitalize()
                )
