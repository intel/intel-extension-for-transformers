...
from .photoai_utils import byte64_to_image, generate_random_name, image_to_byte64
from .voicechat_api import create_speaker_embedding as talkingbot_embd
from .voicechat_api import handle_talkingbot_asr as talkingbot_asr
...
class PhotoAIAPIRouter(APIRouter):
    ...
    async def handle_voice_chat_request(
        self, prompt: str, audio_output_path: Optional[str] = None
    ) -> str:
...
async def handle_ai_photos_upload_images(
    ...
    for image in image_list:
        ...
        try:
            ...
            return JSONResponse(
                content=f"Database insert failed for image {img_path}", status_code=500
            )
        ...
        try:
            ...
            return JSONResponse(
                content=f"Database select failed for image {img_path}", status_code=500
            )
        img_id = result["image_id"]
        frontend_path = format_image_path(user_id, img_name)
        item = {"img_id": img_id, "img_path": frontend_path}
        logger.info(
            f"<uploadImages> Image id is {img_id}, image path is {frontend_path}"
        )
...
async def handle_ai_photos_get_image_detail(request: Request):
    ...
    logger.info(
        f"<getImageDetail> Getting image detail of image {image_id} by user {user_id}"
    )
    ...
    try:
        ...
        return JSONResponse(
            content=f"Exception {e} occurred when selecting image {image_id} from MySQL."
        )
    finally:
        mysql_db._close()
    ...
    if image_info:
        ...
        logger.info(
            f"<getImageDetail> Image detail of image {image_id} is: {image_detail}"
        )
        return image_detail
    else:
        return JSONResponse(
...
async def handle_ai_photos_update_label(request: Request):
    ...
    try:
        ...
        for label_obj in label_list:
            ...
            logger.info(
                f"<updateLabel> Label {label} updated from {label_from} to {label_to}."
            )
    except Exception as e:
        return JSONResponse(
...
async def handle_ai_photos_chat_to_image(request: Request):
    ...
    query = params["query"]
    logger.info(
        f"<chatWithImage> generating chat to image for user {user_id} with query: {query}"
    )
...
async def handle_image_to_image(request: Request):
    ...
    for img_info in image_list:
        ...
        logger.info(
            f"<image2Image> current image id: {img_id}, image path: {image_path}"
        )

        img_b64 = image_to_byte64(image_path)
        data = {
            "source_img": img_b64.decode(),
            "prompt": query,
            "steps": steps,
        ...
        generated_images.append(
            {"imgId": img_id, "imgSrc": "data:image/jpeg;base64," + img_str}
        )
