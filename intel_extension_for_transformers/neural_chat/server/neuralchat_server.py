...
from ..config import LoadingModelConfig, PipelineConfig
...
app = FastAPI(title="NeuralChat Serving API", description="Api", version="0.0.1")
    ...
    def init(self, config):
        ...
        if serving:
            ...
            # TGI serving
                ...
                tgi_habana_visible_devices = tgi_params.get(
                ...
                if device == "cpu":
                    ...
                    create_docker_cmd = "git clone https://github.com/huggingface/tgi-gaudi.git && \
                        cd tgi-gaudi && docker build -t tgi_gaudi ."
                    try:
                        # create docker image first
                        logger.info(
                            f"<neuralchat_server> create docker command = {create_docker_cmd}"
                        )
                        sys.stdout.flush()
                        sys.stderr.flush()
                        subprocess.Popen(
                            create_docker_cmd, shell=True, executable="/bin/bash"
                        )  # nosec
                        logger.info("creating tgi habana docker image...")
                        time.sleep(200)
                    except Exception as e:
                        raise RuntimeError(
                            f"Error in tgi habana docker image creation: {e}"
                        )
                    # add tgi_cmd
                    ...
                    logger.error(
                        f"Supported device: [cpu, gpu, hpu]. Your device: {device}"
                    )
                ...
                try:
                    ...
                    sys.stderr.flush()
                    subprocess.Popen(
                        tgi_cmd, shell=True, executable="/bin/bash"
                    )  # nosec
            ...
            for plugin_name, plugin_config in plugins.items():
                ...
                if yaml_config.get("enable"):
                    ...
                    if plugin_name == "tts":
                        ...
                        from ..pipeline.plugins.audio.tts_chinese import (
                            ChineseTextToSpeech,
                        )
                        ...
                        from ..pipeline.plugins.video.face_animation.sadtalker import (
                            SadTalker,
                        )

                        plugins[plugin_name]["class"] = SadTalker
                    elif plugin_name == "image2image":  # pragma: no cover
                        from ..pipeline.plugins.image2image.image2image import (
                            Image2Image,
                        )
                        ...
                        raise ValueError(
                            "NeuralChat Error: Unsupported plugin for service"
                        )
                    print(f"create {plugin_name} plugin instance...")
                    print("plugin parameters: ", plugin_config["args"])
                    plugin_config["instance"] = plugins[plugin_name]["class"](
                        **plugin_config["args"]
                    )
        ...
        # chatbot as service
            ...
            if use_deepspeed:
                if device == "hpu":
                    ...
                    api_str = (
                        f"'{api_list[0]}'"
                        if len(api_list) == 1
                        else ", ".join(f"'{item}'" for item in api_list)
                    )
                    ...
                    try:
                        print(
                            f"{self.__class__.__name__} init(): command = {command_list}"
                        )
                        sys.stdout.flush()
                        sys.stderr.flush()
                        subprocess.Popen(
                            command_list, shell=True, executable="/bin/bash"
                        )  # nosec
                        logger.info("waiting for server to start...")
                        time.sleep(30)
                    except Exception as exc:
                        raise RuntimeError(
                            f"Error in {self.__class__.__name__} init()"
                        ) from exc
                    self.chatbot = None
                elif device == "cpu":
                    ...
                    launch_str = (
                        f"deepspeed hostfile ./config/hostfile {multi_cpu_server_file}"
                    )
            ...
            api_router = setup_router(
                api_list, self.chatbot, True, use_deepspeed, world_size, host, port
            )
            app.include_router(api_router)
            return True
