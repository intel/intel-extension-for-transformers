<script lang="ts">
	...
	} from "$lib/components/talkbot/store";
	import {
		TalkingPhotoLibrary,
		TalkingVoiceLibrary,
		TalkingKnowledgeLibrary
	} from "$lib/components/talkbot/constant";
			...
			return [
				{
					...
					knowledge: allKnowledges[selectKnowledge].name
				},
				...options,
			];
		});
...
>
	<slot>
		<div class="flex h-32 w-32 cursor-pointer flex-col justify-center rounded-md border-2 p-2">
			<svg
				...
				/></svg
			>
			<p class="text-sm text-black-300">Create Template</p>
		</div>
	</slot>
</button>
<dialog bind:this={insertModal}>
    <div>
    ...
    <div class="flex justify-end gap-10 px-10">
        <button class="btn bg-gray-500 text-white hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-all" on:click={() => {selectAvatar = selectVoice = selectKnowledge = -1;insertModal.close()}}>Cancel</button>
        <button class="btn bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all" on:click={() => {addTemplate();insertModal.close()}}>Confirm</button>
    </div>
</dialog>
