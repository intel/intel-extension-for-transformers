<script lang="ts">
	...
	import { TalkingPhotoLibrary } from "$lib/components/talkbot/constant";
	import TalkingPhotoCard from "$lib/components/talkbot/talking-photo-card.svelte";
	import UploadAvatar from "$lib/components/talkbot/upload-avatar.svelte";
	import { currentAvaTar } from "$lib/components/talkbot/store";
	...
	<div class="h-full w-full p-5 shadow sm:mx-5 xl:mx-20">
		...
		{#if $TalkingPhotoCustom.length === 0}
			<div class="flex gap-10 p-2">
				<img class="h-32" src={portraits} alt="" />
				<div class="flex flex-col items-start justify-center gap-5">
					<h3 class="text-2xl">Upload your own Avatars!</h3>
		...
		{:else}
			<div class="flex gap-7 overflow-auto py-4 text-[#0F172A]">
				<UploadAvatar on:upload={handleAvatarUpload} />
