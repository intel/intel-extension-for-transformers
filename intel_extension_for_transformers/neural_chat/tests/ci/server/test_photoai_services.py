...
from intel_extension_for_transformers.neural_chat.server.restful.photoai_services import (
    ...
    format_image_info,
    get_address_list,
    get_face_list_by_user_id,
    get_image_list_by_ner_query,
    get_images_by_type,
    get_process_status,
    get_type_obj_from_attr,
...
class UnitTest(unittest.TestCase):
    ...
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.check_image_status"
    )
    def test_update_image_tags(self, mock_func, mock_db):
        ...
        try:
            image = {"image_id": 1, "tags": {"time": "new_time"}}
    ...
    def test_format_image_info(self, mock_db):
        ...
        self.assertIn(
            "https://test_server_ip/ai_photos/user1/image1.jpg", res["image_path"]
        )

    def test_delete_single_image(self, mock_db):
        mock_db.return_value.fetch_one.return_value = {
            "image_path": "./test_delete_file.txt"
        }
        mock_db.return_value.transaction.return_value = MagicMock()
        mock_db.return_value.update.return_value = True
        try:
    ...
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.process_single_image"
    )
    ...
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.process_face_for_single_image"
    )
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.generate_caption"
    )
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.get_address_from_gps"
    )
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.update_image_attr"
    )
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.find_GPS_image"
    )
    def test_process_single_image(
        self, mock_func1, mock_func2, mock_func3, mock_func4, mock_func5, mock_db
    ):
    ...
    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.transfer_xywh"
    )
    @patch("deepface.DeepFace.verify")
    @patch("deepface.DeepFace.find")
    @patch("deepface.DeepFace.represent")
    def test_process_face_for_single_image(
        self, mock_represent, mock_find, mock_verify, mock_func1, mock_db
    ):
        mock_represent.return_value = [{"facial_area": "mocked_xywh"}]
        df = pd.DataFrame(data=["./mocked_identity1"], columns=["identity"])
        mock_find.return_value = [df]
        mock_verify.return_value = {
            "facial_areas": {"img1": "mocked_xywh1", "img2": "mocked_xywh2"}
        }
        mock_func1.return_value = "mocked_transferred_xywh"
        mock_db.return_value.fetch_all.return_value = [
            {"face_id": 1, "face_tag": "mocked_tag"}
        ]
        mock_db.return_value.transaction.return_value = MagicMock()
        ...
        try:
            process_face_for_single_image(
                image_id=1,
                image_path="./mocked_image_path",
                db_path="./mocked_db_path",
                user_id="1",
            )
        except Exception as e:
            raise Exception(e)
        mock_db.return_value.insert.assert_any_call(
            sql="INSERT INTO face_info VALUES(null, 'person1');", params=None
        )

    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.format_image_path"
    )
    ...
    def test_get_images_by_type(self, mock_db):
        mock_db.return_value.fetch_all.return_value = [
            {"image_path": "mocked/image.jpg", "image_id": 1}
        ]
        res = get_images_by_type(user_id="1", type="address", subtype="default")
        self.assertIn(
            "https://test_server_ip/ai_photos/user1/image.jpg", res[0]["image_path"]
        )

    def test_get_face_list_by_user_id(self, mock_db):
        mock_db.return_value.fetch_all.return_value = [
            {"image_path": "mocked/image.jpg", "face_tag": "person1"}
        ]
        res = get_face_list_by_user_id(user_id="1")
        self.assertIn(
            "https://test_server_ip/ai_photos/user1/image.jpg", res["person1"]
        )

    @patch(
        "intel_extension_for_transformers.neural_chat.server.restful.photoai_services.get_address_list"
    )
    def test_get_image_list_by_ner_query(self, mock_func1, mock_db):
        mock_func1.return_value = ["shanghai"]
        mock_db.return_value.fetch_all.return_value = [
            {"image_path": "mocked/image.jpg", "face_tag": "person1", "image_id": 1}
        ]
        ...
        self.assertIn(
            "https://test_server_ip/ai_photos/user1/image.jpg", res[0]["imgSrc"]
        )
    ...
    def test_delete_user_infos(self, mock_db):
        ...
        mock_db.return_value.delete.assert_any_call(
            sql="DELETE FROM user_info WHERE user_id='1'", params=None
        )
