...
class TestBuildChatbotExceptions(unittest.TestCase):
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch("transformers.AutoModelForCausalLM.from_pretrained")
    def test_adapter_load_model_cache_dir_no_write_permission(
    ...
    def test_adapter_load_model_device_busy(self, mock_from_pretrained):
        ...
        config = PipelineConfig(model_name_or_path="facebook/opt-125m")
        mock_from_pretrained.side_effect = RuntimeError(
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch("transformers.AutoModelForCausalLM.from_pretrained")
    def test_adapter_load_model_value_error_unsupported_device(
        self, mock_from_pretrained
    ):
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch("transformers.AutoModelForCausalLM.from_pretrained")
    def test_adapter_load_model_value_error_unsupported_model(
        self, mock_from_pretrained
    ):
        # Test value error exception handling
        config = PipelineConfig(model_name_or_path="facebook/opt-125m")
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch("transformers.AutoModelForCausalLM.from_pretrained")
    def test_adapter_load_model_value_error_tokenizer_not_found(
        self, mock_from_pretrained
    ):
        # Test value error exception handling
        config = PipelineConfig(model_name_or_path="facebook/opt-125m")
        mock_from_pretrained.side_effect = ValueError(
            "load_model: tokenizer is not found"
        )
        result = build_chatbot(config)
        self.assertIsNone(result)
    ...
    def test_adapter_load_model_value_error_model_not_found(self, mock_from_pretrained):
        ...
        config = PipelineConfig(model_name_or_path="facebook/opt-125m")
        mock_from_pretrained.side_effect = ValueError(
            "load_model: model name or path is not found"
        )
        result = build_chatbot(config)
        self.assertIsNone(result)
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch("transformers.AutoModelForCausalLM.from_pretrained")
    def test_adapter_load_model_value_error_model_config_not_found(
        self, mock_from_pretrained
    ):
        # Test value error exception handling
        config = PipelineConfig(model_name_or_path="facebook/opt-125m")
        mock_from_pretrained.side_effect = ValueError(
            "load_model: model config is not found"
        )
        result = build_chatbot(config)
        self.assertIsNone(result)
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch(
        "intel_extension_for_transformers.neural_chat.pipeline.plugins.retrieval.detector.intent_detection.IntentDetector.intent_detection"
    )
    ...
    def test_model_safety_checker_exception(self):
        ...
        self.assertEqual(
            ...
            "[SafetyChecker ERROR] Sensitive check file not found!",
        )
...
class TestFinetuneModelExceptions(unittest.TestCase):
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch(
        "intel_extension_for_transformers.llm.finetuning.finetuning.Finetuning.finetune"
    )
    ...
    def test_finetune_error_validation_file_not_found(self, mock_finetune):
        ...
        mock_finetune.side_effect = ValueError(
    ...
    def test_finetune_lora_finetune_fail(self, mock_finetune):
        ...
        finetune_args = FinetuningArguments(
            device=self.device, do_lm_eval=False, peft="lora"
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
    ...
    def test_finetune_llama_adapter_finetune_fail(self, mock_finetune):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, do_lm_eval=False, peft="llama_adapter"
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
        ...
        finetune_model(finetune_cfg)
        self.assertEqual(
            get_latest_error(), ErrorCodes.ERROR_LLAMA_ADAPTOR_FINETUNE_FAIL
        )
    ...
    def test_finetune_ptun_finetune_fail(self, mock_finetune):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, do_lm_eval=False, peft="ptun"
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
    ...
    def test_finetune_prefix_finetune_fail(self, mock_finetune):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, do_lm_eval=False, peft="prefix"
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
    ...
    def test_finetune_prompt_finetune_fail(self, mock_finetune):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, do_lm_eval=False, peft="prompt"
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
    ...
    def test_finetune_data_no_permission_finetune_fail(self, mock_finetune):
        ...
        finetune_model(finetune_cfg)
        self.assertEqual(
            get_latest_error(), ErrorCodes.ERROR_DATASET_CACHE_DIR_NO_WRITE_PERMISSION
        )
    ...
    def test_finetune_error_generic(self, mock_finetune):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, do_lm_eval=False, peft=None
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
...
class TestOptimizeModelExceptions(unittest.TestCase):
    ...
    @unittest.skipIf(get_device_type() != "cpu", "Only run this test on CPU")
    @patch(
        "intel_extension_for_transformers.llm.quantization.optimization.Optimization.optimize"
    )
    ...
    def test_weight_only_quant_optimize_fail(self, mock_optimize):
        ...
        optimize_model(model, config)
        self.assertEqual(
            get_latest_error(), ErrorCodes.ERROR_WEIGHT_ONLY_QUANT_OPTIMIZATION_FAIL
        )
    ...
    def test_bitsandbytes_quant_optimize_fail(self, mock_optimize):
        ...
        optimize_model(model, config)
        self.assertEqual(
            get_latest_error(), ErrorCodes.ERROR_BITS_AND_BYTES_OPTIMIZATION_FAIL
        )
