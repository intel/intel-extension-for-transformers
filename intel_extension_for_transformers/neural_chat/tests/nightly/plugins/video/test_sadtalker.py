...
from intel_extension_for_transformers.neural_chat.pipeline.plugins.video.face_animation.sadtalker import (
    SadTalker,
)
...
class TestSadTalker(unittest.TestCase):
    ...
    def tearDownClass(self):
        os.remove(self.output_video_path) if os.path.exists(
    ...
    def test_sadtalker_without_enhancer(self):
        self.sadtalker.convert(
            source_image=self.source_image, driven_audio=self.driven_audio
        )
