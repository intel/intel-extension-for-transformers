...
from intel_extension_for_transformers.neural_chat.config_logging import (
    configure_logging,
)
from intel_extension_for_transformers.utils.device_utils import (
    get_device_type,
    is_hpu_available,
)
...
class TestFinetuning(unittest.TestCase):
    ...
    def test_finetune_clm_qlora(self):
        ...
        finetune_args = FinetuningArguments(
            device=self.device, qlora=True, do_lm_eval=False
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
    ...
    def test_finetune_clm_full_finetuning(self):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, full_finetune=True, bits=16, do_lm_eval=False
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
    ...
    def test_finetune_clm_value1(self):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, full_finetune=True, do_lm_eval=False
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
    ...
    def test_finetune_clm_value2(self):
        ...
        data_args = DataArguments(train_file=test_data_file)
        finetune_args = FinetuningArguments(
            device=self.device, qlora=True, full_finetune=True, do_lm_eval=False
        )
        finetune_cfg = TextGenerationFinetuningConfig(
            model_args=model_args,
            data_args=data_args,
