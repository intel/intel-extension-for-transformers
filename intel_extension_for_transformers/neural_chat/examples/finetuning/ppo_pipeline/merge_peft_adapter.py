...
if peft_config.task_type == "SEQ_CLS":
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        peft_config.base_model_name_or_path, num_labels=1, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True
    )
else:
    model = AutoModelForCausalLM.from_pretrained(
        peft_config.base_model_name_or_path, return_dict=True, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True
    )
