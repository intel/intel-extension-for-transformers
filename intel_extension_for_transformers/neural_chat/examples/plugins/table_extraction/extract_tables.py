...
import util.postprocess as postprocess
...
def get_args():
    ...
    parser.add_argument('--pdf_file',
                        help="Path for pdf file.")
    parser.add_argument('--out_dir',
                        help="Output directory")
    parser.add_argument('--structure_config_path', default='configs/structure_config.json',
    ...
    parser.add_argument('--detection_config_path', default='configs/detection_config.json',
...
def objects_to_crops(img, tokens, objects, class_thresholds, padding=10):
    ...
    for obj in objects:
        ...
        cropped_table['bbox_padded'] = bbox
        cropped_table['image_size'] = img.size
...
class TableExtractionPipeline(object):
    ...
    def extract(self, img, tokens=None, out_objects=True, out_crops=False, out_cells=False,
        ...
        for table in cropped_tables:
            ...
            extracted_table['table_bbox'] = (table['bbox'], table['bbox_padded'], table['image_size'])
...
def get_column_nums(table):
    if not table['cells']:
        return 0
    max_col_id = max([c['column_nums'][0] if c['column_nums'] else -1 for c in table['cells'][0]])
    return max_col_id + 1

def position_match(prev_table, cur_table, threshold=0.01):
    if 'table_bbox' in prev_table and 'table_bbox' in cur_table:
        bbox_p = prev_table['table_bbox'][0]
        bbox_c = cur_table['table_bbox'][0]
        x_p, width_p = bbox_p[0], bbox_p[2]-bbox_p[0]
        x_c, width_c = bbox_c[0], bbox_c[2]-bbox_c[0]
        img_width = (prev_table['table_bbox'][2][0] +  cur_table['table_bbox'][2][0]) / 2
        return abs(x_p-x_c) <= threshold*img_width and abs(width_p-width_c) <= threshold*img_width
    else:
        return False

def not_much_contents_in_between(prev_table, cur_table, threshold=30):
    if 'contents_below' in prev_table and 'contents_above' in cur_table:
        return len(prev_table['contents_below']) < threshold and \
            len(cur_table['contents_above']) < threshold
    else:
        return False

def mergeable(prev_table, cur_table):
    if get_column_nums(prev_table) != get_column_nums(cur_table) or \
        not position_match(prev_table, cur_table) or \
        not not_much_contents_in_between(prev_table, cur_table) or \
        not ('csv' in prev_table and prev_table['csv']) or \
        not ('csv' in cur_table and cur_table['csv']):
        return False
    return True

def remove_empty_rows(csv_data_list):
    res = []
    for csv_data_str in csv_data_list:
        csv_data = [row for row in csv_data_str.split('\n') if set(row) != {','}]
        res.append('\n'.join(csv_data))
    return res

def merge_tables(all_extracted_tables):
    res_tables = []
    table_from_last_page = None
    for page in range(len(all_extracted_tables)):
        extracted_tables = all_extracted_tables[page]
        for extracted_table in extracted_tables:
            extracted_table['csv'] = remove_empty_rows(extracted_table['csv'])
        if table_from_last_page and extracted_tables:
            if mergeable(table_from_last_page, extracted_tables[0]):
                merged_table = table_from_last_page['csv'] + extracted_tables[0]['csv']
                extracted_tables[0]['csv'] = merged_table
            else:
                res_tables += [table_from_last_page['csv']]
        if not extracted_tables:
            table_from_last_page = None
            continue
        table_from_last_page = extracted_tables[-1]
        res_tables += [table['csv'] for table in extracted_tables[:-1]]
    if table_from_last_page and 'csv' in table_from_last_page and table_from_last_page['csv']:
        res_tables += [table_from_last_page['csv']]
    return res_tables
...
def main():
    ...
    if os.path.exists(args.pdf_file):
        pages = pdf2image.convert_from_path(args.pdf_file, dpi=200)
    elif args.pdf_file.startswith('http'):
        pdf = requests.get(args.pdf_file, stream=True)
        pages = pdf2image.convert_from_bytes(pdf.content, dpi=200)
    else:
        raise ValueError(
            f"Invalid path \'{args.pdf_file}\' to pdf file, please provide local path to pdf or url to it."
        )

    if args.out_dir is None:
        args.out_dir = args.pdf_file.split('/')[-1].split('.pdf')[0]

    if not args.out_dir is None and not os.path.exists(args.out_dir):
        os.makedirs(args.out_dir)

    number_max_length = len(str(len(pages)))
    ...
    for count, page in enumerate(tqdm(pages)):
        print("({}/{})".format(count+1, len(pages)))
        img = page
        res = pytesseract.image_to_data(img, lang='eng+chi_sim', output_type='dict')
        bboxes_txt_conf = [
            (
                [
                    res['left'][i],res['top'][i],res['left'][i]+res['width'][i],res['top'][i]+res['height'][i]
                ],
                res['text'][i].strip('|').strip(),
                res['conf'][i]
            )
            for i in range(len(res['conf'])) if res['conf'][i]>0 and res['text'][i].strip() != ''
        ]
        tokens = [{'bbox': item[0], 'text': item[1]} for item in bboxes_txt_conf]
        ...
        idx = "0"*(number_max_length-len(str(count+1))) + str(count+1)
        img_file = f'page_{idx}.jpg'
        extracted_tables = pipe.extract(img, tokens, out_objects=args.objects, out_cells=args.csv,
                                        out_html=args.html, out_csv=args.csv, crop_padding=args.crop_padding)

        if extracted_tables:
            print("Table(s) extracted.")
            y_down_pos = extracted_tables[-1]['table_bbox'][0][3]
            contents_below_last_table = [b for b in bboxes_txt_conf if b[0][1]>=y_down_pos]
            y_upper_pos = extracted_tables[0]['table_bbox'][0][1]
            contents_above_first_table = [b for b in bboxes_txt_conf if b[0][3]<=y_upper_pos]
            for table_idx, extracted_table in enumerate(extracted_tables):
                if  table_idx == 0:
                    extracted_table['contents_above'] = contents_above_first_table
                else:
                    extracted_table['contents_above'] = []
                if table_idx == len(extracted_tables) - 1:
                    extracted_table['contents_below'] = contents_below_last_table
                else:
                    extracted_table['contents_below'] = []
            all_extracted_tables.append(extracted_tables)
    ...
    if all_extracted_tables:
        merged_tables = merge_tables(all_extracted_tables)
        save_path = os.path.join(args.out_dir, "merged_tables")
        os.makedirs(save_path, exist_ok=True)
        for idx, merged_table in enumerate(merged_tables):
            with open(os.path.join(save_path, f"table_{idx+1}.csv"), 'w') as f:
                for elem in merged_table:
                    f.write(elem)
