...
class EnglishNormalizer:
    ...
    def correct_abbreviation(self, text):
        ...
        words = re.split(
            r" |_|/|\*|\#", text
        )  # ignore the characters that not break sentence
        results = []
        ...
        for idx, word in enumerate(words):
            ...
            if word.isdigit():  # if word is positive integer, it must can be num2words
                try:
                    ...
                    if (
                        prev.lower() in prepositions_year
                        and potential_year < 2999
                        and potential_year > 1000
                        and potential_year % 1000 != 0
                    ):
                        word = num2words(word, to="year")
                        word = word.replace(
                            "-", ""
                        )  # nineteen eighty-seven => nineteen eightyseven
                    else:
                        word = num2words(word)
                except Exception as e:
                    logging.info(
                        "num2words fail with word: %s and exception: %s", word, e
                    )
            else:
                try:
                    val = int(word)
