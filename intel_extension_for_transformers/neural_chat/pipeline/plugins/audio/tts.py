...
class TextToSpeech:
    ...
    def __init__(
        ...
        self.device = device
        self.original_model = SpeechT5ForTextToSpeech.from_pretrained(
        ...
        try:
            ...
            logging.warning(
                "[TTS Warning] speaker model fail to load, so speaker embedding creating is disabled."
            )
        ...
        if os.path.exists(
            ...
            try:
                ...
                logging.warning(
                    "Warning! Need to prepare speaker_embeddings, will use the backup embedding."
                )
        ...
        pat_speaker_embedding_path = os.path.join(
            script_dir, "../../../assets/speaker_embeddings/spk_embed_male.pt"
        )
    ...
    def create_speaker_embedding(self, driven_audio_path):
        ...
        if self.speaker_model is None:
            raise Exception(
                "Unable to create a speaker embedding! Please check the speaker model."
            )
        try:
    ...
    def _lookup_voice_embedding(self, voice):
        ...
        if os.path.exists(
            os.path.join(
                script_dir, f"../../../assets/speaker_embeddings/spk_embed_{voice}.pt"
            )
        ):
            specific_speaker_embedding_path = os.path.join(
                script_dir, f"../../../assets/speaker_embeddings/spk_embed_{voice}.pt"
            )
            ...
            logging.warning(
                "No customized speaker embedding is found! Use the default one"
            )
    ...
    def text2speech(
        ...
        if batch_length > 600 or batch_length < 50:
            raise Exception(
                f"[TTS] Invalid batch_length {batch_length}, should be between 50 and 600!"
            )
        ...
        if voice == "male":
            ...
            if self.male_speaker_embeddings == None:  # pragma: no cover
                logging.warning(
                    "Male speaker embedding is not found! Use the default one"
                )
            else:
        ...
        for text_in in texts:
            ...
            with torch.no_grad():
                ...
                all_speech = np.concatenate(
                    [all_speech, np.array([0 for i in range(8000)])]
                )  # pad after each end
        sf.write(output_audio_path, all_speech, samplerate=16000)
        if self.noise_reducer:
            output_audio_path = self.noise_reducer.reduce_audio_amplify(
                output_audio_path, all_speech
            )
    ...
    def post_llm_inference_actions(self, text_or_generator):
        ...
        self.output_audio_path = (
            plugins.tts.args["output_audio_path"]
            if plugins.tts.args["output_audio_path"]
            else "./response.wav"
        )
        if self.stream_mode:  # pragma: no cover
            ...
            def cache_words_into_sentences():
                ...
                for new_text in text_or_generator:
                    ...
                    if len(buffered_texts) > 5:
                        if (
                            new_text.endswith("... ")
                            or new_text.strip()[-1] in hitted_ends
                        ):
            ...
            return self.stream_text2speech(
                cache_words_into_sentences(), self.output_audio_path, self.voice
            )
        else:
            return self.text2speech(
                text_or_generator, self.output_audio_path, self.voice
            )
