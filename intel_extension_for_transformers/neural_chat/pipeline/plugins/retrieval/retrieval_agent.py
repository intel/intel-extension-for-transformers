...
class Agent_QA:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.splitter = RecursiveCharacterTextSplitter(
            chunk_size=kwargs["child_size"] if "child_size" in kwargs else 512
        )
        ...
        assert (
            self.retrieval_type in allowed_retrieval_type
        ), "search_type of {} not allowed.".format(self.retrieval_type)
        assert (
            self.mode in allowed_generation_mode
        ), "generation mode of {} not allowed.".format(self.mode)

        if isinstance(input_path, str):
            if os.path.exists(input_path):
                self.input_path = input_path
            elif os.path.exists(
                os.path.split(os.path.split(os.path.split(script_dir)[0])[0])[0]
                + "/assets/docs/"
            ):
                self.input_path = (
                    os.path.split(os.path.split(os.path.split(script_dir)[0])[0])[0]
            ...
            logging.error(
                "The given file path is unavailable, please check and try again!"
            )
        ...
        if self.retrieval_type == "default":  # Using vector store retriever
            if append:
                knowledge_base = self.database.from_documents(
                    documents=langchain_documents, embedding=self.embeddings, **kwargs
                )
            else:
                knowledge_base = self.database.build(
                    documents=langchain_documents, embedding=self.embeddings, **kwargs
                )
            ...
            self.retrieval_type == "child_parent"
        ):  # Using child-parent store retriever
            ...
            if append:
                ...
                child_knowledge_base = self.database.from_documents(
                ...
                child_knowledge_base = self.database.build(
            ...
            self.retriever = RetrieverAdapter(
                retrieval_type=self.retrieval_type,
                document_store=knowledge_base,
                child_document_store=child_knowledge_base,
                **kwargs,
            ).retriever
    ...
    def reload_localdb(self, local_persist_dir, **kwargs):
        ...
        assert os.path.exists(local_persist_dir) and bool(
            os.listdir(local_persist_dir)
        ), "Please check the local knowledge base was built!"
        knowledge_base = self.database.reload(
            persist_directory=local_persist_dir, embedding=self.embeddings, **kwargs
        )
        if self.retrieval_type == "default":
            ...
            child_knowledge_base = self.database.reload(
                persist_directory=child_persist_dir, embedding=self.embeddings, **kwargs
            )
            self.retriever = RetrieverAdapter(
                retrieval_type=self.retrieval_type,
                document_store=knowledge_base,
    ...
    def pre_llm_inference_actions(self, model_name, query):
        ...
        links = []
        context = ""
        assert self.retriever is not None, logging.info(
            "Please check the status of retriever"
        )
