...
from .context_utils import get_chuck_data, load_structured_data, load_unstructured_data
...
class DocumentParser:
    ...
    def load(self, input, **kwargs):
        ...
        if isinstance(input, str):
            ...
            try:
                ...
                logging.error(
                    "The given link/str is unavailable. Please try another one!"
                )
        else:
    ...
    def parse_document(self, input):
        ...
        if (
            input.endswith("pdf")
            or input.endswith("docx")
            or input.endswith("html")
            or input.endswith("txt")
            or input.endswith("md")
        ):
            content = load_unstructured_data(input)
            if self.process:
                chuck = get_chuck_data(
                    content, self.max_chuck_size, self.min_chuck_size, input
                )
            else:
                chuck = [[content.strip(), input]]
        elif (
            input.endswith("jsonl")
            or input.endswith("xlsx")
            or input.endswith("csv")
            or input.endswith("json")
        ):
            chuck = load_structured_data(
                input, self.process, self.max_chuck_size, self.min_chuck_size
            )
        else:
            logging.info(
                "This file {} is ignored. Will support this file format soon.".format(
                    input
                )
            )
    ...
    def parse_html(self, input):
        ...
        for link in input:
            if re.match(r"^https?:/{2}\w.+$", link):
                ...
                if self.process:
                    chuck = get_chuck_data(
                        content, self.max_chuck_size, self.min_chuck_size, link
                    )
                else:
