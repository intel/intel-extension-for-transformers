...
class TextVoiceChatExecutor(BaseCommandExecutor):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.parser = argparse.ArgumentParser(prog="neuralchat.predict", add_help=True)
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=None,
            help="Audio output path if the prompt is audio file.",
        )
        self.parser.add_argument(
    ...
    def execute(self, argv: List[str]) -> bool:
        ...
        if model_name:
            self.config = PipelineConfig(
                model_name_or_path=model_name, plugins=plugins, device=device
            )
        else:
...
class FinetuingExecutor(BaseCommandExecutor):
    def __init__(self):
        super().__init__()
        self.parser = argparse.ArgumentParser(prog="neuralchat.finetune", add_help=True)
        self.parser.add_argument(
            "--base_model",
            type=str,
            default=None,
            help="Base model path or name for finetuning.",
        )
        self.parser.add_argument(
            "--device",
            type=str,
            default=None,
            help="Specify finetune model on which device.",
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=None,
            help="Specify max steps of finetuning.",
        )
    ...
    def execute(self, argv: List[str]) -> bool:
        ...
        self.finetuneCfg = TextGenerationFinetuningConfig(
...
for com, info in specific_commands.items():
    command_register(
        ...
        description=info[0],
        cls="intel_extension_for_transformers.neural_chat.cli.cli_commands.{}".format(
            info[1]
        ),
    )
