...
void jit_matmul_amx_s8ab_s8Ab4a_s32AB16a16b::generate() {
  ...
  std::shared_ptr<void> use_loacl_label = {(inLocalLabel(), nullptr), [&](...) { outLocalLabel(); }};
  {
    ...
    for (int idx_m = 0; idx_m < M; idx_m += TH_ * 16) {
      // load src0
      const auto src0_offset = idx_m * ld_src0;
      for (int i = 0; i < TH_; ++i) {
        if (idx_m + i * 16 < M) {
          tileloadd(tmm_src0[i], ptr[reg_src0 + reg_ld_src0 + src0_offset + i * 16 * ld_src0]);
        }
      }

      for (int idx_n = 0; idx_n < N; idx_n += TH_ * 16) {
        // load & zero & dp & store
        const auto src1_offset = idx_n * K;
        for (int i = 0; i < TH_; ++i)
          for (int j = 0; j < TW_; ++j)
            if (idx_m + i * 16 < M && idx_n + j * 16 < N) {
              const auto dst_offset = (idx_m + i * 16) * N + (idx_n + j * 16) * 16;
              if (i == 0) tileloadd(tmm_src1[j], ptr[reg_src1 + reg_stride64 + src1_offset + j * 16 * K]);
  ...
  if (need_cfg_amx) {
    sparselib_configure_tiles(required_amx_cfg_, &reqired_tile_cfg_);
