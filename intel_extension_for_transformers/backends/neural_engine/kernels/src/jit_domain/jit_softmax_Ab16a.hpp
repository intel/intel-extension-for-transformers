...
class jit_softmax_Ab16a : public jit_generator {
 ...
 private:
  ...
  const uint16_t magic_number[5] = {
      fp32_to_fp16(std::log2(std::exp(1.f))),  // log_2{e}

      // Coefficient of the 2nd-order polynomial approximating function $f(x) = 2^{x-0.5}$ for $x \in [0, 1)$; idea
      // from https://arxiv.org/abs/2101.01321
      fp32_to_fp16(0.240226507f),
      fp32_to_fp16(0.452920674f),
      fp32_to_fp16(0.713483036f),
