...
bool mha_dense_ref_kd_t::init() {
  ...
  KERNEL_INIT_CHECK(descs[io::SRC_Q].shape() == descs[io::SRC_K].shape());
  KERNEL_INIT_CHECK(descs[io::SRC_Q].shape() == descs[io::SRC_V].shape());
  KERNEL_INIT_CHECK(descs[io::SRC_Q].shape() == descs[io::DST].shape());
  KERNEL_INIT_CHECK(descs[io::MASK].shape() == std::vector<jd::dim_t>{bs()});

  if (descs.size() > io::BINARY_ADD && descs[io::BINARY_ADD].dtype() != data_type::undef) {
    const auto& badd_shape = descs[io::BINARY_ADD].shape();
    KERNEL_INIT_CHECK(descs[io::BINARY_ADD].dtype() == data_type::fp32);
    KERNEL_INIT_CHECK(seq_len() == badd_shape[badd_shape.size() - 1]);
    KERNEL_INIT_CHECK(badd_shape.size() < 3 || seq_len() == badd_shape[badd_shape.size() - 2]);
...
mha_dense_ref_k_t::mha_dense_ref_k_t(const std::shared_ptr<const kernel_desc_t>& kd)
    : kernel_t(kd),
      ...
      badd_stride{
          !has_badd || ts_descs_[io::BINARY_ADD].shape().size() < 4 ? 0 : head_num_ * seq_len_ * seq_len_,
          !has_badd || ts_descs_[io::BINARY_ADD].shape().size() < 3 ? 0 : seq_len_ * seq_len_,
          !has_badd || ts_descs_[io::BINARY_ADD].shape().size() < 2 ? 0 : seq_len_,
...
#pragma omp parallel for collapse(2)
  for (int ibs = 0; ibs < bs_; ibs++) {
    for (int ihn = 0; ihn < head_num_; ihn++) {
      ...
      const auto q_s8 = reinterpret_cast<const int8_t*>(rt_data[io::SRC_Q]) + src_offset;
      const auto k_s8 = reinterpret_cast<const int8_t*>(rt_data[io::SRC_K]) + src_offset;
      const auto v_s8 = reinterpret_cast<const int8_t*>(rt_data[io::SRC_V]) + src_offset;
