...
mha_dense_ref_k_t::mha_dense_ref_k_t(const std::shared_ptr<const kernel_desc_t>& kd)
    : kernel_t(kd),
      ...
      is_dynq10n_dst(ts_descs_.size() > io::DST_SCALE && ts_descs_[io::DST_SCALE].dtype() != dt::undef),
  ...
  for (int ibs = 0; ibs < bs_; ibs++) {
    for (int ihn = 0; ihn < head_num_; ihn++) {
      for (int i = 0; i < sl_m_; ++i) {  // put at the top so it can be paralleled
        const auto curr_sl_n =
            ts_descs_[io::MASK].dtype() == dt::undef ? sl_n_ : reinterpret_cast<const int32_t*>(rt_data[io::MASK])[ibs];
        ...
        for (int j = 0; j < curr_sl_n; ++j) {
          if (ts_descs_[io::SRC_Q].dtype() == dt::s8 && ts_descs_[io::SRC_K].dtype() == dt::s8) {
            int32_t value = 0;
#pragma omp simd
            for (int k = 0; k < head_size_; ++k) value += q_s8[i * ld_q_ + k] * k_s8[j * ld_kv_ + k];
            exp_row[j] = value;
          } else if (ts_descs_[io::SRC_Q].dtype() == dt::bf16 && ts_descs_[io::SRC_K].dtype() == dt::bf16) {
            float value = 0;
#pragma omp simd
...
#pragma omp simd
        for (int j = 0; j < curr_sl_n; ++j) {
          ...
          exp_row[j] = ts_descs_[io::SRC_V].dtype() == dt::s8 ? post_softmax<func_exp>(a_val) : a_val;
        }
        ...
        for (int j = 0; j < head_size_; ++j) {
          ...
          const bool use_requant_v = ts_descs_[io::SRC_V].dtype() == dt::s8 && ts_descs_[io::V_SCALE].size() > 1;
          if (use_requant_v) {
