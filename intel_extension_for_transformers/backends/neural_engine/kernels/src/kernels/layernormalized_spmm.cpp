...
#include "kernels/layernormalized_spmm.hpp"
...
bool layernormalized_spmm_kd_t::init() {
  ...
  if (src_desc.shape().size() == 3) mean_var_shape.push_back(num_mbs);
...
bool layernormalized_spmm_k_t::init() {
  ...
  if (!jd::kernel_desc_t::create<spmm_vnni_kd_t>(spmm_ker_desc, *(kd->spmm_desc))) return false;
  if (!create<spmm_vnni_k_t, spmm_vnni_kd_t>(kernels_[0], spmm_ker_desc)) return false;
  if (!jd::kernel_desc_t::create<layernorm_ba_kd_t>(lnorm_ker_desc, *(kd->lnorm_desc))) return false;
  if (!create<layernorm_ba_k_t, layernorm_ba_kd_t>(kernels_[1], lnorm_ker_desc)) return false;
  return true;
}

bool layernormalized_spmm_k_t::execute(const std::vector<const void*>& rt_data) const {
