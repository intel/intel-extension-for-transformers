...
bool matmul_vnni_p2031_p2013_kd_t::init() {
  ...
  std::vector<dim_t> src0_perm_shape = {
      shapes[ssd::SRC0][2],  // bs0
      shapes[ssd::SRC0][0],  // bs1
      shapes[ssd::SRC0][3],  // m
      shapes[ssd::SRC0][1],  // k
  };
  ...
  bool is_supported = (op_desc_.kernel_prop() == kernel_prop::forward_inference) &&
                      is_any_of({dt::s8}, [&](const dt& a) { return descs[ssd::SRC0].dtype() == a; }) &&
                      is_any_of({dt::s8}, [&](const dt& a) { return descs[ssd::SRC1].dtype() == a; }) &&
                      is_any_of({dt::s8, dt::u8, dt::fp32}, [&](const dt& a) { return descs[ssd::DST0].dtype() == a; });
  if (!is_supported) {
  ...
  if (descs[ssd::DST0].dtype() != (postops.size() != 0 ? postops.back().dt : data_type::fp32)) {
...
matmul_vnni_p2031_p2013_k_t::matmul_vnni_p2031_p2013_k_t(const std::shared_ptr<const kd_t>& kd)
    : kernel_t(kd),
      ...
      dst_type(kd->get_operator_desc().tensor_descs()[ssd::DST0].dtype()),
      has_binary_add(t_shapes_[ssd::SRC2].size() != 0),
...
inline bool matmul_vnni_p2031_p2013_k_t::thread_exec(const std::vector<const void*>& rt_data, const dim_t ibs0,
  ...
  const auto curr_src0 = reinterpret_cast<const uint8_t*>(rt_data[ssd::SRC0]) + ibs0 * M_ + ibs1 * bs0_ * M_ * K_;
  const auto curr_src1 = reinterpret_cast<const int8_t*>(rt_data[ssd::SRC1]) + ibs0 * N_ + ibs1 * bs0_ * N_ * K_;
  const auto curr_dst_offset = (ibs0 * bs1_ + ibs1) * M_ * N_;
  const auto curr_dst = const_cast<dt_dst*>(reinterpret_cast<const dt_dst*>(rt_data[ssd::DST0])) + curr_dst_offset;
  const auto curr_src2 = reinterpret_cast<const float*>(rt_data[ssd::SRC2]) + curr_dst_offset;
...
#pragma omp parallel for collapse(2)
  ...
  const auto base_src0 = static_cast<const uint8_t*>(rt_data[ssd::SRC0]);
  const auto base_src1 = static_cast<const int8_t*>(rt_data[ssd::SRC1]);
  const auto base_dst = const_cast<dt_dst*>(static_cast<const dt_dst*>(rt_data[ssd::DST0]));
  const auto base_src2 = static_cast<const float*>(rt_data[ssd::SRC2]);
