 ...
 */
class SPARSE_API_ matmul_ref_kd_t : public kernel_desc_t {
 public:
 ...
 public:
  ...
  inline const std::vector<std::vector<jd::dim_t>>& perm() const { return *perm_ptr_; }

 private:
  jd::operator_desc op_desc_;
  std::vector<dim_t> shape_;
  const std::vector<std::vector<jd::dim_t>>* perm_ptr_ = nullptr;
};

/**
 * @brief a derived kernel. kd_t and jit_domain are its class members.
 */
class SPARSE_API_ matmul_ref_k_t : public kernel_t {
 public:
 ...
 public:
  ...
  inline const std::vector<std::vector<jd::dim_t>>& perm() const { return derived_kd()->perm(); }
