...
#ifndef ENGINE_SPARSELIB_INCLUDE_KERNELS_TRANSPOSE_ATTENTION_HPP_
#define ENGINE_SPARSELIB_INCLUDE_KERNELS_TRANSPOSE_ATTENTION_HPP_
...
#include "utils.hpp"
#include "transpose_attention_types.hpp"
#include "jit_domain/jit_transpose_attention.hpp"

namespace jd {
class transpose_attention_k_t;

class transpose_attention_kd_t : public kernel_desc_t {
 public:
  explicit transpose_attention_kd_t(const jd::operator_desc& op_desc)
      : kernel_desc_t(kernel_kind::transpose_attention), op_desc_(op_desc) {}

  virtual ~transpose_attention_kd_t() {}

 public:
  bool init() override;
  DECLARE_COMMON_PD_T(transpose_attention_k_t, transpose_attention_kd_t);
...
class transpose_attention_k_t : public kernel_t {
 public:
  using kd_t = transpose_attention_kd_t;
  explicit transpose_attention_k_t(const std::shared_ptr<const kd_t>& kd) : kernel_t(kd) {
  ...
  virtual ~transpose_attention_k_t() {
    //  aligned_free(mTmp);
  }
  // Delete move constructor and move operator
  transpose_attention_k_t(transpose_attention_k_t&& other) = delete;
  transpose_attention_k_t& operator=(transpose_attention_k_t&& other) = delete;
  // Delete copy constructor and copy operator
  transpose_attention_k_t(const transpose_attention_k_t& other) = delete;
  transpose_attention_k_t& operator=(const transpose_attention_k_t& other) = delete;
 ...
 private:
  // uint8_t* mTmp;
  bool amx_enable = false, vnni_enable = false;
...
#endif  // ENGINE_SPARSELIB_INCLUDE_KERNELS_TRANSPOSE_ATTENTION_HPP_