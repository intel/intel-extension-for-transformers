...
std::string to_string(ne_tensor *tensor, bool with_data = true);

void tensor_assign_buffers(ne_tensor *tensor, bool scratch = true, bool force_inplace = false);
...
struct ne_context_deleter_t {
    void operator()(ne_context *ctx) const noexcept { ne_free(ctx); }
};

using unique_ne_context_t = std::unique_ptr<ne_context, ne_context_deleter_t>;

static inline unique_ne_context_t make_unique_ne_context(size_t mem_size, void *mem_buffer, bool no_alloc) {
...
class Embedding {
  public:
    ...
    Embedding(ModelContext *ctx, int num_embeddings, int embedding_dim)
        : weight(ne_new_tensor_2d(ctx->ctx_w.get(), ctx->dtype, embedding_dim, num_embeddings, NE_SIZE_CALC)) {}

    ne_tensor *forward(ModelContext *ctx, ne_tensor *input) const;
...
class Linear {
  public:
    ...
    Linear(ModelContext *ctx, int in_features, int out_features, bool use_bias = true)
        : weight(ne_new_tensor_2d(ctx->ctx_w.get(), ctx->dtype, in_features, out_features, NE_SIZE_CALC)),
          bias(use_bias ? ne_new_tensor_1d(ctx->ctx_w.get(), NE_TYPE_F32, out_features, NE_SIZE_CALC) : nullptr) {}
  ...
  public:
    ne_tensor *weight; // [out_features, in_features]
    ne_tensor *bias;   // [out_features]
};
...
class LayerNorm {
  public:
    ...
    LayerNorm(ModelContext *ctx, int normalized_shape)
        : weight(ne_new_tensor_1d(ctx->ctx_w.get(), NE_TYPE_F32, normalized_shape, NE_SIZE_CALC)),
          bias(ne_new_tensor_1d(ctx->ctx_w.get(), NE_TYPE_F32, normalized_shape, NE_SIZE_CALC)) {}

    ne_tensor *forward(ModelContext *ctx, ne_tensor *input) const;

  public:
    ne_tensor *weight; // [normalized_shape]
    ne_tensor *bias;   // [normalized_shape]
};
...
class RMSNorm {
  public:
    ...
    RMSNorm(ModelContext *ctx, int normalized_shape, bool inplace = true)
        : weight(ne_new_tensor_1d(ctx->ctx_w.get(), NE_TYPE_F32, normalized_shape, NE_SIZE_CALC)), inplace(inplace) {}

    ne_tensor *forward(ModelContext *ctx, ne_tensor *input, float eps = 1e-5f) const;
...
class PerfStreamer : public BaseStreamer {
  public:
    ...
    void put(const std::vector<int> &output_ids) override;
    void end() override { end_us_ = ne_time_us(); }
...
class ModelLoader {
  public:
    ...
    void read_tensor(const std::string &name, ne_tensor *tensor);
...
class BaseModelForConditionalGeneration {
  public:
    ...
    virtual ne_tensor *forward(ModelContext *ctx, ne_tensor *input_ids, int n_past, int n_ctx) const = 0;
  ...
  protected:
    ...
    BaseConfig config_;
    ModelContext ctx_;
    std::vector<std::pair<std::string, ne_tensor *>> state_dict_;
};
...
class GLMMLP {
  public:
    ...
    ne_tensor *forward(ModelContext *ctx, ne_tensor *hidden_states) const;
...
class GLMSelfAttention {
  public:
    ...
    ne_tensor *forward(ModelContext *ctx, ne_tensor *hidden_states, int n_past, int n_ctx) const;

  public:
    Linear query_key_value;
    Linear dense;
    int num_attention_heads;
    ne_tensor *k_cache; // [n_head, maxlen, head_size]
    ne_tensor *v_cache; // [n_head, head_size, maxlen]
};
...
class ChatGLMModel {
  public:
    ...
    ne_tensor *forward(ModelContext *ctx, ne_tensor *input_ids, int n_past, int n_ctx) const;
...
class ChatGLMForConditionalGeneration : public BaseModelForConditionalGeneration {
  public:
    ...
    ne_tensor *forward(ModelContext *ctx, ne_tensor *input_ids, int n_past, int n_ctx) const override;
...
class GLM2SelfAttention {
  public:
    ...
    ne_tensor *forward(ModelContext *ctx, ne_tensor *hidden_states, int n_past) const;
  ...
  public:
    ...
    Linear query_key_value;
    Linear dense;
    ne_tensor *k_cache; // [mqa_n_head, maxlen, head_size]
    ne_tensor *v_cache; // [mqa_n_head, head_size, maxlen]
};
...
class ChatGLM2Model {
  public:
    ...
    ne_tensor *forward(ModelContext *ctx, ne_tensor *input_ids, int n_past) const;
