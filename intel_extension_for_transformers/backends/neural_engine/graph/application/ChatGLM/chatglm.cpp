...
static std::string shape_to_string(ne_tensor *tensor) {
...
static std::string strides_to_string(ne_tensor *tensor) {
    std::ostringstream oss;
    oss << '[';
    for (int i = tensor->n_dims - 1; i >= 0; i--) {
        oss << tensor->nb[i] << (i > 0 ? ", " : "");
...
std::string to_string(ne_tensor *tensor, bool with_data) {
    std::ostringstream oss;
    oss << "ne_tensor(";
                ...
                for (int i1 = 0; i1 < tensor->ne[1]; i1++) {
                    ...
                    for (int i0 = 0; i0 < tensor->ne[0]; i0++) {
                        ...
                        if (tensor->type == NE_TYPE_F32) {
                            ...
                            val = ne_fp16_to_fp32(*(ne_fp16_t *)ptr);
                        } else {
...
void ModelLoader::read_tensor(const std::string &name, ne_tensor *tensor) {
    ...
    {
        ne_type dtype = (ne_type)read_basic<int>();
    ...
    // map tensor data
    {
        ...
        seek(data_offset + ne_nbytes(tensor), SEEK_SET);
    }
}

// ===== modules =====

ne_tensor *Embedding::forward(ModelContext *ctx, ne_tensor *input) const {
    ne_tensor *output = ne_get_rows(ctx->ctx_b.get(), weight, input);
    return output;
}

ne_tensor *Linear::forward(ModelContext *ctx, ne_tensor *input) const {
...
ne_tensor *LayerNorm::forward(ModelContext *ctx, ne_tensor *input) const {
...
ne_tensor *RMSNorm::forward(ModelContext *ctx, ne_tensor *input, float eps) const {
    ne_context *gctx = ctx->ctx_b.get();
...
int BaseModelForConditionalGeneration::generate_next_token(const std::vector<int> &input_ids,
    ...
    ctx_.ctx_b = make_unique_ne_context(ctx_.compute_buffer.size(), ctx_.compute_buffer.data(), false);
    ctx_.gf = {};
    ...
    if (n_threads <= 0) {
        ...
        // if (input_ids.size() >= 32 && ne_cpu_has_blas() && !ne_cpu_has_gpublas()) {
        if (input_ids.size() >= 32 && ne_cpu_has_blas()) {
        n_threads = 1; // use 1 thread if BLAS is enabled
    }

    ne_tensor *input_ids_tensor = ne_new_tensor_1d(ctx_.ctx_b.get(), NE_TYPE_I32, input_ids.size(), NE_SIZE_CALC);
    memcpy(input_ids_tensor->data, input_ids.data(), ne_nbytes(input_ids_tensor));

    ne_tensor *lm_logits = forward(&ctx_, input_ids_tensor, n_past, n_ctx);
...
GLMSelfAttention::GLMSelfAttention(ModelContext *ctx, int hidden_size, int num_attention_heads, int max_length)
    : query_key_value(ctx, hidden_size, 3 * hidden_size), dense(ctx, hidden_size, hidden_size),
      ...
      k_cache(ne_new_tensor_3d(ctx->ctx_kv.get(), NE_TYPE_F16, hidden_size / num_attention_heads, max_length,
                                 num_attention_heads, NE_SIZE_CALC)),
      v_cache(ne_new_tensor_3d(ctx->ctx_kv.get(), NE_TYPE_F16, max_length, hidden_size / num_attention_heads,
                                 num_attention_heads, NE_SIZE_CALC)) {}

ne_tensor *GLMSelfAttention::forward(ModelContext *ctx, ne_tensor *hidden_states, int n_past, int n_ctx) const {
    ne_context *gctx = ctx->ctx_b.get();
    ...
    ne_tensor *qkv = query_key_value.forward(ctx, hidden_states); // [qlen, 3 * hidden]
...
ne_tensor *GLMBlock::forward(ModelContext *ctx, ne_tensor *hidden_states, int n_past, int n_ctx) const {
    ne_context *gctx = ctx->ctx_b.get();

    ne_tensor *alpha = ne_new_f32(gctx, std::sqrt(2.f * num_hidden_layers));

    ne_tensor *attn_input = input_layernorm.forward(ctx, hidden_states);
...
ne_tensor *ChatGLMModel::forward(ModelContext *ctx, ne_tensor *input_ids, int n_past, int n_ctx) const {
    ne_context *gctx = ctx->ctx_b.get();
    ...
    ne_set_scratch(gctx, empty_scratch);
...
ChatGLMForConditionalGeneration::ChatGLMForConditionalGeneration(const ChatGLMConfig &config)
    ...
    constexpr size_t tensor_ovhd = NE_TENSOR_SIZE + NE_OBJECT_SIZE;
    const size_t ctx_w_size = (4 + config.num_hidden_layers * 12) * tensor_ovhd;
    const size_t ctx_kv_size = 2 * config.num_hidden_layers *
                               (config.max_length * config.hidden_size * ne_type_size(NE_TYPE_F16) + tensor_ovhd);
    ctx_.dtype = config.dtype;
    ctx_.ctx_w = make_unique_ne_context(ctx_w_size, nullptr, true);
    ctx_.ctx_kv = make_unique_ne_context(ctx_kv_size, nullptr, false);
    ...
    // CHATGLM_CHECK(ne_used_mem(ctx_.ctx_kv.get()) == ne_get_mem_size(ctx_.ctx_kv.get())) << "corrupted kv cache";
...
ne_tensor *ChatGLMForConditionalGeneration::forward(ModelContext *ctx, ne_tensor *input_ids, int n_past,
                                                      int n_ctx) const {
    ne_tensor *transformer_outputs = transformer.forward(ctx, input_ids, n_past, n_ctx);
...
GLM2SelfAttention::GLM2SelfAttention(ModelContext *ctx, int hidden_size, int num_attention_heads, int num_kv_heads,
    ...
    : num_attention_heads(num_attention_heads), num_kv_heads(num_kv_heads),
      ...
      k_cache(ne_new_tensor_3d(ctx->ctx_kv.get(), NE_TYPE_F16, hidden_size / num_attention_heads, max_length,
                                 num_kv_heads, NE_SIZE_CALC)),
      v_cache(ne_new_tensor_3d(ctx->ctx_kv.get(), NE_TYPE_F16, max_length, hidden_size / num_attention_heads,
                                 num_kv_heads, NE_SIZE_CALC)) {}

ne_tensor *GLM2SelfAttention::forward(ModelContext *ctx, ne_tensor *hidden_states, int n_past) const {
    ne_context *gctx = ctx->ctx_b.get();
    ...
    ne_tensor *qkv = query_key_value.forward(ctx, hidden_states); // [qlen, hidden + 2 * kv_hidden]
...
ne_tensor *GLM2MLP::forward(ModelContext *ctx, ne_tensor *hidden_states) const {
    ne_context *gctx = ctx->ctx_b.get();
...
ne_tensor *GLM2Block::forward(ModelContext *ctx, ne_tensor *hidden_states, int n_past) const {
    ne_context *gctx = ctx->ctx_b.get();

    ne_tensor *residual = hidden_states;
...
ne_tensor *ChatGLM2Model::forward(ModelContext *ctx, ne_tensor *input_ids, int n_past) const {
    ne_context *gctx = ctx->ctx_b.get();
    ne_tensor *hidden_states = word_embeddings.forward(ctx, input_ids);
...
ChatGLM2ForConditionalGeneration::ChatGLM2ForConditionalGeneration(const ChatGLM2Config &config)
    ...
    const size_t ctx_kv_size = 2 * config.num_hidden_layers *
                               (config.max_length * config.hidden_size / config.num_attention_heads *
                                    config.num_kv_heads * ne_type_size(NE_TYPE_F16) +
                                tensor_ovhd);
    ctx_.dtype = config.dtype;
    ctx_.ctx_w = make_unique_ne_context(ctx_w_size, nullptr, true);
    ctx_.ctx_kv = make_unique_ne_context(ctx_kv_size + 1 * MB, nullptr, false); // 1MB extra for MPS
...
ne_tensor *ChatGLM2ForConditionalGeneration::forward(ModelContext *ctx, ne_tensor *input_ids, int n_past,
                                                       int n_ctx) const {
    ne_tensor *transformer_outputs = transformer.forward(ctx, input_ids, n_past);
    // NOTE: only compute next_token_logits for the last token
    if (input_ids->ne[0] > 1) {
        transformer_outputs =
            ne_view_1d(ctx->ctx_b.get(), transformer_outputs, config.hidden_size,
