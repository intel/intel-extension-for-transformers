...
struct Allocator {
  ...
  virtual uint8_t* alloc(size_t size) {
    return reinterpret_cast<uint8_t*>(AlignedMalloc(size, inner::ALIGN_PAGE_SIZE));
  }
...
class CodeArray {
  ...
  struct AddrInfo {
    ...
    uint64_t getVal(const uint8_t* top) const {
      uint64_t disp =
          (mode == inner::LaddTop) ? jmpAddr + size_t(top) : (mode == inner::LasIs) ? jmpAddr : jmpAddr - size_t(top);
