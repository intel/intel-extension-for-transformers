...
void dynamic_quant_bench::gen_case() {
  ...
  void* src = malloc(fp32_vec.size() * get_data_size(src_dt));
  if (src_dt == jd::data_type::fp32) {
    memcpy(src, fp32_vec.data(), fp32_vec.size() * sizeof(float));
  } else {
    cast_from_float_array<bfloat16_t>(fp32_vec, src, fp32_vec.size());
  }
  ...
  operator_desc op_desc(kernel_kind::dynamic_quant, kernel_prop::forward_inference, engine_kind::cpu, ts_descs,
                        op_attrs);
