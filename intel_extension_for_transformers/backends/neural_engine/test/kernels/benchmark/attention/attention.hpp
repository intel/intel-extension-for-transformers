...
class attention_bench : public kernel_bench {
 ...
 public:
  ...
  double calc_flop() const override {
    ...
    int oc = ts_descs[attention_io::V_WEIGHT].shape()[0];
    int ic = ts_descs[attention_io::V_WEIGHT].shape()[1];
    const int other_dim =
        std::accumulate(ts_descs[attention_io::MERGE_SRC].shape().begin(),
                        ts_descs[attention_io::MERGE_SRC].shape().end(), 1, std::multiplies<size_t>()) /
        ic;
    ...
    FLOPs += static_cast<double>(seq_len) * seq_len * head_num * ts_descs[attention_io::MERGE_DST].shape()[0] *
             ts_descs[attention_io::MERGE_DST].shape()[1] * 2;
    return FLOPs;
  }
  std::vector<int> get_refresh_data_idx() const override {
    return std::vector<int>{attention_io::MERGE_SRC, attention_io::MERGE_DST};
  }
