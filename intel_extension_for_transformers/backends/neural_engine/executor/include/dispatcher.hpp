...
class Dispatcher {
 public:
  ...
  explicit Dispatcher(const shared_ptr<OperatorConfig>& conf,
                      const ExecutionOptions* e_ptr, const Model* m_ptr);

  explicit Dispatcher(const shared_ptr<Operator>& op,
                      const ExecutionOptions* e_ptr, const Model* m_ptr);
  ...
  // need to adjust.
  void AdaptAttrs(const vector<Tensor*>& input, const vector<Tensor*>& output,
                  const string& stage);

  // modify tensors when dispatched kernel needs different format, etc.
  // should call it before (in) or after (out) Forward
  void AdaptTensors(const vector<Tensor*>& input, const vector<Tensor*>& output,
                    const string& stage);

  // reset op like tensor format after finishing inference iteration
  void ResetOpStatus(const vector<Tensor*>& input,
                     const vector<Tensor*>& output);

  void GetExecuteKernel(const vector<Tensor*>& input,
                        const vector<Tensor*>& output,
  ...
  inline const shared_ptr<OperatorConfig>& operator_conf() const {
    return operator_conf_;
  }
  ...
  // for profiling
  inline void set_post_op(const string& post_op) {
    kernel_handler_[execute_kernel_]->set_post_op(post_op);
  }
  inline const string& post_op() {
    return kernel_handler_[execute_kernel_]->post_op();
  }
  inline void set_latency(const float latency) {
    kernel_handler_[execute_kernel_]->set_latency(latency);
  }
  inline const vector<float>& latency() {
    return kernel_handler_[execute_kernel_]->latency();
  }
  inline void set_enable_sparse(const bool enable_sparse) {
    kernel_handler_[execute_kernel_]->set_enable_sparse(enable_sparse);
  }
  inline const float& enable_sparse() {
    return kernel_handler_[execute_kernel_]->enable_sparse();
  }
  inline const KERNEL_TYPE& kernel_type() {
    return kernel_handler_[execute_kernel_]->kernel_type();
  }
  inline const float& weight_zero_ratio() {
    return kernel_handler_[execute_kernel_]->weight_zero_ratio();
  }
  ...
  inline const vector<int64_t>& weight_shape() {
    return kernel_handler_[execute_kernel_]->weight_shape();
  }
  inline void set_table_id(string table_id) {
    kernel_handler_[execute_kernel_]->set_table_id(table_id);
  }
  inline const string& table_id() {
    return kernel_handler_[execute_kernel_]->table_id();
  }
  inline void set_perf_ratio_id(string perf_ratio_id) {
    kernel_handler_[execute_kernel_]->set_perf_ratio_id(perf_ratio_id);
  }
  inline const string& perf_ratio_id() {
    return kernel_handler_[execute_kernel_]->perf_ratio_id();
  }
  ...
  inline const std::map<string, string>& get_attrs() {
    return kernel_handler_[execute_kernel_]->get_attrs();
  }
