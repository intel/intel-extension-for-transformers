...
ResamplingOperator::ResamplingOperator(const shared_ptr<OperatorConfig>& conf): Operator(conf) {
  auto attrs_map = operator_conf_->attributes();
  auto iter = attrs_map.find("scales");
  if (iter != attrs_map.end())
    StringSplit<int64_t>(&scales_, attrs_map["scales"], ",");
}

void ResamplingOperator::Prepare(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  // dnnl resampling supports f32 / bf16 / f16 / s32 / s8 / u8
  output[0]->set_dtype(input[0]->dtype());
}

void ResamplingOperator::Reshape(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  int dims = input[0]->shape().size();
  if (scales_.size() != dims) {
    LOG(ERROR) << "The dims of scales and input are not the same. " << \
    input[0]->shape().size() << "scales dims = " << scales_.size();
  }

  for (int idx = 0; idx < dims; ++idx) {
    if (scales_[idx] == 1) {
      dst_shape.push_back(input[0]->shape()[idx]);
      continue;
    }
    int extend_shape = scales_[idx] * input[0]->shape()[idx];
    dst_shape.push_back(extend_shape);
  }
...
void ResamplingOperator::Forward(const vector<Tensor*>& input, const vector<Tensor*>& output) {
...
REGISTER_OPERATOR_CLASS(Resampling);
}  // namespace executor
