...
MultiHeadAttenionOperator::MultiHeadAttenionOperator(const shared_ptr<OperatorConfig>& conf)
  ...
  iter = attrs_map.find("is_package");
  if (iter != attrs_map.end()) {
    is_package_ = attrs_map["is_package"] == "true";
  }
  if (dst_reshape_.size() > 0 && dst_reshape_[0] != -1) {
    is_sparse_ = true;
    int max_threads = std::min(32, omp_get_max_threads());
    trans_mha_tmpbuf = (uint8_t*)aligned_alloc(64, max_threads * Size2M);
  }
}

MultiHeadAttenionOperator::~MultiHeadAttenionOperator() {
  if (is_sparse_) {
    aligned_free(trans_mha_tmpbuf);
  }
}
...
void MultiHeadAttenionOperator::ReshapeDense(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  attr_map["Q_scale"] = std::to_string(Q_scale_);
  attr_map["K_scale"] = std::to_string(K_scale_);
  attr_map["V_scale"] = std::to_string(V_scale_);
  attr_map["DST_scale"] = std::to_string(DST_scale_);
  attr_map["QK_output_scale"] = std::to_string(output_scale_);
  attr_map["is_package"] = is_package_ ? "True" : "False";
...
void MultiHeadAttenionOperator::ForwardDense(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  int real_bs = att_mask_->shape()[0];
  rt_data_ = {Q_data, K_data, V_data, att_mask_data, dst_data, &real_bs};
...
void MultiHeadAttenionOperator::ReshapeSparse(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  jd::operator_desc trans_attention_desc(jd::kernel_kind::transpose_attention, jd::kernel_prop::forward_inference,
                                         jd::engine_kind::cpu, ts_descs, op_attrs);
  jd::transpose_attention_desc transpose_attention_desc(trans_attention_desc);
  mha_transpose_ = jd::transpose_attention(transpose_attention_desc);
...
void MultiHeadAttenionOperator::ForwardSparse(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  rt_data_ = {K_data,     Q_data,      att_mask_data, V_data,  dst_data, trans_mha_tmpbuf, &seq_len_, &bs_,
              &head_num_, &head_size_, &seq_len_,     &scaleQ, &scaleK,  &scaleV,          &scaleRet, &QKV_zeropoint_};
