...
std::unordered_map<string, jd::data_type> Operator::type_2_sparsemem = {
...
template <int N>
void logsoftmax_u8(void* out, void* in, const float oscale, const int64_t ld) {
...
#pragma unroll N
    for (int i = 0; i < N; ++i) {
      ...
      // Clip [0, 255]
      auto logsoftmax_f32_clip_255 = _mm512_min_ps(logsoftmax_f32, __255);
      auto logsoftmax_f32_clip_0 = _mm512_max_ps(logsoftmax_f32_clip_255, __0);
      auto logsoftmax_s32 = _mm512_cvtps_epi32(logsoftmax_f32_clip_0);
      _mm512_mask_cvtusepi32_storeu_epi8(pout + i * ld + d, 0xffff, logsoftmax_s32);
    }
  }
...
#pragma unroll N
    for (int i = 0; i < N; ++i) {
      ...
      auto logsoftmax_f32_clip_0 = _mm512_max_ps(logsoftmax_f32_clip_255, __0);
      auto logsoftmax_s32 = _mm512_cvtps_epi32(logsoftmax_f32_clip_0);
      _mm512_mask_cvtusepi32_storeu_epi8(pout + i * ld + d, res_mask, logsoftmax_s32);
    }
  }
...
static inline void logsoftmax_int_kernel(uint8_t* out, float* in, float oscale, int64_t ld, int l) {
  switch (l) {
    case 1:
      return logsoftmax_u8<1>(out, in, oscale, ld);
    case 2:
      return logsoftmax_u8<2>(out, in, oscale, ld);
    case 3:
      return logsoftmax_u8<3>(out, in, oscale, ld);
    case 4:
      return logsoftmax_u8<4>(out, in, oscale, ld);
    case 5:
      return logsoftmax_u8<5>(out, in, oscale, ld);
    case 6:
      return logsoftmax_u8<6>(out, in, oscale, ld);
    case 7:
      return logsoftmax_u8<7>(out, in, oscale, ld);
    case 8:
      return logsoftmax_u8<8>(out, in, oscale, ld);
    case 9:
      return logsoftmax_u8<9>(out, in, oscale, ld);
    case 10:
      return logsoftmax_u8<10>(out, in, oscale, ld);
    case 11:
      return logsoftmax_u8<11>(out, in, oscale, ld);
    case 12:
      return logsoftmax_u8<12>(out, in, oscale, ld);
    case 13:
      return logsoftmax_u8<13>(out, in, oscale, ld);
    case 14:
      return logsoftmax_u8<14>(out, in, oscale, ld);
    case 15:
      return logsoftmax_u8<15>(out, in, oscale, ld);
    case 16:
      return logsoftmax_u8<16>(out, in, oscale, ld);
  }
  ...
  logsoftmax_int_kernel(pout, pin, oscale, ld, l1);
  logsoftmax_int_kernel(pout + l1 * ld, pin + l1 * ld, oscale, ld, l2);
}
#endif
...
LogSoftmaxOperator::LogSoftmaxOperator(const shared_ptr<OperatorConfig>& conf) : Operator(conf) {
  ...
  if (attrs_map.find("lut_logsoftmax") != attrs_map.end()) {
...
void LogSoftmaxOperator::MapTensors(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  switch (input_size) {
    ...
    default: {
      // LOG(ERROR) << "Input size in Logsoftmax is: " << input_size << ", not supported!";
      src_ = input[0];
      dst_min_ = input[1];
      dst_max_ = input[2];
    }
...
void LogSoftmaxOperator::Reshape(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  if (output_dtype_ == "fp32" || output_dtype_ == "bf16" || is_dynamic_) {
    ...
    if (input[0]->dtype() != "u8" && input[0]->dtype() != "s8") {
      LOG(ERROR) << "LUT logsoftmax only support int8 input dt.";
    }
    Reshape_Sparselib(input, output);
  } else if (output_dtype_ == "u8") {
    Reshape_u8(input, output);
  } else {
    LOG(ERROR) << "Output dtype in Logsoftmax is: " << output_dtype_ << ", not supported!";
  }
}
...
void LogSoftmaxOperator::Reshape_Sparselib(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  if (lut_optimization_) {
    ...
    jd::operator_desc op_desc(jd::kernel_kind::logsoftmax, jd::kernel_prop::forward_inference, jd::engine_kind::cpu,
                              ts_descs, op_attrs, {dequantize_attr, quantize_attr});
    jd::logsoftmax_desc logsoftmax_desc(op_desc);
    logsoftmax_ker_ = jd::logsoftmax(logsoftmax_desc);
  }
}
...
void LogSoftmaxOperator::Forward_dnnl(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  if (output.size() > 1) {
    // quantize the fp32 result of logsoftmax
...
#pragma omp parallel for
  for (auto d0 = 0; d0 < dim0; ++d0) {
    ...
    logsoftmax_int_kernel(p_out, p_in, dst_scales_[0], dim2, dim1);
  }
...
void LogSoftmaxOperator::Forward_Sparselib(const vector<Tensor*>& input, const vector<Tensor*>& output) {
  ...
  if (lut_optimization_) {
    ...
    logsoftmax_ker_.execute(runtime_data);
  }
  this->unref_tensors(input);
}
