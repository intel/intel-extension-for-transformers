        ...
        if layer_config is None or i in layer_config:
            if length_drop_prob is not None:
                ...
                length = np.random.randint(
                    int(np.ceil(length * (1 - length_drop_ratio_bound))), length + 1
                )
...
def sample_layer_configuration(
    ...
    if layer_dropout_prob is not None:
        return tuple(
            i
            for i in range(num_hidden_layers)
            if np.random.random() >= layer_dropout_prob
        )
        ...
        return tuple(
            range(num_hidden_layers - np.random.randint(0, layer_dropout_bound + 1))
        )
    return None
