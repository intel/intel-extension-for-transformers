...
class AutoDistillation(NASBase):
    ...
    def search(self, res_save_path=None, model_cls=None):
        ...
        for i in range(self.max_trials):
            ...
            if self.framework == "pytorch":
                ...
                if torch.distributed.is_initialized():
                    model_arch_paras_sync = (
                        [model_arch_paras]
                        if torch.distributed.get_rank() == 0
                        else [None]
                    )
                    torch.distributed.broadcast_object_list(
                        model_arch_paras_sync, src=0
                    )
        ...
        if (
            ...
            self.dump_search_results(
                os.path.join(save_path, "Final_results.txt".format())
            )
    ...
    def init_by_cfg(self, conf_fname_or_obj):
        ...
        if isinstance(conf_fname_or_obj, str):  # pragma: no cover
            if os.path.isfile(conf_fname_or_obj):
                ...
                raise FileNotFoundError(
                    "{} is not a file, please provide a file path.".format(
                        conf_fname_or_obj
                    )
                )
        ...
        assert (
            self.config.auto_distillation is not None
    ...
    def create_distillers(self):
        ...
        def create_tf_distiller(distillation_cfg):
            block_names = (
                distillation_cfg.block_names if distillation_cfg.block_names else [""]
            )
            train_steps = (
                distillation_cfg.train_steps if distillation_cfg.train_steps else [500]
            )
            conf = DotDict(
                {
                    "model": {
                        "name": "flash_distillation_{}".format(0),
        ...
        def create_distiller(distillation_cfg):
            ...
            layer_mappings = distillation_cfg.layer_mappings_for_knowledge_transfer
            block_names = (
                distillation_cfg.block_names
                if distillation_cfg.block_names
                else [""] * len(layer_mappings)
            )
            loss_types = (
                distillation_cfg.loss_types
                if distillation_cfg.loss_types
                else [["MSE"] * len(_) for _ in layer_mappings]
            )
            loss_weights = (
                distillation_cfg.loss_weights
                if distillation_cfg.loss_weights
                else [[1.0 / len(_)] * len(_) for _ in layer_mappings]
            )
            add_origin_loss = (
                distillation_cfg.add_origin_loss
                if distillation_cfg.add_origin_loss
                else [False] * len(layer_mappings)
            )
            train_steps = (
                distillation_cfg.train_steps
                if distillation_cfg.train_steps
                else [500] * len(layer_mappings)
            )
            temperatures = (
                distillation_cfg.temperatures
                if distillation_cfg.temperatures
                else [1.0] * len(layer_mappings)
            )
            assert (
                len(layer_mappings)
                == len(block_names)
            ...
            for i, lm in enumerate(layer_mappings):
                conf = DotDict(
                    {
                        "model": {
                            "name": "flash_distillation_{}".format(i),
                            "framework": self.framework,
                        },
                        "distillation": {
                            "train": {
        ...
        if flash_distillation_config.knowledge_transfer:
            ...
            if self.framework == "pytorch":
                (
                    self.flash_distillers,
                    self.flash_block_names,
                    self.flash_train_steps,
        ...
        if flash_distillation_config.regular_distillation:
            ...
            if self.framework == "pytorch":
                (
                    self.regular_distillers,
                    self.regular_block_names,
                    self.regular_train_steps,
