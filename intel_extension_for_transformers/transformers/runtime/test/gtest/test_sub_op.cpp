...
bool CheckResult(const TestParams& t) {
  ...
  try {
    ...
    executor::LLGAOPCreator::GetInstance().CreateSubtractOp(&llga_info, p.conf);
    executor::LLGAKernel sub(p.conf, &llga_info);
    sub.Prepare(p.input, p.output);
    sub.Reshape(p.input, p.output);
    sub.Forward(p.input, p.output);
...
class SubTest : public testing::TestWithParam<TestParams> {
 protected:
  SubTest() {}
  ~SubTest() {}
  void SetUp() override {}
...
std::pair<OpArgs, OpArgs> GenerateFp32Case(const std::vector<std::vector<int64_t>>& input_shape) {
  ...
  auto op_config = std::make_shared<OperatorConfig>("sub", "Sub", input_config, output_config, op_attr);
...
INSTANTIATE_TEST_SUITE_P(Prefix, SubTest, CasesFp32());
