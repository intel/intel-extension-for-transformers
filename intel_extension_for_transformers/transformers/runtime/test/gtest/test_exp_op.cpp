...
#include "../../include/operators/exp.hpp"
...
bool CheckResult(const TestParams& t) {
  ...
  try {
    executor::ExpOperator exp(p.conf);
    exp.Prepare(p.input, p.output);
    exp.Reshape(p.input, p.output);
    exp.Forward(p.input, p.output);
...
class ExpTest : public testing::TestWithParam<TestParams> {
 protected:
  ExpTest() {}
  ~ExpTest() {}
  void SetUp() override {}
...
std::pair<OpArgs, OpArgs> GenerateFp32Case(const std::vector<std::vector<int64_t>>& input_shape) {
  ...
  shared_ptr<OperatorConfig> op_config = std::make_shared<OperatorConfig>("exp", "Exp",
...
INSTANTIATE_TEST_SUITE_P(Prefix, ExpTest, CasesFp32());
