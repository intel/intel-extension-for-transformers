...
bool layernormalized_spmm_k_t::init() {
  ...
  if (!kernel_desc_t::create<spmm_vnni_kd_t>(spmm_ker_desc, *(kd->spmm_desc))) return false;
  if (!create<spmm_vnni_k_t, spmm_vnni_kd_t>(kernels_[0], spmm_ker_desc)) return false;
  if (!kernel_desc_t::create<layernorm_ba_kd_t>(lnorm_ker_desc, *(kd->lnorm_desc))) return false;
