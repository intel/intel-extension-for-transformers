    ...
    def from_pretrained(cls, model_name_or_path: str, **kwargs):
        ...
        if config.torch_dtype is not torch.int8:
            ...
            if not os.path.isdir(model_name_or_path) and not os.path.isfile(
                ...
                q_model = (
                    load(resolved_weights_file, model, dataloader=dataloader)
                    if Version(__version__) > Version("1.13")
                    else load(resolved_weights_file, model)
                )
            else:
                ...
                q_model = (
                    load(weights_file, model, dataloader=dataloader)
                    if Version(__version__) > Version("1.13")
                    else load(weights_file, model)
                )
