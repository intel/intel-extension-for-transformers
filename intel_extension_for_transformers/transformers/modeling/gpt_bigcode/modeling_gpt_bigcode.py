...
class GPTBigCodeAttention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        attn_weights = scale_factor * torch.matmul(
            query, key
        )  # + beta * attn_weights (not needed, it is 0)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            query, key, value = self.c_attn(hidden_states).split(
                (self.embed_dim, self.kv_dim, self.kv_dim), dim=2
            )
        else:
...
class GPTBigCodeForSequenceClassification(GPTBigCodePreTrainedModel):
    ...
    def forward(
        ...
        r"""Labels (`torch.Tensor` of shape `(batch_size,)`, *optional*):
