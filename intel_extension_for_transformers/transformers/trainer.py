...
from .dynamic.evolution import Evolution, approx_ratio, inverse, store2str
...
class BaseTrainer:
    ...
    def builtin_eval_func(self, model):
        ...
        logger.info(
            "Throughput: {} samples/sec".format(results.get("eval_samples_per_second"))
        )
        return result
    ...
    def builtin_train_func(self, model):
        ...
        train_result = self.train(
            component=self.component, resume_from_checkpoint=self._resuming_checkpoint
        )
    ...
    def init_quantizer(
        ...
        if self.quant_config.framework == "pytorch":
            if (
                self.quant_config.approach != QuantizationMode.POSTTRAININGDYNAMIC.value
    ...
    def _save_inc_int8(self, opt_model, output_dir):
        ...
        logger.info(
            "quantized model and configure file have saved to {}".format(output_dir)
        )
    ...
    def prune(
        ...
        if self._eval_func is not None:
            ...
            assert (
                self.metrics is not None
            ), "Please pass metrics to trainer.pruning.metrics!"
            assert (
                self.pruning_config.pruner_config[0].prune_type
                == PruningMode.BASICMAGNITUDE.value
    ...
    def orchestrate_optimizations(
        ...
        """
        from intel_extension_for_transformers.transformers.optimizer import (
        ...
        self.orchestrate_optimizer = Orchestrate_optimizer(
            self.model,
            components,
    ...
    def create_optimizer_builtin(self, config_list, teacher_model=None):
        ...
        for config in config_list:
            if isinstance(config, QuantizationConfig):
                ...
                assert False, (
                    "Orchestrate_optimizations config_list requires at least one"
        ...
        **kwargs,
        ...
        if isinstance(component, Component):
            ...
            component.pre_epoch_begin(
                self.calib_dataloader if self.calib_dataloader else None
            )
            if (
                component.combination is not None
                and "Quantization" in component.combination
            ):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                training_step = (
                    self.training_step_length_adaptive
                    if self.dynamic_config is not None
                    and self.dynamic_config.dynamic_training
    ...
    def training_step_length_adaptive(
        ...
        for i in range(self.dynamic_config.num_sandwich + 1):
            ...
            layer_config = sample_layer_configuration(
                ...
                layer_dropout=(
                    self.dynamic_config.layer_dropout_bound if i == 0 else None
                ),
            ...
            length_config = sample_length_configuration(
                ...
                layer_config,
                length_drop_ratio=(
                    self.dynamic_config.length_drop_ratio_bound if i == 0 else None
                ),
            ...
            if version.parse(__version__) < version.parse("4.20"):
                if self.use_amp:
                    ...
                    with autocast():
                        _, outputs_sub = self.compute_loss(
                            model, inputs, return_outputs=True
                        )
                else:
                    _, outputs_sub = self.compute_loss(
            ...
            start_kl_loss = loss_fct(
                F.log_softmax(start_logits, -1), F.softmax(start_logits_sub, -1)
            )
            end_kl_loss = loss_fct(
                F.log_softmax(end_logits, -1), F.softmax(end_logits_sub, -1)
            )
    ...
    def compute_loss(self, model, inputs, return_outputs=False):  # pragma: no cover
        ...
        teacher_logits = (
            inputs.pop("teacher_logits") if "teacher_logits" in inputs else None
        )
        ...
        if (
            self.in_training
            and hasattr(self, "component")
            and hasattr(self.component, "criterion")
        ):
                ...
                if isinstance(outputs, dict):
                    if "logits" in outputs:
                        ...
                        raise AssertionError(
                            "Logits of outputs not included, can't compute loss"
                        )
            ...
            if teacher_logits is not None:
                if (
                    "start_positions" in inputs and "end_positions" in inputs
                ):  # for SQuAD
            ...
            if version.parse(nc_version) <= version.parse("1.12"):
                ...
                if hasattr(self.component, "on_post_forward"):
                    self.component.on_post_forward(
                        inputs, teacher_output=teacher_logits
                    )
                    if hasattr(self.component.criterion, "teacher_outputs"):
                        self.component.criterion.teacher_outputs = get_logits(
                            self.component.criterion.teacher_outputs
                        )
                loss = self.component.criterion(logits, labels)
                if (
                    hasattr(self.component.criterion, "add_origin_loss")
                    and self.component.criterion.add_origin_loss
                ):
                ...
                loss = self.component.on_after_compute_loss(
                    inputs, logits, loss, teacher_logits
                )
    ...
    def nas(
        ...
        def take_eval_steps(model, trainer, metric_names, save_metrics=False):
            ...
            metric_names = (
                ["eval_loss", "latency"] if metric_names is None else metric_names
            )
            ...
            return {
                metric_name: metrics.get(metric_name) for metric_name in metric_names
            }

        def train_func_builtin(model):
            return take_train_steps(model, trainer=self)

        def eval_func_builtin(model):
            return take_eval_steps(
                model, trainer=self, metric_names=agent.metrics, save_metrics=True
            )
        ...
        def train_func_builtin(model):
            ...
            def run_distillers(
                ...
                for i, elements in enumerate(
                    zip(distillers, train_steps, block_names, checkpoints)
                ):
                    start_time = time.time()
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if (
                ...
                metrics = self.compute_metrics(
                    EvalPrediction(
                        predictions=nested_numpify(logits),
    ...
    def export_to_int8_onnx(
        ...
        if self.enable_executor:  # pragma: no cover
            ...
            if "dynamic" in self.opt_model.q_config["approach"]:
                ...
                if quant_format == "QDQ":
                    ...
                    addition_op_to_quantize = list(
                        ortq.registry.QLinearOpsRegistry.keys()
                    )
        ...
        if version.parse(__version__) > version.parse("4.21.0"):
            ...
            for node in model.graph.node:
                if node.op_type == "Constant" and len(node.attribute) != 0:
                    ...
                    if (
                        constant_value.shape == ()
                        and constant_value == torch.finfo(torch.float32).min
                    ):
        ...
        if self.opt_model.q_config["approach"] == "quant_aware_training":
            ...
            for name, param in model_dict.items():
                ...
                if "_packed_params._packed_weight" in name:
                    name = name.replace("._packed_params._packed_weight", "").split(
                        ".module"
                    )[0]
                    int8_model_dict[name + ".weight"] = param.dequantize()
                # '_packed_params._packed_params' is specific for quantized Linear
                elif "_packed_params._packed_params" in name and isinstance(
                    param, tuple
                ):
                    name = name.replace("._packed_params._packed_params", "").split(
                        ".module"
                    )[0]
        ...
        if (
            scale_mapping
            and self.opt_model.q_config["approach"] != "post_training_dynamic_quant"
        ):  # pragma: no cover
            ...
            for name, module in self.opt_model.model.named_modules():
                if (
                    isinstance(module, q_modules.Conv1d)
                    or isinstance(module, q_modules.Conv2d)
                    or isinstance(module, q_modules.Linear)
                ):
            ...
            if node.op_type not in op_types_to_quantize:
                for inp in node.input:
                    if inp in weight_name_list and "weight" in inp:
                        tmp_node_mapping.update(
                            {node.output[0]: inp.split(".weight")[0]}
                        )
        ...
        for k, v in self.opt_model.q_config["op"].items():  # pragma: no cover
            if (
                k[1] not in pytorch_op_types_to_quantize
                or "int8" in v["weight"]["dtype"]
            ):
                continue
        ...
        # Quantization
        quant_format = (
            ortq.QuantFormat.QOperator
            if quant_format != "QDQ"
            else ortq.QuantFormat.QDQ
        )
        ...
        if "U8U8" in dtype:  # pragma: no cover
            ...
            if not self.enable_executor:
                logger.error(
                    "Right now, we don't support dtype: {}, please use \
            ...
            logger.error(
                "Right now, we don't support dtype: {}, \
                          please use U8U8/U8S8/S8S8.".format(
                    dtype
                )
            )
            sys.exit(0)
            ...
            if scale_mapping:  # pragma: no cover
                ...
                for tensor in tensor_list:
                    if tensor.name in scale_zp_dict:
                        ...
                        if (
                            "zero_point" in tensor.name
                            and activation_type == ortq.QuantType.QInt8
                        ):
                            value -= 128
                        new_tensor = helper.make_tensor(
                            name=tensor.name,
    ...
    def get_export_args(self, model):
        ...
        if (
            model.__class__.__name__ == "XLNetForSequenceClassification"
        ):  # pragma: no cover
    ...
    def benchmark(
        ...
        **kwargs,
        ...
        benchmark(
            model_name_or_path, config, example_inputs=None, dataloader=eval_dataloader
        )
    ...
    def set_dynamic_config(
        ...
        if self.dynamic_config.length_config is not None:
            ...
            assert (
                self.dynamic_config.max_length is not None
            ), """
    ...
    def run_evolutionary_search(self):
        ...
        assert (
            self.dynamic_config is not None
        ), """
        ...
        length_drop_ratios = [
            ...
            for r in np.linspace(
                approx_ratio(self.dynamic_config.length_drop_ratio_bound),
                1,
                self.dynamic_config.population_size + 2,
            )[1:-1]
        ]
        ...
        for i in range(self.dynamic_config.evo_iter + 1):
            ...
            evolution.save_store(
                os.path.join(self.args.output_dir, f"store-iter{i}.tsv")
            )
            evolution.save_population(
                os.path.join(self.args.output_dir, f"population-iter{i}.tsv"),
                population,
            )
            evolution.save_population(
                os.path.join(self.args.output_dir, f"parents-iter{i}.tsv"), parents
            )
...
class NLPSeq2SeqTrainer(BaseTrainer, Seq2SeqTrainer):
    ...
    def builtin_eval_func(self, model):
        ...
        assert (
            ...
            """
        logger.info(
            "max_length = {}, num_beams = {}".format(self.max_length, self.num_beams)
        )
        self.model = model
        # pylint: disable=E1101
        if self.args.seed:
