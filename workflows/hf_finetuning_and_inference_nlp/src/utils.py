...
class Arguments:
    ...
    smoke_test: Optional[bool] = field(
        default=False, metadata={"help": "Whether to execute in sanity check mode."}
    )
    ...
    dataset: Optional[str] = field(
        default="imdb",
        metadata={"help": "Select dataset ('imdb' / 'sst2'). Default is 'imdb'"},
    )
    ...
    profiler: int = field(
        default=0,
        metadata={"help": "whether using pytorch profiler"},
    )
    profiler_name: str = field(
        default="test",
        metadata={"help": "log name for pytorch profiler"},
    )
    ...
    use_tpp: bool = field(
        default=False,
        metadata={"help": "Use TPP Extension for PyTorch for fine-Tuning."},
    )
    unpad: bool = field(
        default=False,
        metadata={"help": "Use TPP Extension for PyTorch for fine-Tuning."},
    ...
    multi_instance: bool = field(
        default=False,
        metadata={"help": "Whether to use multi-instance mode"},
    )
    ...
    save_detailed_performance_metrics: bool = field(
        default=True, metadata={"help": "Whether to save performance metrics or not"}
    )
...
def save_performance_metrics(trainer, data, output_file):
    ...
    predictions_report = {}
    predictions_report["label_id"] = [
        label_map[i] for i in predictions.label_ids.tolist()
    ]
    predictions_report["predictions_label"] = [
        label_map[i] for i in np.argmax(predictions.predictions, axis=1).tolist()
    ]
    predictions_report["predictions_probabilities"] = softmax(
