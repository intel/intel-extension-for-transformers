...
class TextToSpeech:
    ...
    def __init__(self):
        ...
        with tempfile.TemporaryFile(
            dir=os.path.join("/tmp", self.spk_model_name), mode="w+"
        ) as file:
        ...
        self.vocoder.eval()
        self.default_speaker_embedding = torch.load(
            "speaker_embeddings/spk_embed_default.pt"
        ...
        if os.path.exists("finetuned_model_1000_125_few_shot.pt"):
            self.demo_model = torch.load(
                "finetuned_model_1000_125_few_shot.pt", map_location=torch.device("cpu")
            )
        ...
        if os.path.exists("speaker_embeddings/spk_embed_male.pt"):
            self.male_speaker_embeddings = torch.load(
                "speaker_embeddings/spk_embed_male.pt"
            )
    ...
    def create_speaker_embedding(self, driven_audio_path):
        ...
        """
        audio_dataset = Dataset.from_dict({"audio": [driven_audio_path]}).cast_column(
    ...
    def text2speech(self, text, voice="default"):
        ...
        print(
            f"text to speech in {time.time() - start} seconds, and dump the video at {output_video_path}"
        )
