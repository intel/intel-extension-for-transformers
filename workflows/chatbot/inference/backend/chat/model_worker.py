...
async def api_generate_stream(request: Request):
    ...
    return StreamingResponse(
        generator, background=background_tasks, media_type="text/event-stream"
    )
...
async def talkingbot(request: Request):
    ...
    input_tokens = worker.tokenizer.batch_encode_plus(
        [prompt], return_tensors="pt", padding=True
    )
    input_token_len = input_tokens.input_ids.shape[-1]

    stop = StopOnTokens(
        min_length=44, start_length=input_token_len, stop_token_id=stop_token_ids
    )
    ...
    generated_texts = worker.tokenizer.decode(
...
def validate_device(value):
    ...
    if value in valid_devices:
        ...
        raise argparse.ArgumentTypeError(
            f"Invalid device. Must be one of {', '.join(valid_devices)}."
        )
...
def validate_limit_model_concurrency(value):
    if value >= 0:
        ...
        raise argparse.ArgumentTypeError(
            "Limit model concurrency must be a non-negative integer."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--worker-address", type=str, default="http://localhost:8080")
    parser.add_argument("--controller-address", type=str, default="http://localhost:80")
    parser.add_argument("--model-path", type=str, default="facebook/opt-350m")
    parser.add_argument("--model-name", type=str)
    parser.add_argument(
        "--device", type=validate_device, choices=["cpu", "cuda", "mps"], default="cpu"
    )
    parser.add_argument("--num-gpus", type=int, default=1)
    parser.add_argument("--load-8bit", action="store_true")
    parser.add_argument(
        "--limit-model-concurrency", type=validate_limit_model_concurrency, default=5
    )
