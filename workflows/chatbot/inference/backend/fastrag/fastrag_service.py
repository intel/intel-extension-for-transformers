...
logger = build_logger("fastrag_service", "fastrag_service.log")
        ...
        for dirpath, dirnames, filenames in os.walk(folder_path):
            for filename in filenames:
                if filename.endswith(".json"):
                    documents = doc_index.d_load_jsonl_file(
                        os.path.join(dirpath, filename), process_content, documents
                    )
                elif filename.endswith(".xlsx"):
                    documents = doc_index.d_load_xlsx(
                        os.path.join(dirpath, filename), process_content
                    )
                else:
                    print(
                    ...
                    continue
        doc_index.persist_embedding(
            documents, temp_dir, model_path="shibing624/text2vec-large-chinese"
        )
        doc_index.persist_embedding(
            documents, temp_dir, model_path="hkunlp/instructor-large"
        )
...
if not os.path.exists("/tmp/ask_gm_dense_retrieval_chinese") and not os.path.exists(
    "/tmp/ask_gm_dense_retrieval_english"
):
...
reranker = SentenceTransformersRanker(
    model_name_or_path="cross-encoder/ms-marco-MiniLM-L-6-v2", batch_size=32, top_k=1
)
...
with tempfile.TemporaryDirectory(dir="/tmp/my_subdirectory") as temp_dir:
    english_embeddings = HuggingFaceInstructEmbeddings(
        model_name="hkunlp/instructor-large"
    )
    chinese_embeddings = HuggingFaceInstructEmbeddings(
        model_name="shibing624/text2vec-base-chinese"
    )

    young_pat_vectordb = Chroma(
        persist_directory=temp_dir, embedding_function=english_embeddings
    )
    young_pat_dense_retriever = young_pat_vectordb.as_retriever(
        search_type="mmr", search_kwargs={"k": 2, "fetch_k": 5}
    )

    ask_gm_eng_vectordb = Chroma(
        persist_directory=temp_dir, embedding_function=english_embeddings
    )
    ask_gm_eng_retriever = ask_gm_eng_vectordb.as_retriever(
        search_type="mmr", search_kwargs={"k": 2, "fetch_k": 5}
    )

    ask_gm_chn_vectordb = Chroma(
        persist_directory=temp_dir, embedding_function=chinese_embeddings
    )
    ask_gm_chn_retriever = ask_gm_chn_vectordb.as_retriever(
        search_type="mmr", search_kwargs={"k": 2, "fetch_k": 5}
    )
...
custom_template = PromptTemplate(
    name="llama-instruct",
    prompt_text="""Have a conversation with a human, answer the following questions as best you can. You can refer to the following document and context.\n### Question: $query\n### Context: $documents\n### Response:""",
)
...
def dense_retrieval_execution(request, docs):
    ...
    with torch.no_grad():
        with torch.cpu.amp.autocast(
            ...
            output = langchain_model.generate(
                **inputs, generation_config=generation_config
            )
        output = output[:, inputs.input_ids.shape[1] :]
    generated_texts = langchain_tok.batch_decode(
...
def intent_detect_optimize(query):
    ...
    streamer = TextIteratorStreamer(
        langchain_tok, timeout=10.0, skip_prompt=True, skip_special_tokens=True
    )
    stop = StopOnTokens()
    generate_kwargs = dict(
        input_ids=input_ids,
...
def sparse_chitchat(request):
    ...
    stop = StopOnTokensWithPeriod(
        min_length=108, start_length=input_token_len, stop_token_id=stop_token_ids
    )
    generate_kwargs = dict(
        eos_token_id=0,
        pad_token_id=0,
    ...
    with torch.no_grad():
        with torch.cpu.amp.autocast(
            ...
            generation_output = langchain_model.generate(
                **input_tokens, **generate_kwargs
            )
    generated_texts = langchain_tok.decode(
    ...
    if domain == "INC":
        ...
        if request.embedding == "dense":
            ...
            if "qa" not in intent.lower():
                intent = "chitchat"
            print(
                "Intent detection time is {} seconds and the intent is {}".format(
                    time_2 - time_1, intent
                )
            )
            if "chitchat" in intent.lower():
                return sparse_chitchat(request)
            else:
                askgm_document_store = ElasticsearchDocumentStore(
                    host="localhost",
                    index="elastic_askgm_sparse",
                    port=9200,
                    search_fields=["content", "title"],
                ...
                askgm_retriever = BM25Retriever(
                    document_store=askgm_document_store, top_k=10
                )
                askgm_pipeline.add_node(
                    component=askgm_retriever, name="Retriever", inputs=["Query"]
                )
                askgm_pipeline.add_node(
                    component=reranker, name="Reranker", inputs=["Retriever"]
                )
                askgm_pipeline.add_node(
                    component=shaper, name="Shaper", inputs=["Reranker"]
                )
                askgm_pipeline.add_node(
                    component=prompt, name="Prompter", inputs=["Shaper"]
                )
        ...
        if request.embedding == "dense":
            ...
            if "chitchat" in intent.lower():
                ...
                customized_document_store = InMemoryDocumentStore(
                    use_gpu=False, use_bm25=True
                )
                customized_document_store = doc_index.s_load_young_pat_xlsx(
                    "./doc/young_pat/pat.xlsx", True, customized_document_store
                )
                young_pat_pipeline = Pipeline()
                young_pat_retriever = BM25Retriever(
                    document_store=customized_document_store, top_k=10
                )
                young_pat_pipeline.add_node(
                    component=young_pat_retriever, name="Retriever", inputs=["Query"]
                )
                young_pat_pipeline.add_node(
                    component=reranker, name="Reranker", inputs=["Retriever"]
                )
        ...
        if request.filename.endswith("md"):
            ...
            if (
                request.embedding == "dense"
            ):  # currently use Chroma as the dense retrieval datastore
                documents = []
                if request.filename.endswith("jsonl") or request.filename.endswith(
                    "json"
                ):
                    documents = doc_index.d_load_jsonl_file(file_path, True, documents)
                elif request.filename.endswith("pdf") or request.filename.endswith(
                    "docx"
                ):
                    documents = doc_index.d_load_file(file_path, True)
                elif request.filename.endswith(".xlsx") or request.filename.endswith(
                    ".csv"
                ):
                ...
                doc_index.persist_embedding(
                    documents, output_path, model_path="hkunlp/instructor-large"
                )
                embeddings = HuggingFaceInstructEmbeddings(
                    model_name="hkunlp/instructor-large"
                )
                vectordb = Chroma(
                    persist_directory=output_path, embedding_function=embeddings
                )
                ...
                if request.filename.endswith("jsonl") or request.filename.endswith(
                    ...
                    customized_document_store = doc_index.s_load_jsonl_file(
                        file_path, True, customized_document_store
                    )
                elif request.filename.endswith("pdf") or request.filename.endswith(
                    "docx"
                ):
                    customized_document_store = doc_index.s_load_file(
                        file_path, True, customized_document_store
                    )
                elif request.filename.endswith(".xlsx") or request.filename.endswith(
                    ".csv"
                ):
                    customized_document_store = doc_index.s_load_xlsx(
                        file_path, True, customized_document_store
                    )
        customized_pipeline = Pipeline()
        customized_retriever = BM25Retriever(
            document_store=customized_document_store, top_k=10
        )
        customized_pipeline.add_node(
            component=customized_retriever, name="Retriever", inputs=["Query"]
        )
        customized_pipeline.add_node(
            component=reranker, name="Reranker", inputs=["Retriever"]
        )
...
def save_chat_feedback_to_db(request: FeedbackRequest) -> None:
    logger.info("fastrag feedback received.")
    ...
    logger.info(
        f"feedback question: [{question}], answer: [{answer}], feedback: [{feedback_str}]"
    )
    ...
    try:
        ...
        raise Exception(
            "Exception occurred when inserting data into MySQL, please check the db session and your syntax."
        )
    else:
...
def _process_request(pipeline, request) -> Dict[str, Any]:
    ...
    for key in params.keys():
        if (
            isinstance(params[key], collections.Mapping)
            and "filters" in params[key].keys()
        ):
            params[key]["filters"] = _format_filters(params[key]["filters"])

    prediction = pipeline.run(query=request.query, params=params, debug=request.debug)
    # Ensure answers and documents exist, even if they're empty lists
    if "documents" not in prediction:
        prediction["documents"] = []
    if "answers" not in prediction:
        prediction["answers"] = []
