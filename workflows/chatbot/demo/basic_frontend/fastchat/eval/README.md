...
Unfortunately, Bard has not release its public APIs till now. You may have to enter the answers manually. Or you could find a third-party project that interfaces with Bard.
...
`model.jsonl` contains model information we used for generating answers.
...
`review/xxx.jsonl` contains reviews given by reviewers, comparing performance between a pair of models. Each row contains a record of a review with the following field:
