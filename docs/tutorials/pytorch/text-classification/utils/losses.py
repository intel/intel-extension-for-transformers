...
class ConstLambdaLoss:
    ...
    def __call__(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        loss_list = []
        has_been_learned = torch.zeros(
            (logits.shape[0], target.shape[0]), device=logits.device
        )
        for i in range(logits.shape[0]):
            ...
            if i > 0:
                loss = loss * (
                    1
                    - self.loss_lambda * torch.mean(has_been_learned[0:i, ...], axis=0)
                )
...
class BaselineLoss:
    ...
    def __call__(
        ...
        for i in range(logits.shape[0]):
            ...
            # LTE loss
            lte_gold = torch.eq(
                torch.argmax(logits[i, ...], dim=1), target
        ...
        return torch.sum(torch.stack(lte_loss_list, dim=0)) + torch.sum(
            torch.stack(loss_list, dim=0)
        )
        ...
        for i in range(logits.shape[0]):
            ...
            if i > 0:
                loss = loss * (
                    1
                    - self.loss_lambda
                    * torch.max(has_been_learned[0:i, ...], axis=0).values
                )
            loss = torch.mean(loss)
            loss_list.append(loss)
            predictions = torch.argmax(current_logits, axis=1)
...
class MultiExitMSELossForLTE:
    ...
    def __call__(
        ...
        for i in range(logits.shape[0]):
            lte_gold = (
                torch.eq(torch.argmax(logits[i, ...], dim=1), target)
                .float()
                .unsqueeze(1)
            )  # 0 for wrong/continue, 1 for right/exit
