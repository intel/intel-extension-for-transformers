    ...
    def get_source_files(self):
        ...
        for ext in self.extensions:
            if not isinstance(ext, CMakeExtension):
                continue
            files.extend(
                os.path.relpath(f, cwd) for f in self._get_files(ext.sourcedir, cwd)
            )
        return files
    ...
    if IS_INTEL_GPU:
        ...
        ext_modules = [
            CMakeExtension(
                ...
                "intel_extension_for_transformers/llm/operator/csrc",
                lib_only=True,
            )
        ]
    if not SKIP_RUNTIME:
        ...
        ext_modules.extend(
            [
                CMakeExtension(
                    "intel_extension_for_transformers.neural_engine_py",
