...
from intel_extension_for_transformers.utils.data_augmentation import (
    EOS,
    DataAugmentation,
)
...
class TestDataAugmentation(unittest.TestCase):
    ...
    def test_keyboard_augmentation(self):
        ...
        raw_datasets = load_dataset(
            "csv", data_files=aug.output_path, delimiter="\t", split="train"
        )
    ...
    def test_contextualwordembsforsentence_augmentation(self):
        ...
        aug.output_path = os.path.join(self.result_path, "test2.cvs")
        aug.augmenter_arguments = {
            "model_path": "hf-internal-testing/tiny-random-xlnet"
        }
        aug.data_augment()
        raw_datasets = load_dataset(
            "csv", data_files=aug.output_path, delimiter="\t", split="train"
