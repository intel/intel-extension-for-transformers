...
from intel_extension_for_transformers.transformers.modeling.modeling_seq2seq import (
...
def get_seq2seq_example_inputs(model):
    onnx_config_class = TasksManager.get_exporter_config_constructor(
        model_type=model.config.model_type, exporter="onnx", task="text2text-generation"
    )
    onnx_config = onnx_config_class(
        model.config,
        use_past=model.config.use_cache,
        use_past_in_inputs=model.config.use_cache,
    )
    ...
    decoder_with_past_onnx_config = onnx_config.with_behavior(
        "decoder", use_past=True, use_past_in_inputs=model.config.use_cache
    )
    ...
    decoder_dummy_inputs["encoder_outputs"] = tuple(
        decoder_dummy_inputs["encoder_outputs"][0:1]
    )
    ...
    decoder_with_past_dummy_inputs["encoder_outputs"] = tuple(
        decoder_with_past_dummy_inputs["encoder_outputs"][0:1]
    )
    decoder_with_past_dummy_inputs["past_key_values"] = tuple(
        decoder_with_past_dummy_inputs["past_key_values"]
    )
...
class TestModeling(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.input_ids = self.tokenizer(
            self.prompt_texts[0], return_tensors="pt"
        ).input_ids
    ...
    def test_seq2seq_jit_model(self):
        ...
        jit_gen_ids = jit_model.generate(
            input_ids, max_new_tokens=32, **self.generate_kwargs
        )
    ...
    def test_seq2seq_model_quant_with_ipex(self):
        ...
        for backend in ("default",):
            ...
            if backend == "ipex":
                self.assertTrue(
                    isinstance(loaded_model.encoder_model, RecursiveScriptModule)
                )
            else:
                self.assertTrue(
                    isinstance(
                        loaded_model.encoder_model.embed_tokens,
