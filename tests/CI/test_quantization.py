...
class TestQuantization(unittest.TestCase):
    ...
    def test_fx_model_quant(self):
        ...
        for mode in QuantizationMode:
            ...
            if mode == QuantizationMode.POSTTRAININGSTATIC:
                ...
                self.assertTrue(
                    check_onnx("fp32-model.onnx", self.trainer.get_eval_dataloader())
                )
            ...
            quantized_model = self.trainer.quantize(
                quant_config=quantization_config, provider="inc"
            )
            ...
            if mode == QuantizationMode.POSTTRAININGSTATIC:
                ...
                self.assertTrue(
                    check_onnx("int8-model.onnx", self.trainer.get_eval_dataloader())
                )

            if mode == QuantizationMode.QUANTIZATIONAWARETRAINING:
                model = onnx.load("int8-model.onnx")
                tensor_list = {
                    tensor.name: tensor for tensor in model.graph.initializer
                }
                torch_data = (
                    quantized_model.classifier.state_dict()[
                ...
                self.assertTrue(
                    np.allclose(torch_data, onnx_data * onnx_scale, atol=0.001)
                )
    ...
    def test_fx_model_with_smooth_quant(self):
        ...
        quantized_model = trainer.quantize(
            quant_config=quantization_config, train_func=train_func, eval_func=eval_func
        )
        self.assertTrue("quantize" in str(type(quantized_model.classifier.module)))
    ...
    def test_functional_quant(self):
        ...
        quantization_config = QuantizationConfig(
            approach="PostTrainingStatic", objectives=[objectives.performance]
        )
        self.trainer.quantize(
    ...
    def test_no_trainer_quant(self):
        ...
        opt_model = self.optimizer.quantize(
            quant_config=quantization_config,
            provider="inc",
            train_func=train_func,
            eval_func=eval_func,
    ...
    def test_seq2seq_models(self):
        ...
        trainer.num_beams = 3
        trainer.metrics = metrics.Metric(name="eval_loss")
    ...
    def test_quantization_for_llm(self):
        ...
        fp32_model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, use_llm_runtime=False
        )
        ...
        q_model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, quantization_config=sq_config, use_llm_runtime=False
        )
        ...
        self.assertTrue(
            isclose(float(output[0][0][0][0]), 0.16387596726417542, rel_tol=1e-04)
        )
        # AWQ
        ...
        print("output:", float(output[0][0][0][0]))
        self.assertTrue(
            isclose(float(output[0][0][0][0]), 0.17239853739738464, rel_tol=1e-04)
        )
        # TEQ
        woq_config = WeightOnlyQuantConfig(
            weight_dtype="int4_fullrange",
        ...
        # fp8
        woq_config = WeightOnlyQuantConfig(
            weight_dtype="fp8_e5m2", scale_dtype="fp8_e8m0"
        )
        woq_model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, quantization_config=woq_config, use_llm_runtime=False
        )
        ...
        amp_model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, quantization_config=amp_config, use_llm_runtime=False
        )
        ...
        bab_model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, quantization_config=bab_config, use_llm_runtime=False
        )
        # load_in_4bit
        bit4_model = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, load_in_4bit=True, use_llm_runtime=False
        )
        output = bit4_model(dummy_input)
        print("output:", float(output[0][0][0][0]))
        self.assertTrue(
            isclose(float(output[0][0][0][0]), 0.18726778030395508, rel_tol=1e-04)
        )
        ...
        print("output:", float(output[0][0][0][0]))
        self.assertTrue(
            isclose(float(output[0][0][0][0]), 0.1675747185945511, rel_tol=1e-04)
        )
        ...
        print("output:", float(output[0][0][0][0]))
        self.assertTrue(
            isclose(float(output[0][0][0][0]), 0.17126554250717163, rel_tol=1e-04)
        )
