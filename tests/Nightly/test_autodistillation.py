...
from transformers import AutoModelForPreTraining, AutoTokenizer, TrainingArguments
...
def main_worker(rank, world_size, model, teacher_model, dataset):
    ...
    autodistillation_config = AutoDistillationConfig(
        ...
        regular_distillation=FlashDistillationConfig(
            layer_mappings_for_knowledge_transfer=[[[("cls", "0")]]],
            loss_types=[["KL"]],
    ...
    best_model_archs = trainer.autodistillation(
        autodistillation_config, teacher_model, model_cls=AutoModelForPreTraining
    )
