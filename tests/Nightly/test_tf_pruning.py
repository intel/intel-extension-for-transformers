...
class TestTFPruning(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.args = parser.parse_args_into_dataclasses(
            args=[
                "--output_dir",
                "./quantized_model",
    ...
    def test_tf_model_quant(self):
        ...
        from intel_extension_for_transformers.transformers.utils.utility_tf import (
            distributed_init,
        )
        ...
        self.assertTrue(
            get_filepath("dummy", "worker", 0) != get_filepath("dummy", "worker", 1)
        )
        ...
        p_model = self.optimizer.prune(
            pruning_config=pruning_conf,
            train_dataset=self.dummy_dataset,
            eval_dataset=self.dummy_dataset,
        )
        ...
        self.optimizer.prune(
            pruning_config=pruning_conf, train_func=train_func, eval_func=eval_func
        )
