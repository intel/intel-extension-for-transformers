...
std::pair<OpArgs, OpArgs> GenerateFp32Case(std::vector<tensor_desc> const& ts_descs,
  ...
  auto make_tensor_obj = [&](const tensor_desc a_tensor_config) {
    ...
    if (a_tensor_config.shape().size() == 1) {
      ...
      for (int i = 0; i < a_tensor_config.size(); ++i) {
        int32_t index = (int32_t)(rand_r(&seed) % src0_shape[0]);
        memcpy((tensor_data + i * 4), &index, sizeof(int32_t));
      }
  ...
  auto src0_data_copy = src0_tensors.second;
        ...
        if (append_ops[k] == "append_sum") {
          ...
          if (input_dt == data_type::s8) {
            binary_add<int8_t>(dst_data_copy + (i * dst_shape[1] + j) * get_data_size(input_dt),
                               append_vec_copys[k] + (broad_cast_i * dst_shape[1] + j) * get_data_size(input_dt));
