...
#include "../../include/operators/pow.hpp"
...
bool CheckResult(const TestParams& t) {
  ...
  try {
    executor::PowOperator pow(p.conf);
    pow.Prepare(p.input, p.output);
    pow.Reshape(p.input, p.output);
    pow.Forward(p.input, p.output);
...
class PowTest : public testing::TestWithParam<TestParams> {
 protected:
  PowTest() {}
  ~PowTest() {}
  void SetUp() override {}
...
std::pair<OpArgs, OpArgs> GenerateFp32Case(const std::vector<std::vector<int64_t>>& input_shape) {
  ...
  OperatorConfig op_config = OperatorConfig("pow", "Pow", input_config, output_config, op_attr);
...
INSTANTIATE_TEST_SUITE_P(Prefix, PowTest, CasesFp32());
