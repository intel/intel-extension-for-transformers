...
#include "../../include/operators/erf.hpp"
...
bool CheckResult(const TestParams& t) {
  ...
  try {
    executor::ErfOperator erf(p.conf);
    erf.Prepare(p.input, p.output);
    erf.Reshape(p.input, p.output);
    erf.Forward(p.input, p.output);
...
class ErfTest : public testing::TestWithParam<TestParams> {
 protected:
  ErfTest() {}
  ~ErfTest() {}
  void SetUp() override {}
...
std::pair<OpArgs, OpArgs> GenerateFp32Case(const std::vector<std::vector<int64_t>>& input_shape) {
  ...
  OperatorConfig op_config = OperatorConfig("erf", "Erf", input_config, output_config, op_attr);
...
INSTANTIATE_TEST_SUITE_P(Prefix, ErfTest, CasesFp32());
