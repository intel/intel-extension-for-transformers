...
def run():
    ...
    dataset = MRPCDataSet(1, "/home/tensorflow/.cache/nlp_toolkit/glue_mrpc_data", "/tf_dataset2/models/nlp_toolkit/bert_mini_mrpc")
    model = load_model("ir")
...
"""
        with open('run.py', 'w', encoding='utf-8') as f:
            f.write(code)
        mrpc_dataset = load_dataset('glue', 'mrpc',cache_dir='/home/tensorflow/.cache/nlp_toolkit/glue_mrpc_data', split='validation')
        # export onnx model
        torch_model = BertForSequenceClassification.from_pretrained(
            "/tf_dataset2/models/nlp_toolkit/bert_mini_mrpc")
        with torch.no_grad():
            inputs = {
        ...
        self.dispatch_table_dir = os.path.join(
            os.environ['HOME'], '.cache/neural_engine_workspace/engine_dispatch_table.txt')
    ...
    def test_dispatcher_tuning_sharing_acc(self):
        if os.path.exists(self.dispatch_table_dir):
            os.remove(self.dispatch_table_dir)
        cmd = "numactl -l -C 0-3 python run.py log0_dt0.txt & " \
              "numactl -l -C 4-7 python run.py log1_dt0.txt & " \
              "numactl -l -C 8-11 python run.py log2_dt0.txt & " \
              "numactl -l -C 12-15 python run.py log3_dt0.txt &" \
              "numactl -l -C 16-19 python run.py log4_dt0.txt &" \
              "numactl -l -C 20-23 python run.py log5_dt0.txt &" \
              "numactl -l -C 24-27 python run.py log6_dt0.txt"
        ...
        # open kernel tuning
        os.environ['ENGINE_DISPATCHER_TUNING_ON'] = '1'
        os.environ['WEIGHT_SHARING'] = '1'
        os.environ['INST_NUM'] = '7'
        cmd = "numactl -l -C 0-3 python run.py log0_dt1.txt & " \
              "numactl -l -C 4-7 python run.py log1_dt1.txt & " \
              "numactl -l -C 8-11 python run.py log2_dt1.txt & " \
              "numactl -l -C 12-15 python run.py log3_dt1.txt &" \
              "numactl -l -C 16-19 python run.py log4_dt1.txt &" \
              "numactl -l -C 20-23 python run.py log5_dt1.txt &" \
              "numactl -l -C 24-27 python run.py log6_dt1.txt"
        process = subprocess.Popen(cmd, shell=True)  # nosec
        process.wait()
        if process.returncode != 0:
            raise subprocess.CalledProcessError(returncode=process.returncode, cmd=cmd)
        ...
        del os.environ['ENGINE_DISPATCHER_TUNING_ON']
        cmd = "numactl -l -C 0-3 python run.py log0_dt2.txt & " \
              "numactl -l -C 4-7 python run.py log1_dt2.txt & " \
              "numactl -l -C 8-11 python run.py log2_dt2.txt & " \
              "numactl -l -C 12-15 python run.py log3_dt2.txt &" \
              "numactl -l -C 16-19 python run.py log4_dt2.txt &" \
              "numactl -l -C 20-23 python run.py log5_dt2.txt &" \
              "numactl -l -C 24-27 python run.py log6_dt2.txt"
        process = subprocess.Popen(cmd, shell=True)  # nosec
        process.wait()
        if process.returncode != 0:
            raise subprocess.CalledProcessError(returncode=process.returncode, cmd=cmd)
