...
bool spmm_avx512f_kd_t::init() {
  ...
  bool is_supported = (op_desc_.kernel_prop() == kernel_prop::forward_inference) &&
                      is_any_of({dt::fp32}, [&](const dt& a) { return wei_desc.dtype() == a; }) &&
                      is_any_of({dt::fp32}, [&](const dt& a) { return src_desc.dtype() == a; }) &&
                      (!has_bias || is_any_of({dt::fp32}, [&](const dt& a) { return bias_desc.dtype() == a; })) &&
                      is_any_of({dt::fp32}, [&](const dt& a) { return dst_desc.dtype() == a; });
...
bool spmm_avx512f_kd_t::spmm_params_init(const jd::operator_desc& op_desc) {
...
#pragma omp parallel for
  for (int i = 0; i < jit_kers_.size(); ++i) {
