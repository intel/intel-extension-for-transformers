...
template <typename dtype_in_, typename dtype_out_, typename tile_shape_,
        mem_space mem_space_in_, mem_space mem_space_out_, int SIMD_,
        int thread_num_, int softmax_size_>
struct xetla_softmax_fwd_t {
    ...
    using softmax_store_payload_t = subgroup::mem_payload_t<dtype_out,
            softmax_tile_desc_t, msg_type::block_2d, mem_layout::row_major,
            mem_space_out, gpu_arch::Xe>;
    ...
    __XETLA_API KERNEL_FUNC void operator()(
        ...
        int local_offset_y = block_height * ei.get_local_linear_id();
        ...
        xetla_vector<float, softmax_size> row_data_32_0;

        int inner_loop_count = (wg_tile_m % thread_num == 0)
        ...
        softmax_store_payload.init(
                args->data_out_ptr + ei.get_group(1) * wg_tile_m * wg_tile_n,
                SIMD, block_height * wg_tile_m, SIMD, 0, local_offset_y);
        for (int row = 0; row < inner_loop_count; ++row) {
            ...
            row_data_32_0 = softmax_load.reg.xetla_select<softmax_size, 1>(0);

            // get max
            float xmax = hmax<float, float, softmax_size>(row_data_32_0);

            // get exp_sum
            row_data_32_0 -= xmax;
            row_data_32_0 = exp(row_data_32_0);
            float exp_sum = sum<float, float, softmax_size>(row_data_32_0);

            // get softmax elementwise result
            row_data_32_0 /= exp_sum;

            softmax_store.reg.xetla_select<softmax_size, 1>(0)
                    = xetla_cvt<dtype_out, float, softmax_size>(row_data_32_0);
