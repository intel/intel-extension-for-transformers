...
""" Finetuning the library models for sequence classification on GLUE with Multi-Model."""
import logging
import os
import random
import sys
...
def main():
    ...
    configs = [my_config.from_pretrained(
    ...
    models = [my_model.from_pretrained(
    ...
    if training_args.do_eval:
        ...
        for thr_i, thr in enumerate(exit_thresholds):
            ...
            for pred_i, prediction in enumerate(layer_predictions):
                ...
                predicting_model = np.argmax(should_early_exit[thr_i, pred_i, :]) if was_predicted else (
                            len(models) - 1)
                total_predictions[pred_i, :predicting_model + 1, ...] = prediction[:predicting_model + 1, ...].squeeze(
                    1)
