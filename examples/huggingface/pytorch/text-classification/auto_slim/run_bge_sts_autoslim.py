...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="keyword identifier to save the model"
    )
...
def main():
    ...
    if args.task_name is not None:
        ...
        raw_datasets = load_dataset("mteb/stsbenchmark-sts") # train datasets 
        raw_datasets["train"] = raw_datasets["train"].rename_column("score", "label")
        raw_datasets["test"] = raw_datasets["test"].rename_column("score", "label")
        raw_datasets["validation"] = raw_datasets["validation"].rename_column("score", "label")
    else:
    ...
    # method 2: old API using compression manager
    # pruner = Pruning(config)
    # pruner.model = model
    # pruner.on_train_begin()
    # method 3: compression_manager
    ...
    for epoch in range(args.num_train_epochs):
        ...
        torch.save(model, f"{args.keyword}-{epoch}-1117.pt")
    ...
    #-----------------------------start auto slim----------------------------------#
    # define the function to evaluate accuracy (should be zero loss) and latency
    def eval_acc_and_latency(model, eval_dataloader):
        ...
        eval_metric = metric.compute()
        logger.info(f"Before quantization : {eval_metric}")
        logger.info(f"Average inference time: {avg_latency} ms")
        del my_timer

    def eval_latency(model):
        print("Test inference latency")
        dummy_inputs = torch.ones([1, 512], dtype=torch.long)
        dummy_inputs = move_input_to_device(dummy_inputs, torch.device("cuda:0"))
        if torch.cuda.is_available():
            my_timer = GPUTimer(timelogs = [])
        else:
            my_timer = CPUTimer(timelogs = [])
        # should use 512 length data
    ...
    if args.auto_slim:
        ...
        if args.keyword != None:
            torch.save(model, f"{args.keyword}_before_autoslim.pt")
        ...
        eval_acc_and_latency(model, eval_dataloader)
        eval_latency(model)
        # save the dense slim model
        if args.keyword != None:
            torch.save(model, f"{args.keyword}_after_autoslim.pt")
    #-----------------------------finish auto slim---------------------------------#
