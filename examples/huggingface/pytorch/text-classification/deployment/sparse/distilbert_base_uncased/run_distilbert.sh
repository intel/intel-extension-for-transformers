...
OUTPUT_DIR=$(pwd)
LOG_NAME="bert_mini.log"
DATASET="squad"
MODEL_NAME_OR_PATH="Intel/distilbert-base-uncased-squadv1.1-sparse-80-1X4-block"
BATCH_SIZE=8
WARM_UP=100
SEQUENCE_LEN=128
ITERATION=1000
...
if [[ -f ${inference_model} ]]; then
    ...
    echo "===== Transpose to IR ========"
    python export_transpose_ir.py \
      --input_model=./model_and_tokenizer/${PRECISION}-model.onnx \
      --output_dir=./sparse_${PRECISION}_ir
    inference_model="./sparse_${PRECISION}_ir"
else
...
if [[ ${MODE} == "accuracy" ]]; then
    ...
    benchmark_cmd="python run_executor.py --input_model=${inference_model} --mode=performance --batch_size=${BATCH_SIZE} --seq_len=${SEQUENCE_LEN} --warm_up=${WARM_UP} --iteration=${ITERATION} --dataset_name=${DATASET} --tokenizer_dir=./model_and_tokenizer ${mode_cmd} "
