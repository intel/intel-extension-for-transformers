...
if [[ ${MODE} == "accuracy" ]]; then
    ...
    benchmark_cmd="python run_executor.py --input_model=${inference_model} --mode=performance --batch_size=${BATCH_SIZE} --seq_len=${SEQUENCE_LEN} --warm_up=${WARM_UP} --iteration=${ITERATION} --task_name=${DATASET} --dataset_nam=glue --tokenizer_dir=distilbert-base-uncased-finetuned-sst-2-english ${mode_cmd} "
