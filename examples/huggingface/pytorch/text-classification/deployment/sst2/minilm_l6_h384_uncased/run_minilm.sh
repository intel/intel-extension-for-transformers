...
if [[ ${MODE} == "accuracy" ]]; then
    ...
    python run_executor.py \
      ...
      --dataset_name=glue \
      --task_name=${DATASET} \
      --tokenizer_dir=philschmid/MiniLM-L6-H384-uncased-sst2 \
    ...
    benchmark_cmd="python run_executor.py --input_model=${inference_model} --mode=performance --batch_size=${BATCH_SIZE} --seq_len=${SEQUENCE_LEN} --warm_up=${WARM_UP} --iteration=${ITERATION} --task_name=${DATASET} --dataset_name=glue --tokenizer_dir=philschmid/MiniLM-L6-H384-uncased-sst2 ${mode_cmd} "
