...
DATASET="sst2"
MODEL_NAME_OR_PATH="moshew/bert-mini-sst2-distilled"
BATCH_SIZE=8
WARM_UP=100
SEQUENCE_LEN=128
ITERATION=1000
...
if [[ ${MODE} == "accuracy" ]]; then
    ...
    python run_executor.py \
      ...
      --dataset_name=glue \
      --task_name=${DATASET} \
      --tokenizer_dir=moshew/bert-mini-sst2-distilled \
    ...
    benchmark_cmd="python run_executor.py --input_model=${inference_model} --mode=performance --batch_size=${BATCH_SIZE} --seq_len=${SEQUENCE_LEN} --warm_up=${WARM_UP} --iteration=${ITERATION} --task_name=${DATASET} --dataset_name=glue --tokenizer_dir=moshew/bert-mini-sst2-distilled ${mode_cmd} "
