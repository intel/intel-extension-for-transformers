...
class OptimizationArguments:
    ...
    num_sandwich: int = field(
        default=2,
    )
    length_drop_ratio_bound: float = field(
        default=0.2,
    )
    layer_dropout_prob: float = field(
        default=None,
    )
    layer_dropout_bound: int = field(
        default=None,
    )
    do_search: Optional[bool] = field(default=False)
    load_store_file: Optional[str] = field(default=None)
    evo_iter: Optional[int] = field(default=30)
...
def main():
    ...
    if optim_args.onnx:
        assert model_args.tokenizer_name , "For evaluating onnx model please provide a tokenizer"
        config = None
    ...
    if optim_args.length_config and not optim_args.length_adaptive:
        dynamic_length_config = DynamicLengthConfig(
            length_config=optim_args.length_config
        )
        trainer.set_dynamic_config(dynamic_config=dynamic_length_config)

    if optim_args.measure_rate:
        import torchprofile
        ...
        out_file_path = os.path.join(training_args.output_dir, 'measure_out.log')
        with open(out_file_path, 'w') as out_file:
            for rate in rates:
                out_file.write(str(rate) + ":\n")
                dynamic_length_config = DynamicLengthConfig(
                    max_length=data_args.max_seq_length,
                    const_rate=float(rate)
                )
                trainer.set_dynamic_config(dynamic_config=dynamic_length_config)
                ...
                res_str = "rate={}\nmacs={}\nevalTime={}\neval={}\n".format(rate, macs, evalTime, results)
    ...
    if optim_args.length_adaptive:
        ...
        dynamic_length_config = DynamicLengthConfig(
            ...
            num_sandwich=optim_args.num_sandwich,
            length_drop_ratio_bound=optim_args.length_drop_ratio_bound,
            layer_dropout_prob=optim_args.layer_dropout_prob,
            layer_dropout_bound=optim_args.layer_dropout_bound,
            max_length=data_args.max_seq_length
        )
    ...
    # Training
    if training_args.do_train and not optim_args.tune:
    ...
    if optim_args.benchmark or optim_args.accuracy_only:
        ...
        # FLOPs
        flops = None
        if not optim_args.int8 and not optim_args.onnx: # can't measure flops for int8 model
            import torchprofile
            flops = torchprofile.profile_macs(model, dummy_inputs)

        if optim_args.onnx:
            trainer.set_onnx_path(model_args.model_name_or_path)
    ...
    # Evolutionary Search
    if optim_args.do_search and training_args.local_rank in [-1, 0]:
        import warnings
