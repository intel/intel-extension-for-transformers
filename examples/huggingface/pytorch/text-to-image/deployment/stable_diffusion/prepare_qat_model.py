...
if export2onnx:
    ...
    if os.path.exists(os.path.dirname(onnx_model_path)):
        ...
        #     dtype="fp32",
        #     example_inputs={'sample':torch.randn(2, 4, 64, 64, dtype=dtype), 'timestep':torch.randn(2).to(device=device, dtype=torch.int64), 'encoder_hidden_states':torch.randn(2, 77, 768, dtype=dtype)},
    ...
    if os.path.exists(os.path.dirname(onnx_model_path)):
        ...
        torch.onnx.export(
            ...
            args=(
                torch.randn(2, 4, 64, 64).to(device=device,dtype=dtype),
                torch.randn(2).to(device=device, dtype=torch.int64),
                torch.randn(2, 77, 768).to(device=device, dtype=dtype),
                #False,
            ),
            f=onnx_model_path,
