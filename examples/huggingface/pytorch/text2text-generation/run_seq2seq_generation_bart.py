...
def main():
    ...
    class Evaluator:
        ...
        def tokenize_function(self, examples):
            example = self.tokenizer(examples["dialogue"])
            return example
    ...
    if args.quantize:
        ...
        model.model.encoder.config.return_dict = False
        encoder_model = quantization.fit(
            model.model.encoder,
            encoder_conf,
            calib_func=encoder_calib_func,
        )
        inc_model = INCModelForSeq2SeqLM(encoder_model=encoder_model,
                                         decoder_model=model, config=model.config)
        print("=======encoder quantized=====")
        if args.ipex:
            def decoder_with_past_calib_func(prepared_model):
                inc_model.decoder_with_past_model = prepared_model
                for i, (input_ids, last_ind) in enumerate(calib_dataloader):
                    input_bs, input_len = input_ids.shape
                    attention_mask = torch.ones(input_bs, input_len)
                    if i >= args.calib_iters:
            ...
            inc_model.decoder_model.config.return_dict = False
            decoder_with_past_model = quantization.fit(
                copy.deepcopy(inc_model.decoder_model),
                decoder_with_past_conf,
                calib_func=decoder_with_past_calib_func,
            )
            inc_model.decoder_with_past_model = decoder_with_past_model
            print("=======decoder_with_past quantized=====")
        def decoder_calib_func(prepared_model):
            inc_model.decoder_model = prepared_model
            for i, (input_ids, last_ind) in enumerate(calib_dataloader):
                input_bs, input_len = input_ids.shape
                attention_mask = torch.ones(input_bs, input_len)
                if i >= args.calib_iters:
        ...
        decoder_model = quantization.fit(
            inc_model.decoder_model,
            decoder_conf,
            calib_func=decoder_calib_func,
        )
        inc_model.decoder_model = decoder_model
        print("=======decoder quantized=====")
        if args.ipex:
            inc_model.config.torchscript = True
        inc_model.config.torch_dtype = "int8"
        inc_model.save_pretrained(args.output_dir)
    ...
    if args.benchmark:
        ...
        with torch.inference_mode(), torch.no_grad():
            for i in range(num_iter):
                ...
                gen_ids = model.generate(input_ids, max_new_tokens=args.max_new_tokens, **generate_kwargs)
