        ...
        if (gemm_t::is_2d_block_a) {
            implementable
                    &= kernel::block_2d<gpu_arch::Xe, dtype_a>::check_tensor(
                            (uint64_t)(args.matA_base.base), args.matrix_k,
                            args.matrix_m * args.batch_size, args.matA_ld);
        } else {
        ...
        if (gemm_t::is_2d_block_b) {
            implementable
                    &= kernel::block_2d<gpu_arch::Xe, dtype_b>::check_tensor(
                            (uint64_t)(args.matB_base.base), args.matrix_n,
                            args.matrix_k * args.batch_size, args.matB_ld);
        } else {
            implementable &= kernel::general_1d<gpu_arch::Xe,
        ...
        if (epilogue_t::is_2d_block_c) {
            implementable
                    &= kernel::block_2d<gpu_arch::Xe, dtype_c>::check_tensor(
                            ...
                            args.matrix_m * args.batch_size, args.matC_ld);
        } else {
            implementable &= kernel::general_1d<gpu_arch::Xe,
