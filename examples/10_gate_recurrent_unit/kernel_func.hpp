...
#define BRGEMM_CALL(id, acc_id, ptr_a, ptr_b) \
...
struct gru_layer {
    ...
    using work_group_t = typename brgemm_op::work_group_t;
    using brgemm_arguments = typename brgemm_op::arguments_t;
    ...
    static void inline call(xetla_exec_item<3> &ei, fused_config_t<T> *args) {
        ...
        for (uint32_t seq_id = 0; seq_id < seq_len; ++seq_id) {
            for (int j = (hidden_size + wg_tile_n - 1) / wg_tile_n - 1; j >= 0;
                ...
                //   elemwise_op: [m, n] -> [m, n], acc |-> tile_op_t(acc)
                matAcc_1.init(0);
                BRGEMM_CALL(1, 1, args->hx_ptr, args->W_hn_ptr);
                ...
                mem_desc_c.init({args->hx_ptr},
                        ...
                        {start_n + brgemm_op::get_matC_offset_x(g),
                                start_m + brgemm_op::get_matC_offset_y(g)});
