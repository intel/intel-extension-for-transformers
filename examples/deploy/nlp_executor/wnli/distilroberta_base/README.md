...
```shell
python prepare_dataset.py --dataset_name=glue --task_name=wnli --output_dir=./data
```
...
```shell
bash prepare_model.sh --input_model=roberta-base  --task_name=wnli --output_dir=./model_and_tokenizer --precision=fp32
```
And for better perfromance, you can also get a PTQ int8 model by setting precision=int8.
```shell
bash prepare_model.sh --input_model=roberta-base  --task_name=wnli --output_dir=./model_and_tokenizer --precision=int8
```
