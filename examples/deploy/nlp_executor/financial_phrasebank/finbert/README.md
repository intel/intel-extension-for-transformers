...
```shell
cd <NLPToolkit_folder>/examples/deploy/nlp_executor/financial_phrasebank/finbert
...
```shell
python prepare_dataset.py --dataset_name=financial_phrasebank --task_name=sentences_50agree --output_dir=./data
```
...
```shell
bash prepare_model.sh --input_model=ProsusAI/finbert  --task_name=sentences_50agree --output_dir=./model_and_tokenizer --precision=fp32
```
And for better perfromance, you can also get a PTQ int8 model by setting precision=int8.
```shell
bash prepare_model.sh --input_model=ProsusAI/finbert  --task_name=sentences_50agree --output_dir=./model_and_tokenizer --precision=int8
```
