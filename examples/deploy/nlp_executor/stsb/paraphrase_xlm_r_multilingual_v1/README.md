...
```shell
cd <NLPToolkit_folder>/examples/deploy/nlp_executor/stsb/paraphrase_xlm_r_multilingual_v1
...
```shell
python prepare_dataset.py --dataset_name=glue --task_name=stsb --output_dir=./data
```
...
```shell
bash prepare_model.sh --input_model=sentence-transformers/paraphrase-xlm-r-multilingual-v1  --task_name=stsb --output_dir=./model_and_tokenizer --precision=fp32
```
And for better perfromance, you can also get a PTQ int8 model by setting precision=int8.
```shell
bash prepare_model.sh --input_model=sentence-transformers/paraphrase-xlm-r-multilingual-v1  --task_name=stsb --output_dir=./model_and_tokenizer --precision=int8
```
...
### Benchmark
  ...
  run python
  ```shell
  GLOG_minloglevel=2 python run_executor.py --input_model=./model_and_tokenizer/int8-model.onnx --mode=performance --batch_size=4 --seq_len=128
  ```
  or run shell
  ```shell
  bash run_benchmark.sh --input_model=./model_and_tokenizer/int8-model.onnx  --mode=performance --batch_size=4 --seq_len=128
  ```
