...
```shell
cd <NLPToolkit_folder>/examples/deploy/nlp_executor/stsb/bert_base_nli_mean_tokens
...
```shell
bash prepare_model.sh --input_model=sentence-transformers/bert-base-nli-mean-tokens  --task_name=stsb --output_dir=./model_and_tokenizer --precision=fp32
```
And for better perfromance, you can also get a PTQ int8 model by setting precision=int8.
```shell
bash prepare_model.sh --input_model=sentence-transformers/bert-base-nli-mean-tokens  --task_name=stsb --output_dir=./model_and_tokenizer --precision=int8
```
