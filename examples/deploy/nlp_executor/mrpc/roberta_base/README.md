...
```shell
cd <NLPToolkit_folder>/examples/deploy/nlp_executor/mrpc/roberta_base
...
```shell
bash prepare_model.sh --input_model=textattack/roberta-base-MRPC  --task_name=mrpc --output_dir=./model_and_tokenizer --precision=fp32
```
And for better perfromance, you can also get a PTQ int8 model by setting precision=int8.
```shell
bash prepare_model.sh --input_model=textattack/roberta-base-MRPC  --task_name=mrpc --output_dir=./model_and_tokenizer --precision=int8
```
