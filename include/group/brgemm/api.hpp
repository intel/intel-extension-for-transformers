...
/// @brief Brgemm default pre_processing functor.
/// @tparam tile_shape Is the workgroup-level tile shape.
/// @tparam arch Is the HW architecture.
template <typename tile_shape, gpu_arch arch = gpu_arch::Xe>
struct pre_processing_default_t {};

/// @brief Brgemm pre_processing functor with applying relu op to matA.
/// @tparam tile_shape Is the workgroup-level tile shape.
/// @tparam arch Is the HW architecture.
template <typename tile_shape, gpu_arch arch = gpu_arch::Xe>
struct pre_processing_matA_neg_filter_t {};

/// @brief Brgemm functor.
/// @tparam compute_policy Is the compute algorithm of brgemm implementation.
/// @tparam tile_shape Is the workgroup-level tile shape.
...
template <typename compute_policy, typename tile_shape, typename mem_desc_a,
        typename mem_desc_b,
        typename pre_processing = pre_processing_default_t<tile_shape>>
class brgemm_t {};

/// @brief Brgemm selection functor.
