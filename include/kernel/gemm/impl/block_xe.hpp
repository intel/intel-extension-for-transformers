...
/// @brief Persistent-Thread GEMM_UNIVERSAL functor, specialized for Xe architecture.
///
...
template <int wg_num_n_, gpu_arch arch_tag_, typename gemm_t_,
        typename epilogue_t_>
class gemm_universal_t<dispatch_policy_block<wg_num_n_, arch_tag_>, gemm_t_,
        epilogue_t_, std::enable_if_t<(arch_tag_ == gpu_arch::Xe)>> {
    ...
    using dispatch_policy = dispatch_policy_block<wg_num_n, arch_tag>;
...
public:
    ...
    /// @brief Host helper function to auto padding the group range under the current GEMM_UNIVERSAL config.
