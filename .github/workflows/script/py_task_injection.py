...
class InsertCode:
    ...
    def insert(self) -> None:
        ...
        if self.exists(original_code):
            ...
            original_code = (
                original_code.split("# insert code start")[0]
                + original_code.split("# insert code end")[-1]
            )
...
if __name__ == "__main__":
    ...
    if code_params:
        code_inserter = InsertCode(
            args.file_name, code_params.get("code_content"), code_params.get("pattern")
        )
        if code_params.get("params") and code_params.get("params_flag"):
            for parameter, flag in zip(
                code_params.get("params"), code_params.get("params_flag")
            ):
