    ...
    try:
        ...
        df.set_index(
            [
                c
                for c in df.columns
                if not c.startswith("Unnamed") and c not in ["acc", "perf"]
            ],
            inplace=True,
        )
...
df_comp.loc[:, ("time", PCT_COL_NAME)] = (
    df_comp.loc[:, ("time", "new")] / df_comp.loc[:, ("time", "last")] * 100
)
df_comp.loc[:, ("gflops", PCT_COL_NAME)] = (
    df_comp.loc[:, ("gflops", "new")] / df_comp.loc[:, ("gflops", "last")] * 100
)
...
df_comp_styler = (
    df_comp.style.format("{:0.2f}", na_rep="-")
    .format("{:0.4f}", subset=idx_time, na_rep="-")
    .format("{:0.2f}%", subset=idx_pct, na_rep="-")
    .format_index(
        {
            df_comp.index.get_level_values(i).name: "{:.2f}"
            for i in range(len(df_comp.index.levels))
            if df_comp.index.get_level_values(i).dtype in [np.float32, np.float64]
        },
        axis=0,
    )
...
)  # remove the last line showing dtype
geo_mean_repr = f"<pre>GeoMean:\n{geo_mean_repr}</pre>"
print(f"<div class='summary-wrapper {name_no_ext}'>{table_repr}{geo_mean_repr}</div>")
...
if min_gflops_pct < THRESHOLD_REGRESSION:
    print(
        f"Min GFLOPS ({min_gflops_pct:.2f}%) is less than {THRESHOLD_REGRESSION}% in {new_summary_file}",
        file=sys.stderr,
    )
