...
class MptModel(BaseModel):
    def init_model(self, config):
        ...
        if config.device == "hpu":
            ...
            if config.cpu_jit:
                from mpt.mpt_trace import jit_trace_mpt_7b, MPTTSModelForCausalLM

                model = jit_trace_mpt_7b(model)
                config = AutoConfig.from_pretrained(config.model_name_or_path, trust_remote_code=True)
                model = MPTTSModelForCausalLM(
                    model, config, use_cache=config.use_cache, model_dtype=torch.bfloat16
                )
